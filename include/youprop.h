...
#define Sick_resistance                                   \
  (HSick_resistance || youmonst.data->mlet == S_FUNGUS || \
   youmonst.data == &mons[PM_GHOUL] || defends(AD_DISE, uwep))
#define Invulnerable player.uprops[INVULNERABLE].intrinsic /* [Tom] */

/* Extrinsics only */
#define EAntimagic player.uprops[ANTIMAGIC].extrinsic
#define Antimagic (EAntimagic || (Upolyd && resists_magm(&youmonst)))
...
#define Stunned \
  (HStun || player.umonnum == PM_STALKER || youmonst.data->mlet == S_BAT)
...
#define Halluc_resistance \
  (EHalluc_resistance || (Upolyd && dmgtype(youmonst.data, AD_HALU)))
...
#define Infravision (HInfravision || EInfravision || infravision(youmonst.data))
...
#define Invis ((HInvis || EInvis || pm_invisible(youmonst.data)) && !BInvis)
...
#define Teleport_control \
  (HTeleport_control || ETeleport_control || control_teleport(youmonst.data))

#define HLevitation player.uprops[LEVITATION].intrinsic
#define ELevitation player.uprops[LEVITATION].extrinsic
#define Levitation (HLevitation || ELevitation || is_floater(youmonst.data))
...
#define Lev_at_will                                                     \
  (((HLevitation & I_SPECIAL) != 0L || (ELevitation & W_ARTI) != 0L) && \
   ...
   (ELevitation & ~W_ARTI) == 0L && !is_floater(youmonst.data))

#define EFlying player.uprops[FLYING].extrinsic
#ifdef STEED
#define Flying                           \
  (EFlying || is_flyer(youmonst.data) || \
...
#define Wwalking \
  (player.uprops[WWALKING].extrinsic && !Is_waterlevel(&player.uz))
...
#else
#define Swimming (HSwimming || ESwimming || is_swimmer(youmonst.data))
#endif
...
#define Reflecting (EReflecting || (youmonst.data == &mons[PM_SILVER_DRAGON]))
