 ...
 */
#define canseeself() \
  (Blind || player.uswallow || (!Invisible && !player.uundetected))
...
#define maybe_display_usteed                    \
  (player.usteed && mon_visible(player.usteed)) \
...
#define obj_to_glyph(obj)                                               \
  (Hallucination                                                        \
       ...
       : ((obj)->otyp == CORPSE ? (int)(obj)->corpsenm + GLYPH_BODY_OFF \
...
#define glyph_to_obj(glyph)                                        \
  (glyph_is_body(glyph) ? CORPSE : glyph_is_normal_object(glyph)   \
                                       ? ((glyph) - GLYPH_OBJ_OFF) \
                                       : NO_GLYPH)
