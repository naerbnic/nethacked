...
static bool ranged_attk(MonsterType *);
static int mstrength(MonsterType *);
...
static char *xcrypt(char const *str) { /* duplicated in src/hacklib.c */
...
/* data format [COMPRESS excluded] (27..31) */
  ...
  version.struct_sizes = (((unsigned long)sizeof(struct flag) << 24) |
                          ...
                          ((unsigned long)sizeof(Monster) << 10) |
                          ((unsigned long)sizeof(struct Player)));
  return;
}
...
#endif
    save_bones_compat_buf,                         "basic NetHack features"};

static const char *window_opts[] = {"traditional tty-based graphics", 0};
...
static std::vector<char const*> special_oracle = {
    ...
    "and Solovay, 1975].",
    "",
    "Theorem 7.18  There exist recursive "
    "languages A and B such that",
    "  (1)  P(A) == NP(A), and", "  (2)  P(B) != NP(B)", "",
    "This provides impressive evidence that "
    "the techniques that are",
    "currently available will not suffice "
    "for proving that P != NP or          ",
    "that P == NP.\"  [Garey and Johnson, "
    "p. 185.]"};
...
static bool ranged_attk(MonsterType *ptr) {
  int i, j;
 ...
 */
static int mstrength(MonsterType *ptr) {
 ...
 */
  ...
  (void)fwrite((genericptr_t) & (qt_hdr.id[0][0]), sizeof(char) * LEN_HDR,
               qt_hdr.n_hdr, ofp);
  (void)fwrite((genericptr_t) & (qt_hdr.offset[0]), sizeof(long), qt_hdr.n_hdr,
               ofp);
#ifdef DEBUG
  for (i = 0; i < qt_hdr.n_hdr; i++)
...
#endif
    (void)fwrite((genericptr_t) & (msg_hdr[i].n_msg), sizeof(int), 1, ofp);
    (void)fwrite((genericptr_t) & (msg_hdr[i].qt_msg[0]), sizeof(struct qtmsg),
                 msg_hdr[i].n_msg, ofp);
#ifdef DEBUG
    {
      int j;
