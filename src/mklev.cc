...
STATIC_DCL void do_room_or_subroom(struct mkroom *, int, int, int, int, bool,
                                   schar, bool, bool);
...
STATIC_OVL void finddpos(coord *cc, xchar xl, xchar yl, xchar xh, xchar yh) {
  ...
  for (x = xl; x <= xh; x++)
    for (y = yl; y <= yh; y++)
      if (IS_DOOR(levl[x][y].typ) || levl[x][y].typ == SDOOR) goto gotit;
...
STATIC_OVL void do_room_or_subroom(struct mkroom *croom, int lowx, int lowy,
                                   int hix, int hiy, bool lit, schar rtype,
                                   bool special, bool is_room) {
  int x, y;
  struct rm *lev;
  ...
  if (lit) {
    for (x = lowx - 1; x <= hix + 1; x++) {
      ...
      for (y = lowy - 1; y <= hiy + 1; y++) lev++->lit = 1;
    }
    croom->rlit = 1;
  } else
    croom->rlit = 0;
  ...
  if (!special) {
    ...
    for (x = lowx; x <= hix; x++) {
      lev = &levl[x][lowy];
      for (y = lowy; y <= hiy; y++) lev++->typ = ROOM;
    }
    if (is_room) {
...
void add_room(int lowx, int lowy, int hix, int hiy, bool lit, schar rtype,
              bool special) {
  struct mkroom *croom;
...
void add_subroom(struct mkroom *proom, int lowx, int lowy, int hix, int hiy,
                 bool lit, schar rtype, bool special) {
  struct mkroom croom;
  do_room_or_subroom(&croom, lowx, lowy, hix, hiy, lit, rtype, special,
                     (bool)FALSE);
...
STATIC_OVL void join(int a, int b, bool nxcor) {
  ...
  tx = tt.x - dx;
  ty = tt.y - dy;
  if (nxcor && levl[xx + dx][yy + dy].typ) return;
  if (okdoor(xx, yy) || !nxcor) dodoor(xx, yy, croom);
  ...
  if (okdoor(tt.x, tt.y) || !nxcor) dodoor(tt.x, tt.y, troom);
...
void makecorridors() {
  ...
  for (a = 0; a < nroom - 2; a++)
    if (smeq[a] != smeq[a + 2]) join(a, a + 2, FALSE);
      ...
      if (trap_type || !rn2(4)) {
        ...
        if (rn2(7))
          ...
          if (!level.flags.noteleport)
            (void)MakeSpecificObjectAt(SCR_TELEPORTATION, xx, yy + dy, TRUE,
                                       FALSE);
...
STATIC_OVL void makevtele() { makeniche(TELEP_TRAP); }
...
/* check for special levels */
...
#endif
    {
      ...
      strcat(fillname, (player.uz.dlevel < loc_lev->dlevel.dlevel) ? "a" : "b");
      makemaz(fillname);
      return;
...
/* otherwise, fall through - it's a "regular" level. */
  ...
  if (do_vault()) {
    ...
    fill_vault:
      add_room(vault_x, vault_y, vault_x + w, vault_y + h, TRUE, VAULT, FALSE);
...
skip0:
  ...
  for (croom = rooms; croom->hx > 0; croom++) {
    ...
    if (!goldseen && !rn2(3)) (void)MakeGold(0L, somex(croom), somey(croom));
#ifdef REINCARNATION
     ...
     */
    if (!rn2(nroom * 5 / 2))
      (void)MakeSpecificObjectAt((rn2(3)) ? LARGE_BOX : CHEST, somex(croom),
  ...
  for (x = 2; x < (COLNO - 2); x++)
    for (y = 1; y < (ROWNO - 1); y++)
      if ((levl[x][y].typ == POOL && !rn2(10)) ||
        ...
        (void)MakeSpecificObjectAt(KELP_FROND, x, y, TRUE, FALSE);
...
void mklev() {
  ...
  if (level.flags.has_morgue) level.flags.graveyard = 1;
...
#endif
{
  ...
  if ((int)levl[lowx][lowy].roomno == roomno || croom->irregular) return;
#ifdef SPECIALIZATION
#ifdef REINCARNATION
  if (Is_rogue_level(&player.uz))
...
STATIC_OVL struct mkroom *find_branch_room(coord *mp) {
  ...
  if (nroom == 0) {
    ...
    do {
      if (!somexy(croom, mp)) impossible("Can't place branch!");
    } while (occupied(mp->x, mp->y) || (levl[mp->x][mp->y].typ != CORR &&
...
void place_branch(branch *br, xchar x, xchar y) {
  ...
  if (br->type == BR_PORTAL) {
    ...
    sstairs.sy = y;
    sstairs.up =
        (char)on_level(&br->end1, &player.uz) ? br->end1_up : !br->end1_up;
...
bool occupied(xchar x, xchar y) {
  return ((bool)(t_at(x, y) || IS_FURNITURE(levl[x][y].typ) || is_lava(x, y) ||
...
void mktrap(int num, int mazeflag, struct mkroom *croom, coord *tm) {
    ...
    do {
      ...
      switch (kind) {
        ...
        case SLP_GAS_TRAP:
          ...
          break;
        case LEVEL_TELEP:
          if (lvl < 5 || level.flags.noteleport) kind = NO_TRAP;
          break;
        case SPIKED_PIT:
  ...
  if (tm)
    ...
    int tryct = 0;
    bool avoid_boulder =
        (kind == PIT || kind == SPIKED_PIT || kind == TRAPDOOR || kind == HOLE);
  ...
  if (kind == WEB) (void)makemon(&mons[PM_GIANT_SPIDER], m.x, m.y, NO_MM_FLAGS);
}
...
STATIC_OVL
void mkfount(int mazeflag, struct mkroom *croom) {
  coord m;
  int tryct = 0;
...
static void mkgrave(struct mkroom *croom) {
  ...
  if (dobell) (void)MakeSpecificObjectAt(BELL, m.x, m.y, TRUE, FALSE);
  return;
}
...
STATIC_OVL void mkinvpos(xchar x, xchar y, int dist) {
  ...
  if (!within_bounded_area(x, y, x_maze_min + 1, y_maze_min + 1, x_maze_max - 1,
                           y_maze_max - 1)) {
...
STATIC_OVL void mk_knox_portal(xchar x, xchar y) {
  ...
  if (on_level(&knox_level, &br->end1)) {
    ...
    if (Is_branchlev(&player.uz)) return;
    source = &br->end1;
  }
