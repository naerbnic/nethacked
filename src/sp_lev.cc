...
STATIC_DCL void create_trap(trap_info *, struct mkroom *);
...
STATIC_DCL bool search_door(struct mkroom *, xchar *, xchar *, xchar, int);
STATIC_DCL void fix_stair_rooms();
STATIC_DCL void create_corridor(corridor *);

STATIC_DCL bool create_subroom(struct mkroom *, xchar, xchar, xchar, xchar,
                               xchar, xchar);
...
STATIC_DCL void get_location(schar *, schar *, int);
...
STATIC_DCL bool load_rooms(dlb *);
STATIC_DCL void maze1xy(coord *, int);
STATIC_DCL bool load_maze(dlb *);
...
STATIC_DCL bool is_ok_location(schar, schar, int);
...
STATIC_OVL bool create_subroom(struct mkroom *proom, xchar x, xchar y, xchar w,
                               xchar h, xchar rtype, xchar rlit) {
  xchar width, height;
...
STATIC_OVL void create_door(room_door *dd, struct mkroom *broom) {
  ...
  do {
    ...
    if (dpos == -1) /* The position is RANDOM */
      dpos =
          rn2((dwall == W_WEST || dwall == W_EAST) ? (broom->hy - broom->ly)
                                                   : (broom->hx - broom->lx));
...
#ifdef DEBUG
    debugpline("dig_corridor: bad coords : (%d,%d) (%d,%d).", xx, yy, tx, ty);
#endif
    return FALSE;
  }
  if (tx > xx)
...
STATIC_OVL void build_room(room *r, room *pr) {
  ...
  if (pr) {
    okroom = create_subroom(pr->mkr, r->x, r->y, r->w, r->h, rtype, r->rlit);
    aroom = &subrooms.back();
  } else {
    aroom = &rooms[nroom];
    okroom = create_room(r->x, r->y, r->w, r->h, r->xalign, r->yalign, rtype,
                         r->rlit);
    r->mkr = aroom;
  }
...
STATIC_OVL bool load_rooms(dlb *fd) {
  ...
  for (i = 0; i < nrooms; i++) {
    ...
    /* read the engravings */
    Fread((genericptr_t) & r->nengraving, 1, sizeof(r->nengraving), fd);
    if ((n = r->nengraving) != 0) {
  ...
  for (i = 0; i < nrooms; i++)
    if (tmproom[i]->parent) {
      ...
      for (j = 0; j < nrooms; j++)
        if (tmproom[j]->name && !strcmp(tmproom[j]->name, tmproom[i]->parent)) {
    ...
    for (x = 2; x <= x_maze_max; x++)
      for (y = 0; y <= y_maze_max; y++)
        if (filling == -1) {
          ...
          levl[x][y].typ = STONE;
#else
          levl[x][y].typ = (y < 2 || ((x % 2) && (y % 2))) ? STONE : HWALL;
#endif
        } else {
  ...
  while (numpart--) {
      ...
      for (y = ystart; y < ystart + ysize; y++)
        for (x = xstart; x < xstart + xsize; x++) {
          ...
          if (levl[x][y].typ == SDOOR || IS_DOOR(levl[x][y].typ)) {
             ...
             */
            if (x != xstart &&
                (IS_WALL(levl[x - 1][y].typ) || levl[x - 1][y].horizontal))
    ...
    if (n) {
      if (num_lregions) {
        ...
        num_lregions = n;
        lregions = (lev_region *)alloc(sizeof(lev_region) * (unsigned)n);
      }
    }
    ...
    while (n--) {
      ...
      if (!tmplregion.del_islev) {
        get_location(&tmplregion.delarea.x1, &tmplregion.delarea.y1, DRY | WET);
        get_location(&tmplregion.delarea.x2, &tmplregion.delarea.y2, DRY | WET);
      }
    ...
    while (n--) {
      ...
      if (tmpregion.rlit < 0)
        tmpregion.rlit =
            (rnd(1 + abs(depth(&player.uz))) < 11 && rn2(77)) ? TRUE : FALSE;
      ...
      if (tmpregion.rirreg) {
        ...
        flood_fill_rm(tmpregion.x1, tmpregion.y1, nroom + ROOMOFFSET,
                      tmpregion.rlit, TRUE);
        add_room(min_rx, min_ry, max_rx, max_ry, FALSE, tmpregion.rtype, TRUE);
        troom->rlit = tmpregion.rlit;
        troom->irregular = TRUE;
      } else {
        add_room(tmpregion.x1, tmpregion.y1, tmpregion.x2, tmpregion.y2,
    ...
    while (n--) {
      ...
      set_wall_property(tmpdig.x1, tmpdig.y1, tmpdig.x2, tmpdig.y2,
                        W_NONDIGGABLE);
    }
    ...
    while (n--) {
      ...
      create_feature(tmpfountain.x, tmpfountain.y, (struct mkroom *)0,
                     FOUNTAIN);
    }
...
/* no need for IS_DOOR check; out of map bounds */
  ...
  if (nwalk_sav && (mapcount > (int)(mapcountmax / 10))) {
    ...
    for (x = rnd((int)(20 * mapfact) / 100); x; x--) {
      maze1xy(&mm, DRY);
      (void)MakeRandomObjectAt(rn2(2) ? GEM_CLASS : RANDOM_CLASS, mm.x, mm.y,
                               TRUE);
    }
    ...
    for (x = rn2((int)(15 * mapfact) / 100); x; x--) {
      ...
      if (sobj_at(BOULDER, mm.x, mm.y))
        while (trytrap == PIT || trytrap == SPIKED_PIT || trytrap == TRAPDOOR ||
               trytrap == HOLE)
          trytrap = rndtrap();
