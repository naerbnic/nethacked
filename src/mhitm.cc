...
STATIC_DCL char *mon_nam_too(char *, Monster *, Monster *);
STATIC_DCL void mrustm(Monster *, Monster *, Object *);
STATIC_DCL int hitmm(Monster *, Monster *, struct Attack *);
STATIC_DCL int gazemm(Monster *, Monster *, struct Attack *);
STATIC_DCL int gulpmm(Monster *, Monster *, struct Attack *);
STATIC_DCL int explmm(Monster *, Monster *, struct Attack *);
STATIC_DCL int mdamagem(Monster *, Monster *, struct Attack *);
STATIC_DCL void mswingsm(Monster *, Monster *, Object *);
STATIC_DCL void noises(Monster *, struct Attack *);
STATIC_DCL void missmm(Monster *, Monster *, struct Attack *);
STATIC_DCL int passivemm(Monster *, Monster *, bool, int);
...
STATIC_OVL char *mon_nam_too(char *outbuf, Monster *mon, Monster *other_mon) {
  strcpy(outbuf, mon_nam(mon));
  if (mon == other_mon) switch (pronoun_gender(mon)) {
      case 0:
...
STATIC_OVL void noises(Monster *magr, struct Attack *mattk) {
...
STATIC_OVL
void missmm(Monster *magr, Monster *mdef, struct Attack *mattk) {
  const char *fmt;
 ...
 */
int mattackm(Monster *magr, Monster *mdef) {
  int i,          /* loop counter */
  ...
  for (i = 0; i < NATTK; i++) {
    ...
    switch (mattk->aatyp) {
      ...
      case AT_TENT:
        ...
        if (distmin(magr->mx, magr->my, mdef->mx, mdef->my) > 1) return MM_MISS;
      ...
      case AT_HUGS: /* automatic if prev two attacks succeed */
        ...
        if (strike) res[i] = hitmm(magr, mdef, mattk);
...
STATIC_OVL int hitmm(Monster *magr, Monster *mdef, struct Attack *mattk) {
  if (vis) {
    int compat;
    char buf[BUFSZ], mdef_name[BUFSZ];
...
STATIC_OVL int gazemm(Monster *magr, Monster *mdef, struct Attack *mattk) {
  char buf[BUFSZ];
  ...
  if (magr->data == &mons[PM_MEDUSA] && mon_reflects(mdef, (char *)0)) {
    ...
    if (mdef->mcansee) {
      ...
      if (canseemon(magr)) pline("%s is turned to stone!", Monnam(magr));
      monstone(magr);
...
STATIC_OVL int gulpmm(Monster *magr, Monster *mdef, struct Attack *mattk) {
  xchar ax, ay, dx, dy;
  int status;
  char buf[BUFSZ];
  Object *obj;
  ...
  for (obj = mdef->minvent; obj; obj = obj->nobj) (void)snuff_lit(obj);
...
STATIC_OVL int explmm(Monster *magr, Monster *mdef, struct Attack *mattk) {
  int result;
 ...
 */
STATIC_OVL int mdamagem(Monster *magr, Monster *mdef, struct Attack *mattk) {
  Object *obj;
  char buf[BUFSZ];
  MonsterType *pa = magr->data, *pd = mdef->data;
  ...
  switch (mattk->adtyp) {
    case AD_DGST:
      ...
      if (!corpse_chance(mdef, magr, TRUE) || magr->mhp < 1) break;
    ...
    case AD_FIRE:
      ...
      if (vis) pline("%s is %s!", Monnam(mdef), on_fire(mdef->data, mattk));
      ...
      if (resists_fire(mdef)) {
        if (vis) pline_The("fire doesn't seem to burn %s!", mon_nam(mdef));
    ...
    case AD_COLD:
      ...
      if (resists_cold(mdef)) {
        if (vis) pline_The("frost doesn't seem to chill %s!", mon_nam(mdef));
        shieldeff(mdef->mx, mdef->my);
    ...
    case AD_ACID:
      ...
      if (resists_acid(mdef)) {
        if (vis)
          pline("%s is covered in acid, but it seems harmless.", Monnam(mdef));
        tmp = 0;
      } else if (vis) {
    ...
    case AD_BLND:
      if (can_blnd(magr, mdef, mattk->aatyp, (Object *)0)) {
        ...
        if (vis && mdef->mcansee) pline("%s is blinded.", Monnam(mdef));
    ...
    case AD_CURS:
      ...
      if (!magr->mcan && !rn2(10)) {
        ...
        mdef->mstrategy &= ~STRAT_WAITFORU;
        if (is_were(pd) && pd->mlet != S_HUMAN) were_change(mdef);
    ...
    case AD_SGLD:
      ...
      if (!tele_restrict(magr)) {
        ...
        if (vis && !canspotmon(magr)) pline("%s suddenly disappears!", buf);
      }
      break;
    case AD_DRLI:
    ...
    case AD_SEDU:
      ...
      for (obj = mdef->minvent; obj; obj = obj->nobj)
        if (!magr->mtame || !obj->cursed) break;
      ...
      if (obj) {
        ...
        otmp = obj;
#ifdef STEED
        if (player.usteed == mdef && otmp == which_armor(mdef, W_SADDLE))
        ...
        if (otmp->owornmask) {
          ...
          if (otmp->owornmask & W_WEP) setmnotwielded(mdef, otmp);
          otmp->owornmask = 0L;
          update_mon_intrinsics(mdef, otmp, FALSE, FALSE);
        }
        /* AddObjectToMonsterInventory() might free otmp [if it merges] */
        if (vis) strcpy(onambuf, doname(otmp));
...
int sleep_monst(Monster *mon, int amt, int how) {
...
STATIC_OVL void mswingsm(Monster *magr, Monster *mdef, Object *otemp) {
  char buf[BUFSZ];
 ...
 */
STATIC_OVL int passivemm(Monster *magr, Monster *mdef, bool mhit, int mdead) {
  MonsterType *mddat = mdef->data;
  MonsterType *madat = magr->data;
  char buf[BUFSZ];
  int i, tmp;
  ...
  switch (mddat->mattk[i].adtyp) {
    case AD_ACID:
      if (mhit && !rn2(2)) {
        strcpy(buf, Monnam(magr));
        if (canseemon(magr))
          pline("%s is splashed by %s acid!", buf, s_suffix(mon_nam(mdef)));
        if (resists_acid(magr)) {
          if (canseemon(magr)) pline("%s is not affected.", Monnam(magr));
          tmp = 0;
        }
      } else
        tmp = 0;
  ...
  if (rn2(3)) switch (mddat->mattk[i].adtyp) {
      case AD_PLYS: /* Floating eye */
        ...
        if (mddat == &mons[PM_FLOATING_EYE]) {
          ...
          strcpy(buf, Monnam(magr));
          if (canseemon(magr)) pline("%s is frozen by %s.", buf, mon_nam(mdef));
          magr->mcanmove = 0;
          magr->mfrozen = tmp;
          return (mdead | mhit);
        }
      ...
      case AD_COLD:
        ...
        if (canseemon(magr)) pline("%s is suddenly very cold!", Monnam(magr));
        mdef->mhp += tmp / 2;
      ...
      case AD_FIRE:
        ...
        if (canseemon(magr)) pline("%s is suddenly very hot!", Monnam(magr));
        break;
      case AD_ELEC:
