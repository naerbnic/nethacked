...
bool is_home_elemental(MonsterType *ptr) {
  if (ptr->mlet == S_ELEMENTAL) switch (monsndx(ptr)) {
      case PM_AIR_ELEMENTAL:
        return Is_airlevel(&player.uz);
      case PM_FIRE_ELEMENTAL:
        return Is_firelevel(&player.uz);
      case PM_EARTH_ELEMENTAL:
        return Is_earthlevel(&player.uz);
      case PM_WATER_ELEMENTAL:
        return Is_waterlevel(&player.uz);
    }
  return FALSE;
}
...
STATIC_OVL void m_initweap(Monster *mtmp) {
  ...
  switch (ptr->mlet) {
    case S_GIANT:
      if (rn2(2)) (void)mongets(mtmp, (mm != PM_ETTIN) ? BOULDER : CLUB);
      break;
    case S_HUMAN:
      if (is_mercenary(ptr)) {
        int w1 = 0, w2 = 0;
        ...
        if (rn2(2))
          (void)mongets(mtmp, rn2(2) ? ELVEN_MITHRIL_COAT : ELVEN_CLOAK);
        if (rn2(2))
    ...
    case S_ANGEL: {
      ...
      otmp = MakeSpecificObject(
          !rn2(4) || is_lord(ptr) ? SHIELD_OF_REFLECTION : LARGE_SHIELD, FALSE,
          FALSE);
      otmp->cursed = FALSE;
      otmp->oerodeproof = TRUE;
      otmp->spe = 0;
    ...
    case S_HUMANOID:
      if (mm == PM_HOBBIT) {
        ...
        if (!rn2(4)) {
          ...
          if (!rn2(3)) (void)mongets(mtmp, DWARVISH_MITHRIL_COAT);
        } else {
      ...
      switch (mm != PM_ORC_CAPTAIN ? mm : rn2(2) ? PM_MORDOR_ORC
        ...
        default:
          if (mm != PM_ORC_SHAMAN && rn2(2))
            (void)mongets(mtmp, (mm == PM_GOBLIN || rn2(2) == 0) ? ORCISH_DAGGER
                                                                 : SCIMITAR);
      }
      break;
    case S_OGRE:
    ...
    case S_ZOMBIE:
      ...
      if (!rn2(4)) (void)mongets(mtmp, (rn2(3) ? KNIFE : SHORT_SWORD));
      break;
    case S_LIZARD:
      if (mm == PM_SALAMANDER)
        (void)mongets(mtmp, (rn2(7) ? SPEAR : rn2(3) ? TRIDENT : STILETTO));
      break;
    case S_DEMON:
      switch (mm) {
        case PM_BALROG:
  ...
  if ((int)mtmp->m_lev > rn2(75)) (void)mongets(mtmp, rnd_offensive_item(mtmp));
}
...
STATIC_OVL void m_initinv(Monster *mtmp) {
  int cnt;
  Object *otmp;
  MonsterType *ptr = mtmp->data;
#ifdef REINCARNATION
  ...
  switch (ptr->mlet) {
    case S_HUMAN:
      if (is_mercenary(ptr)) {
        ...
        if (mac < -1 && rn2(5))
          mac += 7 + mongets(mtmp, (rn2(5)) ? PLATE_MAIL : CRYSTAL_PLATE_MAIL);
        else if (mac < 3 && rn2(5))
          mac += 6 + mongets(mtmp, (rn2(3)) ? SPLINT_MAIL : BANDED_MAIL);
        else if (rn2(5))
          mac +=
              3 + mongets(mtmp, (rn2(3)) ? RING_MAIL : STUDDED_LEATHER_ARMOR);
        else
        ...
        if (ptr != &mons[PM_GUARD] && ptr != &mons[PM_WATCHMAN] &&
          ...
          if (ptr != &mons[PM_SOLDIER] && !rn2(3)) (void)mongets(mtmp, BUGLE);
        ...
        switch (rn2(4)) {
          ...
          case 3:
            (void)mongets(mtmp, WAN_STRIKING);
                 ...
                 quest_mon_represents_role(ptr, PM_PRIEST)) {
        (void)mongets(mtmp, rn2(7) ? ROBE : rn2(3) ? CLOAK_OF_PROTECTION
    ...
    case S_LICH:
      if (ptr == &mons[PM_MASTER_LICH] && !rn2(13))
        ...
        otmp = MakeSpecificObject(rn2(3) ? ATHAME : QUARTERSTAFF, TRUE,
                                  rn2(13) ? FALSE : TRUE);
  ...
  if ((int)mtmp->m_lev > rn2(50)) (void)mongets(mtmp, rnd_defensive_item(mtmp));
  if ((int)mtmp->m_lev > rn2(100)) (void)mongets(mtmp, rnd_misc_item(mtmp));
#ifndef GOLDOBJ
...
Monster *clone_mon(Monster *mon, xchar x, xchar y) {
  coord mm;
  Monster *m2;
...
Monster *makemon(MonsterType *ptr, int x, int y, int mmflags) {
  ...
  if (ptr) {
    ...
    if (mvitals[mndx].mvflags & G_GENOD) return ((Monster *)0);
  ...
  *mtmp = zeromonst; /* clear all entries in structure */
  ...
  if (mtmp->data->msound == MS_LEADER) quest_status.leader_m_id = mtmp->m_id;
  mtmp->mxlth = xlth;
  mtmp->mnum = mndx;
  ...
  if (is_golem(ptr)) {
    ...
    if (is_home_elemental(ptr)) mtmp->mhpmax = (mtmp->mhp *= 3);
  }
  ...
  switch (ptr->mlet) {
    ...
    case S_SNAKE:
      if (in_mklev)
        if (x && y) (void)MakeRandomObjectAt(0, x, y, TRUE);
      if (hides_under(ptr) && OBJ_AT(x, y)) mtmp->mundetected = TRUE;
      break;
    case S_LIGHT:
    case S_ELEMENTAL:
      ...
      break;
    case S_EEL:
      if (is_pool(x, y)) mtmp->mundetected = TRUE;
      break;
    case S_LEPRECHAUN:
      mtmp->msleeping = 1;
      break;
    ...
    case S_ORC:
      ...
      break;
    case S_UNICORN:
      if (is_unicorn(ptr) && sgn(player.ualign.type) == sgn(ptr->maligntyp))
        mtmp->mpeaceful = TRUE;
      break;
    case S_BAT:
      if (Inhell && is_bat(ptr)) mon_adjust_speed(mtmp, 2, nullptr);
      break;
  }
  if ((ct = emits_light(mtmp->data)) > 0)
    new_light_source(mtmp->mx, mtmp->my, ct, LS_MONSTER, (genericptr_t)mtmp);
  ...
  if ((mcham = pm_to_cham(mndx)) != CHAM_ORDINARY) {
    ...
    mtmp->iswiz = TRUE;
    flags.no_of_wizards++;
    if (flags.no_of_wizards == 1 && Is_earthlevel(&player.uz)) mitem = SPE_DIG;
  } else if (mndx == PM_DJINNI) {
    flags.djinni_count++;
  } else if (mndx == PM_GHOST) {
    flags.ghost_count++;
    if (!(mmflags & MM_NONAME)) mtmp = christen_monst(mtmp, rndghostname());
   ...
   * same expression.
   */
  if (mndx == PM_LONG_WORM && (mtmp->wormno = get_wormno(), mtmp->wormno != 0))
#endif
  {
  ...
  if (allow_minvent) {
    if (is_armed(ptr)) m_initweap(mtmp); /* equip with weapons / armor */
  ...
  if (!in_mklev) newsym(mtmp->mx, mtmp->my); /* make sure the mon shows up */
...
bool create_critters(int cnt, MonsterType *mptr) {
  coord c;
  int x, y;
  Monster *mon;
  bool known = FALSE;
 ...
 */
STATIC_OVL int align_shift(MonsterType *ptr) {
...
/* evidently they've all been exterminated */
    ...
    for (; mndx < SPECIAL_PM; mndx++) {
      ...
      if (ct < 0 || ct > 127) panic("rndmonst: bad count [#%d: %d]", mndx, ct);
...
MonsterType *mkclass(char class_id, int spc) {
  ...
  if (first == SPECIAL_PM) return (MonsterType *)0;
  ...
  for (last = first; last < SPECIAL_PM && mons[last].mlet == class_id; last++)
    if (!(mvitals[last].mvflags & G_GONE) && !(mons[last].geno & mask) &&
      ...
      /* consider it */
      if (num && toostrong(last, maxmlev) && monstr[last] != monstr[last - 1] &&
          rn2(2))
        break;
      num += mons[last].geno & G_FREQ;
    }

  if (!num) return ((MonsterType *)0);
  ...
  for (num = rnd(num); num > 0; first++)
    if (!(mvitals[first].mvflags & G_GONE) && !(mons[first].geno & mask) &&
      ...
      if (num && adj_lev(&mons[first]) > (player.ulevel * 2)) {
        ...
        if (mons[first].mlevel != mons[first + 1].mlevel) num--;
      }
    }
...
int adj_lev(MonsterType *ptr) {
  ...
  if (tmp2 < 0)
    tmp--; /* if mlevel > player.uz decrement tmp */
  else
...
MonsterType *grow_up(Monster *mtmp, Monster *victim) {
  ...
  if (mtmp->mhp <= 0) return ((MonsterType *)0);
  ...
  if ((int)++mtmp->m_lev >= mons[newtype].mlevel && newtype != oldtype) {
    ...
    if (mvitals[newtype].mvflags & G_GENOD) { /* allow G_EXTINCT */
      if (sensemon(mtmp))
        pline("As %s grows up into %s, %s %s!", mon_nam(mtmp), an(ptr->mname),
...
bool peace_minded(MonsterType *ptr) {
  ...
  return (
      (bool)(!!rn2(16 +
                   (player.ualign.record < -15 ? -15 : player.ualign.record)) &&
             !!rn2(2 + abs(mal))));
}
...
/* KAA, modified by ERS */
void set_mimic_sym(Monster *mtmp) {
  int typ, roomno, rt;
  ...
  if (roomno >= 0) rt = rooms[roomno].rtype;
  ...
  if (OBJ_AT(mx, my)) {
     ...
     */
    if (mx != 0 &&
        (levl[mx - 1][my].typ == HWALL || levl[mx - 1][my].typ == TLCORNER ||
         levl[mx - 1][my].typ == TRWALL || levl[mx - 1][my].typ == BLCORNER ||
         levl[mx - 1][my].typ == TDWALL || levl[mx - 1][my].typ == CROSSWALL ||
         levl[mx - 1][my].typ == TUWALL))
      appear = S_hcdoor;
    else
      appear = S_vcdoor;

    if (!mtmp->minvis || See_invisible) block_point(mx, my); /* vision */
    ...
    if (s_sym < 0) {
      ...
      if (s_sym == RANDOM_CLASS) s_sym = syms[rn2((int)sizeof(syms) - 2) + 2];
      goto assign_sym;
    }
  } else {
