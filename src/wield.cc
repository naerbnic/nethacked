...
STATIC_DCL int ready_weapon(Object *);
...
STATIC_OVL int ready_weapon(Object *wep) {
  ...
  if (!wep) {
    ...
    You("wield the %s corpse in your bare %s.", mons[wep->corpsenm].mname,
...
void setuqwep(Object *obj) {
  setworn(obj, W_QUIVER);
  update_inventory();
}

void setuswapwep(Object *obj) {
...
static const char wield_objs[] = {ALL_CLASSES, ALLOW_NONE, WEAPON_CLASS,
                                  TOOL_CLASS, 0};
...
int dowield() {
  ...
  if (!(wep = getobj(wield_objs, "wield"))) /* Cancelled */
    return (0);
  else if (wep == uwep) {
...
int doswapweapon() {
  ...
  if (uwep == oldwep) /* Wield failed for some reason */
    setuswapwep(oldswap);
  else {
    setuswapwep(oldwep);
    if (uswapwep)
...
int dowieldquiver() {
  ...
  const char *quivee_types =
      (uslinging() || (uswapwep && objects[uswapwep->otyp].oc_skill == P_SLING))
          ? bullets
          : ready_objs;
  ...
  if (!(newquiver = getobj(quivee_types, "ready"))) /* Cancelled */
    return (0);
...
/* (moved from apply.c) */
bool wield_tool(Object *obj, const char *verb) {
  const char *what;
  bool more_than_1;
  ...
  if (obj->owornmask & (W_ARMOR | W_RING | W_AMUL | W_TOOL)) {
    ...
    You_cant("%s %s %s while wearing %s.", verb, shk_your(yourbuf, obj), what,
             more_than_1 ? "them" : "it");
    return FALSE;
  }
  if (welded(uwep)) {
...
void erode_obj(Object *target, bool acid_dmg, bool fade_scrolls) {
  int erosion;
  Monster *victim;
  bool vismon;
  bool visobj;
  ...
  if (target->greased) {
    ...
    if (victim == &youmonst)
      Your(
          "%s%s!", aobjnam(target, acid_dmg ? "corrode" : "rust"),
          erosion + 1 == MAX_ERODE ? " completely" : erosion ? " further" : "");
    else if (vismon)
      pline(
          "%s's %s%s!", Monnam(victim),
    ...
    if (flags.verbose) {
      if (victim == &youmonst)
        Your("%s completely %s.", aobjnam(target, Blind ? "feel" : "look"),
             acid_dmg ? "corroded" : "rusty");
      else if (vismon)
        pline("%s's %s completely %s.", Monnam(victim), aobjnam(target, "look"),
              acid_dmg ? "corroded" : "rusty");
      else if (visobj)
        pline_The("%s completely %s.", aobjnam(target, "look"),
                  acid_dmg ? "corroded" : "rusty");
    }
  }
}

int chwepon(Object *otmp, int amount) {
  ...
  if (((uwep->spe > 5 && amount >= 0) || (uwep->spe < -5 && amount < 0)) &&
      rn2(3)) {
    if (!Blind)
      Your("%s %s for a while and then %s.", aobjnam(uwep, "violently glow"),
           color, otense(uwep, "evaporate"));
    else
  ...
  if (!Blind) {
    ...
    Your("%s %s for a %s.",
         aobjnam(uwep, amount == 0 ? "violently glow" : "glow"), color, xtime);
