...
void vision_recalc(int control) {
  ...
  if (player.uswallow || control == 2) {
     ...
     */
    view_from(player.uy, player.ux, next_array, next_rmin, next_rmax, 0,
              (void (*)(int, int, genericptr_t))0, (genericptr_t)0);
    ...
    if (player.xray_range >= 0) {
      if (player.xray_range) {
        ...
        for (row = player.uy - player.xray_range;
             row <= player.uy + player.xray_range; row++) {
          if (row < 0) continue;
          if (row >= ROWNO) break;
          dy = v_abs(player.uy - row);
          next_row = next_array[row];
    ...
    if (has_night_vision && player.xray_range < player.nv_range) {
      if (!player.nv_range) { /* range is 0 */
        ...
        for (row = player.uy - player.nv_range;
             row <= player.uy + player.nv_range; row++) {
          if (row < 0) continue;
          if (row >= ROWNO) break;
          dy = v_abs(player.uy - row);
          next_row = next_array[row];
  ...
  for (row = 0; row < ROWNO; row++) {
    ...
    sv = &seenv_matrix[dy + 1][start < player.ux ? 0
                                                 : (start > player.ux ? 2 : 1)];

    for (col = start; col <= stop; lev += ROWNO, sv += (int)colbump[++col]) {
      if (next_row[col] & IN_SIGHT) {
         ...
         */
        if ((IS_DOOR(lev->typ) || lev->typ == SDOOR || IS_WALL(lev->typ)) &&
            !viz_clear[row][col]) {
          /*
      ...
      not_in_sight:
        if ((old_row[col] & IN_SIGHT) ||
            ((next_row[col] & COULD_SEE) ^ (old_row[col] & COULD_SEE)))
          newsym(col, row);
      }
...
STATIC_OVL void dig_point(int row, int col) {
  ...
  if (col == 0) { /* left edge */
    if (viz_clear[row][1]) {
      ...
      right_ptrs[row][0] = 1;
      for (i = 1; i <= right_ptrs[row][1]; i++) left_ptrs[row][i] = 1;
    }
...
STATIC_OVL void fill_point(int row, int col) {
  ...
  if (col == 0) {
    if (viz_clear[row][1]) { /* adjacent is clear */
      ...
      right_ptrs[row][0] = right_ptrs[row][1];
      for (i = 1; i <= right_ptrs[row][1]; i++) left_ptrs[row][i] = 0;
    }
  } else if (col == COLNO - 1) {
   ...
   */
    ...
    for (i = col; i < right_ptrs[row][col + 1]; i++) left_ptrs[row][i] = col;
    ...
    /* Right side clear, left side blocked. */
    for (i = left_ptrs[row][col - 1]; i <= col; i++) right_ptrs[row][i] = col;
...
#define set_min(z) \
  if (*row_min > (z)) *row_min = (z)
#define set_max(z) \
  if (*row_max < (z)) *row_max = (z)
...
STATIC_OVL void view_init() {
  ...
  for (i = 0; i < CLOSE_MAX_BC_DY; i++) close_dy[i] = &close_table[i];

  for (i = 0; i < FAR_MAX_BC_DY; i++) far_dy[i] = &far_table[i];
}
...
STATIC_OVL int close_shadow(int side, int this_row, int block_row,
                            int block_col) {
  ...
  if ((sdy = (start_row - block_row)) < 0) sdy = -sdy;
  --sdy;                                               /* src   dy */
  ...
  if (side == FROM_RIGHT) return block_col + offset;

  return block_col - offset;
}

STATIC_OVL int far_shadow(int side, int this_row, int block_row,
                          int block_col) {
  int sdy, sdx, pdy, offset;
  ...
  if ((sdx = (start_col - block_col)) < 0) sdx = -sdx;
  --sdx; /* src   dx */
  if ((pdy = (block_row - this_row)) < 0) pdy = -pdy;
  --pdy; /* point dy */
 ...
 */
STATIC_OVL void right_side(int row, int cb_row, int cb_col, int fb_row,
                           int fb_col, int left, int right_mark, char *limits) {
  int i;
  char *rowp;
  int hit_stone = 0;
  ...
  while (left <= right_mark) {
    ...
    if ((loc_right < right_shadow) ||
      ...
      if (deeper) {
        if (hit_stone)
          right_side(nrow, row, lblock_col, row, loc_right, left, loc_right,
                     limits);
        else
          right_side(nrow, cb_row, cb_col, row, loc_right, left, loc_right,
                     limits);
      }
     ...
     */
      ...
      if (deeper) {
        if (hit_stone)
          right_side(nrow, row, lblock_col, fb_row, fb_col, left, right_shadow,
                     limits);
        else
          right_side(nrow, cb_row, cb_col, fb_row, fb_col, left, right_shadow,
                     limits);
      }
 ...
 */
STATIC_OVL void left_side(int row, int cb_row, int cb_col, int fb_row,
                          int fb_col, int left_mark, int right, char *limits) {
  int i;
  char *rowp;
  int hit_stone = 0;
  ...
  while (right >= left_mark) {
    ...
    if ((loc_left > left_shadow) || (fb_row >= 0 && loc_left == left_shadow)) {
      if (vis_func) {
        for (i = loc_left; i <= right; i++) (*vis_func)(i, row, varg);
      } else {
        for (i = loc_left; i <= right; i++) set_cs(rowp, i);
      ...
      if (deeper) {
        if (hit_stone)
          left_side(nrow, row, lblock_col, row, loc_left, loc_left, right,
                    limits);
        else
          left_side(nrow, cb_row, cb_col, row, loc_left, loc_left, right,
                    limits);
      }
    ...
    /*  The opening extends beyond the left mark. */
      ...
      if (deeper) {
        if (hit_stone)
          left_side(nrow, row, lblock_col, fb_row, fb_col, left_shadow, right,
                    limits);
        else
          left_side(nrow, cb_row, cb_col, fb_row, fb_col, left_shadow, right,
                    limits);
      }
 ...
 */
STATIC_OVL void view_from(srow, scol, loc_cs_rows, left_most, right_most, range,
                          func, arg) int srow,
...
genericptr_t arg;
{
  ...
  if (viz_clear_rows[srow][scol]) {
    ...
    left = (!scol) ? 0
                   : (viz_clear_rows[srow][scol - 1] ? left_ptrs[srow][scol - 1]
                                                     : scol - 1);
    right = (scol == COLNO - 1) ? COLNO - 1 : (viz_clear_rows[srow][scol + 1]
  ...
  if ((nrow = srow + 1) < ROWNO) {
    ...
    if (scol) left_side(nrow, -1, scol, left_row, left, left, scol, limits);
  }
  ...
  while (left <= right_mark) {
    ...
    if (!is_clear(row, left)) {
      ...
      if (right_edge > right_mark) {
         ...
         */
        right_edge =
            is_clear(row - step, right_mark) ? right_mark + 1 : right_mark;
      }
      if (vis_func) {
  ...
  while (right >= left_mark) {
    ...
    if (!is_clear(row, right)) {
      ...
      if (left_edge < left_mark) {
        ...
        left_edge = is_clear(row - step, left_mark) ? left_mark - 1 : left_mark;
      }
      if (vis_func) {
...
STATIC_OVL void view_from(
  ...
  if (is_clear(srow, scol)) {
     ...
     */
    left = (!scol) ? 0 : (is_clear(srow, scol - 1) ? left_ptrs[srow][scol - 1]
                                                   : scol - 1);
    right = (scol == COLNO - 1) ? COLNO - 1 : (is_clear(srow, scol + 1)
                                                   ? right_ptrs[srow][scol + 1]
                                                   : scol + 1);
  }
...
void do_clear_area(int scol, int srow, int range,
  ...
  if (scol != player.ux || srow != player.uy)
    ...
    if (vision_full_recalc) vision_recalc(0); /* recalc vision if dirty */
