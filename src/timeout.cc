      ...
      switch (upp - player.uprops) {
        ...
        case INVIS:
          ...
          if (!Invis && !BInvis && !Blind) {
            You(!See_invisible ? "are no longer invisible."
...
void attach_egg_hatch_timeout(Object *egg) {
  ...
  for (i = (MAX_EGG_HATCH_TIME - 50) + 1; i <= MAX_EGG_HATCH_TIME; i++)
    if (rnd(i) > 150) {
      /* egg will hatch */
      (void)start_timer((long)i, TIMER_OBJECT, HATCH_EGG, (genericptr_t)egg);
      break;
    }
}
...
void hatch_egg(genericptr_t arg, long timeout) {
  ...
  if (get_obj_location(egg, &x, &y, 0)) {
	 ...
	 * set up for it...
	 */
	else if (Object->where == OBJ_MIGRATING) {
	    /*
  ...
  if (mon) {
    ...
    switch (egg->where) {
      case OBJ_INVENT:
        ...
        if (!cansee_hatchspot)
          ...
          You("see %s %s out of your pack!", monnambuf,
              locomotion(mon->data, "drop"));
        if (yours) {
          pline("%s cries sound like \"%s%s\"", siblings ? "Their" : "Its",
      ...
      case OBJ_MINVENT:
        if (cansee_hatchspot) {
          ...
          You("see %s %s out of %s!", monnambuf, locomotion(mon->data, "drop"),
              carriedby);
        }
        break;
#if 0
    ...
    if (egg->quan > 0) {
      ...
      if (egg->timed) {
        ...
        (void)stop_timer(HATCH_EGG, (genericptr_t)egg);
        (void)start_timer((long)rnd(12), TIMER_OBJECT, HATCH_EGG,
                          (genericptr_t)egg);
      }
    } else if (carried(egg)) {
      useup(egg);
...
STATIC_OVL void slip_or_trip() {
  ...
  if (otmp && on_foot) { /* trip over something in particular */
    ...
    what = !otmp->nexthere
               ...
               : (otmp->dknown || !Blind)
                     ? doname(otmp)
                     : ((otmp = sobj_at(ROCK, player.ux, player.uy)) == 0
                            ? something
            ...
            switch (obj->where) {
              ...
              case OBJ_MINVENT:
                pline("%s %scandle%s getting short.", whose,
                      menorah ? "candelabrum's " : "", many ? "s are" : " is");
                break;
              case OBJ_FLOOR:
            ...
            if (menorah) {
              ...
              switch (obj->where) {
                ...
                case OBJ_FLOOR:
                  ...
                  */
                  You("see %s%s consumed!", many ? "some " : "",
...
STATIC_DCL TimerElement *remove_timer(TimerElement **, short, genericptr_t);
...
STATIC_OVL const char *kind_name(short kind) {
  switch (kind) {
    case TIMER_LEVEL:
      return "level";
    case TIMER_GLOBAL:
...
#ifdef VERBOSE_TIMER
      sprintf(buf, " %4ld   %4ld  %-6s %s(%s)", curr->timeout, curr->tid,
              kind_name(curr->kind), timeout_funcs[curr->func_index].name,
              fmt_ptr((genericptr_t)curr->arg, arg_address));
#else
      sprintf(buf, " %4ld   %4ld  %-6s #%d(%s)", curr->timeout, curr->tid,
              kind_name(curr->kind), curr->func_index,
              fmt_ptr((genericptr_t)curr->arg, arg_address));
#endif
      putstr(win, 0, buf);
    }
...
STATIC_OVL TimerElement *remove_timer(TimerElement **base, short func_index,
                                      genericptr_t arg) {
...
STATIC_OVL bool timer_is_local(TimerElement *timer) {
  switch (timer->kind) {
    ...
    case TIMER_GLOBAL:
      return FALSE;
    case TIMER_OBJECT:
      return obj_is_local((Object *)timer->arg);
    case TIMER_MONSTER:
      return mon_is_local((Monster *)timer->arg);
  }
