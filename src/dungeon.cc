...
STATIC_DCL void dumpit();

STATIC_OVL void dumpit() {
  int i;
  s_level *x;
  branch *br;
  ...
  for (x = sp_levchn; x; x = x->next) {
    ...
    fprintf(stderr, "flags:%s%s%s%s\n",
            ...
            x->flags.hellish ? " hellish" : "", x->flags.town ? " town" : "");
    getchar();
  }
  fprintf(stderr, "\nBranches:\n");
  for (br = branches; br; br = br->next) {
    fprintf(stderr, "%d: %s, end1 %d %d, end2 %d %d, %s\n", br->id,
            br->type == BR_STAIR ? "stair" : br->type == BR_NO_END1
                                                 ? "no end1"
                                                 : br->type == BR_NO_END2
                                                       ? "no end2"
                                                       : br->type == BR_PORTAL
                                                             ? "portal"
                                                             : "unknown",
            br->end1.dnum, br->end1.dlevel, br->end2.dnum, br->end2.dlevel,
...
void save_dungeon(int fd, bool perform_write, bool free_data) {
  ...
  if (perform_write) {
    ...
    for (count = 0, curr = branches; curr; curr = curr->next) count++;
...
STATIC_OVL int find_branch(const char *s, struct proto_dungeon *pd) {
  int i;
 ...
 */
STATIC_OVL xchar parent_dnum(const char *s, struct proto_dungeon *pd) {
  int i;
  xchar pdnum;
  ...
  for (pdnum = 0; strcmp(pd->tmpdungeon[pdnum].name, s); pdnum++)
    if ((i -= pd->tmpdungeon[pdnum].branches) < 0) return (pdnum);
...
STATIC_OVL int level_range(xchar dgn, int base, int rand, int chain,
  ...
  if (base < 1 || base > lmax) panic("level_range: base value out of range");
...
STATIC_OVL xchar parent_dlevel(const char *s, struct proto_dungeon *pd) {
  ...
  i = find_branch(s, pd);
  num = level_range(dnum, pd->tmpbranch[i].lev.base, pd->tmpbranch[i].lev.rand,
...
STATIC_OVL branch *add_branch(int dgn, int child_entry_level,
...
STATIC_OVL void init_level(int dgn, int proto_index, struct proto_dungeon *pd) {
  ...
  pd->final_lev[proto_index] = new_level = (s_level *)alloc(sizeof(s_level));
...
STATIC_OVL int possible_places(int idx, bool *map, struct proto_dungeon *pd) {
  ...
  count = level_range(lev->dlevel.dnum, pd->tmplevel[idx].lev.base,
                      ...
                      &start);
  for (i = start; i < start + count; i++) map[i] = TRUE;
...
STATIC_OVL xchar pick_level(bool *map, int nth) {
  int i;
...
void init_dungeons() {
  ...
  if (n_dgns >= MAXDUNGEON) panic("init_dungeons: too many dungeons");
  ...
  for (i = 0; i < n_dgns; i++) {
    ...
    if (!wizard)
      ...
      if (pd.tmpdungeon[i].chance && (pd.tmpdungeon[i].chance <= rn2(100))) {
        ...
        for (j = 0; j < pd.tmpdungeon[i].branches; j++)
          Fread((genericptr_t) & pd.tmpbranch[cb], sizeof(struct tmpbranch), 1,
                dgn_file);
        n_dgns--;
        i--;
        continue;
    ...
    if (pd.tmpdungeon[i].lev.rand)
      dungeons[i].num_dunlevs =
          (xchar)rn1(pd.tmpdungeon[i].lev.rand, pd.tmpdungeon[i].lev.base);
    else
    ...
    if (dungeons[i].num_dunlevs > MAXLEVEL) dungeons[i].num_dunlevs = MAXLEVEL;

    pd.start = pd.n_levs; /* save starting point */
    pd.n_levs += pd.tmpdungeon[i].levels;
    if (pd.n_levs > LEV_LIMIT) panic("init_dungeon: too many special levels");
    /*
    ...
    if (pd.n_brs > BRANCH_LIMIT) panic("init_dungeon: too many branches");
    for (; cb < pd.n_brs; cb++)
      Fread((genericptr_t) & pd.tmpbranch[cb], sizeof(struct tmpbranch), 1,
            dgn_file);
  }
...
xchar dunlev(d_level *lev) { return (lev->dlevel); }
...
xchar deepest_lev_reached(bool noquest) {
...
branch *Is_branchlev(d_level *lev) {
  ...
  for (curr = branches; curr; curr = curr->next) {
    if (on_level(lev, &curr->end1) || on_level(lev, &curr->end2)) return curr;
  }
  return (branch *)0;
}
...
void prev_level(bool at_stairs) {
  if (at_stairs && player.ux == sstairs.sx && player.uy == sstairs.sy) {
    ...
    if (!player.uz.dnum && player.uz.dlevel == 1 && !player.uhave.amulet)
      done(ESCAPED);
    else
      goto_level(&sstairs.tolev, at_stairs, FALSE, FALSE);
  } else {
...
void u_on_newpos(int x, int y) {
  ...
  if (player.usteed)
    player.usteed->mx = player.ux, player.usteed->my = player.uy;
#endif
}

void u_on_sstairs() { /* place you on the special staircase */
...
bool Can_dig_down(d_level *lev) {
  return ((bool)(!level.flags.hardfloor && !Is_botlevel(lev) &&
                 !Invocation_lev(lev)));
}
...
bool In_mines(d_level *lev) { return ((bool)(lev->dnum == mines_dnum)); }
...
bool In_V_tower(d_level *lev) { return ((bool)(lev->dnum == tower_dnum)); }
...
bool In_W_tower(int x, int y, d_level *lev) {
   ...
   */
  if (dndest.nlx > 0)
    return (bool)within_bounded_area(x, y, dndest.nlx, dndest.nly, dndest.nhx,
                                     dndest.nhy);
  else
...
STATIC_OVL void print_branch(winid win, int dnum, int lower_bound,
                             int upper_bound, bool bymenu,
  ...
  for (br = branches; br; br = br->next) {
    if (br->end1.dnum == dnum && lower_bound < br->end1.dlevel &&
      ...
      if (bymenu) {
        ...
        add_menu(win, NO_GLYPH, &any, lchoices->menuletter, 0, ATR_NONE, buf,
                 MENU_UNSELECTED);
...
schar print_dungeon(bool bymenu, schar *rlev, xchar *rdgn) {
  ...
  for (i = 0, dptr = dungeons; i < n_dgns; i++, dptr++) {
    ...
    for (slev = sp_levchn, last_level = 0; slev; slev = slev->next) {
      ...
      if (Is_stronghold(&slev->dlevel)) sprintf(eos(buf), " (tune %s)", tune);
      if (bymenu) {
