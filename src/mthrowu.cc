...
#define URETREATING(x, y) \
  (distmin(player.ux, player.uy, x, y) > distmin(player.ux0, player.uy0, x, y))
...
STATIC_OVL const char *breathwep[] = {
    "fragments",              "fire",             "frost",      "sleep gas",
    ...
    if (mtmp->mhp < 1) {
      if (vis || verbose)
        pline("%s is %s!", Monnam(mtmp),
              (nonliving(mtmp->data) || !canspotmon(mtmp)) ? "destroyed"
                                                           : "killed");
...
void m_throw(Monster *mon, int x, int y, int dx, int dy, int range,
             Object *obj) {
  Monster *mtmp;
  Object *singleobj;
  char sym = obj->oclass;
    ...
    if ((mtmp = m_at(bhitpos.x, bhitpos.y)) != 0) {
      ...
      if (singleobj->oclass == GEM_CLASS &&
        ...
        if (singleobj->otyp > LAST_GEM) {
          ...
          You("are not interested in %s junk.", s_suffix(mon_nam(mon)));
          makeknown(singleobj->otyp);
          dropy(singleobj);
        } else {
          You("accept %s gift in the spirit in which it was intended.",
              s_suffix(mon_nam(mon)));
          (void)hold_another_object(singleobj, "You catch, but drop, %s.",
      ...
      if (hitu && can_blnd(nullptr, &youmonst,
        ...
        if (singleobj->otyp == CREAM_PIE) {
          if (!Blind)
            ...
            pline("There's %s sticky all over your %s.", something,
                  body_part(FACE));
  ...
  if (lined_up(mtmp)) {
    ...
    if (!mtmp->mspec_used && rn2(3)) {
      if ((typ >= AD_MAGM) && (typ <= AD_ACID)) {
        ...
        buzz((int)(-20 - (typ - 1)), (int)mattk->damn, mtmp->mx, mtmp->my,
             sgn(tbx), sgn(tby));
        nomul(0, 0);
...
bool linedup(xchar ax, xchar ay, xchar bx, xchar by) {
  ...
  if ((!tbx || !tby || abs(tbx) == abs(tby)) /* straight line or diagonal */
    ...
    if (ax == player.ux && ay == player.uy)
      return ((bool)(couldsee(bx, by)));
 ...
 */
Object *m_carrying(Monster *mtmp, int type) {
  Object *otmp;
  ...
  if (!hits)
    switch (otmp->oclass) {
      case WEAPON_CLASS: {
        ...
        hits =
            (oskill != -P_BOW && oskill != -P_CROSSBOW && oskill != -P_DART &&
             oskill != -P_SHURIKEN && oskill != P_SPEAR &&
             oskill != P_JAVELIN && oskill != P_KNIFE); /* but not dagger */
        break;
      }
      case ARMOR_CLASS:
...
#endif
                obj_type != TALLOW_CANDLE && obj_type != WAX_CANDLE &&
                obj_type != LENSES && obj_type != TIN_WHISTLE &&
      ...
      case FOOD_CLASS:
        if (obj_type == CORPSE && mons[otmp->corpsenm].msize > MZ_TINY)
          ...
          hits = (obj_type == MEAT_STICK || obj_type == HUGE_CHUNK_OF_MEAT);
        break;
      case SPBOOK_CLASS:
      case WAND_CLASS:
      case BALL_CLASS:
