...
STATIC_OVL bool rm_waslit() {
  ...
  if (levl[player.ux][player.uy].typ == ROOM &&
      levl[player.ux][player.uy].waslit)
    return (TRUE);
 ...
 */
STATIC_OVL void mkcavepos(xchar x, xchar y, int dist, bool waslit,
                          bool rockit) {
  struct rm *lev;
...
STATIC_OVL void mkcavearea(bool rockit) {
  ...
  if (rockit)
    ...
    pline(
        "A mysterious force %s cave around you!",
...
STATIC_OVL int dig() {
  ...
  if (Fumbling && !rn2(3)) {
    switch (rn2(3)) {
      case 0:
...
#ifdef STEED
          if (player.usteed)
            Your("%s %s and %s %s!", xname(uwep), otense(uwep, "bounce"),
                 otense(uwep, "hit"), mon_nam(player.usteed));
          else
#endif
        ...
        break;
      case 1:
        pline("Bang!  You hit with the broad side of %s!", the(xname(uwep)));
        break;
      default:
  ...
  if (digging.effort > 100) {
    ...
    if (Is_earthlevel(&player.uz) && !rn2(3)) {
      ...
      switch (rn2(2)) {
        case 0:
          mtmp = makemon(&mons[PM_EARTH_ELEMENTAL], dpx, dpy, NO_MM_FLAGS);
          break;
        default:
...
schar fillholetyp(int x, int y) {
  int x1, y1;
  int lo_x = max(1, x - 1), hi_x = min(x + 1, COLNO - 1), lo_y = max(0, y - 1),
...
void digactualhole(int x, int y, Monster *madeby, int ttyp) {
  ...
  if (ttyp == PIT) {
    ...
    if (madeby_u)
      ...
      pline("%s digs a hole through the %s.", Monnam(madeby), surface_type);
    else if (cansee(x, y) && flags.verbose)
...
STATIC_OVL void dig_up_grave() {
  ...
  switch (rn2(5)) {
    ...
    case 3:
      if (!Blind)
        pline(Hallucination ? "I want my mummy!" : "You've disturbed a tomb!");
...
int use_pick_axe(Object *obj) {
  ...
  while (*sdp) {
    ...
    if (player.dz > 0 || (player.dz == 0 && isok(rx, ry) &&
  ...
  if (player.uswallow && attack(player.ustuck)) {
    ...
    if (dig_target == DIGTYP_UNDIGGABLE) {
      ...
      if (trap && trap->ttyp == WEB) {
        ...
        Your("%s entangled in the web.", aobjnam(obj, "become"));
        ...
        You("need a pick to dig rock.");
               ...
               (sobj_at(STATUE, rx, ry) || sobj_at(BOULDER, rx, ry))) {
        bool vibrate = !rn2(3);
      ...
      if (digging.pos.x != rx || digging.pos.y != ry ||
        ...
        if (flags.autodig && dig_target == DIGTYP_ROCK && !digging.down &&
            digging.pos.x == player.ux && digging.pos.y == player.uy &&
 ...
 */
void watch_dig(Monster *mtmp, xchar x, xchar y, bool zap) {
  struct rm *lev = &levl[x][y];
...
bool mdig_tunnel(Monster *mtmp) {
  ...
  if (IS_WALL(here->typ)) {
    ...
    if (level.flags.is_maze_lev) {
      here->typ = ROOM;
    } else if (level.flags.is_cavernous_lev && !in_town(mtmp->mx, mtmp->my)) {
      here->typ = CORR;
    } else {
      here->typ = DOOR;
      here->doormask = D_NODOOR;
    ...
    here->typ = CORR;
    if (pile && pile < 5)
      (void)MakeSpecificObjectAt((pile == 1) ? BOULDER : ROCK, mtmp->mx,
                                 mtmp->my, TRUE, FALSE);
  }
  newsym(mtmp->mx, mtmp->my);
...
Object *bury_an_obj(Object *otmp) {
  ...
  if (otmp->otyp == CORPSE) {
    ...
    (void)start_timer((under_ice ? 0L : 250L) + (long)rnd(250), TIMER_OBJECT,
                      ROT_ORGANIC, (genericptr_t)otmp);
  }
...
void rot_corpse(genericptr_t arg, long timeout) {
  xchar x = 0, y = 0;
  Object *obj = (Object *)arg;
  bool on_floor = obj->where == OBJ_FLOOR, in_invent = obj->where == OBJ_INVENT;
  ...
  if (on_floor) {
    ...
    if (flags.verbose) {
      ...
      Your("%s%s %s away%c", obj == uwep ? "wielded " : nul, cname,
