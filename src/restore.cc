...
STATIC_OVL void find_lev_obj() {
  ...
  for (x = 0; x < COLNO; x++)
    for (y = 0; y < ROWNO; y++) level.objects[x][y] = nullptr;
...
STATIC_OVL void restdamage(int fd, bool ghostly) {
  ...
  while (--counter >= 0) {
    ...
    if (ghostly) tmp_dam->when += (monstermoves - omoves);
    ...
    if (player.uz.dlevel) {
      ...
      for (shp = damaged_shops; *shp; shp++) {
        ...
        if (shkp && inhishop(shkp) && repair_damage(shkp, tmp_dam, TRUE)) break;
      }
    }
    if (!shp || !*shp) {
...
STATIC_OVL Monster *restmonchn(int fd, bool ghostly) {
  ...
  while (1) {
    ...
    if (mtmp->minvent) {
      ...
      for (obj = mtmp->minvent; obj; obj = obj->nobj) obj->ocarry = mtmp;
    }
    if (mtmp->mw) {
      Object *obj;
...
bool restgamestate(int fd, unsigned int *stuckid, unsigned int *steedid) {
  ...
  for (otmp = invent; otmp; otmp = otmp->nobj)
    if (otmp->owornmask) setworn(otmp, otmp->owornmask);
  ...
  restore_oracles(fd);
  if (player.ustuck) mread(fd, (genericptr_t)stuckid, sizeof(*stuckid));
#ifdef STEED
  if (player.usteed) mread(fd, (genericptr_t)steedid, sizeof(*steedid));
#endif
 ...
 * don't dereference a wild player.ustuck when saving the game state, for
 * instance)
 */
...
int dorecover(int fd) {
  ...
  if (!wizard && !discover) (void)delete_savefile();
#ifdef REINCARNATION
...
/* CHECK:  This may prevent restoration */
  ...
  if (player.uz.dlevel) {
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp2) {
      ...
      if (ghostly) {
        ...
        if (!mtmp->isshk) /* shopkeepers will reset based on name */
  ...
  ftrap = 0;
  while (trap = newtrap(), mread(fd, (genericptr_t)trap, sizeof(Trap)),
  ...
  for (x = 0; x < COLNO; x++)
    for (y = 0; y < ROWNO; y++) level.monsters[x][y] = (Monster *)0;
  ...
  if (ghostly) {
    ...
    if (lev > ledger_no(&medusa_level) && lev < ledger_no(&stronghold_level) &&
        xdnstair == 0) {
      coord cc;
      ...
      switch (br->type) {
        ...
        case BR_PORTAL: /* max of 1 portal per level */
        {
          ...
          for (ttmp = ftrap; ttmp; ttmp = ttmp->ntrap)
            if (ttmp->ttyp == MAGIC_PORTAL) break;
...
static int mgetc() {
  if (inbufp >= inbufsz) {
    ...
    if (!inbufsz) {
      if (inbufp > sizeof inbuf) error("EOF on file #%d.\n", mreadfd);
