...
STATIC_DCL int dog_goal(Monster *, struct edog *, int, int, int);

STATIC_DCL Object *DROPPABLES(Monster *);
STATIC_DCL bool can_reach_location(Monster *, xchar, xchar, xchar, xchar);
STATIC_DCL bool could_reach_item(Monster *, xchar, xchar);

STATIC_OVL Object *DROPPABLES(Monster *mon) {
  Object *obj;
  Object *wep = mon->weapon();
  bool item1 = FALSE, item2 = FALSE;

  if (is_animal(mon->data) || mindless(mon->data)) item1 = item2 = TRUE;
  if (!tunnels(mon->data) || !needspick(mon->data)) item1 = TRUE;
...
int dog_nutrition(Monster *mtmp, Object *obj) {
  int nutrit;
...
int dog_eat(Monster *mtmp, Object *obj, int x, int y, bool devour) {
...
#else
    edog->apport +=
        (int)(200L / ((long)edog->dropdist + monstermoves - edog->droptime));
#endif
  ...
  if (grow && (int)mtmp->m_lev < (int)mtmp->data->mlevel + 15) {
    if (!grow_up(mtmp, (Monster *)0)) return 2;
  }
 ...
 */
STATIC_OVL int dog_invent(Monster *mtmp, struct edog *edog, int udist) {
  int omx, omy;
  Object *obj;
        ...
        if (rn2(20) < edog->apport + 3) {
          if (rn2(udist) || !rn2(edog->apport)) {
            if (cansee(omx, omy) && flags.verbose)
              pline("%s picks up %s.", Monnam(mtmp), distant_name(obj, doname));
 ...
 */
STATIC_OVL int dog_goal(Monster *mtmp, struct edog *edog, int after, int udist,
                        int whappr) {
  int omx, omy;
  ...
  if (mtmp == player.usteed) return (-2);
#endif
...
#define SQSRCHRADIUS 5
  ...
  if (gtyp == UNDEF ||
    ...
    gx = player.ux;
    gy = player.uy;
    if (after && udist <= 4 && gx == player.ux && gy == player.uy) return (-2);
    appr = (udist >= 9) ? 1 : (mtmp->mflee) ? -1 : 0;
    if (udist > 1) {
      if (!IS_ROOM(levl[player.ux][player.uy].typ) || !rn2(4) || whappr ||
...
int dog_move(Monster *mtmp, int after) {
...
#define GDIST(x, y) (dist2(x, y, gx, gy))
  ...
  appr =
      dog_goal(mtmp, has_edog ? edog : (struct edog *)0, after, udist, whappr);
  if (appr == -2) return (0);
    ...
    if (!has_edog) {
      ...
      while (i--) {
        ...
        if (enexto(&mm, mm.x, mm.y, &mons[PM_ANGEL]))
          (void)mk_roamer(&mons[PM_ANGEL], player.ualign.type, mm.x, mm.y,
                          FALSE);
      }
      return (2);
    }
  }
  if (!Conflict && !mtmp->mconf && mtmp == player.ustuck &&
  ...
  for (i = 0; i < cnt; i++) {
    ...
    if (!has_edog && (j = distu(nx, ny)) > 16 && j >= udist) continue;
    ...
    if ((info[i] & ALLOW_M) && MON_AT(nx, ny)) {
      ...
      if ((int)mtmp2->m_lev >= (int)mtmp->m_lev + 2 ||
          (mtmp2->data == &mons[PM_FLOATING_EYE] && rn2(10) && mtmp->mcansee &&
           haseyes(mtmp->data) && mtmp2->mcansee &&
...
newdogpos:
  if (nix != omx || niy != omy) {
    ...
    if (!m_in_out_region(mtmp, nix, niy)) return 1;
    if (((IS_ROCK(levl[nix][niy].typ) && may_dig(nix, niy)) ||
      ...
      if (mtmp->weapon_check >= NEED_PICK_AXE && mon_wield_item(mtmp)) return 0;
    }
...
STATIC_OVL bool could_reach_item(Monster *mon, xchar nx, xchar ny) {
 ...
 * deep.
 */
STATIC_OVL bool can_reach_location(Monster *mon, xchar mx, xchar my, xchar fx,
                                   xchar fy) {
  int i, j;
  int dist;
