...
STATIC_VAR short cham_to_pm[] = {
    ...
    PM_CHAMELEON, PM_DOPPELGANGER, PM_SANDESTIN,
};

/* for deciding whether corpse or statue will carry along full monster data */
#define KEEPTRAITS(mon)                                                  \
  ((mon)->isshk || (mon)->mtame || ((mon)->data->geno & G_UNIQ) ||       \
   is_reviver((mon)->data) ||                                            \
   /* normally leader the will be unique, */ /* but he                   \
                                                might have               \
                                                been                     \
                                                polymorphed              \
                                                */                       \
       (mon)->m_id ==                                                    \
       quest_status                                                      \
           .leader_m_id || /* special cancellation handling for these */ \
 ...
 * etc....
 */
STATIC_OVL Object *make_corpse(Monster *mtmp) {
  ...
  switch (mndx) {
    ...
    case PM_IRON_GOLEM:
      num = d(2, 6);
      while (num--) obj = MakeSpecificObjectAt(IRON_CHAIN, x, y, TRUE, FALSE);
      mtmp->mnamelth = 0;
      break;
    case PM_GLASS_GOLEM:
    ...
    case PM_CLAY_GOLEM:
      ...
      break;
    case PM_STONE_GOLEM:
      obj = MakeCorpseOrStatue(STATUE, (Monster *)0, mdat, x, y, FALSE);
      break;
    case PM_WOOD_GOLEM:
      num = d(2, 4);
      while (num--) {
    ...
    default:
      if (mvitals[mndx].mvflags & G_NOCORPSE)
        ...
        obj = MakeCorpseOrStatue(CORPSE, KEEPTRAITS(mtmp) ? mtmp : 0, mdat, x,
                                 y, TRUE);
      break;
  }
  ...
  if (mtmp->mnamelth) obj = oname(obj, mtmp->name());
...
int minliquid(Monster *mtmp) {
  ...
  if (mtmp == player.usteed && (Flying || Levitation)) return (0);
#endif
  ...
  if (mtmp->data == &mons[PM_GREMLIN] && (inpool || infountain) && rn2(3)) {
    if (split_mon(mtmp, (Monster *)0)) dryup(mtmp->mx, mtmp->my, FALSE);
    if (inpool) water_damage(mtmp->minvent, FALSE, FALSE);
    return (0);
  } else if (mtmp->data == &mons[PM_IRON_GOLEM] && inpool && !rn2(5)) {
    int dam = d(2, 6);
    if (cansee(mtmp->mx, mtmp->my)) pline("%s rusts.", Monnam(mtmp));
    mtmp->mhp -= dam;
  ...
  if (inlava) {
    ...
    if (!is_clinger(mtmp->data) && !likes_lava(mtmp->data)) {
      if (!resists_fire(mtmp)) {
        if (cansee(mtmp->mx, mtmp->my))
          pline("%s %s.", Monnam(mtmp), mtmp->data == &mons[PM_WATER_ELEMENTAL]
                                            ? "boils away"
                                            : "burns to a crisp");
        mondead(mtmp);
      } else {
      ...
      if (mtmp->mhp > 0) {
        (void)fire_damage(mtmp->minvent, FALSE, FALSE, mtmp->mx, mtmp->my);
    ...
    if (!is_clinger(mtmp->data) && !is_swimmer(mtmp->data) &&
      ...
      if (player.ustuck && player.uswallow && player.ustuck == mtmp) {
        ...
        pline("%s sinks as water rushes in and flushes you out.", Monnam(mtmp));
      }
      mondead(mtmp);
      if (mtmp->mhp > 0) {
        (void)rloc(mtmp, FALSE);
  ...
  for (mtmp = fmon; mtmp; mtmp = nmtmp) {
    ...
    if (Conflict && !mtmp->iswiz && mtmp->mcansee) {
      ...
      if (couldsee(mtmp->mx, mtmp->my) &&
          (distu(mtmp->mx, mtmp->my) <= BOLT_LIM * BOLT_LIM) && fightm(mtmp))
...
#define mstoning(obj) \
  (ofood(obj) &&      \
   (touch_petrifies(&mons[(obj)->corpsenm]) || (obj)->corpsenm == PM_MEDUSA))
...
int meatmetal(Monster *mtmp) {
  ...
  for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp->nexthere) {
    if (mtmp->data == &mons[PM_RUST_MONSTER] && !is_rustprone(otmp)) continue;
    if (is_metallic(otmp) && !obj_resists(otmp, 5, 95) &&
      ...
      if (mtmp->data == &mons[PM_RUST_MONSTER] && otmp->oerodeproof) {
        ...
        if (otmp == uball) {
          ...
          ptr = mtmp->data;
          if (poly) {
            if (newcham(mtmp, (MonsterType *)0, FALSE, FALSE)) ptr = mtmp->data;
          } else if (grow) {
            ptr = grow_up(mtmp, (Monster *)0);
          } else if (mstone) {
            if (poly_when_stoned(ptr)) {
              mon_to_stone(mtmp);
              ptr = mtmp->data;
            } else if (!resists_ston(mtmp)) {
              if (canseemon(mtmp)) pline("%s turns to stone!", Monnam(mtmp));
              monstone(mtmp);
              ptr = (MonsterType *)0;
            }
          } else if (heal) {
...
int meatobj(Monster *mtmp) {
  ...
  for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp2) {
    ...
    if (is_organic(otmp) && !obj_resists(otmp, 5, 95) &&
      ...
      if (ptr != &mons[PM_GELATINOUS_CUBE]) return !ptr ? 2 : 1;
    } else if (otmp->oclass != ROCK_CLASS && otmp != uball && otmp != uchain) {
      ++ecount;
      if (ecount == 1) {
...
bool mpickstuff(Monster *mtmp, const char *str) {
  ...
  for (otmp = level.objects[mtmp->mx][mtmp->my]; otmp; otmp = otmp2) {
    ...
    if (!str ? searches_for_item(mtmp, otmp) : !!(index(str, otmp->oclass))) {
      ...
      if (cansee(mtmp->mx, mtmp->my) && flags.verbose)
        pline("%s picks up %s.", Monnam(mtmp),
              (distu(mtmp->mx, mtmp->my) <= 5) ? doname(otmp)
                                               : distant_name(otmp, doname));
      RemoveObjectFromStorage(otmp);
...
int curr_mon_load(Monster *mtmp) {
  ...
  for (obj = mtmp->minvent; obj; obj = obj->nobj) {
    if (obj->otyp != BOULDER || !throws_rocks(mtmp->data)) curload += obj->owt;
  }

  return curload;
}

int max_mon_load(Monster *mtmp) {
  long maxload;
...
bool can_carry(Monster *mtmp, Object *otmp) {
  ...
  if (otyp == CORPSE && touch_petrifies(&mons[otmp->corpsenm]) &&
    ...
    return FALSE;
  if (otyp == CORPSE && is_rider(&mons[otmp->corpsenm])) return FALSE;
  ...
  if (throws_rocks(mdat) && otyp == BOULDER) return (TRUE);

  /* nymphs deal in stolen merchandise, but not boulders or statues */
  if (mdat->mlet == S_NYMPH) return (bool)(otmp->oclass != ROCK_CLASS);
...
int mfndpos(Monster *mon, coord *poss, long *info, long flag) {
  ...
  if (flag & ALLOW_DIG) {
    ...
    if (!needspick(mdat)) {
      ...
      rockok =
          (m_carrying(mon, PICK_AXE) ||
           (m_carrying(mon, DWARVISH_MATTOCK) && !which_armor(mon, W_ARMS)));
      treeok = (m_carrying(mon, AXE) ||
                (m_carrying(mon, BATTLE_AXE) && !which_armor(mon, W_ARMS)));
    }
      ...
      if ((is_pool(nx, ny) == wantpool || poolok) &&
        ...
        if ((nx == player.ux && ny == player.uy) ||
          ...
          if (level.flags.has_temple && *in_rooms(nx, ny, TEMPLE) &&
              !*in_rooms(x, y, TEMPLE) &&
              in_your_sanctuary((Monster *)0, nx, ny)) {
         ...
         */
        {
          ...
          if (ttmp) {
            ...
            if ((ttmp->ttyp != RUST_TRAP || mdat == &mons[PM_IRON_GOLEM]) &&
                ...
                (ttmp->ttyp != WEB || (!amorphous(mdat) && !webmaker(mdat)))) {
              if (!(flag & ALLOW_TRAPS)) {
                if (mon->mtrapseen & (1L << (ttmp->ttyp - 1))) continue;
              }
...
STATIC_OVL long mm_aggression(
    Monster *magr,   /* monster that is currently deciding where to move */
    Monster *mdef) { /* another monster which is next to it */
...
bool monnear(Monster *mon, int x, int y) {
...
/* finish adding its replacement */
  ...
  if (emits_light(mtmp2->data)) {
    ...
    new_light_source(mtmp2->mx, mtmp2->my, emits_light(mtmp2->data), LS_MONSTER,
                     (genericptr_t)mtmp2);
...
void relmon(Monster *mon) {
  Monster *mtmp;

  if (fmon == (Monster *)0) panic("relmon: no fmon available.");
...
STATIC_OVL void m_detach(Monster *mtmp, MonsterType *mptr) {
  ...
  if (emits_light(mptr)) del_light_source(LS_MONSTER, (genericptr_t)mtmp);
  newsym(mtmp->mx, mtmp->my);
  unstuck(mtmp);
...
STATIC_OVL void lifesaved_monster(Monster *mtmp) {
...
void mondead(Monster *mtmp) {
  ...
  if (mtmp == player.usteed) dismount_steed(DISMOUNT_GENERIC);
#endif
  ...
  if (mtmp->data == &mons[PM_MEDUSA]) achieve.killed_medusa = 1;
#endif
...
bool corpse_chance(Monster *mon, Monster *magr, bool was_swallowed) {
  MonsterType *mdat = mon->data;
  int i, tmp;
      ...
      if (was_swallowed && magr) {
        if (magr == &youmonst) {
          ...
          if (magr->mhp < 1) { /* maybe lifesaved */
            if (canspotmon(magr)) pline("%s rips open!", Monnam(magr));
  ...
  if (bigmonst(mdat) || mdat == &mons[PM_LIZARD] || is_golem(mdat) ||
      is_mplayer(mdat) || is_rider(mdat))
    return TRUE;
  return (bool)(!rn2((int)(2 + ((int)(mdat->geno & G_FREQ) < 2) +
                           verysmall(mdat))));
}

/* drop (perhaps) a cadaver and remove monster */
void mondied(Monster *mdef) {
  mondead(mdef);
  if (mdef->mhp > 0) return; /* lifesaved */

  if (corpse_chance(mdef, (Monster *)0, FALSE) &&
      (accessible(mdef->mx, mdef->my) || is_pool(mdef->mx, mdef->my)))
    (void)make_corpse(mdef);
}

/* monster disappears, not dies */
void mongone(Monster *mdef) {
  mdef->mhp = 0; /* can skip some inventory bookkeeping */
#ifdef STEED
  /* Player is thrown from his steed when it disappears */
  if (mdef == player.usteed) dismount_steed(DISMOUNT_GENERIC);
#endif
...
void monstone(Monster *mdef) {
  ...
  if ((int)mdef->data->msize > MZ_TINY ||
    ...
    while ((obj = mdef->minvent) != 0) {
      ...
      if (obj->owornmask) update_mon_intrinsics(mdef, obj, FALSE, TRUE);
      obj_no_longer_held(obj);
      if (obj->owornmask & W_WEP) setmnotwielded(mdef, obj);
      obj->owornmask = 0L;
      if (obj->otyp == BOULDER ||
    ...
    otmp = MakeCorpseOrStatue(STATUE, KEEPTRAITS(mdef) ? mdef : 0, mdef->data,
                              x, y, FALSE);
    ...
    if (mdef->data->geno & G_UNIQ) otmp->spe = 1;
  ...
  if (glyph_is_invisible(levl[x][y].glyph)) unmap_object(x, y);
...
void monkilled(Monster *mdef, const char *fltxt, int how) {
...
void killed(Monster *mtmp) { xkilled(mtmp, 1); }
...
void xkilled(
  ...
  if (dest & 1) {
    ...
    if (!wasinside && !canspotmon(mtmp))
      ...
      You("%s %s!", verb,
          !mtmp->mtame
              ? mon_nam(mtmp)
              : x_monnam(mtmp, mtmp->mnamelth ? ARTICLE_NONE : ARTICLE_THE,
                         "poor", mtmp->mnamelth ? SUPPRESS_SADDLE : 0, FALSE));
    }
  }

  if (mtmp->mtrapped && (t = t_at(x, y)) != 0 &&
      (t->ttyp == PIT || t->ttyp == SPIKED_PIT) && sobj_at(BOULDER, x, y))
    dest |= 2; /*
  ...
  if ((dest & 2) || LEVEL_SPECIFIC_NOCORPSE(mdat)) goto cleanup;
  ...
  if ((!accessible(x, y) && !is_pool(x, y)) ||
    ...
    if (!rn2(6) && !(mvitals[mndx].mvflags & G_NOCORPSE)
      ...
      typ = otmp->otyp;
      if (mdat->msize < MZ_HUMAN && typ != FOOD_RATION && typ != LEASH &&
          typ != FIGURINE &&
     ...
     */
    if (corpse_chance(mtmp, (Monster *)0, FALSE)) (void)make_corpse(mtmp);
  }
...
cleanup:
  ...
  if (is_unicorn(mdat) && sgn(player.ualign.type) == sgn(mdat->maligntyp)) {
    change_luck(-5);
    You_feel("guilty...");
  }
...
void mon_to_stone(Monster *mtmp) {
  if (mtmp->data->mlet == S_GOLEM) {
    /* it's a golem, and not a stone golem */
    if (canseemon(mtmp)) pline("%s solidifies...", Monnam(mtmp));
    if (newcham(mtmp, &mons[PM_STONE_GOLEM], FALSE, FALSE)) {
      if (canseemon(mtmp)) pline("Now it's %s.", an(mtmp->data->mname));
    } else {
 ...
 *	0 - in most cases.
 */
bool mnearto(Monster *mtmp, xchar x, xchar y, bool move_other) {
  Monster *othermon = (Monster *)0;
  xchar newx, newy;
  coord mm;
  ...
  if (move_other && othermon) {
    ...
    if ((othermon->mx != x) || (othermon->my != y)) return (TRUE);
  }
...
static const char *poiseff[] = {
    " feel weaker",             "r brain is on fire", "r judgement is impaired",
    "r muscles won't obey you", " feel very sick",    " break out in hives"};

void poisontell(int typ) { pline("You%s.", poiseff[typ]); }
...
void setmangry(Monster *mtmp) {
  ...
  if (!flags.mon_moving && /* should always be the case here */
    ...
    Monster *mon;
    MonsterType *q_guardian = &mons[quest_info(MS_GUARDIAN)];
    int got_mad = 0;
    ...
    if (got_mad && !Hallucination)
      pline_The(
          "%s appear%s to be angry too...",
          got_mad == 1 ? q_guardian->mname : makeplural(q_guardian->mname),
          got_mad == 1 ? "s" : "");
  }
}

void wakeup(Monster *mtmp) {
  mtmp->msleeping = 0;
...
void wake_nearby() {
  ...
  for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
    if (!mtmp->dead() && distu(mtmp->mx, mtmp->my) < player.ulevel * 20) {
      mtmp->msleeping = 0;
      if (mtmp->mtame && !mtmp->isminion) EDOG(mtmp)->whistletime = moves;
    }
  }
}
...
void rescham() {
  ...
  for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
    ...
    if (mcham) {
      ...
      (void)newcham(mtmp, &mons[cham_to_pm[mcham]], FALSE, FALSE);
    }
    if (is_were(mtmp->data) && mtmp->data->mlet != S_HUMAN) new_were(mtmp);
...
STATIC_OVL int select_newcham_form(Monster *mon) {
  int mndx = NON_PM;
  ...
  if (wizard && iflags.mon_polycontrol) {
    ...
    int tries = 0;
    do {
      sprintf(pprompt, "Change %s into what kind of monster? [type the name]",
              mon_nam(mon));
      getlin(pprompt, buf);
...
int newcham(Monster *mtmp, MonsterType *mdat, bool polyspot, bool msg) {
  int mhp, hpn, hpd;
  int mndx, tryct;
  MonsterType *olddata = mtmp->data;
  char oldname[BUFSZ];
  ...
  if (mdat == 0) {
    while (++tryct <= 100) {
      ...
      if (is_mplayer(mdat) || (!is_human(mdat) && polyok(mdat))) break;
    }
...
/* new hp: same fraction of max as before */
  ...
  if (emits_light(olddata) != emits_light(mtmp->data)) {
    ...
    if (emits_light(olddata)) del_light_source(LS_MONSTER, (genericptr_t)mtmp);
    if (emits_light(mtmp->data))
      new_light_source(mtmp->mx, mtmp->my, emits_light(mtmp->data), LS_MONSTER,
                       (genericptr_t)mtmp);
  }
  ...
  if (mdat == &mons[PM_LONG_WORM] &&
    ...
    initworm(mtmp, rn2(5));
    if (count_wsegs(mtmp)) place_worm_tail_randomly(mtmp, mtmp->mx, mtmp->my);
  }
...
void golemeffects(Monster *mon, int damtype, int dam) {
  ...
  if (slow) {
    if (mon->mspeed != MSLOW) mon_adjust_speed(mon, -1, nullptr);
  }
  if (heal) {
    ...
    if (!silent) { /* do we want pline msgs? */
      ...
      if (nct || sct) {
        if (nct)
          ...
          You("see %sangry guard%s approaching!", sct == 1 ? "an " : "",
              sct > 1 ? "s" : "");
...
void mimic_hit_msg(Monster *mtmp, short otyp) {
  ...
  switch (mtmp->m_ap_type) {
    ...
    case M_AP_OBJECT:
      if (otyp == SPE_HEALING || otyp == SPE_EXTRA_HEALING) {
        pline("%s seems a more vivid %s than before.", The(simple_typename(ap)),
