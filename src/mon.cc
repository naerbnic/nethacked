...
STATIC_OVL Object *make_corpse(Monster *mtmp) {
  ...
  switch (mndx) {
    ...
    case PM_CLAY_GOLEM:
      ...
      break;
    case PM_STONE_GOLEM:
      obj = MakeCorpseOrStatue(STATUE, nullptr, mdat, x, y, FALSE);
      break;
    case PM_WOOD_GOLEM:
      num = d(2, 4);
      while (num--) {
    ...
    if (is_metallic(otmp) && !obj_resists(otmp, 5, 95) &&
      ...
      if (mtmp->data == &mons[PM_RUST_MONSTER] && otmp->oerodeproof) {
        ...
        if (otmp == uball) {
          ...
          ptr = mtmp->data;
          if (poly) {
            if (newcham(mtmp, nullptr, FALSE, FALSE))
              ptr = mtmp->data;
          } else if (grow) {
            ptr = grow_up(mtmp, nullptr);
          } else if (mstone) {
      ...
      if ((is_pool(nx, ny) == wantpool || poolok) &&
        ...
        if ((nx == player.ux && ny == player.uy) ||
          ...
          if (level.flags.has_temple && *in_rooms(nx, ny, TEMPLE) &&
              !*in_rooms(x, y, TEMPLE) &&
              in_your_sanctuary(nullptr, nx, ny)) {
            if (!(flag & ALLOW_SANCT))
              continue;
...
void mondied(Monster *mdef) {
  ...
  if (corpse_chance(mdef, nullptr, FALSE) &&
...
void monstone(Monster *mdef) {
  ...
  if (player.uswallow && player.ustuck == mdef)
    wasinside = TRUE;
  mondead(mdef);
  if (wasinside) {
...
void xkilled(
  ...
  if ((!accessible(x, y) && !is_pool(x, y)) ||
     ...
     */
    if (corpse_chance(mtmp, nullptr, FALSE))
...
void mnexto(Monster *mtmp) {
  ...
  if (!enexto(&mm, player.ux, player.uy, mtmp->data))
    return;
  ...
  if (msg) {
    ...
    pline("%s turns into %s!", oldname,
          mdat == &mons[PM_GREEN_SLIME]
              ? "slime"
              : x_monnam(mtmp, ARTICLE_A, nullptr, SUPPRESS_SADDLE, FALSE));
