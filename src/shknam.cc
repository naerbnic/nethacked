...
static const char *const shkliquors[] = {
    ...
    /* Schweiz */
    "Leuk",      "Brig",            "Brienz",    "Thun",
    "Sarnen",    "Burglen",         "Elm",       "Flims",
...
static const char *const shkwands[] = {
    ...
    "Rhaeader",    "Llandrindod",   "Llanfair-ym-muallt", "Y-Fenni",
    "Maesteg",     "Rhydaman",      "Beddgelert",         "Curig",
    "Llanrwst",    "Llanerchymedd", "Caergybi",
    /* Scotland */
    "Nairn",       "Turriff",       "Inverurie",          "Braemar",
    "Lochnagar",   "Kerloch",       "Beinn a Ghlo",       "Drumnadrochit",
    "Morven",      "Uist",          "Storr",              "Sgurr na Ciche",
    "Cannich",     "Gairloch",      "Kyleakin",           "Dunvegan",
    0};
...
static const char *const shkfoods[] = {
    ...
    "Ardjawinangun", "Berbek",      "Papar",      "Baliga",      "Tjisolok",
    "Siboga",        "Banjoewangi", "Trenggalek", "Karangkobar", "Njalindoeng",
    "Pasawahan",     "Pameunpeuk",  "Patjitan",   "Kediri",      "Pemboeang",
    "Tringanoe",     "Makin",       "Tipor",      "Semai",       "Berhala",
    "Tegal",         "Samoe",       0};
...
static const char *const shktools[] = {
    /* Spmi */
    "Ymla",        "Eed-morra",   "Cubask",      "Nieb",       "Bnowr Falr",
    "Telloc Cyaj", "Sperc",       "Noskcirdneh", "Yawolloh",   "Hyeghu",
    "Niskal",      "Trahnil",     "Htargcm",     "Enrobwem",   "Kachzi Rellim",
    "Regien",      "Donmyar",     "Yelpur",      "Nosnehpets", "Stewe",
    "Renrut",      "_Zlaw",       "Nosalnef",    "Rewuorb",    "Rellenk",
    "Yad",         "Cire Htims",  "Y-crad",      "Nenilukah",  "Corsh",
    "Aned",
#ifdef OVERLAY
...
static const char *const shklight[] = {
    ...
    "Pernik",   "Lom",         "Haskovo",        "Dobrinishte", "Varvara",
    "Oryahovo", "Troyan",      "Lovech",         "Sliven",      0};
...
static const char *const shkgeneral[] = {
    ...
    "Chibougamau", "Matagami",     "Kipawa",       "Kinojevis",  "Abitibi",
    "Maganasipi",
    /* Iceland */
 ...
 * (by testing the sign) whether to use MakeRandomObject() or
 *MakeSpecificObject().
 */
...
STATIC_OVL void mkshobj_at(struct shclass const *shp, int sx, int sy) {
  Monster *mtmp;
  int atype;
  MonsterType *ptr;

  if (rn2(100) < depth(&player.uz) && !MON_AT(sx, sy) &&
...
bool saleable(Monster *shkp, Object *obj) {
  ...
  if (shp->symb == RANDOM_CLASS)
    ...
    for (i = 0; i < SIZE(shtypes[0].iprobs) && shp->iprobs[i].iprob; i++)
      if (shp->iprobs[i].itype < 0 ? shp->iprobs[i].itype == -obj->otyp
