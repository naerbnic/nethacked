...
NhRect *rnd_rect() { return rect_cnt > 0 ? &rect[rn2(rect_cnt)] : 0; }
...
static bool intersect(NhRect *r1, NhRect *r2, NhRect *r3) {
  ...
  if (r3->lx > r3->hx || r3->ly > r3->hy) return FALSE;
  return TRUE;
}
...
void remove_rect(NhRect *r) {
  ...
  ind = get_rect_ind(r);
  if (ind >= 0) rect[ind] = rect[--rect_cnt];
}
...
void split_rects(NhRect *r1, NhRect *r2) {
  ...
  for (i = rect_cnt - 1; i >= 0; i--)
    if (intersect(&rect[i], r2, &r)) split_rects(&rect[i], &r);
