    ...
    for (sy = sroom->ly; sy <= sroom->hy; sy++) {
      if (sroom->irregular) {
        if ((int)levl[sx][sy].roomno != rmno || levl[sx][sy].edge ||
            (sroom->doorct && distmin(sx, sy, doors[sh].x, doors[sh].y) <= 1))
          continue;
      ...
      switch (type) {
        ...
        case BEEHIVE:
          if (!rn2(3))
            (void)MakeSpecificObjectAt(LUMP_OF_ROYAL_JELLY, sx, sy, TRUE,
                                       FALSE);
          break;
        case BARRACKS:
            ...
            if (sobj) {
              for (i = rn2(5); i; i--)
                (void)AddObjectToContainer(
                    sobj, MakeRandomObject(RANDOM_CLASS, FALSE));
...
struct mkroom *search_special(schar type) {
  ...
  for (croom = &rooms[0]; croom->hx >= 0; croom++)
    if ((type == ANY_TYPE && croom->rtype != OROOM) ||
        (type == ANY_SHOP && croom->rtype >= SHOPBASE) || croom->rtype == type)
      return croom;
