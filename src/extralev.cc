...
#define doorhere (r[x][y].doortable)
    if (x > 0 && (!(doorhere & LEFT)) && (!r[x - 1][y].doortable || !rn2(10)))
      dirs[q++] = 0;
    if (x < 2 && (!(doorhere & RIGHT)) && (!r[x + 1][y].doortable || !rn2(10)))
      dirs[q++] = 1;
    if (y > 0 && (!(doorhere & UP)) && (!r[x][y - 1].doortable || !rn2(10)))
      dirs[q++] = 2;
    if (y < 2 && (!(doorhere & DOWN)) && (!r[x][y + 1].doortable || !rn2(10)))
      dirs[q++] = 3;
  ...
  for (y = 0; y < 3; y++)
    for (x = 0; x < 3; x++) {
      if (here.real) { /* Make a room */
         ...
         */
        add_room(lowx, lowy, hix, hiy, (bool)!rn2(7), OROOM, FALSE);
      }
    }
