...
static struct Bool_Opt {
  ...
  int optflags;
      ...
      {"altmeta", nullptr, TRUE, DISP_IN_GAME},
      {"ascii_map", &iflags.wc_ascii_map, !PREFER_TILED, SET_IN_GAME}, /*WC*/
      {"asksavedisk", nullptr, FALSE, SET_IN_FILE},
...
#else
      {"checkpoint", nullptr, FALSE, SET_IN_FILE},
#endif
      {"checkspace", nullptr, FALSE, SET_IN_FILE},
...
#else
      {"DECgraphics", nullptr, FALSE, SET_IN_FILE},
#endif
...
#else
      {"fast_map", nullptr, TRUE, SET_IN_FILE},
#endif
...
#else
      {"flush", nullptr, FALSE, SET_IN_FILE},
#endif
...
#else
      {"IBMgraphics", nullptr, FALSE, SET_IN_FILE},
#endif
      ...
      {"Macgraphics", nullptr, FALSE, SET_IN_FILE},
#ifdef MAIL
      {"mail", &flags.biff, TRUE, SET_IN_GAME},
#else
      {"mail", nullptr, TRUE, SET_IN_FILE},
#endif
#ifdef MENU_COLOR
      {"menucolors", &iflags.use_menu_color, FALSE, SET_IN_GAME},
#else
      {"menucolors", nullptr, FALSE, SET_IN_GAME},
#endif
#ifdef WIZARD
      /* for menu debugging only*/
      {"menu_tab_sep", &iflags.menu_tab_sep, FALSE, SET_IN_GAME},
#else
      {"menu_tab_sep", nullptr, FALSE, SET_IN_FILE},
#endif
      {"mouse_support", &iflags.wc_mouse_support, TRUE, DISP_IN_GAME}, /*WC*/
#ifdef NEWS
      {"news", &iflags.news, TRUE, DISP_IN_GAME},
#else
      {"news", nullptr, FALSE, SET_IN_FILE},
#endif
      {"null", &flags.null, TRUE, SET_IN_GAME},
      {"page_wait", nullptr, FALSE, SET_IN_FILE},
#ifdef PARANOID
...
#else
      {"showexp", nullptr, FALSE, SET_IN_FILE},
#endif
...
#else
      {"showscore", nullptr, FALSE, SET_IN_FILE},
#endif
...
#else
      {"timed_delay", nullptr, FALSE, SET_IN_GAME},
#endif
...
static struct Comp_Opt {
  ...
  int optflags;
      ...
      {"boulder", "the symbol to use for displaying boulders", 1, SET_IN_GAME},
      {"catname", "the name of your (first) cat (e.g., catname:Tabby)", PL_PSIZ,
       DISP_IN_GAME},
...
#ifdef CHANGE_COLOR
      {"palette", "palette (00c/880/-fff is blue/yellow/reverse white)", 15,
       SET_IN_GAME},
#endif
      {"pettype", "your preferred initial pet type", 4, DISP_IN_GAME},
      {"pickup_burden", "maximum burden picked up before prompt", 20,
       SET_IN_GAME},
      {"pickup_types", "types of objects to pick up automatically", MAXOCLASSES,
       SET_IN_GAME},
...
STATIC_DCL char *string_for_opt(char *, bool);
STATIC_DCL char *string_for_env_opt(const char *, char *, bool);
STATIC_DCL void bad_negation(const char *, bool);
...
STATIC_DCL bool special_handling(const char *, bool, bool);
...
STATIC_OVL bool is_wc_option(const char *);
STATIC_OVL bool wc_supported(const char *);
STATIC_OVL bool is_wc2_option(const char *);
STATIC_OVL bool wc2_supported(const char *);
...
void initoptions() {
  ...
  iflags.opt_booldup = iflags.opt_compdup = nullptr;
  ...
  (void)memcpy((genericptr_t)flags.inv_order, (genericptr_t)def_inv_order,
               sizeof flags.inv_order);
...
STATIC_OVL char *string_for_env_opt(const char *optname, char *opts,
                                    bool val_optional) {
  if (!initial) {
...
STATIC_OVL void graphics_opts(char *opts, const char *optype, int maxlen,
                              int offset) {
...
STATIC_OVL void warning_opts(char *opts, const char *optype) {
  ...
  for (i = 0; i < length; i++)
    translate[i] =
        (((i < WARNCOUNT) && opts[i]) ? (uchar)opts[i] : def_warnsyms[i].sym);
...
void set_duplicate_opt_detection(int on_or_off) {
  ...
  if (on_or_off != 0) {
    ...
    iflags.opt_booldup = nullptr;
    if (iflags.opt_compdup)
      free((genericptr_t)iflags.opt_compdup);
    iflags.opt_compdup = nullptr;
  }
}

STATIC_OVL void duplicate_opt_detection(const char *opts, int bool_or_comp) {
  int i, *optptr;
  ...
  } else if ((bool_or_comp == 1) && iflags.opt_compdup && initial &&
             from_file) {
...
void parseoptions(char *opts, bool tinitial, bool tfrom_file) {
  ...
  if (match_optname(opts, fullname, 3, TRUE)) {
    ...
    if (negated)
      bad_negation(fullname, FALSE);
    else if ((op = string_for_opt(opts, !tfrom_file)) != 0 && strlen(op) > 1)
  ...
  if (!strncmpi(opts, fullname, 4)) {
    ...
    if (!strncmpi(fontopts, "map", 3) || !strncmpi(fontopts, "_map", 4))
      ...
      if (wintype > 0 && !negated && (op = string_for_opt(opts, FALSE)) != 0) {
        switch (wintype) {
          case NHW_MAP:
    ...
    if (wintype > 0 && (op = string_for_opt(opts, FALSE)) != 0) {
  ...
  if (match_optname(opts, "palette", 3, TRUE)) {
    ...
    color_number = 0;
    color_incr = 1;
    if ((op = string_for_opt(opts, FALSE)) != nullptr) {
      char *pt = op;
      int cnt, tmp, reverse;
      long rgb;
  ...
  if (match_optname(opts, fullname, 7, TRUE)) {
    ...
    if (clash) {
      ...
      pline("Badoption - boulder symbol '%c' conflicts with a %s symbol.",
...
#if defined(TERMLIB) || defined(ASCIIGRAPH)
      if (FALSE
#ifdef TERMLIB
...
static const char *menutype[] = {"traditional", "combination", "partial",
                                 "full"};

static const char *burdentype[] = {"unencumbered", "burdened",  "stressed",
                                   "strained",     "overtaxed", "overloaded"};

static const char *runmodes[] = {"teleport", "run", "walk", "crawl"};
...
int doset() {
    ...
    for (i = 0; boolopt[i].name; i++)
      if ((bool_p = boolopt[i].addr) != 0 &&
        ...
        if (is_wc_option(boolopt[i].name) && !wc_supported(boolopt[i].name))
          continue;
        if (is_wc2_option(boolopt[i].name) && !wc2_supported(boolopt[i].name))
          continue;
        any.a_int = (pass == 0) ? 0 : i + 1;
        if (!iflags.menu_tab_sep)
          sprintf(buf, "%s%-13s [%s]", pass == 0 ? "    " : "", boolopt[i].name,
                  *bool_p ? "true" : "false");
        else
          sprintf(buf, "%s\t[%s]", boolopt[i].name, *bool_p ? "true" : "false");
  ...
  add_menu(tmpwin, NO_GLYPH, &any, 0, 0, iflags.menu_headings,
           "Compounds (selecting will prompt for new value):", MENU_UNSELECTED);
  ...
  sprintf(buf, "autopickup exceptions (%d currently set)",
          count_ape_maps(nullptr, nullptr));
  add_menu(tmpwin, NO_GLYPH, &any, 0, 0, ATR_NONE, buf, MENU_UNSELECTED);
...
#ifdef AUTOPICKUP_EXCEPTIONS
      if (opt_indx == -2) {
        special_handling("autopickup_exception", setinitial, fromfile);
      } else
#endif
...
STATIC_OVL bool special_handling(const char *optname, bool setinitial,
                                 bool setfromfile) {
  winid tmpwin;
  anything any;
  int i;
  ...
  if (!strcmp("menustyle", optname)) {
    ...
    for (i = 0; i < SIZE(menutype); i++) {
      ...
      any.a_int = i + 1;
      add_menu(tmpwin, NO_GLYPH, &any, *style_name, 0, ATR_NONE, style_name,
               MENU_UNSELECTED);
    }
    ...
    menu_item *disclosure_category_pick = nullptr;
    /*
    ...
    for (i = 0; i < NUM_DISCLOSURE_OPTIONS; i++) {
      ...
      any.a_int = i + 1;
      add_menu(tmpwin, NO_GLYPH, &any, disclosure_options[i], 0, ATR_NONE,
               disclosure_name, MENU_UNSELECTED);
      disc_cat[i] = 0;
    }
    end_menu(tmpwin, "Change which disclosure options categories:");
    if ((pick_cnt = select_menu(tmpwin, PICK_ANY, &disclosure_category_pick)) >
        0) {
      for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx) {
      ...
      disclosure_category_pick = nullptr;
    }
    destroy_nhwindow(tmpwin);
    ...
    for (i = 0; i < NUM_DISCLOSURE_OPTIONS; i++) {
      if (disc_cat[i]) {
        char dbuf[BUFSZ];
        menu_item *disclosure_option_pick = nullptr;
    ...
    menu_item *mode_pick = nullptr;
    tmpwin = create_nhwindow(NHW_MENU);
    start_menu(tmpwin);
    for (i = 0; i < SIZE(runmodes); i++) {
      mode_name = runmodes[i];
      any.a_int = i + 1;
      add_menu(tmpwin, NO_GLYPH, &any, *mode_name, 0, ATR_NONE, mode_name,
               MENU_UNSELECTED);
    }
    ...
    static const char *npchoices[3] = {"0 (off)", "1 (on)",
                                       "2 (on, DOS compatible)"};
    ...
    for (i = 0; i < SIZE(npchoices); i++) {
      any.a_int = i + 1;
      add_menu(tmpwin, NO_GLYPH, &any, npletters[i], 0, ATR_NONE, npchoices[i],
               MENU_UNSELECTED);
    }
    ...
    menu_item *sortl_pick = nullptr;
    tmpwin = create_nhwindow(NHW_MENU);
    start_menu(tmpwin);
    for (i = 0; i < SIZE(sortltype); i++) {
      sortl_name = sortltype[i];
      any.a_char = *sortl_name;
      add_menu(tmpwin, NO_GLYPH, &any, *sortl_name, 0, ATR_NONE, sortl_name,
               MENU_UNSELECTED);
    }
    ...
    for (i = 0; i < SIZE(mhchoices); i++) {
      any.a_int = i + 1;
      add_menu(tmpwin, NO_GLYPH, &any, npletters[i], 0, ATR_NONE, mhchoices[i],
               MENU_UNSELECTED);
    }
...
STATIC_OVL const char *get_compopt_value(const char *optname, char *buf) {
  ...
  if (!strcmp(optname, "align_message"))
    sprintf(buf, "%s", iflags.wc_align_message == ALIGN_TOP
                           ? "top"
                           : iflags.wc_align_message == ALIGN_LEFT
                                 ? "left"
                                 : iflags.wc_align_message == ALIGN_BOTTOM
    ...
    sprintf(buf, "%s", iflags.wc_align_status == ALIGN_TOP
                           ? "top"
                           : iflags.wc_align_status == ALIGN_LEFT
                                 ? "left"
                                 : iflags.wc_align_status == ALIGN_BOTTOM
    ...
    sprintf(buf, "%c", iflags.bouldersym
                           ? iflags.bouldersym
                           : oc_syms[(int)objects[BOULDER].oc_class]);
    ...
    sprintf(buf, "%s",
            (!iflags.num_pad) ? "0=off" : (iflags.num_pad_mode)
                                              ? "2=on, DOS compatible"
                                              : "1=on");
    ...
    sprintf(buf, "%d-%d-%d-%d-%d-%d-%d-%d-%d-%d-%d-%d", ttycolors[CLR_RED],
            ttycolors[CLR_GREEN], ttycolors[CLR_BROWN], ttycolors[CLR_BLUE],
            ttycolors[CLR_MAGENTA], ttycolors[CLR_CYAN], ttycolors[CLR_ORANGE],
            ttycolors[CLR_BRIGHT_GREEN], ttycolors[CLR_YELLOW],
            ttycolors[CLR_BRIGHT_BLUE], ttycolors[CLR_BRIGHT_MAGENTA],
    ...
    sprintf(buf, "ON, for %s objects%s", ocl[0] ? ocl : "all",
            ...
            (iflags.autopickup_exceptions[AP_LEAVE] ||
             iflags.autopickup_exceptions[AP_GRAB])
                ? ((count_ape_maps(nullptr, nullptr) == 1)
                       ? ", with one exception"
...
int add_autopickup_exception(const char *mapping) {
  ...
  if (sscanf(mapping, "\"%255[^\"]\"", text) == 1) {
    ...
    ape->grab = grab;
    if (!*apehead)
      ape->next = (struct autopickup_exception *)0;
    else
      ape->next = *apehead;
    *apehead = ape;
  } else {
...
#define CONFIG_SLOT 3 /* fill in next value at run-time */
    nullptr, "or use `NETHACKOPTIONS=\"<options>\"' in your environment",
...
static const char *opt_epilog[] = {
    ...
    "items will not be selectable in the 'O' command's menu.", nullptr};
...
int fruitadd(char *str) {
  ...
  if (user_specified) {
    ...
    if (found || numeric || !strncmp(str, "cursed ", 7) ||
        ...
        !strcmp(str, "empty tin") || ((!strncmp(eos(str) - 7, " corpse", 7) ||
...
int choose_classes_menu(const char *prompt, int category, bool way,
  ...
  if (class_list == nullptr || class_select == nullptr)
    return 0;
  accelerator = 0;
  next_accelerator = 'a';
  any.a_void = 0;
...
STATIC_OVL int wc_set_window_colors(char *op) {
  ...
  static char **fgp[] = {&iflags.wc_foregrnd_menu, &iflags.wc_foregrnd_message,
                         &iflags.wc_foregrnd_status, &iflags.wc_foregrnd_text};
  static char **bgp[] = {&iflags.wc_backgrnd_menu, &iflags.wc_backgrnd_message,
                         &iflags.wc_backgrnd_status, &iflags.wc_backgrnd_text};
