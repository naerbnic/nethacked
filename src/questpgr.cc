...
STATIC_DCL const char *intermed();
STATIC_DCL const char *neminame();
STATIC_DCL const char *guardname();
STATIC_DCL const char *homebase();
STATIC_DCL struct qtmsg *msg_in(struct qtmsg *, int);
STATIC_DCL void convert_arg(char);
STATIC_DCL void convert_line();
...
STATIC_OVL struct qtmsg *construct_qtlist(long hdr_offset) {
  ...
  msg_list =
      (struct qtmsg *)alloc((unsigned)(n_msgs + 1) * sizeof(struct qtmsg));
...
const char *ldrname() {
  ...
  sprintf(nambuf, "%s%s", type_is_pname(&mons[i]) ? "" : "the ", mons[i].mname);
  return nambuf;
}

/* return your intermediate target string */
STATIC_OVL const char *intermed() { return (urole.intermed); }
...
STATIC_OVL const char *homebase() { return (urole.homebase); }
...
STATIC_OVL void convert_arg(char c) {
  ...
  switch (c) {
    ...
    case 'c':
      ...
      break;
    case 'r':
      str = rank_of(player.ulevel, Role_switch, flags.female);
      break;
    case 'R':
    ...
    case 'g':
      ...
      break;
    case 'G':
      str = align_gtitle(player.ualignbase[A_ORIGINAL]);
      break;
    case 'H':
      str = homebase();
      break;
    case 'a':
      str = align_str(player.ualignbase[A_ORIGINAL]);
      break;
    case 'A':
      str = align_str(player.ualign.type);
      break;
    case 'd':
      str = align_gname(player.ualignbase[A_ORIGINAL]);
      break;
    case 'D':
