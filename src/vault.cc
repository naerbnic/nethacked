...
STATIC_OVL bool clear_fcorr(Monster *grd, bool forceshow) {
  ...
  while ((fcbeg = EGD(grd)->fcbeg) < EGD(grd)->fcend) {
    ...
    if ((player.ux == fcx && player.uy == fcy && grd->mhp > 0) ||
        ...
        (Punished && !carried(uball) && uball->ox == fcx && uball->oy == fcy))
      return FALSE;
...
char vault_occupied(char *array) {
  ...
  for (ptr = array; *ptr; ptr++)
    if (rooms[*ptr - ROOMOFFSET].rtype == VAULT) return (*ptr);
  return ('\0');
}
...
void invault() {
  ...
  guard = findgd();
  if (++player.uinvault % 30 == 0 &&
      !guard) { /* if time ok and no guard now. */
    char buf[BUFSZ];
    int x, y, dd, gx, gy;
    int lx = 0, ly = 0;
#ifdef GOLDOBJ
  ...
  fnd:
    ...
    if (!player.ugold && !hidden_gold())
      ...
      if (!player.ugold) verbalize("You have hidden gold.");
    ...
    if (!umoney && !hidden_gold())
      ...
      if (!umoney) verbalize("You have hidden money.");
...
STATIC_OVL void wallify_vault(Monster *grd) {
  ...
  for (x = lox; x <= hix; x++)
    for (y = loy; y <= hiy; y++) {
      ...
      if (!IS_WALL(levl[x][y].typ) && !in_fcorridor(grd, x, y)) {
        ...
        if ((trap = t_at(x, y)) != 0) deltrap(trap);
        if (x == lox)
  ...
  if (movedgold || fixed) {
    ...
    if (movedgold) pline("A mysterious force moves the gold into the vault.");
    if (fixed) pline_The("damaged vault's walls are magically restored!");
  }
}
...
int gd_move(Monster *grd) {
  ...
  nx = ny = m = n = 0;
  if (!u_in_vault && !grd_in_vault) wallify_vault(grd);
  if (!grd->mpeaceful) {
    if (semi_dead) {
      egrd->gddone = 1;
      goto newpos;
    ...
    if (!in_fcorridor(grd, grd->mx, grd->my)) (void)clear_fcorr(grd, TRUE);
    return (-1);
  }
      ...
      if (egrd->warncnt == 3)
        verbalize("I repeat, %sfollow me!",
                  u_carry_gold
                      ? (
#ifndef GOLDOBJ
                            !player.ugold ? "drop that hidden gold and "
                                          : "drop that gold and ")
                      : "");
#else
    ...
    if (!u_in_vault) {
      ...
      letknow:
        if (!cansee(grd->mx, grd->my) || !mon_visible(grd))
          ...
          You(um_dist(grd->mx, grd->my, 2) ? "see an angry %s approaching."
  ...
  if (egrd->fcend > 1) {
    ...
    if (u_carry_gold && (in_fcorridor(grd, player.ux, player.uy) ||
                         /* cover a 'blind' spot */
    ...
    for (ny = y - 1; ny <= y + 1; ny++) {
      if ((nx == x || ny == y) && (nx != x || ny != y) && isok(nx, ny)) {
        ...
        if (!IS_STWALL(typ) && !IS_POOL(typ)) {
          if (in_fcorridor(grd, nx, ny)) goto nextnxy;

          if (*in_rooms(nx, ny, VAULT)) continue;
...
newpos:
  ...
  cleanup:
    ...
    restfakecorr(grd);
    if (!semi_dead &&
        (in_fcorridor(grd, player.ux, player.uy) || cansee(x, y))) {
...
#ifndef GOLDOBJ
         player.ugold, currency(player.ugold));
#else
...
long hidden_gold() {
  ...
  for (obj = invent; obj; obj = obj->nobj)
    if (Has_contents(obj)) value += contained_gold(obj);
