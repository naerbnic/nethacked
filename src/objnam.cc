  ...
  switch (ocl->oc_class) {
    ...
    case AMULET_CLASS:
      if (nn)
        ...
        strcpy(buf, "amulet");
      if (un) sprintf(eos(buf), " called %s", un);
      if (dn) sprintf(eos(buf), " (%s)", dn);
      return (buf);
    default:
      if (nn) {
        strcpy(buf, actualn);
        if (GemStone(otyp)) strcat(buf, " stone");
        if (un) sprintf(eos(buf), " called %s", un);
        if (dn) sprintf(eos(buf), " (%s)", dn);
      } else {
...
#endif
{
  ...
  switch (obj->oclass) {
    ...
    case VENOM_CLASS:
    case TOOL_CLASS:
      if (typ == LENSES) strcpy(buf, "pair of ");
    ...
    case BALL_CLASS:
      ...
      break;
    case POTION_CLASS:
      if (obj->dknown && obj->odiluted) strcpy(buf, "diluted ");
      if (nn || un || !obj->dknown) {
        strcat(buf, "potion");
        if (!obj->dknown) break;
        if (nn) {
          strcat(buf, " of ");
          if (typ == POT_WATER && obj->bknown &&
    ...
    case SPBOOK_CLASS:
      if (!obj->dknown) {
        ...
        if (typ != SPE_BOOK_OF_THE_DEAD) strcpy(buf, "spellbook of ");
        strcat(buf, actualn);
      } else if (un) {
 ...
 */
  ...
  switch (obj->oclass) {
    case AMULET_CLASS:
      if (obj->owornmask & W_AMUL) strcat(bp, " (being worn)");
      break;
    case WEAPON_CLASS:
      if (ispoisoned) strcat(prefix, "poisoned ");
    plus:
    ...
    case TOOL_CLASS:
      ...
      if (!is_weptool(obj)) add_erosion_words(obj, prefix);
      ...
      if (objects[obj->otyp].oc_charged) goto charges;
      break;
    case WAND_CLASS:
      add_erosion_words(obj, prefix);
    charges:
      ...
      break;
    case POTION_CLASS:
      if (obj->otyp == POT_OIL && obj->lamplit) strcat(bp, " (lit)");
      break;
    case RING_CLASS:
      add_erosion_words(obj, prefix);
    ring:
    ...
    case FOOD_CLASS:
      ...
      if (obj->otyp == CORPSE) {
        ...
        if (obj->corpsenm >= LOW_PM &&
          ...
          strcat(prefix, mons[obj->corpsenm].mname);
          strcat(prefix, " ");
          if (obj->spe) strcat(bp, " (laid by you)");
        }
      }
    ...
    case CHAIN_CLASS:
      add_erosion_words(obj, prefix);
      if (obj->owornmask & W_BALL) strcat(bp, " (chained to you)");
      break;
  }
...
bool not_fully_identified(Object *otmp) {
  ...
  if (!otmp->known || !otmp->dknown ||
    ...
    return TRUE;
  if (otmp->oartifact && undiscovered_artifact(otmp->oartifact)) return TRUE;
  ...
  if (otmp->rknown ||
    ...
    return (bool)(is_rustprone(otmp) || is_corrodeable(otmp) ||
                  IsFlammable(otmp));
}
...
char *cxname(Object *obj) {
  if (obj->otyp == CORPSE) return corpse_xname(obj, FALSE);
  return xname(obj);
}
#ifdef SORTLOOT
char *cxname2(Object *obj) {
  if (obj->otyp == CORPSE) return corpse_xname(obj, TRUE);
 ...
 */
const char *singular(Object *otmp, char *(*func)(Object *)) {
  long savequan;
  char *nam;

  /* Note: using xname for corpses will not give the monster type */
  if (otmp->otyp == CORPSE && func == xname) return corpse_xname(otmp, TRUE);
...
char *otense(Object *otmp, const char *verb) {
   ...
   */
  if (!is_plural(otmp)) return vtense((char *)0, verb);
...
static const char *const special_subjs[] = {
    ...
    "Cyclops", "Hippocrates", "Pelias", "aklys", "amnesia", "paralysis", 0};
...
char *vtense(const char *subj, const char *verb) {
   ...
   */
  if (subj) {
    if (!strncmpi(subj, "a ", 2) || !strncmpi(subj, "an ", 3)) goto sing;
    spot = (const char *)0;
...
char *makeplural(const char *oldstr) {
   ...
   */
  if (!strncmp(str, "pair of ", 8)) goto bottom;

  /* Search for common compounds, ex. lump of royal jelly */
  for (spot = str; *spot; spot++) {
    if (!strncmp(spot, " of ", 4) || !strncmp(spot, " labeled ", 9) ||
        !strncmp(spot, " called ", 8) || !strncmp(spot, " named ", 7) ||
        !strcmp(spot, " above") /* lurkers above */
        || !strncmp(spot, " versus ", 8) || !strncmp(spot, " from ", 6) ||
        !strncmp(spot, " in ", 4) || !strncmp(spot, " on ", 4) ||
        !strncmp(spot, " a la ", 6) ||
        !strncmp(spot, " with", 5) /* " with "? */
        || !strncmp(spot, " de ", 4) || !strncmp(spot, " d'", 3) ||
        !strncmp(spot, " du ", 4)) {
  ...
  *(spot + 1) = 0;
  ...
  if ((len == 2 && !strcmp(str, "ya")) ||
      ...
      (len >= 4 && (!strcmp(spot - 3, "fish") || !strcmp(spot - 3, "tuna") ||
                    !strcmp(spot - 3, "deer") || !strcmp(spot - 3, "yaki"))) ||
      (len >= 5 &&
       (!strcmp(spot - 4, "sheep") || !strcmp(spot - 4, "ninja") ||
        !strcmp(spot - 4, "ronin") || !strcmp(spot - 4, "shito") ||
        !strcmp(spot - 7, "shuriken") || !strcmp(spot - 4, "tengu") ||
        !strcmp(spot - 4, "manes"))) ||
  ...
  if (len >= 6 &&
      (!strcmp(spot - 5, "matzoh") || !strcmp(spot - 5, "matzah"))) {
    strcpy(spot - 1, "ot");
    goto bottom;
  }
...
char *makesingular(const char *oldstr) {
  ...
  if (p >= bp + 1 && p[-1] == 's') {
    if (p >= bp + 2 && p[-2] == 'e') {
      if (p >= bp + 3 && p[-3] == 'i') {
        if (!BSTRCMP(bp, p - 7, "cookies") || !BSTRCMP(bp, p - 4, "pies"))
          goto mins;
        strcpy(p - 3, "y");
        return bp;
      }
      ...
      if (!BSTRCMP(bp, p - 6, "gloves") || !BSTRCMP(bp, p - 6, "lenses") ||
          !BSTRCMP(bp, p - 5, "shoes") || !BSTRCMP(bp, p - 6, "scales"))
        return bp;
               ...
               !BSTRCMP(bp, p - 5, "glass") || !BSTRCMP(bp, p - 4, "ness") ||
...
#endif
               !BSTRCMP(bp, p - 5, "aklys") || !BSTRCMP(bp, p - 6, "fungus"))
      return bp;
  mins:
    p[-1] = '\0';
...
static bool wishymatch(const char *u_str, const char *o_str,
                       bool retry_inverted) {
 ...
 */
#ifdef WIZARD
  if (wizard && !strcmp(o_str, "beartrap")) return !strncmpi(o_str, u_str, 8);
#endif
...
#define SPINACH 2
  ...
  */
  ...
  if (spe > SCHAR_LIM) spe = SCHAR_LIM;
  ...
  */
  ...
  /* Alternate spellings (pick-ax, silver sabre, &c) */
  {
    ...
    if (!strncmpi(bp, "grey spell", 10)) *(bp + 2) = 'a';
  }
  ...
  if (!BSTRCMPI(bp, p - 10, "gold piece") || !BSTRCMPI(bp, p - 7, "zorkmid") ||
    ...
    if (cnt < 1) cnt = 1;
#ifndef GOLDOBJ
    if (from_user) pline("%d gold piece%s.", cnt, plur(cnt));
    player.ugold += cnt;
    flags.botl = 1;
    return (&zeroobj);
#else
...
srch:
  ...
  while (i < NUM_OBJECTS && (!oclass || objects[i].oc_class == oclass)) {
    ...
    if (dn && (zn = OBJ_DESCR(objects[i])) != 0 && wishymatch(dn, zn, FALSE)) {
  ...
  if (wizard && from_user) {
    ...
    for (trap = NO_TRAP + 1; trap < TRAPNUM; trap++) {
      ...
      if (!strncmpi(tname, bp, strlen(tname))) {
        ...
        if ((trap == TRAPDOOR || trap == HOLE) && !Can_fall_thru(&player.uz))
          trap = ROCKTRAP;
    ...
    if (!BSTRCMP(bp, p - 8, "fountain")) {
      ...
      if (!strncmpi(bp, "magic ", 6)) levl[player.ux][player.uy].blessedftn = 1;
      pline("A %sfountain.",
...
int rnd_class(int first, int last) {
  ...
  for (i = first; i <= last; i++)
    if (objects[i].oc_prob && (x -= objects[i].oc_prob) <= 0) return i;
  return 0;
}
  ...
  if (cloak) {
    switch (cloak->otyp) {
      ...
      case MUMMY_WRAPPING:
        return "wrapping";
      case ALCHEMY_SMOCK:
        return (objects[cloak->otyp].oc_name_known && cloak->dknown) ? "smock"
                                                                     : "apron";
      default:
        break;
    }
...
const char *mimic_obj_name(Monster *mtmp) {
