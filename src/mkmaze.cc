...
STATIC_DCL bool put_lregion_here(xchar, xchar, xchar, xchar, xchar, xchar,
                                 xchar, bool, d_level *);
...
STATIC_OVL bool iswall(int x, int y) {
  ...
  type = levl[x][y].typ;
  return (IS_WALL(type) || IS_DOOR(type) || type == SDOOR || type == IRONBARS);
}
...
void wallification(int x1, int y1, int x2, int y2) {
  ...
  static xchar spine_array[16] = {
      VWALL, HWALL,    HWALL,    HWALL,  VWALL, TRCORNER, TLCORNER, TDWALL,
...
bool bad_location(xchar x, xchar y, xchar lx, xchar ly, xchar hx, xchar hy) {
  return ((bool)(occupied(x, y) || within_bounded_area(x, y, lx, ly, hx, hy) ||
...
void place_lregion(xchar lx, xchar ly, xchar hx, xchar hy, xchar nlx, xchar nly,
...
STATIC_OVL bool put_lregion_here(xchar x, xchar y, xchar nlx, xchar nly,
                                 xchar nhx, xchar nhy, xchar rtype,
                                 bool oneshot, d_level *lev) {
...
STATIC_OVL void fixup_special() {
  ...
  /* KMH -- Sokoban levels */
  if (In_sokoban(&player.uz)) sokoban_detect();
  ...
  if (lregions) free((genericptr_t)lregions), lregions = 0;
  num_lregions = 0;
}
...
void makemaz(const char *s) {
    ...
    if (dunlevs_in_dungeon(&player.uz) > 1) {
      if (sp && sp->rndlevs)
        sprintf(protofile, "%s%d-%d", dungeons[player.uz.dnum].proto,
                dunlev(&player.uz), rnd((int)sp->rndlevs));
      else
      ...
      strcpy(protofile, dungeons[player.uz.dnum].proto);
  ...
  for (x = 2; x < x_maze_max; x++)
    for (y = 2; y < y_maze_max; y++) levl[x][y].typ = STONE;
#else
  for (x = 2; x <= x_maze_max; x++)
...
#define INVPOS_DISTANCE 11
    ...
    if (x_range <= INVPOS_X_MARGIN || y_range <= INVPOS_Y_MARGIN ||
      ...
      panic("inv_pos: maze is too small! (%d x %d)", x_maze_max, y_maze_max);
#endif
...
#undef y_maze_min
  ...
  for (x = rn1(6, 7); x; x--) mktrap(0, 1, (struct mkroom *)0, (coord *)0);
}
...
#ifdef DEBUG
  pline("mkportal: at (%d,%d), to %s, level %d", x, y, dungeons[todnum].dname,
        todlevel);
#endif
...
void movebubbles() {
  ...
  int x, y, i, j;
  Trap *btrap;
  static const struct rm water_pos = {cmap_to_glyph(S_water), WATER, 0, 0, 0,
                                      0,                      0,     0, 0};
...
void water_friction() {
  ...
  if (Swimming && rn2(4)) return; /* natural swimmers have advantage */
...
void save_waterlevel(int fd, int mode) {
  ...
  if (release_data(mode)) unsetup_waterlevel();
}
...
const char *waterbody_name(xchar x, xchar y) {
  ...
  if (is_lava(x, y))
    return "lava";
  else if (ltyp == ICE || (ltyp == DRAWBRIDGE_UP &&
                           (levl[x][y].drawbridgemask & DB_UNDER) == DB_ICE))
    return "ice";
  else if (((ltyp != POOL) && (ltyp != WATER) && !Is_medusa_level(&player.uz) &&
            !Is_waterlevel(&player.uz) && !Is_juiblex_level(&player.uz)) ||
           (ltyp == DRAWBRIDGE_UP &&
...
STATIC_OVL void setup_waterlevel() {
  ...
  for (x = xmin; x <= xmax; x++)
    for (y = ymin; y <= ymax; y++) levl[x][y].glyph = water_glyph;
  ...
  for (x = bxmin; x <= bxmax; x += xskip)
    for (y = bymin; y <= bymax; y += yskip) mk_bubble(x, y, rn2(7));
}
...
STATIC_OVL void mk_bubble(int x, int y, int n) {
  ...
  static uchar bm2[] = {2, 1, 0x3}, bm3[] = {3, 2, 0x7, 0x7},
               bm4[] = {4, 3, 0x6, 0xf, 0x6}, bm5[] = {5, 3, 0xe, 0x1f, 0xe},
