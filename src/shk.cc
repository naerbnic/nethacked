...
STATIC_DCL void call_kops(Monster *, bool);
#ifdef OVLB
...
STATIC_DCL void setpaid(Monster *);
STATIC_DCL long addupbill(Monster *);
STATIC_DCL void pacify_shk(Monster *);
STATIC_DCL struct bill_x *onbill(Object *, Monster *, bool);
STATIC_DCL Monster *next_shkp(Monster *, bool);
STATIC_DCL long shop_debt(struct eshk *);
STATIC_DCL char *shk_owns(char *, Object *);
STATIC_DCL char *mon_owns(char *, Object *);
STATIC_DCL void clear_unpaid(Object *);
STATIC_DCL long check_credit(long, Monster *);
STATIC_DCL void pay(long, Monster *);
STATIC_DCL long get_cost(Object *, Monster *);
STATIC_DCL long set_cost(Object *, Monster *);
STATIC_DCL const char *shk_embellish(Object *, long);
STATIC_DCL long cost_per_charge(Monster *, Object *, bool);
STATIC_DCL long cheapest_item(Monster *);
STATIC_DCL int dopayobj(Monster *, struct bill_x *, Object **, int, bool);
STATIC_DCL long stolen_container(Object *, Monster *, long, bool);
STATIC_DCL long getprice(Object *, bool);
STATIC_DCL void shk_names_obj(Monster *, Object *, const char *, long,
                              const char *);
STATIC_DCL Object *bp_to_obj(struct bill_x *);
STATIC_DCL bool inherits(Monster *, int, int);
STATIC_DCL void set_repo_loc(struct eshk *);
STATIC_DCL bool angry_shk_exists();
STATIC_DCL void rile_shk(Monster *);
STATIC_DCL void rouse_shk(Monster *, bool);
STATIC_DCL void remove_damage(Monster *, bool);
STATIC_DCL void sub_one_frombill(Object *, Monster *);
STATIC_DCL void add_one_tobill(Object *, bool);
 ...
 */
long money2mon(Monster *mon, long amount) {
  Object *ygold = findgold(invent);
 ...
 */
void money2u(Monster *mon, long amount) {
  Object *mongold = findgold(mon->minvent);
...
STATIC_OVL Monster *next_shkp(Monster *shkp, bool withbill) {
...
/* called in do_name.c */
char *shkname(Monster *mtmp) { return (ESHK(mtmp)->shknam); }

/* called in mon.c */
void shkgone(Monster *mtmp) {
  ...
  if (on_level(&eshk->shoplevel, &player.uz)) {
    ...
    if (!search_special(ANY_SHOP)) level.flags.has_shop = 0;
...
STATIC_OVL void setpaid(Monster *shkp) {
  Object *obj;
  Monster *mtmp;
  ...
  for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) clear_unpaid(mtmp->minvent);
...
STATIC_OVL long addupbill(Monster *shkp) {
...
STATIC_OVL void call_kops(Monster *shkp, bool nearshop) {
  ...
  if (flags.soundok) pline("An alarm sounds!");
...

  {
    ...
    if (nearshop) {
      ...
      if (flags.verbose) pline_The("Keystone Kops appear!");
      mm.x = player.ux;
      mm.y = player.uy;
      makekops(&mm);
      return;
    }
    if (flags.verbose) pline_The("Keystone Kops are after you!");
...
static bool rob_shop(Monster *shkp) {
  struct eshk *eshkp;
  long total;
  ...
  if (eshkp->credit >= total) {
    Your("credit of %ld %s is used to cover your shopping bill.", eshkp->credit,
...
void u_entered_shop(char *enterstring) {
  ...
  if (!inhishop(shkp)) {
    ...
    if (!index(empty_shops, *enterstring)) pline(no_shk);
    strcpy(empty_shops, player.ushops);
    player.ushops[0] = '\0';
    return;
  }
  ...
  if (!inside_shop(player.ux, player.uy)) {
    ...
    int cnt;
    const char *tool;
    Object *pick = carrying(PICK_AXE), *mattock = carrying(DWARVISH_MATTOCK);
    ...
    if (pick || mattock) {
      ...
      verbalize(NOTANGRY(shkp) ? "Will you please leave your %s%s outside?"
 ...
 */
bool same_price(Object *obj1, Object *obj2) {
  Monster *shkp1, *shkp2;
...
int inhishop(Monster *mtmp) {
  return (index(in_rooms(mtmp->mx, mtmp->my, SHOPBASE), ESHK(mtmp)->shoproom) &&
          on_level(&(ESHK(mtmp)->shoplevel), &player.uz));
}

Monster *shop_keeper(char rmno) {
  Monster *shkp = rmno >= ROOMOFFSET ? rooms[rmno - ROOMOFFSET].resident : 0;
...
STATIC_OVL struct bill_x *onbill(Object *obj, Monster *shkp, bool silent) {
  if (shkp) {
    struct bill_x *bp = ESHK(shkp)->bill_p;
    int ct = ESHK(shkp)->billct;
...
void delete_contents(Object *obj) {
  Object *curr;
...
void obfree(Object *obj, Object *merge) {
  struct bill_x *bp;
  struct bill_x *bpm;
  Monster *shkp;
...
STATIC_OVL void pay(long tmp, Monster *shkp) {
...
void home_shk(Monster *shkp, bool killkops) {
...
STATIC_OVL bool angry_shk_exists() {
  Monster *shkp;

  for (shkp = next_shkp(fmon, FALSE); shkp; shkp = next_shkp(shkp->nmon, FALSE))
    if (ANGRY(shkp)) return (TRUE);
  return (FALSE);
}

/* remove previously applied surcharge from all billed items */
STATIC_OVL void pacify_shk(Monster *shkp) {
...
STATIC_OVL void rile_shk(Monster *shkp) {
...
STATIC_OVL void rouse_shk(Monster *shkp, bool verbosely) {
...
void make_happy_shk(Monster *shkp, bool silentkops) {
  ...
  eshkp->following = 0;
  eshkp->robbed = 0L;
  if (!Role_if(PM_ROGUE)) adjalign(sgn(player.ualign.type));
  if (!inhishop(shkp)) {
    char shk_nam[BUFSZ];
    ...
    if (vanished) pline("Satisfied, %s suddenly disappears!", shk_nam);
  } else if (wasmad)
 ...
 */
void make_angry_shk(Monster *shkp, xchar ox, xchar oy) {
  xchar sx, sy;
  struct eshk *eshkp = ESHK(shkp);
...
STATIC_OVL long cheapest_item(Monster *shkp) {
  ...
  while (ct--) {
    if (bp->price * bp->bquan < gmin) gmin = bp->price * bp->bquan;
    bp++;
  }
  return (gmin);
}
...
proceed:
  ...
  if (ltmp || eshkp->billct || eshkp->debit) rouse_shk(shkp, TRUE);
...
#endif
    if (!ltmp) You("do not owe %s anything.", mon_nam(shkp));
#ifndef GOLDOBJ
      ...
      if (stashed_gold) pline("But you have some gold stashed away.");
    } else {
#ifndef GOLDOBJ
      ...
      if (umoney > ltmp) {
#endif
        You("give %s the %ld gold piece%s %s asked for.", mon_nam(shkp), ltmp,
            plur(ltmp), mhe(shkp));
        pay(ltmp, shkp);
      } else {
...
#ifndef GOLDOBJ
      if (player.ugold < ltmp / 2L || (player.ugold < ltmp && stashed_gold)) {
        if (!player.ugold)
#else
      if (umoney < ltmp / 2L || (umoney < ltmp && stashed_gold)) {
        if (!umoney)
  ...
  if (eshkp->debit) {
    ...
    umoney = money_cnt(invent);
#endif
    sprintf(sbuf, "You owe %s %ld %s ", shkname(shkp), dtmp, currency(dtmp));
    if (loan) {
      if (loan == dtmp)
    ...
    if (umoney + eshkp->credit < dtmp) {
      ...
      pline("But you don't%s have enough gold%s.",
            stashed_gold ? " seem to" : "", eshkp->credit ? " or credit" : "");
      return (1);
    } else {
  ...
  if (eshkp->billct) {
...
#endif
      You("%shave no money or credit%s.", stashed_gold ? "seem to " : "",
          paid ? " left" : "");
      return (0);
    }
#ifndef GOLDOBJ
    ...
    if ((umoney + eshkp->credit) < cheapest_item(shkp)) {
      ...
      if (stashed_gold) pline("Maybe you have some gold stashed away?");
      return (0);
    }
  ...
  thanks:
    if (!itemize) update_inventory(); /* Done in dopayobj() if itemize. */
  }
  if (!ANGRY(shkp) && paid)
...
STATIC_OVL int dopayobj(Monster *shkp, struct bill_x *bp, Object **obj_p,
...
#endif
  int buy;
  bool stashed_gold = (hidden_gold() > 0L), consumed = (which == 0);
...
#endif
    You("%shave no money or credit left.", stashed_gold ? "seem to " : "");
    return PAY_BROKE;
  }
...
STATIC_OVL bool inherits(Monster *shkp, int numsk, int croaked) {
  long loss = 0L;
#ifdef GOLDOBJ
  long umoney;
#endif
  ...
  if (numsk > 1) {
    if (cansee(shkp->mx, shkp->my && croaked))
      pline("%s %slooks at your corpse%s and %s.", Monnam(shkp),
            ...
            !rn2(2)
                ? (shkp->female ? ", shakes her head," : ", shakes his head,")
                : "",
  ...
  if (roomno == eshkp->shoproom && inhishop(shkp) && !eshkp->billct &&
      !eshkp->robbed && !eshkp->debit && NOTANGRY(shkp) && !eshkp->following) {
    if (invent)
      pline("%s gratefully inherits all your possessions.", shkname(shkp));
    set_repo_loc(eshkp);
    goto clear;
  }
...
#endif
    takes[0] = '\0';
    if (!shkp->mcanmove || shkp->msleeping) strcat(takes, "wakes up and ");
    if (distu(shkp->mx, shkp->my) > 2) strcat(takes, "comes and ");
    strcat(takes, "takes");
...
STATIC_OVL Object *bp_to_obj(struct bill_x *bp) {
  Object *obj;
...
STATIC_OVL long get_cost(Object *obj, Monster *shkp) {
  ...
  if (!obj->dknown || !objects[obj->otyp].oc_name_known) {
    if (obj->oclass == GEM_CLASS && objects[obj->otyp].oc_material == GLASS) {
      ...
      bool pseudorand = (((int)player.ubirthday % obj->otyp) >= obj->otyp / 2);
 ...
 */
long contained_cost(Object *obj, Monster *shkp, long price, bool usell,
                    bool unpaid_only) {
  Object *otmp;
  ...
  for (otmp = obj->cobj; otmp; otmp = otmp->nobj) {
    ...
    if (usell) {
      if (saleable(shkp, otmp) && !otmp->unpaid && otmp->oclass != BALL_CLASS &&
...
long contained_gold(Object *obj) {
  Object *otmp;
  long value = 0L;
...
STATIC_OVL void dropped_container(Object *obj, Monster *shkp, bool sale) {
  Object *otmp;
...
void picked_container(Object *obj) {
  Object *otmp;
...
long unpaid_cost(Object *unp_obj) {
...
STATIC_OVL void add_one_tobill(Object *obj, bool dummy) {
  Monster *shkp;
  struct bill_x *bp;
  int bct;
...
STATIC_OVL void add_to_billobjs(Object *obj) {
  if (obj->where != OBJ_FREE) panic("add_to_billobjs: obj not free");
...
STATIC_OVL void bill_box_content(Object *obj, bool ininv, bool dummy,
                                 Monster *shkp) {
  Object *otmp;
  ...
  for (otmp = obj->cobj; otmp; otmp = otmp->nobj) {
    ...
    if (!otmp->no_charge) add_one_tobill(otmp, dummy);
...
STATIC_OVL void shk_names_obj(Monster *shkp, Object *obj, const char *fmt,
...
void addtobill(Object *obj, bool ininv, bool dummy, bool silent) {
  ...
  if (/* perhaps we threw it away earlier */
      onbill(obj, shkp, FALSE) || (obj->oclass == FOOD_CLASS && obj->oeaten))
    return;
...
speak:
  if (shkp->mcanmove && !shkp->msleeping && !silent) {
    ...
    if (ANGRY(shkp))
      ...
      static const char *honored[5] = {"good", "honored", "most gracious",
                                       "esteemed", "most renowned and sacred"};
    ...
    if (ininv) {
      ...
      pline("%s will cost you %ld %s%s.", The(xname(obj)), ltmp, currency(ltmp),
            (obj->quan > 1L) ? " each" : "");
  } else if (!silent) {
    if (ltmp)
      pline_The("list price of %s is %ld %s%s.", the(xname(obj)), ltmp,
...
void splitbill(Object *obj, Object *otmp) {
...
STATIC_OVL void sub_one_frombill(Object *obj, Monster *shkp) {
  struct bill_x *bp;
...
void subfrombill(Object *obj, Monster *shkp) {
  Object *otmp;
...
STATIC_OVL long stolen_container(Object *obj, Monster *shkp, long price,
  ...
  if (ininv && obj->unpaid)
    ...
    if (!obj->no_charge) price += get_cost(obj, shkp);
    obj->no_charge = 0;
  }
  ...
  for (otmp = obj->cobj; otmp; otmp = otmp->nobj) {
    ...
    if (!Has_contents(otmp)) {
      if (ininv) {
        if (otmp->unpaid) price += otmp->quan * get_cost(otmp, shkp);
      } else {
        if (!otmp->no_charge) {
...
long stolen_value(Object *obj, xchar x, xchar y, bool peaceful, bool silent) {
  ...
  Monster *shkp = shop_keeper(*in_rooms(x, y, SHOPBASE));

  if (!shkp || !inhishop(shkp)) return (0L);
  ...
  if (peaceful) {
    ...
    if (!silent) {
      if (cansee(shkp->mx, shkp->my)) {
        Norep("%s booms: \"%s, you are a thief!\"", Monnam(shkp), plname);
      } else
...
void sellobj(Object *obj, xchar x, xchar y) {
  Monster *shkp;
  struct eshk *eshkp;
  ...
  if (!isgold && ((offer + gltmp) == 0L || sell_how == SELL_DONTSELL)) {
    bool unpaid = (obj->unpaid || (container && count_unpaid(obj->cobj)));

    if (container) {
      dropped_container(obj, shkp, FALSE);
      if (!obj->unpaid && !saleitem) obj->no_charge = 1;
      if (obj->unpaid || count_unpaid(obj->cobj)) subfrombill(obj, shkp);
    } else
      obj->no_charge = 1;
  ...
  if (eshkp->robbed) { /* shkp is not angry? */
    if (isgold)
      ...
      offer += cgold;
    if ((eshkp->robbed -= offer < 0L)) eshkp->robbed = 0L;
    if (offer)
      verbalize(
          "Thank you for your contribution to restock this recently "
          "plundered shop.");
    subfrombill(obj, shkp);
    return;
  }
...
move_on:
  if ((!saleitem && !(container && cltmp > 0L)) || eshkp->billct == BILLSZ ||
      obj->oclass == BALL_CLASS || obj->oclass == CHAIN_CLASS || offer == 0L ||
      (obj->oclass == FOOD_CLASS && obj->oeaten) ||
  ...
  if (!money_cnt(shkp->minvent)) {
    ...
    if (!sell_response) {
      ...
      sprintf(
          qbuf, "%s offers%s %ld gold piece%s for%s %s %s.  Sell %s?",
          Monnam(shkp), short_funds ? " only" : "", offer, plur(offer),
...
int doinvbill(int mode) {
  ...
  if (mode == 0) {
    ...
    for (bp = eshkp->bill_p, end_bp = &eshkp->bill_p[eshkp->billct];
         bp < end_bp; bp++)
      if (bp->useup || ((obj = bp_to_obj(bp)) != 0 && obj->quan < bp->bquan))
        cnt++;
    return cnt;
  }
  ...
  totused = 0L;
  for (bp = eshkp->bill_p, end_bp = &eshkp->bill_p[eshkp->billct]; bp < end_bp;
       bp++) {
    obj = bp_to_obj(bp);
    if (!obj) {
  ...
  if (eshkp->debit) {
    ...
    totused += eshkp->debit;
    buf_p = xprname(nullptr, "usage charges and/or other fees", GOLD_SYM, FALSE,
                    eshkp->debit, 0L);
    putstr(datawin, 0, buf_p);
  }
...
STATIC_OVL long getprice(Object *obj, bool shk_buying) {
  ...
  switch (obj->oclass) {
    ...
    case WAND_CLASS:
      ...
      break;
    case POTION_CLASS:
      if (obj->otyp == POT_WATER && !obj->blessed && !obj->cursed) tmp = 0L;
      break;
    case ARMOR_CLASS:
    case WEAPON_CLASS:
...
Monster *shkcatch(Object *obj, xchar x, xchar y) {
  Monster *shkp;

  if (!(shkp = shop_keeper(inside_shop(x, y))) || !inhishop(shkp)) return (0);

  if (shkp->mcanmove && !shkp->msleeping &&
      (*player.ushops != ESHK(shkp)->shoproom ||
       !inside_shop(player.ux, player.uy)) &&
      dist2(shkp->mx, shkp->my, x, y) < 3 &&
      /* if it is the shk's pos, you hit and anger him */
      (shkp->mx != x || shkp->my != y)) {
    if (mnearto(shkp, x, y, TRUE)) verbalize("Out of my way, scum!");
    if (cansee(x, y)) {
...
void add_damage(xchar x, xchar y, long cost) {
  ...
  if (cansee(x, y)) levl[x][y].seenv = SVALL;
}
 ...
 */
STATIC_OVL
void remove_damage(Monster *shkp, bool croaked) {
  ...
  while (tmp_dam) {
    ...
    if (index(shops, ESHK(shkp)->shoproom)) {
      if (croaked)
        ...
        if (closed_door(x, y)) stop_picking = picking_at(x, y);
        disposition = repair_damage(shkp, tmp_dam, FALSE);
        if (!disposition) stop_picking = FALSE;
      }
    }
  ...
  if (stop_picking) stop_occupation();
}
 ...
 * 3: untrap
 */
int repair_damage(Monster *shkp, struct damage *tmp_dam, bool catchup) {
  xchar x, y, i;
  xchar litter[9];
  Monster *mtmp;
  Object *otmp;
  ...
  if ((monstermoves - tmp_dam->when) < REPAIR_DELAY) return (0);
  ...
  if (!IS_ROOM(tmp_dam->typ)) {
    ...
    if ((mtmp = m_at(x, y)) && (!passes_walls(mtmp->data))) return (0);
  }
  if ((ttmp = t_at(x, y)) != 0) {
    if (x == player.ux && y == player.uy)
      if (!Passes_walls) return (0);
    if (ttmp->ttyp == LANDMINE || ttmp->ttyp == BEAR_TRAP) {
      ...
      otmp = MakeSpecificObject((ttmp->ttyp == LANDMINE) ? LAND_MINE : BEARTRAP,
                                TRUE, FALSE);
      otmp->quan = 1;
...
#define vert(i) ((i / 3) - 1)
    for (i = 0; i < 9; i++) {
      if ((i == 4) || (!ZAP_POS(levl[x + horiz(i)][y + vert(i)].typ))) continue;
      litter[i] = OPEN;
      if (inside_shop(x + horiz(i), y + vert(i)) == ESHK(shkp)->shoproom)
        litter[i] |= INSHOP;
    }
    if (Punished && !player.uswallow && ((uchain->ox == x && uchain->oy == y) ||
  ...
  for (i = 0; i < 9; i++)
    if (litter[i] & NEED_UPDATE) newsym(x + horiz(i), y + vert(i));
  return (2);
#undef NEED_UPDATE
#undef OPEN
#undef INSHOP
...
int shk_move(Monster *shkp) {
  ...
  if (inhishop(shkp)) remove_damage(shkp, FALSE);

  if ((udist = distu(omx, omy)) < 3 &&
      (shkp->data != &mons[PM_GRID_BUG] ||
       (omx == player.ux || omy == player.uy))) {
    if (ANGRY(shkp) || (Conflict && !resist(shkp, RING_CLASS, 0, 0))) {
      if (Displaced) Your("displaced image doesn't fool %s!", mon_nam(shkp));
      (void)mattacku(shkp);
      return (0);
    }
    if (eshkp->following) {
  ...
  if (eshkp->following || ((z = holetime()) >= 0 && z * z <= udist)) {
    ...
    if (udist > 4 && eshkp->following) return (-1); /* leave it to m_move */
    gx = player.ux;
    gy = player.uy;
  } else if (ANGRY(shkp)) {
...
void after_shk_move(Monster *shkp) {
  struct eshk *eshkp = ESHK(shkp);
...
void shopdig(int fall) {
  ...
  if (!fall) {
    ...
    if (distu(shkp->mx, shkp->my) > 2) {
      ...
      if (distu(shkp->mx, shkp->my) > 2) {
        if (lang == 2)
          pline("%s curses you in anger and frustration!", shkname(shkp));
        rile_shk(shkp);
        return;
      } else
...
void pay_for_damage(const char *dmgstr, bool cant_mollify) {
  ...
  for (tmp_dam = level.damagelist; (tmp_dam && (tmp_dam->when == monstermoves));
       tmp_dam = tmp_dam->next) {
    char *shp;

    if (!tmp_dam->cost) continue;
    ...
    for (shp = shops_affected; *shp; shp++) {
      ...
      if (!(tmp_shk = shop_keeper(*shp))) continue;
      if (tmp_shk == shkp) {
...
Object *shop_object(xchar x, xchar y) {
  Object *otmp;
  Monster *shkp;
...
void price_quote(Object *first_obj) {
  Object *otmp;
      ...
      case 4:
        ...
        if (objects[itm->otyp].oc_name_known
                ...
                : (o == AMULET_CLASS || o == RING_CLASS || o == WAND_CLASS ||
                   o == POTION_CLASS || o == SCROLL_CLASS || o == SPBOOK_CLASS))
...
void shk_chat(Monster *shkp) {
  ...
  if (ANGRY(shkp))
    ...
    pline("%s reminds you that you owe %s %ld %s.", shkname(shkp), mhim(shkp),
...
void check_unpaid_usage(Object *otmp, bool altusage) {
  ...
  if (!otmp->unpaid || !*player.ushops ||
    ...
    return;
  if (!(shkp = shop_keeper(*player.ushops)) || !inhishop(shkp)) return;
  if ((tmp = cost_per_charge(shkp, otmp, altusage)) == 0L) return;
...
void costly_gold(xchar x, xchar y, long amount) {
  ...
  if (eshkp->credit >= amount) {
    ...
    if (eshkp->credit) Your("credit is erased.");
    if (eshkp->debit)
...
bool block_door(xchar x, xchar y) {
  ...
  if (!(shkp = shop_keeper((char)roomno)) || !inhishop(shkp)) return (FALSE);
...
bool block_entry(xchar x, xchar y) {
  ...
  if (!(IS_DOOR(levl[player.ux][player.uy].typ) &&
        levl[player.ux][player.uy].doormask == D_BROKEN))
    return (FALSE);
...
STATIC_OVL char *shk_owns(char *buf, Object *obj) {
  Monster *shkp;
  xchar x, y;
...
#ifdef __SASC
void sasc_bug(Object *op, unsigned x) { op->unpaid = x; }
#endif
