...
STATIC_DCL MonsterType *lookat(int, int, char *, char *);
STATIC_DCL void checkfile(char *, MonsterType *, bool, bool);
...
STATIC_OVL MonsterType *lookat(int x, int y, char *buf, char *monbuf) {
  ...
  if (player.ux == x && player.uy == y && senseself()) {
    ...
    sprintf(buf, "%s%s%s called %s", Invis ? "invisible " : "", race,
            mons[player.umonnum].mname, plname);
    ...
    if (mtmp != nullptr) {
...

      {
        ...
        xraydist = (player.xray_range < 0) ? -1 : player.xray_range *
                                                      player.xray_range;
        /* normal vision */
    ...
    switch (glyph_to_cmap(glyph)) {
      ...
      case S_cloud:
        strcpy(buf,
               Is_airlevel(&player.uz) ? "cloudy area" : "fog/vapor cloud");
        break;
      default:
 ...
 */
STATIC_OVL void checkfile(char *inp, MonsterType *pm, bool user_typed_name,
                          bool without_asking) {
  dlb *fp;
  ...
  do {
    ...
    if (from_screen) {
      ...
      if (glyph_is_cmap(glyph)) {
        ...
        if (sym == '`' && iflags.bouldersym &&
            (int)glyph_to_obj(glyph) == BOULDER)
        ...
        impossible("do_look:  bad glyph %d at (%d,%d)", glyph, (int)cc.x,
                   (int)cc.y);
        sym = ' ';
      }
    }
    ...
    for (i = 0; i < MAXMCLASSES; i++) {
      if (sym == (from_screen ? monsyms[i] : def_monsyms[i]) && monexplain[i]) {
        need_to_look = TRUE;
        if (!found) {
    ...
    if ((from_screen ? (sym == monsyms[S_HUMAN] && cc.x == player.ux &&
                        cc.y == player.uy)
...
#define is_cmap_drawbridge(i) ((i) >= S_vodbridge && (i) <= S_hcdbridge)
    ...
    for (i = 1; i < WARNCOUNT; i++) {
      ...
      if (sym == (from_screen ? warnsyms[i] : def_warnsyms[i].sym)) {
        if (!found) {
          sprintf(out_str, "%c       %s", sym, def_warnsyms[i].explanation);
...
int dowhatis() { return do_look(FALSE); }

int doquickwhatis() { return do_look(TRUE); }
...
int doidtrap() {
  ...
  for (trap = ftrap; trap; trap = trap->ntrap)
    if (trap->tx == x && trap->ty == y) {
      ...
      tt = what_trap(tt);
      pline("That is %s%s%s.", an(defsyms[trap_to_defsym(tt)].explanation),
            !trap->madeby_u
                ? ""
                : (tt == WEB) ? " woven" :
...
STATIC_OVL bool help_menu(int *sel) {
  ...
  for (i = 0; help_menu_items[i]; i++)
...
#endif
    {
      ...
      add_menu(tmpwin, NO_GLYPH, &any, 0, 0, ATR_NONE, help_menu_items[i],
               MENU_UNSELECTED);
    }
