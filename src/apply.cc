...
#ifdef TOURIST
STATIC_DCL int use_camera(Object *);
#endif
STATIC_DCL int use_towel(Object *);
STATIC_DCL bool its_dead(int, int, int *);
STATIC_DCL int use_stethoscope(Object *);
STATIC_DCL void use_whistle(Object *);
STATIC_DCL void use_magic_whistle(Object *);
STATIC_DCL void use_leash(Object *);
STATIC_DCL int use_mirror(Object *);
STATIC_DCL void use_bell(Object **);
STATIC_DCL void use_candelabrum(Object *);
STATIC_DCL void use_candle(Object **);
STATIC_DCL void use_lamp(Object *);
STATIC_DCL void light_cocktail(Object *);
STATIC_DCL void use_tinning_kit(Object *);
STATIC_DCL void use_figurine(Object **);
STATIC_DCL void use_grease(Object *);
STATIC_DCL void use_trap(Object *);
STATIC_DCL void use_stone(Object *);
STATIC_PTR int set_trap(); /* occupation callback */
STATIC_DCL int use_whip(Object *);
STATIC_DCL int use_pole(Object *);
STATIC_DCL int use_cream_pie(Object *);
STATIC_DCL int use_grapple(Object *);
STATIC_DCL int do_break_wand(Object *);
STATIC_DCL bool figurine_location_checks(Object *, coord *, bool);
...
#ifdef TOURIST
STATIC_OVL int use_camera(Object *obj) {
  Monster *mtmp;
  ...
  if (obj->cursed && !rn2(2)) {
    ...
    You("take a picture of %s %s.", s_suffix(mon_nam(player.ustuck)),
        mbodypart(player.ustuck, STOMACH));
  } else if (player.dz) {
    You("take a picture of the %s.", (player.dz > 0)
                                         ? surface(player.ux, player.uy)
                                         : ceiling(player.ux, player.uy));
  } else if (!player.dx && !player.dy) {
    (void)zapyourself(obj, TRUE);
  } else if ((mtmp = bhit(player.dx, player.dy, COLNO, FLASHED_LIGHT,
                          (int (*)(Monster *, Object *))0,
                          (int (*)(Object *, Object *))0, obj, NULL)) != 0) {
...
STATIC_OVL int use_towel(Object *obj) {
  if (!freehand()) {
    ...
    *resp = 1;
    ...
    if (otmp->otyp == CORPSE) {
      You("determine that %s unfortunate being is dead.",
          (rx == player.ux && ry == player.uy) ? "this" : "that");
    } else {
      ttmp = t_at(rx, ry);
      pline(
          "%s appears to be in %s health for a statue.",
...
STATIC_OVL int use_stethoscope(Object *obj) {
  static long last_used_move = -1;
  static short last_used_movement = 0;
  Monster *mtmp;
  struct rm *lev;
  int rx, ry, res;
  bool interference = (player.uswallow && is_whirly(player.ustuck->data) &&
  ...
  if (player.usteed && player.dz > 0) {
    if (interference) {
      pline("%s interferes.", Monnam(player.ustuck));
      mstatusline(player.ustuck);
    } else
      mstatusline(player.usteed);
    return res;
  } else
#endif
      if (player.uswallow && (player.dx || player.dy || player.dz)) {
    mstatusline(player.ustuck);
    return res;
  } else if (player.uswallow && interference) {
    pline("%s interferes.", Monnam(player.ustuck));
    mstatusline(player.ustuck);
    return res;
  } else if (player.dz) {
    ...
    else if (player.dz < 0 || !can_reach_floor())
      You_cant("reach the %s.", (player.dz > 0)
                                    ? surface(player.ux, player.uy)
                                    : ceiling(player.ux, player.uy));
    else if (its_dead(player.ux, player.uy, &res))
      ; /* message already given */
    else if (Is_stronghold(&player.uz))
      You_hear("the crackling of hellfire.");
    else
      pline_The("%s seems healthy enough.", surface(player.ux, player.uy));
    return res;
...
STATIC_OVL void use_whistle(Object *obj) {
  You(whistle_str, obj->cursed ? "shrill" : "high");
  wake_nearby();
}

STATIC_OVL void use_magic_whistle(Object *obj) {
  Monster *mtmp, *nextmon;
...
bool um_dist(xchar x, xchar y, xchar n) {
  return ((bool)(abs(player.ux - x) > n || abs(player.uy - y) > n));
}
...
void o_unleash(Object *otmp) {
  Monster *mtmp;

  for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
    if (mtmp->m_id == (unsigned)otmp->leashmon) mtmp->mleashed = 0;
  otmp->leashmon = 0;
}

/* mtmp is about to die, or become untame */
void m_unleash(Monster *mtmp, bool feedback) {
  Object *otmp;
...
void unleash_all() {
  ...
  for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) mtmp->mleashed = 0;
}

#define MAXLEASHED 2

/* ARGSUSED */
STATIC_OVL void use_leash(Object *obj) {
  coord cc;
  Monster *mtmp;
  int spotmon;
...
Object *get_mleash(Monster *mtmp) {
  Object *otmp;

  otmp = invent;
  while (otmp) {
    if (otmp->otyp == LEASH && otmp->leashmon == (int)mtmp->m_id) return (otmp);
    otmp = otmp->nobj;
  }
  return (nullptr);
}
...
bool next_to_u() {
  ...
  if (player.usteed && mon_has_amulet(player.usteed)) return FALSE;
#endif
  return (TRUE);
}
    ...
    if (dist2(player.ux, player.uy, mtmp->mx, mtmp->my) >
      ...
      if (!um_dist(mtmp->mx, mtmp->my, 3)) {
        ...
        if (um_dist(mtmp->mx, mtmp->my, 5) || (mtmp->mhp -= rnd(2)) <= 0) {
          ...
          if (mtmp->mhp > 0) player.uconduct.killer = save_pacifism;
        } else {
        ...
        if (um_dist(mtmp->mx, mtmp->my, 5)) {
          ...
          if (mtmp->data->msound != MS_SILENT) switch (rn2(3)) {
              case 0:
                growl(mtmp);
                break;
              case 1:
...
STATIC_OVL int use_mirror(Object *obj) {
  Monster *mtmp;
  char mlet;
  bool vis;
  ...
  if (!getdir((char *)0)) return 0;
  if (obj->cursed && !rn2(2)) {
    if (!Blind) pline_The("mirror fogs up and doesn't reflect!");
    return 1;
  }
  if (!player.dx && !player.dy && !player.dz) {
    if (!Blind && !Invisible) {
      if (player.umonnum == PM_FLOATING_EYE) {
        if (!Free_action) {
          pline(Hallucination ? "Yow!  The mirror stares back!"
                              : "Yikes!  You've frozen yourself!");
          nomul(-rnd((MAXULEV + 6) - player.ulevel), "gazing into a mirror");
        } else
          You("stiffen momentarily under your gaze.");
      } else if (youmonst.data->mlet == S_VAMPIRE)
        You("don't have a reflection.");
      else if (player.umonnum == PM_UMBER_HULK) {
        ...
        You("look as %s as ever.",
            ACURR(A_CHA) > 14 ? (poly_gender() == 1 ? "beautiful" : "handsome")
                              : "ugly");
    } else {
  ...
  if (player.uswallow) {
    if (!Blind)
      You("reflect %s %s.", s_suffix(mon_nam(player.ustuck)),
          mbodypart(player.ustuck, STOMACH));
    return 1;
  }
  if (Underwater) {
  ...
  if (player.dz) {
    if (!Blind)
      You("reflect the %s.", (player.dz > 0) ? surface(player.ux, player.uy)
                                             : ceiling(player.ux, player.uy));
    return 1;
  }
  mtmp = bhit(player.dx, player.dy, COLNO, INVIS_BEAM,
              (int (*)(Monster *, Object *))0, (int (*)(Object *, Object *))0,
              obj, NULL);
  if (!mtmp || !haseyes(mtmp->data)) return 1;

  vis = canseemon(mtmp);
  mlet = mtmp->data->mlet;
  if (mtmp->msleeping) {
    if (vis) pline("%s is too tired to look at your mirror.", Monnam(mtmp));
  } else if (!mtmp->mcansee) {
    if (vis) pline("%s can't see anything right now.", Monnam(mtmp));
    /* some monsters do special things */
  } else if (mlet == S_VAMPIRE || mlet == S_GHOST) {
    if (vis) pline("%s doesn't have a reflection.", Monnam(mtmp));
  } else if (!mtmp->mcan && !mtmp->minvis && mtmp->data == &mons[PM_MEDUSA]) {
    if (mon_reflects(mtmp, "The gaze is reflected away by %s %s!")) return 1;
    if (vis) pline("%s is turned to stone!", Monnam(mtmp));
    stoned = TRUE;
    killed(mtmp);
    ...
    if (vis) pline("%s confuses itself!", Monnam(mtmp));
    mtmp->mconf = 1;
  } else if (!mtmp->mcan && !mtmp->minvis &&
    ...
    if (vis) pline("%s is frightened by its reflection.", Monnam(mtmp));
...
STATIC_OVL void use_bell(Object **optr) {
  ...
  if (Underwater || (player.uswallow && ordinary)) {
    ...
    if (obj->cursed && !rn2(4) &&
        ...
        (mtmp = makemon(mkclass(S_NYMPH, 0), player.ux, player.uy,
                        NO_MINVENT)) != 0) {
...
STATIC_OVL void use_candelabrum(Object *obj) {
...
STATIC_OVL void use_candle(Object **optr) {
  Object *obj = *optr;
  Object *otmp;
  ...
  if (yn(qbuf) == 'n') {
    if (!obj->lamplit) You("try to light %s...", the(xname(obj)));
    use_lamp(obj);
    return;
  } else {
    ...
    You("attach %ld%s %s to %s.", obj->quan, !otmp->spe ? "" : " more", s,
        the(xname(otmp)));
    if (!otmp->spe || otmp->age > obj->age) otmp->age = obj->age;
    ...
    if (obj->unpaid)
      verbalize("You %s %s, you bought %s!", otmp->lamplit ? "burn" : "use",
                (obj->quan > 1L) ? "them" : "it",
                (obj->quan > 1L) ? "them" : "it");
...
bool snuff_candle(Object *otmp) /* call in drop, throw, and put in box, etc. */
{
...
bool catch_lit(Object *obj) {
  ...
  if (!obj->lamplit && (obj->otyp == MAGIC_LAMP || ignitable(obj))) {
    if ((obj->otyp == MAGIC_LAMP || obj->otyp == CANDELABRUM_OF_INVOCATION) &&
      ...
      return FALSE;
    if (!get_obj_location(obj, &x, &y, 0)) return FALSE;
...
STATIC_OVL void use_lamp(Object *obj) {
  char buf[BUFSZ];
...
STATIC_OVL void light_cocktail(Object *obj) {
  char buf[BUFSZ];
...
int jump(int magic) {
  ...
  if (!magic && (nolimbs(youmonst.data) || slithy(youmonst.data))) {
    ...
    if (magic) {
      You("writhe a little in the grasp of %s!", mon_nam(player.ustuck));
      return 1;
    }
    You("cannot escape from %s!", mon_nam(player.ustuck));
    return 0;
  } else if (Levitation || Is_airlevel(&player.uz) ||
    ...
    return 0;
  } else if (!magic && (player.uhunger <= 100 || ACURR(A_STR) < 6)) {
...
#ifdef STEED
    if (player.usteed)
      pline("%s is in no shape for jumping.", Monnam(player.usteed));
    else
#endif
      Your("%s%s %s in no shape for jumping.",
           (wl == LEFT_SIDE) ? "left " : (wl == RIGHT_SIDE) ? "right " : "", bp,
           (wl == BOTH_SIDES) ? "are" : "is");
    return 0;
  }
#ifdef STEED
  else if (player.usteed && player.utrap) {
    pline("%s is stuck in a trap.", Monnam(player.usteed));
    return (0);
  }
#endif
  ...
  if (!magic && !(HJumping & ~INTRINSIC) && !EJumping &&
    ...
    if (player.utrap) switch (player.utraptype) {
        case TT_BEARTRAP: {
    ...
    if (In_sokoban(&player.uz)) change_luck(-1);
...
bool tinnable(Object *corpse) {
  if (corpse->oeaten) return 0;
  if (!mons[corpse->corpsenm].cnutrit) return 0;
  return 1;
}

STATIC_OVL void use_tinning_kit(Object *obj) {
  Object *corpse, *can;
  ...
  if (touch_petrifies(&mons[corpse->corpsenm]) && !Stone_resistance && !uarmg) {
    ...
    if (poly_when_stoned(youmonst.data))
      You("tin %s without wearing gloves.", an(mons[corpse->corpsenm].mname));
    else {
  ...
  if ((can = MakeSpecificObject(TIN, FALSE, FALSE)) != 0) {
    ...
    if (carried(corpse)) {
      if (corpse->unpaid) verbalize(you_buy_it);
      useup(corpse);
    } else {
...
void use_unicorn_horn(Object *obj) {
...
#define attr_trouble(Y) trouble_list[trouble_count++] = attr2trbl(Y)
  ...
  if (Blinded > (long)player.ucreamed) prop_trouble(BLINDED);
  ...
  if (did_attr)
    pline("This makes you feel %s!",
          (did_prop + did_attr) == (trouble_count + unfixable_trbl) ? "great"
                                                                    : "better");
  else if (!did_prop)
    ...
    switch (figurine->where) {
      case OBJ_INVENT:
        if (Blind)
          ...
          You("see %s %s out of your pack!", monnambuf,
              locomotion(mtmp->data, "drop"));
        break;
      ...
      case OBJ_MINVENT:
        if (cansee_spot && !silent) {
          ...
          You("see %s %s out of %s!", monnambuf, locomotion(mtmp->data, "drop"),
              carriedby);
        }
        break;
#if 0
...
STATIC_OVL bool figurine_location_checks(Object *obj, coord *cc, bool quietly) {
  xchar x, y;

  if (carried(obj) && player.uswallow) {
    if (!quietly) You("don't have enough room in here.");
    return FALSE;
  }
  x = cc->x;
  y = cc->y;
  ...
  if (sobj_at(BOULDER, x, y) && !passes_walls(&mons[obj->corpsenm]) &&
    ...
    if (!quietly) You("cannot fit the figurine on the boulder.");
    return FALSE;
  }
  return TRUE;
}

STATIC_OVL void use_figurine(Object **optr) {
  Object *obj = *optr;
  xchar x, y;
  coord cc;
  ...
  if (player.uswallow) {
    ...
    if (!figurine_location_checks(obj, (coord *)0, FALSE)) return;
  }
  if (!getdir((char *)0)) {
    flags.move = multi = 0;
    return;
      ...
      (player.dx || player.dy)
          ...
          : (Is_airlevel(&player.uz) || Is_waterlevel(&player.uz) ||
             is_pool(cc.x, cc.y))
                ...
                : (player.dz < 0 ? "toss the figurine into the air"
...
STATIC_OVL void use_grease(Object *obj) {
  Object *otmp;
  char buf[BUFSZ];
...
STATIC_OVL void use_stone(Object *tstone) {
  Object *obj;
  bool do_scratch;
  ...
  if ((obj = getobj(choices, stonebuf)) == 0) return;
#ifndef GOLDOBJ
  ...
  if (tstone->otyp == TOUCHSTONE && tstone->cursed &&
    ...
    if (Blind)
      ...
      pline("A sharp crack shatters %s%s.", (obj->quan > 1) ? "one of " : "",
            the(xname(obj)));
#ifndef GOLDOBJ
/* assert(obj != &goldobj); */
#endif
...
STATIC_OVL void use_trap(Object *otmp) {
  ...
  if (nohands(youmonst.data))
    ...
    what =
        is_animal(player.ustuck->data) ? "while swallowed" : "while engulfed";
  else if (Underwater)
    what = "underwater";
  else if (Levitation)
    ...
    what = (player.ux == xdnladder || player.ux == xupladder) ? "on the ladder"
                                                              : "on the stairs";
  else if (IS_FURNITURE(levl[player.ux][player.uy].typ) ||
           IS_ROCK(levl[player.ux][player.uy].typ) ||
           closed_door(player.ux, player.uy) || t_at(player.ux, player.uy))
    what = "here";
  if (what) {
  ...
  if (otmp == trapinfo.tobj && player.ux == trapinfo.tx &&
      player.uy == trapinfo.ty) {
  ...
  trapinfo.tobj = otmp;
  trapinfo.tx = player.ux, trapinfo.ty = player.uy;
  tmp = ACURR(A_DEX);
...
/*[fumbling and/or confusion and/or cursed object check(s)
  ...
  if (player.usteed && P_SKILL(P_RIDING) < P_BASIC) {
    ...
    if (Fumbling || otmp->cursed)
      ...
      chance = (rnl(10) > 5);
    You("aren't very skilled at reaching from %s.", mon_nam(player.usteed));
...
int set_trap() {
  ...
  if (!otmp || !carried(otmp) || player.ux != trapinfo.tx ||
      player.uy != trapinfo.ty) {
    /* ?? */
    reset_trapset();
    return 0;
  ...
  ttyp = (otmp->otyp == LAND_MINE) ? LANDMINE : BEAR_TRAP;
  ttmp = maketrap(player.ux, player.uy, ttyp);
  if (ttmp) {
    ttmp->tseen = 1;
    ttmp->madeby_u = 1;
    newsym(player.ux, player.uy); /* if our hero happens to be invisible */
    if (*in_rooms(player.ux, player.uy, SHOPBASE)) {
      add_damage(player.ux, player.uy, 0L); /* schedule removal */
    }
...
STATIC_OVL int use_whip(Object *obj) {
  char buf[BUFSZ];
  Monster *mtmp;
  Object *otmp;
  ...
  if (player.uswallow && attack(player.ustuck)) {
    ...
    You("flick a bug off of the %s.", ceiling(player.ux, player.uy));

  } else if ((!player.dx && !player.dy) || (player.dz > 0)) {
    int dam;

#ifdef STEED
    /* Sometimes you hit your steed by mistake */
    if (player.usteed && !rn2(proficient + 2)) {
      You("whip %s!", mon_nam(player.usteed));
      kick_steed();
      return 1;
    }
#endif
    if (Levitation
      ...
      if (otmp && proficient) {
        You("wrap your bullwhip around %s on the %s.",
            an(singular(otmp, xname)), surface(player.ux, player.uy));
        if (rnl(6) || pickup_object(otmp, 1L, TRUE) < 1) pline(msg_slipsfree);
        return 1;
      }
    }
    ...
    if (otmp) {
      ...
      You("wrap your bullwhip around %s %s.", s_suffix(mon_nam(mtmp)), onambuf);
      if (gotit && otmp->cursed) {
        pline("%s welded to %s %s%c", (otmp->quan == 1L) ? "It is" : "They are",
      ...
      if (gotit) {
        ...
        switch (rn2(proficient + 1)) {
          case 2:
            /* to floor near you */
            You("yank %s %s to the %s!", s_suffix(mon_nam(mtmp)), onambuf,
                surface(player.ux, player.uy));
...
/* right to you */
          ...
          default:
            ...
            You("yank %s from %s %s!", the(onambuf), s_suffix(mon_nam(mtmp)),
                mon_hand);
...
STATIC_OVL int use_pole(Object *obj) {
  ...
  cc.x = player.ux;
  cc.y = player.uy;
  if (getpos(&cc, TRUE, "the spot to hit") < 0) return 0; /* user pressed ESC */
  ...
  if ((mtmp = m_at(cc.x, cc.y)) != (Monster *)0) {
     ...
     */
    if (mtmp->mhp < oldhp) player.uconduct.weaphit++;
  } else
...
STATIC_OVL int use_cream_pie(Object *obj) {
  bool wasblind = Blind;
  bool wascreamed = player.ucreamed;
  bool several = FALSE;
...
STATIC_OVL int use_grapple(Object *obj) {
  ...
  switch (tohit) {
    ...
    case 1: /* Object */
      ...
      break;
    case 2: /* Monster */
      if ((mtmp = m_at(cc.x, cc.y)) == (Monster *)0) break;
    ...
    case 3: /* Surface */
      if (IS_AIR(levl[cc.x][cc.y].typ) || is_pool(cc.x, cc.y))
        ...
        hurtle(sgn(cc.x - player.ux), sgn(cc.y - player.uy), 1, FALSE);
        spoteffects(TRUE);
      }
      return (1);
...
#define BY_OBJECT ((Monster *)nullptr)

/* return 1 if the wand is broken, hence some time elapsed */
STATIC_OVL int do_break_wand(Object *obj) {
  ...
  switch (obj->otyp) {
    ...
    wanexpl:
      explode(player.ux, player.uy, (obj->otyp - WAN_MAGIC_MISSILE), dmg,
              WAND_CLASS, expltype);
  ...
  for (i = 0; i <= 8; i++) {
    ...
    if (obj->otyp == WAN_DIGGING) {
      if (dig_check(BY_OBJECT, FALSE, x, y)) {
        ...
        digactualhole(
            x, y, BY_OBJECT,
            (rn2(obj->spe) < 3 || !Can_dig_down(&player.uz)) ? PIT : HOLE);
      }
      continue;
...
STATIC_OVL bool uhave_graystone() {
  Object *otmp;

  for (otmp = invent; otmp; otmp = otmp->nobj)
    if (is_graystone(otmp)) return TRUE;
  return FALSE;
}
...
int doapply() {
  ...
  if (obj->oclass == WAND_CLASS) return do_break_wand(obj);
  ...
  switch (obj->otyp) {
    ...
    case TIN_OPENER:
      ...
      if (flags.verbose) pline("In order to eat, use the 'e' command.");
      if (obj != uwep)
        pline(
    ...
    case HORN_OF_PLENTY: /* not a musical instrument */
      if (obj->spe > 0) {
        ...
        if (!rn2(13)) {
          ...
          otmp = MakeRandomObject(FOOD_CLASS, FALSE);
