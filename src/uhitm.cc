...
STATIC_DCL bool hmon_hitmon(Monster *, Object *, int);
#ifdef STEED
STATIC_DCL int joust(Monster *, Object *);
#endif
STATIC_DCL void demonpet();
STATIC_DCL bool m_slips_free(Monster *mtmp, struct Attack *mattk);
STATIC_DCL int explum(Monster *, struct Attack *);
STATIC_DCL void start_engulf(Monster *);
STATIC_DCL void end_engulf();
STATIC_DCL int gulpum(Monster *, struct Attack *);
STATIC_DCL bool hmonas(Monster *, int);
STATIC_DCL void nohandglow(Monster *);
STATIC_DCL bool shade_aware(Object *);
...
void hurtmarmor(Monster *mdef, int attk) {
  int hurt;
  Object *target;
...
bool attack_checks(Monster *mtmp, Object *wep) {
  char qbuf[QBUFSZ];
#ifdef PARANOID
  char buf[BUFSZ];
#endif
  ...
  if (player.uswallow && mtmp == player.ustuck) return FALSE;
  ...
  if (flags.forcefight) {
    ...
    if (!canspotmon(mtmp) &&
        !glyph_is_invisible(levl[player.ux+player.dx][player.uy+player.dy].glyph))
            map_invisible(player.ux+player.dx, player.uy+player.dy);
     */
    return FALSE;
  }
  ...
  if (!canspotmon(mtmp) &&
      !glyph_is_warning(
          glyph_at(player.ux + player.dx, player.uy + player.dy)) &&
      !glyph_is_invisible(
          levl[player.ux + player.dx][player.uy + player.dy].glyph) &&
    ...
    if (mtmp->m_ap_type && !Protection_from_shape_changers) {
      if (!player.ustuck && !mtmp->mflee && dmgtype(mtmp->data, AD_STCK))
        player.ustuck = mtmp;
    }
  ...
  if (flags.confirm && mtmp->mpeaceful && !Confusion && !Hallucination &&
    ...
    if (canspotmon(mtmp)) {
#ifdef PARANOID
      sprintf(qbuf, "Really attack %s? [no/yes]", mon_nam(mtmp));
 ...
 */
void check_caitiff(Monster *mtmp) {
  if (Role_if(PM_KNIGHT) && player.ualign.type == A_LAWFUL &&
      (!mtmp->mcanmove || mtmp->msleeping || (mtmp->mflee && !mtmp->mavenge)) &&
      player.ualign.record > -10) {
    You("caitiff!");
    adjalign(-1);
  }
}

schar find_roll_to_hit(Monster *mtmp) {
  schar tmp;
  int tmp2;

  tmp = 1 + Luck + abon() + find_mac(mtmp) + player.uhitinc +
        maybe_polyd(youmonst.data->mlevel, player.ulevel);

  check_caitiff(mtmp);

  /*	attacking peaceful creatures is bad for the samurai's giri */
  if (Role_if(PM_SAMURAI) && mtmp->mpeaceful && player.ualign.record > -10) {
...
/* player.dx and player.dy must be set */
bool attack(Monster *mtmp) {
  schar tmp;
  ...
  if (is_safepet(mtmp) && !flags.forcefight) {
    if (!uwep || uwep->oartifact != ART_STORMBRINGER) {
      ...
      if (inshop || foo || (IS_ROCK(levl[player.ux][player.uy].typ) &&
  ...
  if (mdat->mlet == S_LEPRECHAUN && !mtmp->mfrozen && !mtmp->msleeping &&
      ...
      (m_move(mtmp, 0) == 2 || /* it died */
       mtmp->mx != player.ux + player.dx ||
       mtmp->my != player.uy + player.dy)) /* it moved */
    return (FALSE);
...
STATIC_OVL bool known_hitum(Monster *mon, int *mhit, struct Attack *uattk) {
  bool malive = TRUE;
  ...
  if (!*mhit) {
    ...
    int oldhp = mon->mhp, x = player.ux + player.dx, y = player.uy + player.dy;

    /* KMH, conduct */
    if (uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))
      player.uconduct.weaphit++;
    ...
    if (malive && player.twoweap && !override_confirmation && m_at(x, y) == mon)
      malive = hmon(mon, uswapwep, 0);
    if (malive) {
      /* monster still alive */
      if (!rn2(25) && mon->mhp < mon->mhpmax / 2 &&
          !(player.uswallow && mon == player.ustuck)) {
        ...
        if (player.ustuck == mon && !player.uswallow && !sticks(youmonst.data))
          player.ustuck = 0;
      }
      ...
      if (mon->wormno && *mhit) cutworm(mon, x, y, uwep);
    }
  }
  return (malive);
}
...
/* returns TRUE if monster still lives */
STATIC_OVL bool hitum(Monster *mon, int tmp, struct Attack *uattk) {
  bool malive;
  int mhit = (tmp > (dieroll = rnd(20)) || player.uswallow);
...
bool /* general "damage monster" routine */ hmon(Monster *mon, Object *obj,
                                                 int thrown) {
  bool result, anger_guards;

  anger_guards = (mon->mpeaceful && (mon->ispriest || mon->isshk ||
...
/* guts of hmon() */
STATIC_OVL bool hmon_hitmon(Monster *mon, Object *obj, int thrown) {
  int tmp;
  MonsterType *mdat = mon->data;
    ...
    if (obj->oclass == WEAPON_CLASS || is_weptool(obj) ||
      ...
      if (/* if you strike with a bow... */
        ...
        if (!thrown && obj == uwep && obj->otyp == BOOMERANG &&
          ...
          pline("As you hit %s, %s%s %s breaks into splinters.", mon_nam(mon),
                more_than_1 ? "one of " : "", shk_your(yourbuf, obj),
                xname(obj));
          if (!more_than_1) uwepgone(); /* set unweapon */
          useup(obj);
          if (!more_than_1) obj = (Object *)0;
          hittxt = TRUE;
          if (mdat != &mons[PM_SHADE]) tmp++;
        }
      } else {
        tmp = dmgval(obj, mon);
        /* a minimal hit doesn't exercise proficiency */
        valid_weapon_attack = (tmp > 1);
        if (!valid_weapon_attack || mon == player.ustuck || player.twoweap) {
          ; /* no special bonuses */
        } else if (mon->mflee && Role_if(PM_ROGUE) && !Upolyd) {
          You("strike %s from behind!", mon_nam(mon));
          tmp += rnd(player.ulevel);
          hittxt = TRUE;
        } else if (dieroll == 2 && obj == uwep && obj->oclass == WEAPON_CLASS &&
                   (bimanual(obj) ||
                    (Role_if(PM_SAMURAI) && obj->otyp == KATANA && !uarms)) &&
                   ((wtype = uwep_skill_type()) != P_NONE &&
                    P_SKILL(wtype) >= P_SKILLED) &&
                   ((monwep = mon->weapon()) != 0 && !is_flimsy(monwep) &&
          ...
          pline("%s %s %s from the force of your blow!", s_suffix(Monnam(mon)),
                xname(monwep), otense(monwep, "shatter"));
          m_useup(mon, monwep);
...
#ifdef STEED
        if (player.usteed && !thrown && tmp > 0 &&
            weapon_type(obj) == P_LANCE && mon != player.ustuck) {
        ...
        if (thrown && (is_ammo(obj) || is_missile(obj))) {
          ...
          if (obj->opoisoned && is_poisonable(obj)) ispoisoned = TRUE;
        }
      }
        ...
        switch (obj->otyp) {
          ...
          case CORPSE: /* fixed by polder@cs.vu.nl */
            if (touch_petrifies(&mons[obj->corpsenm])) {
              ...
              if (!munstone(mon, TRUE)) minstapetrify(mon, TRUE);
  ...
  {                       \
    ...
    o = nullptr;          \
            ...
            if (touch_petrifies(&mons[obj->corpsenm])) {
              ...
              const char *eggp = (obj->corpsenm != NON_PM && obj->known)
...
#undef useup_eggs
          ...
          case BLINDING_VENOM:
            ...
            if (can_blnd(
                    &youmonst, mon,
                    (uchar)(obj->otyp == BLINDING_VENOM ? AT_SPIT : AT_WEAP),
                    obj)) {
              if (Blind) {
                ...
                if (!thrown && obj->quan > 1) what = An(singular(obj, xname));
                /* note: s_suffix returns a modifiable buffer */
                if (haseyes(mdat) && mdat != &mons[PM_FLOATING_EYE])
                  whom =
                      strcat(strcat(s_suffix(whom), " "), mbodypart(mon, FACE));
  ...
  if (ispoisoned) {
    ...
    if (obj && !rn2(nopoison)) {
      obj->opoisoned = FALSE;
      Your("%s %s no longer poisoned.", xname(obj), otense(obj, "are"));
    }
  ...
  if (tmp < 1) {
    ...
    if (mdat == &mons[PM_SHADE]) {
      if (!hittxt) {
        ...
        Your("%s %s harmlessly through %s.", what, vtense(what, "pass"),
             mon_nam(mon));
        hittxt = TRUE;
      }
    } else {
  ...
  if (jousting) {
    ...
    if (jousting < 0) {
      ...
      player.twoweap = FALSE;      /* untwoweapon() is too verbose here */
    ...
    if (mon->mhp > tmp) {
      mhurtle(mon, player.dx, player.dy, 1);
  ...
  if (!hittxt && /*( thrown => obj exists )*/
    ...
    if (thrown)
      ...
      You("%s %s%s", Role_if(PM_BARBARIAN) ? "smite" : "hit", mon_nam(mon),
  ...
  if (silvermsg) {
    ...
    if (canspotmon(mon)) {
      if (barehand_silver_rings == 1)
        ...
        sprintf(silverobjbuf, "Your %s%s %s %%s!",
                strstri(saved_oname, "silver") ? "" : "silver ", saved_oname,
    ...
    /* note: s_suffix returns a modifiable buffer */
    if (!noncorporeal(mdat)) whom = strcat(s_suffix(whom), " flesh");
    pline(fmt, whom);
  }
  ...
  if (poiskilled) {
    ...
    if (!already_killed) killed(mon); /* takes care of most messages */
  } else if (player.umconf && !thrown) {
    nohandglow(mon);
...
STATIC_OVL bool shade_aware(Object *obj) {
  if (!obj) return FALSE;
  /*
...
STATIC_OVL bool m_slips_free(Monster *mdef, struct Attack *mattk) {
  Object *obj;
  ...
  if (obj && (obj->greased || obj->otyp == OILSKIN_CLOAK) &&
      (!obj->cursed || rn2(3))) {
    You("%s %s %s %s!",
        mattk->adtyp == AD_WRAP ? "slip off of" : "grab, but cannot hold onto",
...
STATIC_OVL void demonpet() {
  ...
  i = !rn2(6) ? ndemon(player.ualign.type) : NON_PM;
  pm = i != NON_PM ? &mons[i] : youmonst.data;
  if ((dtmp = makemon(pm, player.ux, player.uy, NO_MM_FLAGS)) != 0)
 ...
 */
STATIC_OVL void steal_it(Monster *mdef, struct Attack *mattk) {
  Object *otmp, *stealoid, **minvent_ptr;
  long unwornmask;
  ...
  if (stealoid) { /* we will be taking everything */
    if (gender(mdef) == (int)player.mfemale && youmonst.data->mlet == S_NYMPH)
      You("charm %s.  She gladly hands over her possessions.", mon_nam(mdef));
    else
      You("seduce %s and %s starts to take off %s clothes.", mon_nam(mdef),
          mhe(mdef), mhis(mdef));
  }
...
int damageum(Monster *mdef, struct Attack *mattk) {
  MonsterType *pd = mdef->data;
  ...
  switch (mattk->adtyp) {
    ...
    case AD_FIRE:
      ...
      if (!Blind) pline("%s is %s!", Monnam(mdef), on_fire(mdef->data, mattk));
      ...
      if (resists_fire(mdef)) {
        if (!Blind) pline_The("fire doesn't heat %s!", mon_nam(mdef));
    ...
    case AD_COLD:
      ...
      if (resists_cold(mdef)) {
        shieldeff(mdef->mx, mdef->my);
        if (!Blind) pline_The("frost doesn't chill %s!", mon_nam(mdef));
    ...
    case AD_ELEC:
      ...
      if (resists_elec(mdef)) {
        if (!Blind) pline_The("zap doesn't shock %s!", mon_nam(mdef));
    ...
    case AD_ACID:
      ...
      break;
    case AD_STON:
      if (!munstone(mdef, TRUE)) minstapetrify(mdef, TRUE);
      tmp = 0;
      break;
#ifdef SEDUCE
    case AD_SSEX:
    ...
    case AD_TLPT:
      ...
      if (!negated && tmp < mdef->mhp) {
        char nambuf[BUFSZ];
        bool u_saw_mon =
            canseemon(mdef) || (player.uswallow && player.ustuck == mdef);
    ...
    case AD_BLND:
      if (can_blnd(&youmonst, mdef, mattk->aatyp, (Object *)0)) {
        if (!Blind && mdef->mcansee) pline("%s is blinded.", Monnam(mdef));
        mdef->mcansee = 0;
        tmp += mdef->mblinded;
        if (tmp > 127) tmp = 127;
        mdef->mblinded = tmp;
    ...
    case AD_DRIN:
      ...
      if (!vegan(mdef->data)) player.uconduct.unvegan++;
      if (!vegetarian(mdef->data)) violated_vegetarian();
    ...
    case AD_STCK:
      if (!negated && !sticks(mdef->data))
        player.ustuck = mdef; /* it's now stuck to you */
      break;
    case AD_WRAP:
      if (!sticks(mdef->data)) {
        ...
        } else if (player.ustuck == mdef) {
          /* Monsters don't wear amulets of magical breathing */
          if (is_pool(player.ux, player.uy) && !is_swimmer(mdef->data) &&
    ...
    case AD_SLEE:
      if (!negated && !mdef->msleeping && sleep_monst(mdef, rnd(10), -1)) {
        if (!Blind) pline("%s is put to sleep by you!", Monnam(mdef));
        slept_monst(mdef);
      }
      break;
    case AD_SLIM:
    ...
    case AD_CONF:
      if (!mdef->mconf) {
        if (canseemon(mdef)) pline("%s looks confused.", Monnam(mdef));
        mdef->mconf = 1;
      }
      break;
    default:
...
STATIC_OVL int explum(Monster *mdef, struct Attack *mattk) {
  int tmp = d((int)mattk->damn, (int)mattk->damd);
...
STATIC_OVL void start_engulf(Monster *mdef) {
  if (!Invisible) {
    map_location(player.ux, player.uy, TRUE);
...
STATIC_OVL int gulpum(Monster *mdef, struct Attack *mattk) {
  int tmp;
  ...
  if (player.uhunger < 1500 && !player.uswallow) {
    for (otmp = mdef->minvent; otmp; otmp = otmp->nobj) (void)snuff_lit(otmp);
      ...
      switch (mattk->adtyp) {
        case AD_DGST:
          ...
          if (mdef->mhp > 0) { /* monster lifesaved */
            ...
            if (corpse_chance(mdef, &youmonst, TRUE) &&
              ...
              player.uhunger += (mdef->data->cnutrit + 1) / 2;
            } else
              tmp = 0;
        ...
        case AD_BLND:
          if (can_blnd(&youmonst, mdef, mattk->aatyp, nullptr)) {
            if (mdef->mcansee) pline("%s can't see in there!", Monnam(mdef));
            mdef->mcansee = 0;
            dam += mdef->mblinded;
            if (dam > 127) dam = 127;
            mdef->mblinded = dam;
...
void missum(Monster *mdef, struct Attack *mattk) {
  if (could_seduce(&youmonst, mdef, mattk))
    ...
    You("miss it.");
  if (!mdef->msleeping && mdef->mcanmove) wakeup(mdef);
}

/* attack monster as a monster. */
STATIC_OVL bool hmonas(Monster *mon, int tmp) {
  ...
  for (i = 0; i < NATTK; i++) {
    ...
    switch (mattk->aatyp) {
      ...
      use_weapon:
        ...
        dhit = (tmp > (dieroll = rnd(20)) || player.uswallow);
        ...
        if (m_at(player.ux + player.dx, player.uy + player.dy) != mon)
          return ((bool)(nsum != 0));
      ...
      case AT_TENT:
        ...
        if ((dhit = (tmp > rnd(20) || player.uswallow)) != 0) {
          int compat;

          if (!player.uswallow &&
              (compat = could_seduce(&youmonst, mon, mattk))) {
            You("%s %s %s.",
                mon->mcansee && haseyes(mon->data) ? "smile at" : "talk to",
                mon_nam(mon), compat == 2 ? "engagingly" : "seductively");
      ...
      case AT_HUGS:
        ...
        else if (!sticks(mon->data) && !player.uswallow) {
          if (mon == player.ustuck) {
            pline("%s is being %s.", Monnam(mon),
                  player.umonnum == PM_ROPE_GOLEM ? "choked" : "crushed");
            sum[i] = damageum(mon, mattk);
      ...
      case AT_MAGC:
         ...
         */
        if (i == 0 &&
            (youmonst.data->mlet == S_KOBOLD || youmonst.data->mlet == S_ORC ||
    ...
    if (sum[i] == 2) return ((bool)passive(mon, 1, 0, mattk->aatyp));
    /* defender dead */
    else {
      (void)passive(mon, sum[i], 1, mattk->aatyp);
      nsum |= sum[i];
    }
    if (!Upolyd) break;   /* No extra attacks if no longer a monster */
    if (multi < 0) break; /* If paralyzed while attacking, i.e. floating eye */
  }
  return ((bool)(nsum != 0));
}
...
int passive(Monster *mon, bool mhit, int malive, uchar aatyp) {
  ...
  switch (ptr->mattk[i].adtyp) {
    case AD_ACID:
      if (mhit && rn2(2)) {
        ...
        if (!Acid_resistance) mdamageu(mon, tmp);
    ...
    case AD_RUST:
      if (mhit && !mon->mcan) {
        if (aatyp == AT_KICK) {
          if (uarmf) (void)rust_dmg(uarmf, xname(uarmf), 1, TRUE, &youmonst);
        } else if (aatyp == AT_WEAP || aatyp == AT_CLAW || aatyp == AT_MAGC ||
                   aatyp == AT_TUCH)
          passive_obj(mon, (Object *)0, &(ptr->mattk[i]));
      }
    ...
    case AD_CORR:
      if (mhit && !mon->mcan) {
        if (aatyp == AT_KICK) {
          if (uarmf) (void)rust_dmg(uarmf, xname(uarmf), 3, TRUE, &youmonst);
        } else if (aatyp == AT_WEAP || aatyp == AT_CLAW || aatyp == AT_MAGC ||
                   aatyp == AT_TUCH)
          passive_obj(mon, (Object *)0, &(ptr->mattk[i]));
      }
  ...
  if (malive && !mon->mcan && rn2(3)) {
    switch (ptr->mattk[i].adtyp) {
      ...
      case AD_COLD: /* brown mold or blue jelly */
        if (monnear(mon, player.ux, player.uy)) {
          if (Cold_resistance) {
            shieldeff(player.ux, player.uy);
 ...
 */
void passive_obj(Monster *mon, Object *obj, struct Attack *mattk) {
  MonsterType *ptr = mon->data;
  int i;

  /* if caller hasn't specified an object, use uwep, uswapwep or uarmg */
  if (!obj) {
    obj = (player.twoweap && uswapwep && !rn2(2)) ? uswapwep : uwep;
...
void stumble_onto_mimic(Monster *mtmp) {
  ...
  if (Blind) {
    ...
    int glyph = levl[player.ux + player.dx][player.uy + player.dy].glyph;

    if (glyph_is_cmap(glyph) &&
        (glyph_to_cmap(glyph) == S_hcdoor || glyph_to_cmap(glyph) == S_vcdoor))
...
STATIC_OVL void nohandglow(Monster *mon) {
  char *hands = makeplural(body_part(HAND));

  if (!player.umconf || mon->mconf) return;
  if (player.umconf == 1) {
    if (Blind)
...
int flash_hits_mon(Monster *mtmp, Object *otmp) {
