...
void magic_map_background(xchar x, xchar y, int show) {
  ...
  if (level.flags.hero_memory) lev->glyph = glyph;
...
void map_background(xchar x, xchar y, int show) {
  ...
  if (level.flags.hero_memory) levl[x][y].glyph = glyph;
  if (show) show_glyph(x, y, glyph);
}
...
void map_invisible(xchar x, xchar y) {
  if (x != player.ux ||
      y != player.uy) { /* don't display I at hero's location */
    if (level.flags.hero_memory) levl[x][y].glyph = GLYPH_INVISIBLE;
...
void unmap_object(int x, int y) {
  ...
  if ((trap = t_at(x, y)) != 0 && trap->tseen && !covers_traps(x, y))
    ...
    if (!lev->waslit && lev->glyph == cmap_to_glyph(S_room) && lev->typ == ROOM)
...
void map_location(int x, int y, int show) { _map_location(x, y, show); }
...
void feel_location(xchar x, xchar y) {
  ...
  if (Underwater && !Is_waterlevel(&player.uz) && !is_pool(x, y)) return;
  ...
  if (Levitation && !Is_airlevel(&player.uz) && !Is_waterlevel(&player.uz)) {
     ...
     */
    if (IS_ROCK(lev->typ) ||
        (IS_DOOR(lev->typ) && (lev->doormask & (D_LOCKED | D_CLOSED)))) {
      ...
      if (lev->glyph == objnum_to_glyph(BOULDER)) {
        if (lev->typ != ROOM && lev->seenv) {
          ...
          lev->glyph =
              lev->waslit ? cmap_to_glyph(S_room) : cmap_to_glyph(S_stone);
    ...
    /* Floor spaces are dark if unlit.  Corridors are dark if unlit. */
    if (lev->typ == ROOM && lev->glyph == cmap_to_glyph(S_room) && !lev->waslit)
  ...
  if ((x != player.ux || y != player.uy) && (mon = m_at(x, y)) && sensemon(mon))
    display_monster(x, y, mon, (tp_sensemon(mon) || MATCH_WARN_OF_MON(mon))
                                   ? PHYSICALLY_SEEN
                                   : DETECTED,
                    is_worm_tail(mon));
}
...
void swallowed(int first) {
  ...
  if (isok(player.ux, player.uy - 1)) {
    if (left_ok)
      show_glyph(player.ux - 1, player.uy - 1,
...
void show_glyph(int x, int y, int glyph) {
  ...
  if (!isok(x, y)) {
    ...
    impossible("show_glyph:  bad pos %d %d with glyph %d [%s %d].", x, y, glyph,
               text, offset);
    return;
  }
...
void flush_screen(int cursor_on_u) {
  ...
  if (cursor_on_u)
    curs(WIN_MAP, player.ux, player.uy); /* move cursor to the hero */
...
static const char *type_names[MAX_TYPE] = {
    "STONE",           "VWALL",    "HWALL",     "TLCORNER",      "TRCORNER",
    "BLCORNER",        "BRCORNER", "CROSSWALL", "TUWALL",        "TDWALL",
    "TLWALL",          "TRWALL",   "DBWALL",    "SDOOR",         "SCORR",
    "POOL",            "MOAT",     "WATER",     "DRAWBRIDGE_UP", "LAVAPOOL",
    "DOOR",            "CORR",     "ROOM",      "STAIRS",        "LADDER",
    "FOUNTAIN",        "THRONE",   "SINK",      "ALTAR",         "ICE",
...
static void error4(int x, int y, int a, int b, int c, int dd) {
  pline("set_wall_state: %s @ (%d,%d) %s%s%s%s", type_to_name(levl[x][y].typ),
        x, y, a ? "1" : "", b ? "2" : "", c ? "3" : "", dd ? "4" : "");
...
STATIC_OVL int set_twall(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                         int y3) {
...
STATIC_OVL int set_corn(int x1, int y1, int x2, int y2, int x3, int y3, int x4,
                        int y4) {
...
void set_wall_state() {
  ...
  for (x = 0; x < COLNO; x++)
    for (lev = &levl[x][0], y = 0; y < ROWNO; y++, lev++) {
      ...
      if (wmode >= 0) lev->wall_info = (lev->wall_info & ~WM_MASK) | wmode;
    }
...
unsigned char seenv_matrix[3][3] = {
    {SV2, SV1, SV0}, {SV3, SVALL, SV7}, {SV4, SV5, SV6}};
      ...
      switch (lev->wall_info & WM_MASK) {
        ...
        case WM_T_LONG:
          if (seenv & (SV3 | SV4) && !(seenv & (SV5 | SV6 | SV7))) {
            ...
            /* only SV0|SV1|SV2 */
            if (!only(seenv, SV0 | SV1 | SV2)) t_warn(lev);
            col = T_stone;
          }
          break;
        case WM_T_BL:
