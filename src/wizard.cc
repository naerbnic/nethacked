...
static const int nasties[] = {
    PM_COCKATRICE,      PM_ETTIN,           PM_STALKER,
    PM_MINOTAUR,        PM_RED_DRAGON,      PM_BLACK_DRAGON,
    PM_GREEN_DRAGON,    PM_OWLBEAR,         PM_PURPLE_WORM,
    PM_ROCK_TROLL,      PM_XAN,             PM_GREMLIN,
    PM_UMBER_HULK,      PM_VAMPIRE_LORD,    PM_XORN,
    PM_ZRUTY,           PM_ELF_LORD,        PM_ELVENKING,
    PM_YELLOW_DRAGON,   PM_LEOCROTTA,       PM_BALUCHITHERIUM,
    PM_CARNIVOROUS_APE, PM_FIRE_GIANT,      PM_COUATL,
    PM_CAPTAIN,         PM_WINGED_GARGOYLE, PM_MASTER_MIND_FLAYER,
    PM_FIRE_ELEMENTAL,  PM_JABBERWOCK,      PM_ARCH_LICH,
    PM_OGRE_KING,       PM_OLOG_HAI,        PM_IRON_GOLEM,
    PM_OCHRE_JELLY,     PM_GREEN_SLIME,     PM_DISENCHANTER};
  ...
  for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
    if (!mtmp->dead() && mtmp->iswiz && mtmp->msleeping && !rn2(40)) {
      ...
      if (distu(mtmp->mx, mtmp->my) > 2)
        You("get the creepy feeling that somebody noticed your taking the "
            "Amulet.");
      return;
    }
}
...
STATIC_OVL bool you_have(int mask) {
  switch (mask) {
    case M3_WANTSAMUL:
      return (bool)(player.uhave.amulet);
    case M3_WANTSBELL:
      return (bool)(player.uhave.bell);
    case M3_WANTSCAND:
      return (bool)(player.uhave.menorah);
    case M3_WANTSBOOK:
      return (bool)(player.uhave.book);
    case M3_WANTSARTI:
      return (bool)(player.uhave.questart);
    default:
      break;
  }
  return (0);
...
void clonewiz() {
  ...
  if ((mtmp2 = makemon(&mons[PM_WIZARD_OF_YENDOR], player.ux, player.uy,
                       NO_MM_FLAGS)) != 0) {
    mtmp2->msleeping = mtmp2->mtame = mtmp2->mpeaceful = 0;
    if (!player.uhave.amulet && rn2(2)) { /* give clone a fake */
      (void)AddObjectToMonsterInventory(
          mtmp2, MakeSpecificObject(FAKE_AMULET_OF_YENDOR, TRUE, FALSE));
    }
    ...
    for (i = rnd(tmp); i > 0; --i)
      for (j = 0; j < 20; j++) {
        ...
        if (mcast && !enexto(&bypos, mcast->mux, mcast->muy, &mons[makeindex]))
          continue;
        if ((mtmp = makemon(&mons[makeindex], bypos.x, bypos.y, NO_MM_FLAGS)) !=
            0) {
...
void cuss(Monster *mtmp) {
  if (mtmp->iswiz) {
    if (!rn2(5)) /* typical bad guy action */
    ...
    else if (player.uhave.amulet && !rn2(SIZE(random_insult)))
      ...
      verbalize("%s %s!", random_malediction[rn2(SIZE(random_malediction))],
                random_insult[rn2(SIZE(random_insult))]);
