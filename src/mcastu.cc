...
STATIC_DCL void cursetxt(Monster *, bool);
STATIC_DCL int choose_magic_spell(int);
STATIC_DCL int choose_clerical_spell(int);
STATIC_DCL void cast_wizard_spell(Monster *, int, int);
STATIC_DCL void cast_cleric_spell(Monster *, int, int);
...
STATIC_OVL
void cursetxt(Monster *mtmp, bool undirected) {
 ...
 */
int castmu(Monster *mtmp, struct Attack *mattk, bool thinks_it_foundyou,
           bool foundyou) {
  ...
  if ((mattk->adtyp == AD_SPEL || mattk->adtyp == AD_CLRC) && ml) {
    ...
    } while (--cnt > 0 && spell_would_be_useless(mtmp, mattk->adtyp, spellnum));
    if (cnt == 0) return 0;
  }
  ...
  if (canspotmon(mtmp) || !is_undirected_spell(mattk->adtyp, spellnum)) {
    pline("%s casts a spell%s!", canspotmon(mtmp) ? Monnam(mtmp) : "Something",
          is_undirected_spell(mattk->adtyp, spellnum)
              ? ""
 ...
 */
STATIC_OVL
void cast_wizard_spell(Monster *mtmp, int dmg, int spellnum) {
  ...
  switch (spellnum) {
    ...
    case MGC_CURE_SELF:
      if (mtmp->mhp < mtmp->mhpmax) {
        if (canseemon(mtmp)) pline("%s looks better.", Monnam(mtmp));
        /* note: player healing does 6d4; this used to do 1d8 */
        if ((mtmp->mhp += d(3, 6)) > mtmp->mhpmax) mtmp->mhp = mtmp->mhpmax;
        dmg = 0;
      }
      break;
    case MGC_PSI_BOLT:
...
STATIC_OVL
void cast_cleric_spell(Monster *mtmp, int dmg, int spellnum) {
  ...
  switch (spellnum) {
    ...
    case CLC_INSECTS: {
      ...
      if (!success)
        ...
        pline("%s transforms a clump of sticks into snakes!", Monnam(mtmp));
      else if (Invisible && !perceives(mtmp->data) &&
               (mtmp->mux != player.ux || mtmp->muy != player.uy))
        pline("%s summons insects around a spot near you!", Monnam(mtmp));
      else if (Displaced && (mtmp->mux != player.ux || mtmp->muy != player.uy))
        pline("%s summons insects around your displaced image!", Monnam(mtmp));
      else
...
STATIC_DCL
bool spell_would_be_useless(Monster *mtmp, unsigned int adtyp, int spellnum) {
  ...
  if (adtyp == AD_SPEL) {
    ...
    if (mtmp->permspeed == MFAST && spellnum == MGC_HASTE_SELF) return TRUE;
    ...
    if (mtmp->mhp == mtmp->mhpmax && spellnum == MGC_CURE_SELF) return TRUE;
    ...
    if (mtmp->mpeaceful && spellnum == CLC_INSECTS) return TRUE;
    /* healing when already healed */
    if (mtmp->mhp == mtmp->mhpmax && spellnum == CLC_CURE_SELF) return TRUE;
    /* don't summon insects if it doesn't think you're around */
    if (!mcouldseeu && spellnum == CLC_INSECTS) return TRUE;
    /* blindness spell on blinded player */
    if (Blinded && spellnum == CLC_BLIND_YOU) return TRUE;
  }
  return FALSE;
}
...
int buzzmu(Monster *mtmp, struct Attack *mattk) {
  /* don't print constant stream of curse messages for 'normal'
     spellcasting monsters at range */
  if (mattk->adtyp > AD_SPC2) return (0);
