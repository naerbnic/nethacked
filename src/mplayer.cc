...
static const char *developers[] = {
    /* devteam */
    "Dave",    "Dean",    "Eric",    "Izchak", "Janet",    "Jessie",  "Ken",
    "Kevin",   "Michael", "Mike",    "Pat",    "Paul",     "Steve",   "Timo",
    "Warwick",
    /* PC team */
    "Bill",    "Eric",    "Keizo",   "Ken",    "Kevin",    "Michael", "Mike",
    "Paul",    "Stephen", "Steve",   "Timo",   "Yitzhak",
    /* Amiga team */
...
STATIC_OVL void get_mplname(Monster *mtmp, char *nam) {
  ...
  strcat(nam, " the ");
  strcat(nam,
         rank_of((int)mtmp->m_lev, monsndx(mtmp->data), (bool)mtmp->female));
}
    ...
    if (special) {
      ...
      if (rn2(8))
        mk_mplayer_armor(mtmp,
                         rnd_class(LEATHER_GLOVES, GAUNTLETS_OF_DEXTERITY));
      if (rn2(8))
