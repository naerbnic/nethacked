...
STATIC_OVL void kickdmg(Monster *mon, bool clumsy) {
  ...
  if (uarmf && uarmf->otyp == KICKING_BOOTS) dmg += 5;
  ...
  if (mon->data == &mons[PM_SHADE]) dmg = 0;

  if ((is_undead(mon->data) || is_demon(mon->data)) && uarmf && uarmf->blessed)
...
doit:
  ...
  if (!rn2(clumsy ? 3 : 4) && (clumsy || !bigmonst(mon->data)) &&
      ...
      !mon->mstun && !mon->mconf && !mon->msleeping && mon->data->mmove >= 12) {
...
bool ghitm(Monster *mtmp, Object *gold) {
  ...
  if (!likes_gold(mtmp->data) && !mtmp->isshk && !mtmp->ispriest &&
    ...
    if (canseemon(mtmp)) {
      pline_The("%s harmlessly %s %s.", xname(gold), otense(gold, "hit"),
                mon_nam(mtmp));
      msg_given = TRUE;
    }
  } else {
    ...
    if (cansee(mtmp->mx, mtmp->my)) pline("%s catches the gold.", Monnam(mtmp));
#ifndef GOLDOBJ
    ...
    if (mtmp->isshk) {
      ...
      if (robbed) {
        ...
        if (!robbed) make_happy_shk(mtmp, FALSE);
      } else {
        if (mtmp->mpeaceful) {
#ifndef GOLDOBJ
...
#ifndef GOLDOBJ
          if (gold->quan >
              goldreqd + (player.ugold + player.ulevel * rn2(5)) / ACURR(A_CHA))
#else
          if (value >
              goldreqd +
...
void container_impact_dmg(Object *obj) {
  ...
  costly =
      ((shkp = shop_keeper(*in_rooms(x, y, SHOPBASE))) && costly_spot(x, y));
  ...
  for (otmp = obj->cobj; otmp; otmp = otmp2) {
    ...
    if (result) {
      ...
      if (costly) loss += stolen_value(otmp, x, y, (bool)shkp->mpeaceful, TRUE);
      if (otmp->quan > 1L)
        useup(otmp);
      else {
...
STATIC_OVL int kick_object(xchar x, xchar y) {
  ...
  if (kickobj->otyp == CORPSE && touch_petrifies(&mons[kickobj->corpsenm]) &&
    ...
    You("kick the %s with your bare %s.", corpse_xname(kickobj, TRUE),
  ...
  if (!ZAP_POS(levl[x + player.dx][y + player.dy].typ) ||
      closed_door(x + player.dx, y + player.dy))
    range = 1;
  ...
  if (IS_ROCK(levl[x][y].typ) || closed_door(x, y)) {
    if ((!martial() && rn2(20) > ACURR(A_DEX)) ||
        ...
        closed_door(player.ux, player.uy)) {
      if (Blind)
  ...
  if (mon) {
    if (mon->isshk && kickobj->where == OBJ_MINVENT && kickobj->ocarry == mon)
  ...
  if (kickobj->where == OBJ_MIGRATING) {
    if (costly) {
      if (isgold)
        ...
        (void)stolen_value(kickobj, x, y, (bool)shkp->mpeaceful, FALSE);
    }
    return 1;
  }
...
int dokick() {
  ...
  kickobj = nullptr;
  if (OBJ_AT(x, y) && (!Levitation || Is_airlevel(&player.uz) ||
                       Is_waterlevel(&player.uz) || sobj_at(BOULDER, x, y))) {
    if (kick_object(x, y)) {
  ...
  if (!IS_DOOR(maploc->typ)) {
    ...
    if (IS_THRONE(maploc->typ)) {
      ...
      if ((Luck < 0 || maploc->doormask) && !rn2(3)) {
        ...
        if (i > 6) i = 6;
        while (i--)
          (void)MakeSpecificObjectAt(
              rnd_class(DILITHIUM_CRYSTAL, LUCKSTONE - 1), x, y, FALSE, TRUE);
        if (Blind)
    ...
    if (IS_GRAVE(maploc->typ) || maploc->typ == IRONBARS) goto ouch;
    if (IS_TREE(maploc->typ)) {
      ...
      if (rn2(15) && !(maploc->looted & TREE_LOOTED) &&
        ...
        if (nfall != nfruit) {
          ...
          treefruit = MakeSpecificObject(frtype, TRUE, FALSE);
    ...
    if (IS_SINK(maploc->typ)) {
      ...
      short washerndx =
          (gend == 1 || (gend == 2 && rn2(2))) ? PM_INCUBUS : PM_SUCCUBUS;
      ...
      if (rn2(5)) {
        ...
        if (makemon(&mons[washerndx], x, y, NO_MM_FLAGS)) newsym(x, y);
        maploc->looted |= S_LDWASHER;
        exercise(A_DEX, TRUE);
        return (1);
      } else if (!rn2(3)) {
        pline("Flupp!  %s.", (Blind ? "You hear a sloshing sound"
                                    : "Muddy waste pops up from the drain"));
        if (!(maploc->looted & S_LRING)) { /* once per sink */
          if (!Blind) You("see a ring shining in its midst.");
    ...
    if (maploc->typ == STAIRS || maploc->typ == LADDER ||
      ...
      if (!IS_STWALL(maploc->typ) && maploc->ladder == LA_DOWN) goto dumb;
    ouch:
...
void impact_drop(Object *missile, xchar x, xchar y, xchar dlev) {
  ...
  for (obj = level.objects[x][y]; obj; obj = obj2) {
    ...
    if (costly) {
      ...
      if (Has_contents(obj)) picked_container(obj); /* does the right thing */
  ...
  if (dct && cansee(x, y)) { /* at least one object fell */
    ...
    if (missile)
      ...
      pline("%s adjacent %s %s.", dct == 1L ? "One of the" : "Some of the",
  ...
  if (costly && shkp && price) {
    ...
    if (ESHK(shkp)->debit > debit) {
      ...
      You("owe %s %ld %s for goods lost.", Monnam(shkp), amt, currency(amt));
    }
  }
}
...
bool ship_object(Object *otmp, xchar x, xchar y, bool shop_floor_obj) {
  ...
  if (unpaid || shop_floor_obj) {
    ...
    if (container) picked_container(otmp); /* happens to do the right thing */
...
schar down_gate(xchar x, xchar y) {
  ...
  if ((xdnstair == x && ydnstair == y) ||
    ...
    return (xdnstair == x && ydnstair == y) ? MIGR_STAIRS_UP : MIGR_SSTAIRS;
  }
  ...
  if (((ttmp = t_at(x, y)) != 0 && ttmp->tseen) &&
    ...
    gate_str =
        (ttmp->ttyp == TRAPDOOR) ? "through the trap door" : "through the hole";
    return MIGR_RANDOM;
  }
  return MIGR_NOWHERE;
}
