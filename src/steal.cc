...
#ifndef GOLDOBJ
long /* actually returns something that fits in an int */ somegold() {
#ifdef LINT /* long conv. ok */
  return (0L);
#else
  return (long)((player.ugold < 100)
                    ? player.ugold
                    : (player.ugold > 10000) ? rnd(10000)
                                             : rnd((int)player.ugold));
#endif
}
...
void stealgold(Monster *mtmp) {
  ...
  if (gold && (!player.ugold || gold->quan > player.ugold || !rn2(5))) {
    ...
    pline("%s quickly snatches some gold from between your %s!", Monnam(mtmp),
...
long /* actually returns something that fits in an int */ somegold(
  ...
  return (0L);
#else
  return (long)((umoney < 100) ? umoney : (umoney > 10000) ? rnd(10000)
                                                           : rnd((int)umoney));
#endif
}
...
void remove_worn_item(Object *obj, bool unchain_ball) {
  ...
  if (obj->owornmask & W_ARMOR) {
    ...
    /* catchall -- should never happen */
      ...
      setworn(nullptr, obj->owornmask & W_ARMOR);
 ...
 */
int steal(Monster *mtmp, char *objnambuf) {
  Object *otmp;
  ...
  for (otmp = invent; otmp; otmp = otmp->nobj)
    if ((!uarm || otmp != uarmc) && otmp != uskin
      ...
      if ((tmp -=
           ((otmp->owornmask & (W_ARMOR | W_RING | W_AMUL | W_TOOL)) ? 5 : 1)) <
          0)
        break;
  if (!otmp) {
        ...
        if (donning(otmp)) {
          ...
          if (flags.female)
            ...
            pline("%s seduces you and %s off your %s.",
                  ...
                  curssv ? "helps you to take" : slowly ? "you start taking"
                                                        : "you take",
                  equipname(otmp));
          named++;
...
int mpickobj(Monster *mtmp, Object *otmp) {
  ...
  if (otmp->oclass == COIN_CLASS) {
    ...
    /* AddObjectToMonsterInventory() might free otmp [if merged with something
       else],
...
void stealamulet(Monster *mtmp) {
  ...
  if (!otmp) {
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj)
      if (otmp->otyp == real || (otmp->otyp == fake && !mtmp->iswiz)) break;
  }
...
void mdrop_special_objs(Monster *mon) {
  Object *obj, *otmp;
...
void relobj(Monster *mtmp, int show, bool is_pet) {
  Object *otmp;
  ...
  if (!is_pet || mindless(mtmp->data) || is_animal(mtmp->data))
    item1 = item2 = TRUE;
  if (!tunnels(mtmp->data) || !needspick(mtmp->data)) item1 = TRUE;
  ...
  while ((otmp = mtmp->minvent) != 0) {
    ...
    if (otmp->owornmask || otmp == wep ||
      ...
      if (is_pet) { /* dont drop worn/wielded item */
        if (otmp->otyp == PICK_AXE) item1 = TRUE;
        if (otmp->otyp == UNICORN_HORN && !otmp->cursed) item2 = TRUE;
        otmp->nobj = keepobj;
        keepobj = otmp;
        continue;
      }
