...
STATIC_DCL int maybe_write_ls(int, int, bool);
...
void del_light_source(int type, genericptr_t id) {
  ...
  switch (type) {
    case LS_OBJECT:
      tmp_id = (genericptr_t)(((Object *)id)->o_id);
      break;
    case LS_MONSTER:
      tmp_id = (genericptr_t)(((Monster *)id)->m_id);
      break;
    default:
      tmp_id = 0;
      break;
...
void do_light_sources(char **cs_rows) {
  ...
  for (ls = light_base; ls; ls = ls->next) {
    ...
    if (ls->type == LS_OBJECT) {
      if (get_obj_location((Object *)ls->id, &ls->x, &ls->y, 0))
        ls->flags |= LSF_SHOW;
    } else if (ls->type == LS_MONSTER) {
      if (get_mon_location((Monster *)ls->id, &ls->x, &ls->y, 0))
        ls->flags |= LSF_SHOW;
    }
  ...
  if (release_data(mode)) {
    for (prev = &light_base; (curr = *prev) != 0;) {
      if (!curr->id) {
        ...
        switch (curr->type) {
          case LS_OBJECT:
            is_global = !obj_is_local((Object *)curr->id);
            break;
          case LS_MONSTER:
            is_global = !mon_is_local((Monster *)curr->id);
            break;
          default:
            is_global = 0;
  ...
  for (ls = light_base; ls; ls = ls->next) {
    if (ls->flags & LSF_NEEDS_FIXUP) {
      if (ls->type == LS_OBJECT || ls->type == LS_MONSTER) {
        ...
        if (!ls->id)
          impossible("relink_light_sources: cant find %c_id %ld", which, nid);
      } else
...
STATIC_OVL int maybe_write_ls(int fd, int range, bool write_it) {
  ...
  for (ls = light_base; ls; ls = ls->next) {
    ...
    switch (ls->type) {
      case LS_OBJECT:
        is_global = !obj_is_local((Object *)ls->id);
        break;
      case LS_MONSTER:
        is_global = !mon_is_local((Monster *)ls->id);
        break;
      default:
        is_global = 0;
...
void obj_move_light_source(Object *src, Object *dest) {
  light_source *ls;
...
bool obj_sheds_light(Object *obj) {
...
void obj_merge_light_sources(Object *src, Object *dest) {
  light_source *ls;
...
int candle_light_range(Object *obj) {
  int radius;
...
int wiz_light_sources() {
  ...
  if (light_base) {
    ...
    for (ls = light_base; ls; ls = ls->next) {
      sprintf(
          buf, "  %2d,%2d   %2d   0x%04x  %s  %s", ls->x, ls->y, ls->range,
          ls->flags,
          (ls->type == LS_OBJECT
               ? "obj"
               : ls->type == LS_MONSTER
                     ? (mon_is_local((Monster *)ls->id)
                            ? "mon"
                            : ((Monster *)ls->id == &youmonst) ? "you" : "<m>")
                     : /* migrating monster */
                     "???"),
