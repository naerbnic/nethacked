...
void del_light_source(int type, genericptr_t id) {
    ...
    switch (type) {
    case LS_OBJECT:	tmp_id = (genericptr_t)(((Object *)id)->o_id);
			break;
    case LS_MONSTER:	tmp_id = (genericptr_t)(((Monster *)id)->m_id);
			break;
    default:		tmp_id = 0;
			break;
    }
...
int wiz_light_sources() {
	...
	for (ls = light_base; ls; ls = ls->next) {
		...
		(ls->type == LS_OBJECT ? "obj" :
		 ls->type == LS_MONSTER ?
		    (mon_is_local((Monster *)ls->id) ? "mon" :
		     ((Monster *)ls->id == &youmonst) ? "you" :
		     "<m>") :		/* migrating monster */
		 "???"),
