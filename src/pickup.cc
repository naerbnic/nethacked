...
#endif
{
  ...
  *itemcount = 0;
#ifndef GOLDOBJ
  if (incl_gold) ilets[iletct++] = def_oc_syms[COIN_CLASS];
#endif
...
int pickup(int what) {
  ...
  if (flags.menu_style != MENU_TRADITIONAL || iflags.menu_requested) {
    ...
    if (count) { /* looking for N of something */
      ...
      for (i = 0; i < n; i++) pick_list[i].count = count;
    } else {
      n = query_objlist(
  ...
  menu_pickup:
    ...
    for (obj = objchain; obj; obj = obj2) {
      ...
      if (!selective && oclasses[0] && !index(oclasses, obj->oclass)) continue;
...
int query_category(const char *qstr, Object *olist, int qflags,
  ...
  *pick_list = (menu_item *)0;
  ...
  n = select_menu(win, how, pick_list);
  destroy_nhwindow(win);
  if (n < 0) n = 0; /* caller's don't expect -1 */
  return n;
}
...
int pickup_object(Object *obj, long count, bool telekinesis) {
  ...
  if ((res = lift_object(obj, nullptr, &count, telekinesis)) <= 0) return res;
...
Object *pick_obj(Object *otmp) {
  ...
  if (!player.uswallow && otmp != uball && costly_spot(otmp->ox, otmp->oy)) {
    ...
    if (!index(player.ushops, *fakeshop)) remote_burglary(otmp->ox, otmp->oy);
  }
...
lootcont:
  ...
  if (c != 'y' && mon_beside(player.ux, player.uy)) {
    ...
    if (cc.x == player.ux && cc.y == player.uy) {
      underfoot = TRUE;
      if (container_at(cc.x, cc.y, FALSE)) goto lootcont;
    } else
      underfoot = FALSE;
    if (player.dz < 0) {
...
STATIC_PTR int out_container(Object *obj) {
  ...
  if (Icebox && !age_is_relative(obj)) {
    obj->age = monstermoves - obj->age; /* actual age */
    if (obj->otyp == CORPSE) StartCorpseTimeout(obj);
  }
