...
STATIC_DCL void shuffle_all();
STATIC_DCL bool interesting_to_discover(int);

static short disco[NUM_OBJECTS] = DUMMY;

#ifdef USE_TILES
STATIC_DCL void shuffle_tiles();
...
void oinit() { setgemprobs(&player.uz); }
...
void savenames(int fd, int mode) {
  ...
  if (perform_bwrite(mode)) {
    ...
    bwrite(fd, (genericptr_t)objects, sizeof(struct objclass) * NUM_OBJECTS);
  }
