    ...
    switch (ep->engr_type) {
      ...
      case HEADSTONE:
        if (!Blind || can_reach_floor()) {
          sensed = 1;
          pline("%s is engraved here on the %s.", Something, surface(x, y));
        }
        break;
      case BURN:
        if (!Blind || can_reach_floor()) {
...
static const char styluses[] = {ALL_CLASSES,  ALLOW_NONE, TOOL_CLASS,
...
int doengrave() {
  ...
  if (player.uswallow) {
    ...
    return (0);
             ...
             IS_FOUNTAIN(levl[player.ux][player.uy].typ)) {
      ...
      if (zappable(otmp)) {
        ...
        switch (otmp->otyp) {
          ...
          case WAN_MAGIC_MISSILE:
            ptext = TRUE;
            if (!Blind) {
              sprintf(post_engr_text, "The %s is riddled by bullet holes!",
                      surface(player.ux, player.uy));
            }
            break;
  ...
  if (zapwand && (otmp->spe < 0)) {
    ...
    if (!IS_GRAVE(levl[player.ux][player.uy].typ))
      You("are not going to get anywhere trying to write in the %s with your "
          "dust.",
  ...
  if (oep) {
    ...
    if (type == HEADSTONE) {
      ...
      c = yn_function("Do you want to add to the current engraving?", ynqchars,
                      'y');
      if (c == 'q') {
        pline(Never_mind);
        return (0);
  ...
  switch (type) {
    default:
      everb =
          (oep && !eow ? "add to the weird writing on" : "write strangely on");
      break;
    case DUST:
  ...
  if (otmp != &zeroobj)
    ...
    You("%s the %s with your %s.", everb, eloc, makeplural(body_part(FINGER)));
  ...
  if (len == 0 || index(ebuf, '\033')) {
    if (zapwand) {
      if (!Blind)
        pline("%s, then %s.", Tobjnam(otmp, "glow"), otense(otmp, "fade"));
      return (1);
    } else {
      pline(Never_mind);
      return (0);
...
static const char *epitaphs[] = {
    "Rest in peace", "R.I.P.", "Rest In Pieces",
    ...
    "Sparky -- he was a very good dog", "Beware of Electric Third Rail",
    "Made in Taiwan", "Og friend. Og good dude. Og died. Og now food",
    "Beetlejuice Beetlejuice Beetlejuice", "Look out below!",
    ...
    "Caution! This grave contains toxic waste", "Sum quod eris",
