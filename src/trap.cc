...
STATIC_DCL void domagictrap();
STATIC_DCL bool emergency_disrobe(bool *);
STATIC_DCL int untrap_prob(Trap *ttmp);
STATIC_DCL void cnv_trap_obj(int, int, Trap *);
STATIC_DCL void move_into_trap(Trap *);
STATIC_DCL int try_disarm(Trap *, bool);
STATIC_DCL void reward_untrap(Trap *, Monster *);
STATIC_DCL int disarm_holdingtrap(Trap *);
STATIC_DCL int disarm_landmine(Trap *);
STATIC_DCL int disarm_squeaky_board(Trap *);
STATIC_DCL int disarm_shooting_trap(Trap *, int);
STATIC_DCL int try_lift(Monster *, Trap *, int, bool);
STATIC_DCL int help_monster_out(Monster *, Trap *);
STATIC_DCL bool thitm(int, Monster *, Object *, int, bool);
STATIC_DCL int mkroll_launch(Trap *, xchar, xchar, short, long);
STATIC_DCL bool isclearpath(coord *, int, schar, schar);
#ifdef STEED
STATIC_OVL int steedintrap(Trap *, Object *);
  ...
  while (1) {
    switch (rn2(5)) {
      ...
      case 1:
        ...
        if (item) (void)burn_dmg(item, "shirt");
#endif
        return TRUE;
      case 2:
 ...
 */
bool rust_dmg(Object *otmp, const char *ostr, int type, bool print,
              Monster *victim) {
    ...
    if (grprot && otmp->greased) {
      ...
      if (flags.verbose) {
        if (victim == &youmonst)
          ...
          pline("Somehow, %s's %s %s not affected.", mon_nam(victim), ostr,
                vtense(ostr, "are"));
      }
    } else {
      if (victim == &youmonst)
        Your("%s %s%s!", ostr, vtense(ostr, action[type]),
             erosion + 1 == MAX_ERODE ? " completely" : erosion ? " further"
                                                                : "");
      else if (vismon)
        pline("%s's %s %s%s!", Monnam(victim), ostr, vtense(ostr, action[type]),
              erosion + 1 == MAX_ERODE ? " completely" : erosion ? " further"
                                                                 : "");
      if (is_primary)
        otmp->oeroded++;
...
void grease_protect(Object *otmp, const char *ostr, Monster *victim) {
...
Trap *maketrap(int x, int y, int typ) {
  Trap *ttmp;
  struct rm *lev;
  bool oldplace;

  if ((ttmp = t_at(x, y)) != 0) {
    if (ttmp->ttyp == MAGIC_PORTAL) return (Trap *)0;
    oldplace = TRUE;
    if (player.utrap && (x == player.ux) && (y == player.uy) &&
        ((player.utraptype == TT_BEARTRAP && typ != BEAR_TRAP) ||
         (player.utraptype == TT_WEB && typ != WEB) ||
         (player.utraptype == TT_PIT && typ != PIT && typ != SPIKED_PIT)))
      player.utrap = 0;
  } else {
    oldplace = FALSE;
    ttmp = newtrap();
  ...
  switch (typ) {
    ...
    case TRAPDOOR:
      lev = &levl[x][y];
      if (*in_rooms(x, y, SHOPBASE) && ((typ == HOLE || typ == TRAPDOOR) ||
                                        IS_DOOR(lev->typ) || IS_WALL(lev->typ)))
        add_damage(
            x, y, /* schedule repair */
            ((IS_DOOR(lev->typ) || IS_WALL(lev->typ)) && !flags.mon_moving)
                ? 200L
                : 0L);
       ...
       */
        ...
        lev->typ = level.flags.is_maze_lev ? ROOM : level.flags.is_cavernous_lev
                                                        ? CORR
                                                        : DOOR;
...
void fall_through(bool td) {
  d_level dtmp;
  char msgbuf[BUFSZ];
  const char *dont_fall = 0;
  int newlevel = dunlev(&player.uz);

  /* KMH -- You can't escape the Sokoban level traps */
  if (Blind && Levitation && !In_sokoban(&player.uz)) return;

  do {
    newlevel++;
  } while (!rn2(4) && newlevel < dunlevs_in_dungeon(&player.uz));
  ...
  if (td) {
    ...
    pline_The("%s opens up under you!", surface(player.ux, player.uy));

  if (In_sokoban(&player.uz) && Can_fall_thru(&player.uz))
    ; /* KMH -- You can't escape the Sokoban level traps */
  else if (Levitation || player.ustuck || !Can_fall_thru(&player.uz) ||
           Flying || is_clinger(youmonst.data) ||
           (Inhell && !player.uevent.invoked &&
            newlevel == dunlevs_in_dungeon(&player.uz))) {
  ...
  if (dont_fall) {
    ...
    impact_drop(nullptr, player.ux, player.uy, 0);
    if (!td) {
  ...
  if (*player.ushops) shopdig(1);
  if (Is_stronghold(&player.uz)) {
    find_hell(&dtmp);
  } else {
    dtmp.dnum = player.uz.dnum;
 ...
 */
Monster *animate_statue(Object *statue, xchar x, xchar y, int cause,
                        int *fail_reason) {
  MonsterType *mptr;
  Monster *mon = 0;
  Object *item;
  coord cc;
  bool historic = (Role_if(PM_ARCHEOLOGIST) && !flags.mon_moving &&
  ...
  if (statue->oxlth && statue->oattached == OATTACHED_MONST) {
    ...
    if (mon && mon->mtame && !mon->isminion) wary_dog(mon, TRUE);
  } else {
  ...
  if (statue->onamelth) mon = christen_monst(mon, ONAME(statue));
  ...
  if ((x == player.ux && y == player.uy) || cause == ANIMATE_SPELL) {
  ...
  if (x == player.ux && y == player.uy && Upolyd &&
      hides_under(youmonst.data) && !OBJ_AT(x, y))
...
Monster *activate_statue_trap(Trap *trap, xchar x, xchar y, bool shatter) {
  ...
  while (otmp) {
    mtmp = animate_statue(
        otmp, x, y, shatter ? ANIMATE_SHATTER : ANIMATE_NORMAL, &fail_reason);
...
#ifdef STEED
STATIC_OVL bool keep_saddle_with_steedcorpse(unsigned steed_mid, Object *objchn,
                                             Object *saddle) {
  if (!saddle) return FALSE;
  while (objchn) {
    if (objchn->otyp == CORPSE && objchn->oattached == OATTACHED_MONST &&
        objchn->oxlth) {
      Monster *mtmp = (Monster *)objchn->oextra;
...
void dotrap(Trap *trap, unsigned trflags) {
  int ttype = trap->ttyp;
  Object *otmp;
  ...
  /* KMH -- You can't escape the Sokoban level traps */
  if (In_sokoban(&player.uz) && (ttype == PIT || ttype == SPIKED_PIT ||
    ...
    if ((Levitation || Flying) && (ttype == PIT || ttype == SPIKED_PIT ||
                                   ttype == HOLE || ttype == BEAR_TRAP)) {
      You("%s over %s %s.", Levitation ? "float" : "fly",
          a_your[trap->madeby_u], defsyms[trap_to_defsym(ttype)].explanation);
      return;
    }
    if (!Fumbling && ttype != MAGIC_PORTAL && ttype != ANTI_MAGIC &&
        !forcebungle && (!rn2(5) || ((ttype == PIT || ttype == SPIKED_PIT) &&
...
#ifdef STEED
  if (player.usteed) player.usteed->mtrapseen |= (1 << (ttype - 1));
#endif
  ...
  switch (ttype) {
    ...
    case DART_TRAP:
      ...
      if (player.usteed && !rn2(2) && steedintrap(trap, otmp)) /* nothing */
        ...
        if (otmp->opoisoned) poisoned("dart", A_CON, "little dart", -10);
        obfree(otmp, nullptr);
      } else {
        PlaceObject(otmp, player.ux, player.uy);
        if (!Blind) otmp->dknown = 1;
    ...
    case ROCKTRAP:
      if (trap->once && trap->tseen && !rn2(15)) {
        ...
        pline("A trap door in %s opens and %s falls on your %s!",
              the(ceiling(player.ux, player.uy)), an(xname(otmp)),
              body_part(HEAD));
        ...
        if (!Blind) otmp->dknown = 1;
        stackobj(otmp);
        newsym(player.ux, player.uy); /* map the rock */
    ...
    case BEAR_TRAP:
      ...
      player.utrap = rn1(4, 4);
      player.utraptype = TT_BEARTRAP;
#ifdef STEED
      if (player.usteed) {
        pline("%s bear trap closes on %s %s!", A_Your[trap->madeby_u],
              s_suffix(mon_nam(player.usteed)), mbodypart(player.usteed, FOOT));
      } else
#endif
      {
        pline("%s bear trap closes on your %s!", A_Your[trap->madeby_u],
              body_part(FOOT));
        if (player.umonnum == PM_OWLBEAR || player.umonnum == PM_BUGBEAR)
          You("howl in anger!");
      }
    ...
    case RUST_TRAP:
      ...
      switch (rn2(5)) {
        ...
        case 1:
          ...
          if (rust_dmg(uarms, "shield", 1, TRUE, &youmonst)) break;
          if (player.twoweap || (uwep && bimanual(uwep)))
            erode_obj(player.twoweap ? uswapwep : uwep, FALSE, TRUE);
        glovecheck:
        ...
        default:
          pline("%s you!", A_gush_of_water_hits);
          for (otmp = invent; otmp; otmp = otmp->nobj) (void)snuff_lit(otmp);
          if (uarmc)
    ...
    case SPIKED_PIT:
      /* KMH -- You can't escape the Sokoban level traps */
      if (!In_sokoban(&player.uz) && (Levitation || Flying)) break;
      seetrap(trap);
      if (!In_sokoban(&player.uz) && is_clinger(youmonst.data)) {
        if (trap->tseen) {
      ...
      if (Role_if(PM_RANGER) && !trap->madeby_u && !trap->once &&
          In_quest(&player.uz) && Is_qlocate(&player.uz)) {
        pline("Fortunately it has a bottom after all...");
        trap->once = 1;
      } else if (player.umonnum == PM_PIT_VIPER ||
                 player.umonnum == PM_PIT_FIEND)
      ...
      if (!Passes_walls) player.utrap = rn1(6, 2);
    ...
    case WEB: /* Our luckless player has stumbled into a web. */
      ...
      if (amorphous(youmonst.data) || is_whirly(youmonst.data) ||
          unsolid(youmonst.data)) {
        if (acidic(youmonst.data) || player.umonnum == PM_GELATINOUS_CUBE ||
            player.umonnum == PM_FIRE_ELEMENTAL) {
          if (webmsgok)
            You("%s %s spider web!",
                (player.umonnum == PM_FIRE_ELEMENTAL) ? "burn" : "dissolve",
                a_your[trap->madeby_u]);
          deltrap(trap);
          newsym(player.ux, player.uy);
          break;
      ...
      /* Time stuck in the web depends on your/steed strength. */
      {
        ...
        if (player.usteed && webmsgok) {
          ...
          if (mintrap(player.usteed)) {
            player.usteed->mtrapped = 0;
            if (strongmonst(player.usteed->data)) str = 17;
          } else {
            break;
          }
...
#endif
        if (str <= 3)
          player.utrap = rn1(6, 6);
        else if (str < 6)
          player.utrap = rn1(6, 4);
        else if (str < 9)
          player.utrap = rn1(4, 4);
        else if (str < 12)
          player.utrap = rn1(4, 2);
        else if (str < 15)
          player.utrap = rn1(2, 2);
        else if (str < 18)
          player.utrap = rnd(2);
        else if (str < 69)
          player.utrap = 1;
          ...
          player.utrap = 0;
          if (webmsgok) You("tear through %s web!", a_your[trap->madeby_u]);
          deltrap(trap);
          newsym(player.ux, player.uy); /* get rid of trap symbol */
        }
      }
      break;

    case STATUE_TRAP:
      (void)activate_statue_trap(trap, player.ux, player.uy, FALSE);
      break;
    ...
    case MAGIC_TRAP: /* A magic trap. */
      ...
      if (!rn2(30)) {
        deltrap(trap);
        newsym(player.ux, player.uy); /* update position */
        You("are caught in a magical explosion!");
        losehp(rnd(10), "magical explosion", KILLED_BY_AN);
        Your("body absorbs some of the magical energy!");
        player.uen = (player.uenmax += 2);
      } else
        domagictrap();
#ifdef STEED
      (void)steedintrap(trap, nullptr);
    ...
    case LANDMINE: {
      ...
      if (Levitation || Flying) {
        ...
        pline("KAABLAMM!!!  %s %s%s off!",
              forcebungle ? "Your inept attempt sets" : "The air currents set",
        ...
        pline("KAABLAMM!!!  You triggered %s land mine!",
              a_your[trap->madeby_u]);
#ifdef STEED
        if (player.usteed) steed_mid = player.usteed->m_id;
        recursive_mine = TRUE;
        (void)steedintrap(trap, nullptr);
        recursive_mine = FALSE;
        saddle = sobj_at(SADDLE, player.ux, player.uy);
#endif
...
#ifdef STEED
      if (steed_mid && saddle && !player.usteed)
        (void)keep_saddle_with_steedcorpse(steed_mid, fobj, saddle);
#endif
      newsym(player.ux, player.uy); /* update trap symbol */
      losehp(rnd(16), "land mine", KILLED_BY_AN);
      /* fall recursively into the pit... */
      if ((trap = t_at(player.ux, player.uy)) != 0) dotrap(trap, RECURSIVETRAP);
    ...
    case ROLLING_BOULDER_TRAP: {
      ...
      if (!launch_obj(BOULDER, trap->launch.x, trap->launch.y, trap->launch2.x,
...
#ifdef STEED
STATIC_OVL int steedintrap(Trap *trap, Object *otmp) {
  Monster *mtmp = player.usteed;
  MonsterType *mptr;
  int tt;
  bool in_sight;
  ...
  if (!player.usteed || !trap) return 0;
  mptr = mtmp->data;
  tt = trap->ttyp;
  mtmp->mx = player.ux;
  mtmp->my = player.uy;
  ...
  switch (tt) {
    ...
    case SLP_GAS_TRAP:
      ...
      break;
    case LANDMINE:
      if (thitm(0, mtmp, nullptr, rnd(16), FALSE)) trapkilled = TRUE;
      steedhit = TRUE;
      break;
    case PIT:
    case SPIKED_PIT:
      if (mtmp->mhp <= 0 ||
          thitm(0, mtmp, nullptr, rnd((tt == PIT) ? 6 : 10), FALSE))
        trapkilled = TRUE;
      steedhit = TRUE;
      break;
    case POLY_TRAP:
...
void blow_up_landmine(Trap *trap) {
  (void)scatter(trap->tx, trap->ty, 4,
                MAY_DESTROY | MAY_HIT | MAY_FRACTURE | VIS_EFFECTS, nullptr);
...
int launch_obj(short otyp, int x1, int y1, int x2, int y2, int style) {
  ...
  while (dist-- > 0 && !used_up) {
    ...
    /* dstage@player.washington.edu -- Delay only if hero sees it */
    ...
    if (style == ROLL) {
      ...
      if (t && otyp == BOULDER) {
        switch (t->ttyp) {
          ...
          case TRAPDOOR:
            ...
            if (flooreffects(singleobj, x2, y2, "fall")) used_up = TRUE;
      ...
      if (otyp == BOULDER &&
        ...
        You_hear("a loud crash%s!", cansee(bhitpos.x, bhitpos.y) ? bmsg : "");
...
STATIC_OVL bool isclearpath(coord *cc, int distance, schar dx, schar dy) {
  ...
  while (distance-- > 0) {
    ...
    y += dy;
    typ = levl[x][y].typ;
    if (!isok(x, y) || !ZAP_POS(typ) || closed_door(x, y)) return FALSE;
  }
  cc->x = x;
  cc->y = y;
  return TRUE;
  ...
  if (!trap) {
    ...
    if (!rn2(40)) {
      if (sobj_at(BOULDER, mtmp->mx, mtmp->my) &&
        ...
        if (!rn2(2)) {
          mtmp->mtrapped = 0;
          if (canseemon(mtmp)) pline("%s pulls free...", Monnam(mtmp));
      ...
      if (trap->ttyp == BEAR_TRAP) {
        if (canseemon(mtmp)) pline("%s eats a bear trap!", Monnam(mtmp));
        deltrap(trap);
        mtmp->meating = 5;
        mtmp->mtrapped = 0;
      } else if (trap->ttyp == SPIKED_PIT) {
        if (canseemon(mtmp)) pline("%s munches on some spikes!", Monnam(mtmp));
        trap->ttyp = PIT;
        mtmp->meating = 5;
      }
    }
    ...
    bool in_sight, tear_web, see_it,
        inescapable = ((tt == HOLE || tt == PIT) && In_sokoban(&player.uz) &&
                       !trap->madeby_u);
    const char *fallverb;

#ifdef STEED
    /* true when called from dotrap, inescapable is not an option */
    if (mtmp == player.usteed) inescapable = TRUE;
#endif
    ...
    if (mtmp == player.usteed) in_sight = TRUE;
#endif
    switch (tt) {
      case ARROW_TRAP:
        if (trap->once && trap->tseen && !rn2(15)) {
      ...
      case ROCKTRAP:
        ...
        otmp->owt = GetWeight(otmp);
        if (in_sight) seetrap(trap);
        if (thitm(0, mtmp, otmp, d(2, 6), FALSE)) trapkilled = TRUE;
        break;
      ...
      case BEAR_TRAP:
        if (mptr->msize > MZ_SMALL && !amorphous(mptr) && !is_flyer(mptr) &&
          ...
          if (in_sight) {
            ...
            if ((mptr == &mons[PM_OWLBEAR] || mptr == &mons[PM_BUGBEAR]) &&
                flags.soundok)
      ...
      case RUST_TRAP: {
        ...
        switch (rn2(5)) {
          case 0:
            if (in_sight)
              pline("%s %s on the %s!", A_gush_of_water_hits, mon_nam(mtmp),
                    mbodypart(mtmp, HEAD));
          ...
          case 1:
            if (in_sight)
              pline("%s %s's left %s!", A_gush_of_water_hits, mon_nam(mtmp),
                    mbodypart(mtmp, ARM));
            target = which_armor(mtmp, W_ARMS);
            if (rust_dmg(target, "shield", 1, TRUE, mtmp)) break;
            target = mtmp->weapon();
            if (target && bimanual(target)) erode_obj(target, FALSE, TRUE);
          glovecheck:
          ...
          case 2:
            if (in_sight)
              pline("%s %s's right %s!", A_gush_of_water_hits, mon_nam(mtmp),
                    mbodypart(mtmp, ARM));
            erode_obj(mtmp->weapon(), FALSE, TRUE);
            goto glovecheck;
          default:
            if (in_sight) pline("%s %s!", A_gush_of_water_hits, mon_nam(mtmp));
            ...
            if (target)
              ...
              if (target) (void)rust_dmg(target, "armor", 1, TRUE, mtmp);
#ifdef TOURIST
              else {
      ...
      mfiretrap:
        if (in_sight)
          ...
          You("see a %s erupt from the %s!", tower_of_flame,
        ...
        if (burn_floor_paper(mtmp->mx, mtmp->my, see_it, FALSE) && !see_it &&
            distu(mtmp->mx, mtmp->my) <= 3 * 3)
          You("smell smoke.");
        if (is_ice(mtmp->mx, mtmp->my)) melt_ice(mtmp->mx, mtmp->my);
      ...
      case TRAPDOOR:
        ...
        if (is_flyer(mptr) || is_floater(mptr) || mptr == &mons[PM_WUMPUS] ||
            (mtmp->wormno && count_wsegs(mtmp) > 5) || mptr->msize >= MZ_HUGE) {
          ...
          if (acidic(mptr) || mptr == &mons[PM_GELATINOUS_CUBE] ||
            ...
            if (in_sight)
              pline("%s %s %s spider web!", Monnam(mtmp),
                    (mptr == &mons[PM_FIRE_ELEMENTAL]) ? "burns" : "dissolves",
                    a_your[trap->madeby_u]);
            deltrap(trap);
            newsym(mtmp->mx, mtmp->my);
            break;
      ...
      case LANDMINE:
        if (rn2(3)) break; /* monsters usually don't set it off */
        if (is_flyer(mptr)) {
          bool already_seen = trap->tseen;
...
void instapetrify(const char *str) {
  ...
  if (poly_when_stoned(youmonst.data) && polymon(PM_STONE_GOLEM)) return;
  You("turn to stone...");
...
void minstapetrify(Monster *mon, bool byplayer) {
  ...
  if (cansee(mon->mx, mon->my)) pline("%s turns to stone.", Monnam(mon));
  if (byplayer) {
    stoned = TRUE;
    xkilled(mon, 0);
  } else
...
void mselftouch(Monster *mon, const char *arg, bool byplayer) {
  Object *mwep = mon->weapon();
...
void float_up() {
  if (player.utrap) {
    ...
    You(is_animal(player.ustuck->data) ? "float away from the %s."
                                       : "spiral up into %s.",
        is_animal(player.ustuck->data) ? surface(player.ux, player.uy)
...
#ifdef STEED
  if (player.usteed && !is_floater(player.usteed->data) &&
      !is_flyer(player.usteed->data)) {
    if (Lev_at_will)
      pline("%s magically floats up!", Monnam(player.usteed));
    else {
      You("cannot stay on %s.", mon_nam(player.usteed));
...
int float_down(long hmask, long emask) {
  ...
  if (player.uswallow) {
    You("float down, but you are still %s.",
        is_animal(player.ustuck->data) ? "swallowed" : "engulfed");
    return (1);
  }
  ...
  if (!Flying) {
    if (!player.uswallow && player.ustuck) {
      if (sticks(youmonst.data))
        You("aren't able to maintain your hold on %s.", mon_nam(player.ustuck));
      else
        pline("Startled, %s can no longer hold you!", mon_nam(player.ustuck));
      player.ustuck = 0;
    }
    /* kludge alert:
     ...
     */
    if (is_pool(player.ux, player.uy) && !Wwalking && !Swimming &&
        !player.uinwater)
      no_msg = drown();
  ...
  if (!trap) {
    trap = t_at(player.ux, player.uy);
    if (Is_airlevel(&player.uz))
      You("begin to tumble in place.");
    else if (Is_waterlevel(&player.uz) && !no_msg)
      You_feel("heavier.");
    /* player.uinwater msgs already in spoteffects()/drown() */
    else if (!player.uinwater && !no_msg) {
#ifdef STEED
      if (!(emask & W_SADDLE))
#endif
      {
        bool sokoban_trap = (In_sokoban(&player.uz) && trap);
        if (Hallucination)
          ...
          if (!sokoban_trap)
            You("float gently to the %s.", surface(player.ux, player.uy));
          else {
...
#ifdef STEED
            if (player.usteed) dismount_steed(DISMOUNT_FELL);
#endif
  ...
  /* can't rely on player.uz0 for detecting trap door-induced level change;
     it gets changed to reflect the new level before we can check it */
  assign_level(&current_dungeon_level, &player.uz);

  if (trap) switch (trap->ttyp) {
      case STATUE_TRAP:
        break;
      case HOLE:
      case TRAPDOOR:
        if (!Can_fall_thru(&player.uz) || player.ustuck) break;
      /* fall into next case */
      default:
        if (!player.utrap) /* not already in the trap */
          dotrap(trap, 0);
    }
  ...
  if (!Is_airlevel(&player.uz) && !Is_waterlevel(&player.uz) &&
      ...
      on_level(&player.uz, &current_dungeon_level))
    (void)pickup(1);
  return 1;
}

STATIC_OVL void dofiretrap(Object *box) {
  bool see_it = !Blind;
  int num, alt;
  ...
  if ((box && !carried(box)) ? is_pool(box->ox, box->oy) : Underwater) {
    pline("A cascade of steamy bubbles erupts from %s!",
          the(box ? xname(box) : surface(player.ux, player.uy)));
    if (Fire_resistance)
      You("are uninjured.");
    else
  ...
  if (Fire_resistance) {
    ...
    if (alt > num) num = alt;
    if (player.mhmax > mons[player.umonnum].mlevel)
      player.mhmax -= rn2(min(player.mhmax, num + 1)), flags.botl = 1;
  } else {
    num = d(2, 4);
    if (player.uhpmax > player.ulevel)
      player.uhpmax -= rn2(min(player.uhpmax, num + 1)), flags.botl = 1;
  }
  if (!num)
    You("are uninjured.");
  else
  ...
  if (!box && burn_floor_paper(player.ux, player.uy, see_it, TRUE) && !see_it)
    You("smell paper burning.");
  if (is_ice(player.ux, player.uy)) melt_ice(player.ux, player.uy);
}
      ...
      case 15:
        if (on_level(&player.uz, &qstart_level))
          ...
          You("suddenly yearn for %s.",
              Hallucination ? "Cleveland" : (In_quest(&player.uz) ||
        ...
        /* tame nearby monsters */
        {
          ...
          for (i = -1; i <= 1; i++)
            for (j = -1; j <= 1; j++) {
              ...
              mtmp = m_at(player.ux + i, player.uy + j);
              if (mtmp) (void)tamedog(mtmp, nullptr);
            }
          break;
        }
 ...
 */
int fire_damage(Object *chain, bool force, bool here, xchar x, xchar y) {
  int chance;
  Object *obj, *otmp, *nobj, *ncobj;
  int retval = 0;
  ...
  for (obj = chain; obj; obj = nobj) {
    ...
    if (Is_container(obj)) {
      ...
      if (!force && (Luck + 5) > rn2(chance)) continue;
      ...
      if (Has_contents(obj)) {
        ...
        for (otmp = obj->cobj; otmp; otmp = ncobj) {
          ncobj = otmp->nobj;
          RemoveObjectFromStorage(otmp);
          if (!flooreffects(otmp, x, y, "")) PlaceObject(otmp, x, y);
        }
      }
      delobj(obj);
      retval++;
      ...
      if (obj->otyp == SCR_FIRE || obj->otyp == SPE_FIREBALL) continue;
  ...
  if (retval && !in_sight) You("smell smoke.");
  return retval;
}

/* returns TRUE if obj is destroyed */
bool water_damage(Object *obj, bool force, bool here) {
  Object *otmp;
  Object *obj_original = obj;
  ...
  while (near_capacity() > (Punished ? UNENCUMBERED : SLT_ENCUMBER)) {
    ...
    if (invc > 0) {
      ...
      for (obj = invent; obj; obj = obj->nobj) {
        ...
        if (!((obj->otyp == LOADSTONE && obj->cursed) || obj == uamul ||
              obj == uleft || obj == uright || obj == ublindf || obj == uarm ||
              obj == uarmc || obj == uarmg || obj == uarmf ||
#ifdef TOURIST
              obj == uarmu ||
#endif
              (obj->cursed && (obj == uarmh || obj == uarms)) || welded(obj)))
          otmp = obj;
    ...
    if (!otmp) {
      ...
      if (player.ugold) {
         ...
         */
        obj = mkgoldobj(player.ugold); /* removes from player.ugold */
        obj->in_use = TRUE;
        player.ugold = obj->quan; /* put the gold back */
...
bool drown() {
  ...
  if (player.uinwater &&
      is_pool(player.ux - player.dx, player.uy - player.dy) &&
      (Swimming || Amphibious)) {
  ...
  if (!player.uinwater) {
    You("%s into the water%c", Is_waterlevel(&player.uz) ? "plunge" : "fall",
        Amphibious || Swimming ? '.' : '!');
    if (!Swimming && !Is_waterlevel(&player.uz))
  ...
  if (player.umonnum == PM_GREMLIN && rn2(3))
    ...
    You("rust!");
    i = d(2, 6);
    if (player.mhmax > i) player.mhmax -= i;
  ...
  if (Amphibious || Swimming) {
    if (Amphibious) {
      if (flags.verbose) pline("But you aren't drowning.");
  ...
  if ((Teleportation || can_teleport(youmonst.data)) && !player.usleep &&
      (Teleport_control || rn2(3) < Luck + 2)) {
    You("attempt a teleport spell."); /* utcsri!carroll */
    if (!level.flags.noteleport) {
      (void)dotele();
      if (!is_pool(player.ux, player.uy)) return (TRUE);
    } else
  ...
  x = y = 0; /* lint suppression */
             /* if sleeping, wake up now so that we don't crawl out of water
                ...
                due to combat is handled; note unmul() clears player.usleep */
  if (player.usleep) unmul("Suddenly you wake up!");
...
crawl:
  if (crawl_ok) {
    ...
    bool succ = Is_waterlevel(&player.uz) ? TRUE : emergency_disrobe(&lost);

    You("try to crawl out of the water.");
    if (lost) You("dump some of your gear to lose weight...");
    if (succ) {
  ...
  killer =
      (levl[player.ux][player.uy].typ == POOL || Is_medusa_level(&player.uz))
          ? "pool of water"
          : "moat";
  done(DROWNING);
  ...
  if (player.uinwater) {
    ...
    You("find yourself back %s.",
        Is_waterlevel(&player.uz) ? "in an air bubble" : "on land");
  }
  return (TRUE);
}

void drain_en(int n) {
  if (!player.uenmax) return;
...
int dountrap() {
  ...
  if ((nohands(youmonst.data) && !webmaker(youmonst.data)) ||
    ...
    return 0;
  } else if (player.ustuck && sticks(youmonst.data)) {
    pline("You'll have to let go of %s first.", mon_nam(player.ustuck));
    return 0;
  }
  if (player.ustuck || (welded(uwep) && bimanual(uwep))) {
...
STATIC_OVL int untrap_prob(Trap *ttmp) {
  ...
  if (ttmp->ttyp == WEB && !webmaker(youmonst.data)) chance = 30;
  ...
  if (Role_if(PM_ROGUE)) {
    if (rn2(2 * MAXULEV) < player.ulevel) chance--;
    if (player.uhave.questart && chance > 1) chance--;
...
STATIC_OVL void move_into_trap(Trap *ttmp) {
  int bc;
  ...
  if (!Punished || drag_ball(x, y, &bc, &bx, &by, &cx, &cy, &unused, TRUE)) {
    player.ux0 = player.ux, player.uy0 = player.uy;
    player.ux = x, player.uy = y;
    player.umoved = TRUE;
    newsym(player.ux0, player.uy0);
    vision_recalc(1);
 ...
 * 2: succeeds
 */
STATIC_OVL int try_disarm(Trap *ttmp, bool force_failure) {
  Monster *mtmp = m_at(ttmp->tx, ttmp->ty);
  int ttype = ttmp->ttyp;
  bool under_u = (!player.dx && !player.dy);
  ...
  if (player.dx && player.dy && bad_rock(youmonst.data, player.ux, ttmp->ty) &&
      bad_rock(youmonst.data, ttmp->tx, player.uy)) {
    ...
    if (rnl(5)) {
      ...
      if (mtmp) { /* must be a trap that holds monsters */
        if (ttype == BEAR_TRAP) {
          ...
          if (!webmaker(youmonst.data)) {
            Trap *ttmp2 = maketrap(player.ux, player.uy, WEB);
            if (ttmp2) {
              pline_The("webbing sticks to you. You're caught too!");
              dotrap(ttmp2, NOWEBMSG);
#ifdef STEED
              if (player.usteed && player.utrap) {
...
STATIC_OVL void reward_untrap(Trap *ttmp, Monster *mtmp) {
  if (!ttmp->madeby_u) {
    ...
    if (!rn2(3) && !rnl(8) && player.ualign.type == A_LAWFUL) {
      adjalign(1);
...
/* Helge Hafting */
STATIC_OVL int disarm_holdingtrap(Trap *ttmp) {
  Monster *mtmp;
  ...
  if ((mtmp = m_at(ttmp->tx, ttmp->ty)) != 0) {
    ...
    You("remove %s %s from %s.", the_your[ttmp->madeby_u],
        (ttmp->ttyp == BEAR_TRAP) ? "bear trap" : "webbing", mon_nam(mtmp));
...
/* Helge Hafting */
STATIC_OVL int disarm_landmine(Trap *ttmp) {
  int fails = try_disarm(ttmp, FALSE);
...
STATIC_OVL int disarm_squeaky_board(Trap *ttmp) {
  Object *obj;
  bool bad_tool;
  int fails;
  ...
  bad_tool = (obj->cursed || ((obj->otyp != POT_OIL || obj->lamplit) &&
...
STATIC_OVL int disarm_shooting_trap(Trap *ttmp, int otyp) {
  int fails = try_disarm(ttmp, FALSE);
...
STATIC_OVL int try_lift(Monster *mtmp, Trap *ttmp, int wt, bool stuff) {
  int wc = weight_cap();
...
STATIC_OVL int help_monster_out(Monster *mtmp, Trap *ttmp) {
  int wt;
  Object *otmp;
  bool uprob;
  ...
  for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) wt += otmp->owt;
...
int untrap(bool force) {
  ...
  for (otmp = level.objects[x][y]; otmp; otmp = otmp->nexthere) {
    if (Is_box(otmp) && !player.dx && !player.dy) {
      box_here = TRUE;
      containercnt++;
    ...
    if (box_here) {
      if (ttmp->ttyp == PIT || ttmp->ttyp == SPIKED_PIT) {
        You_cant("do much about %s%s.", the_trap,
                 player.utrap ? " that you're stuck in"
  ...
  if (!player.dx && !player.dy) {
    for (otmp = level.objects[x][y]; otmp; otmp = otmp->nexthere)
      if (Is_box(otmp)) {
        sprintf(
            qbuf, "There is %s here. Check it for traps?",
...
#endif
        if ((otmp->otrapped &&
             (force || (!confused && rn2(MAXULEV + 1 - player.ulevel) < 10))) ||
  ...
  if ((levl[x][y].doormask & D_TRAPPED &&
       (force || (!confused && rn2(MAXULEV - player.ulevel + 11) < 10))) ||
      (!force && confused && !rn2(3))) {
    You("find a trap on the door!");
    exercise(A_WIS, TRUE);
    if (ynq("Disarm it?") != 'y') return (1);
    if (levl[x][y].doormask & D_TRAPPED) {
      ch = 15 + (Role_if(PM_ROGUE) ? player.ulevel * 3 : player.ulevel);
      exercise(A_DEX, TRUE);
      if (!force &&
          (confused || Fumbling || rnd(75 + level_difficulty() / 2) > ch)) {
        You("set it off!");
  ...
  if (Luck > -13 && rn2(13 + Luck) > 7) { /* saved by luck */
    ...
    switch (rn2(20) ? ((Luck >= 13) ? 0 : rn2(13 - Luck)) : rn2(26)) {
      ...
      case 21: {
         ...
         */
        if (Punished && !carried(uball) &&
            ((uchain->ox == player.ux && uchain->oy == player.uy) ||
             (uball->ox == player.ux && uball->oy == player.uy)))
          unpunish();
      ...
      case 0:
        ...
        if (!Stunned) {
          if (Hallucination)
            ...
            You("%s and get dizzy...", stagger(youmonst.data, "stagger"));
          else
...
STATIC_OVL bool thitm(int tlev, Monster *mon, Object *obj, int d_override,
                      bool nocorpse) {
  int strike;
  bool trapkilled = FALSE;
...
bool unconscious() {
  return ((bool)(multi < 0 && (!nomovemsg || player.usleep ||
  ...
  if (!Fire_resistance) {
    ...
    for (obj = invent; obj; obj = obj2) {
      ...
      if (is_organic(obj) && !obj->oerodeproof) {
        if (obj->owornmask) {
          if (usurvive) Your("%s into flame!", aobjnam(obj, "burst"));
    ...
    You("find yourself back on solid %s.", surface(player.ux, player.uy));
    return (TRUE);
  }

  if (!Wwalking) {
    player.utrap = rn1(4, 4) + (rn1(4, 12) << 8);
    player.utraptype = TT_LAVA;
    You("sink into the lava, but it only burns slightly!");
    if (player.uhp > 1) losehp(1, lava_killer, KILLED_BY);
  }
