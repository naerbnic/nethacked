...
void fall_through(bool td) {
  ...
  if (Blind && Levitation && !In_sokoban(&player.uz))
    return;
...
void dotrap(Trap *trap, unsigned trflags) {
...
#ifdef STEED
  if (player.usteed)
    player.usteed->mtrapseen |= (1 << (ttype - 1));
#endif
  ...
  switch (ttype) {
    ...
    case SPIKED_PIT:
      /* KMH -- You can't escape the Sokoban level traps */
      if (!In_sokoban(&player.uz) && (Levitation || Flying))
        break;
      seetrap(trap);
      ...
      /* Time stuck in the web depends on your/steed strength. */
      {
        ...
        if (player.usteed && webmsgok) {
          ...
          if (mintrap(player.usteed)) {
            ...
            if (strongmonst(player.usteed->data))
              str = 17;
          } else {
            break;
          }
    ...
    case LANDMINE: {
      ...
      if ((trap = t_at(player.ux, player.uy)) != 0)
...
int float_down(long hmask, long emask) {
  ...
  if (trap)
    switch (trap->ttyp) {
      ...
      case HOLE:
      case TRAPDOOR:
        if (!Can_fall_thru(&player.uz) || player.ustuck)
          break;
      /* fall into next case */
      default:
...
STATIC_OVL void dofiretrap(Object *box) {
  ...
  if (is_ice(player.ux, player.uy))
    melt_ice(player.ux, player.uy);
}
...
STATIC_OVL int untrap_prob(Trap *ttmp) {
  ...
  if (ttmp && ttmp->madeby_u)
    chance--;
  if (Role_if(PM_ROGUE)) {
    if (rn2(2 * MAXULEV) < player.ulevel)
      chance--;
    if (player.uhave.questart && chance > 1)
      chance--;
