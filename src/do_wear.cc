...
static const char c_armor[] = "armor", c_suit[] = "suit",
                  ...
                  c_shirt[] = "shirt",
#endif
                  c_cloak[] = "cloak", c_gloves[] = "gloves",
                  c_boots[] = "boots", c_helmet[] = "helmet",
                  c_shield[] = "shield", c_weapon[] = "weapon",
...
STATIC_DCL void Ring_off_or_gone(Object *, bool);
STATIC_PTR int select_off(Object *);
STATIC_DCL Object *do_takeoff();
...
void off_msg(Object *otmp) {
  if (flags.verbose) You("were wearing %s.", doname(otmp));
}

/* for items that involve no delay */
STATIC_OVL void on_msg(Object *otmp) {
  if (flags.verbose) {
    char how[BUFSZ];
  ...
  switch (uarmh->otyp) {
    ...
    case HELM_OF_OPPOSITE_ALIGNMENT:
      if (player.ualign.type == A_NEUTRAL)
        ...
        player.ualign.type = -(player.ualign.type);
...
int Gloves_off() {
  ...
  if (uwep && uwep->otyp == CORPSE && touch_petrifies(&mons[uwep->corpsenm])) {
    ...
    You("wield the %s in your bare %s.", corpse_xname(uwep, TRUE),
  ...
  if (player.twoweap && uswapwep && uswapwep->otyp == CORPSE &&
    ...
    You("wield the %s in your bare %s.", corpse_xname(uswapwep, TRUE),
        body_part(HAND));
...
void Ring_on(Object *obj) {
  ...
  if (obj == uswapwep) setuswapwep((Object *)0);
  if (obj == uquiver) setuqwep((Object *)0);
  ...
  switch (obj->otyp) {
    ...
    adjust_attrib:
      ...
      ABON(which) += obj->spe;
      if (ACURR(which) != old_attrib || (objects[obj->otyp].oc_name_known &&
...
STATIC_OVL void Ring_off_or_gone(Object *obj, bool gone) {
...
void Ring_off(Object *obj) { Ring_off_or_gone(obj, FALSE); }

void Ring_gone(Object *obj) { Ring_off_or_gone(obj, TRUE); }

void Blindf_on(Object *otmp) {
  bool already_blind = Blind, changed = FALSE;

  if (otmp == uwep) setuwep((Object *)0);
  if (otmp == uswapwep) setuswapwep((Object *)0);
  if (otmp == uquiver) setuqwep((Object *)0);
  setworn(otmp, W_TOOL);
  on_msg(otmp);
...
void Blindf_off(Object *otmp) {
  ...
  if (Blind) {
    if (was_blind) {
      ...
      if (otmp->otyp != LENSES) You("still cannot see.");
    } else {
...
bool donning(Object *otmp) {
  ...
  if (otmp == uarm)
    result =
        (afternmv == Armor_on || afternmv == Armor_off || what == WORN_ARMOR);
#ifdef TOURIST
  else if (otmp == uarmu)
    result =
        (afternmv == Shirt_on || afternmv == Shirt_off || what == WORN_SHIRT);
#endif
  else if (otmp == uarmc)
    result =
        (afternmv == Cloak_on || afternmv == Cloak_off || what == WORN_CLOAK);
  else if (otmp == uarmf)
    result =
        (afternmv == Boots_on || afternmv == Boots_off || what == WORN_BOOTS);
  else if (otmp == uarmh)
...
static const char accessories[] = {RING_CLASS, AMULET_CLASS, TOOL_CLASS,
                                   FOOD_CLASS, 0};
...
#define MOREARM(x) \
  ...
  if (!armorpieces) {
    ...
    if (uskin)
      pline_The("%s merged with your skin!", uskin->otyp >= GRAY_DRAGON_SCALES
                                                 ? "dragon scales are"
...
int cursed(Object *otmp) {
  /* Curses, like chickens, come home to roost. */
  if ((otmp == uwep) ? welded(otmp) : (int)otmp->cursed) {
    You("can't.  %s cursed.",
        (is_boots(otmp) || is_gloves(otmp) || otmp->quan > 1L) ? "They are"
                                                               : "It is");
    otmp->bknown = TRUE;
    return (1);
  }
...
int armoroff(Object *otmp) {
  ...
  if (delay) {
    ...
    if (is_cloak(otmp))
      ...
      setworn(nullptr, otmp->owornmask & W_ARMOR);
    off_msg(otmp);
  }
 ...
 */
int canwearobj(Object *otmp, long *mask, bool noisy) {
  int err = 0;
  const char *which;
  ...
  if (welded(uwep) && bimanual(uwep) && (is_suit(otmp)
#ifdef TOURIST
                                         || is_shirt(otmp)
#endif
                                         )) {
  ...
  if (is_helmet(otmp)) {
    if (uarmh) {
      ...
      if (noisy)
        pline_The("%s won't fit over your horn%s.", c_helmet,
    ...
    if (uarms) {
      ...
      if (noisy)
        You("cannot wear a shield while wielding a two-handed %s.",
            is_sword(uwep) ? c_sword : (uwep->otyp == BATTLE_AXE) ? c_axe
                                                                  : c_weapon);
      err++;
    } else if (player.twoweap) {
      if (noisy) You("cannot wear a shield while wielding two weapons.");
      err++;
    } else
      *mask = W_ARMS;
  } else if (is_boots(otmp)) {
    if (uarmf) {
      ...
      if (player.utraptype == TT_BEARTRAP) {
        ...
        if (noisy)
          Your("%s are stuck in the %s!", makeplural(body_part(FOOT)),
...
int doputon() {
  ...
  if (uleft && uright && uamul && ublindf) {
    Your("%s%s are full, and you're already wearing an amulet and %s.",
         humanoid(youmonst.data) ? "ring-" : "", makeplural(body_part(FINGER)),
  ...
  if (otmp->oclass == RING_CLASS || otmp->otyp == MEAT_RING) {
    ...
    if (uleft && uright) {
      There("are no more %s%s to fill.", humanoid(youmonst.data) ? "ring-" : "",
    ...
    if (uleft)
      ...
      do {
        ...
        sprintf(qbuf, "Which %s%s, Right or Left?",
                humanoid(youmonst.data) ? "ring-" : "", body_part(FINGER));
    ...
    if (otmp->oartifact && !touch_artifact(otmp, &youmonst)) return 1;
    setworn(otmp, W_AMUL);
  ...
  if (is_worn(otmp)) prinv((char *)0, otmp, 0L);
  return (1);
}
...
void glibr() {
  ...
  leftfall =
      (uleft && !uleft->cursed && (!uwep || !welded(uwep) || !bimanual(uwep)));
  ...
  if (player.twoweap && otmp) {
    ...
    Your("%s %sslips from your %s.", otherwep, xfl ? "also " : "",
         makeplural(body_part(HAND)));
    setuswapwep(nullptr);
    xfl++;
    if (otmp->otyp != LOADSTONE || !otmp->cursed) dropx(otmp);
  }
  otmp = uwep;
  if (otmp && !welded(otmp)) {
    const char *thiswep;
    ...
    Your("%s%s %sslips from your %s.", otherwep ? "other " : "", thiswep,
...
Object *some_armor(Monster *victim) {
  ...
  if (!otmph) otmph = (victim == &youmonst) ? uarm : which_armor(victim, W_ARM);
#ifdef TOURIST
  if (!otmph)
...
Object *stuck_ring(Object *ring, int otyp) {
...
int select_off(Object *otmp) {
  ...
  *buf = '\0'; /* lint suppresion */
  ...
  if (otmp == uarmg) {
    if (welded(uwep)) {
      You("are unable to take off your %s while wielding that %s.", c_gloves,
          is_sword(uwep) ? c_sword : c_weapon);
      uwep->bknown = TRUE;
      return 0;
    } else if (Glib) {
      You_cant("take off the slippery %s with your slippery %s.", c_gloves,
               makeplural(body_part(FINGER)));
      return 0;
    }
  }
...
STATIC_OVL int menu_remarm(int retry) {
  ...
  if (retry) {
    ...
    n = query_category("What type of things do you want to take off?", invent,
                       WORN_TYPES | ALL_TYPES, &pick_list, PICK_ANY);
    if (!n) return 0;
    for (i = 0; i < n; i++) {
  ...
  n = query_objlist("What do you want to take off?", invent,
                    ...
                    PICK_ANY, all_worn_categories ? is_worn : is_worn_by_type);
  if (n > 0) {
    for (i = 0; i < n; i++) (void)select_off(pick_list[i].item.a_obj);
    free((genericptr_t)pick_list);
...
void adj_abon(Object *otmp, schar delta) {
