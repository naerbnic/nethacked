...
STATIC_DCL MonsterType *muse_newcham_mon(Monster *);
STATIC_DCL int precheck(Monster *, Object *);
STATIC_DCL void mzapmsg(Monster *, Object *, bool);
STATIC_DCL void mreadmsg(Monster *, Object *);
STATIC_DCL void mquaffmsg(Monster *, Object *);
STATIC_PTR int mbhitm(Monster *, Object *);
STATIC_DCL void mbhit(Monster *, int, int (*)(Monster *, Object *),
                      int (*)(Object *, Object *), Object *);
STATIC_DCL void you_aggravate(Monster *);
STATIC_DCL void mon_consume_unstone(Monster *, Object *, bool, bool);
 ...
 */
STATIC_OVL int precheck(Monster *mon, Object *obj) {
  bool vis;
...
#define POTION_OCCUPANT_CHANCE(n) (13 + 2 * (n)) /* also in potion.c */
    ...
    if (potion_descr && !strcmp(potion_descr, "smoky") &&
      ...
      if (!mtmp) {
        ...
        if (vis) pline("In a cloud of smoke, %s emerges!", a_monnam(mtmp));
...
STATIC_OVL void mzapmsg(Monster *mtmp, Object *otmp, bool self) {
  if (!canseemon(mtmp)) {
    if (flags.soundok)
      You_hear("a %s zap.",
    ...
    pline("%s zaps %sself with %s!", Monnam(mtmp), mhim(mtmp), doname(otmp));
  else {
...
STATIC_OVL void mreadmsg(Monster *mtmp, Object *otmp) {
  bool vismon = canseemon(mtmp);
  char onambuf[BUFSZ];
  short saverole;
  unsigned savebknown;

  if (!vismon && !flags.soundok) return; /* no feedback */
...
STATIC_OVL void mquaffmsg(Monster *mtmp, Object *otmp) {
  if (canseemon(mtmp)) {
    otmp->dknown = 1;
...
bool find_defensive(Monster *mtmp) {
  ...
  if (dist2(x, y, mtmp->mux, mtmp->muy) > 25) return FALSE;
  ...
  if (mtmp->mconf || mtmp->mstun || !mtmp->mcansee) {
    if (!is_unicorn(mtmp->data) && !nohands(mtmp->data)) {
      for (obj = mtmp->minvent; obj; obj = obj->nobj)
        if (obj->otyp == UNICORN_HORN && !obj->cursed) break;
    }
  ...
  if (levl[x][y].typ == STAIRS && !stuck && !immobile) {
    ...
    if (x == xupladder && y == yupladder) m.has_defense = MUSE_UP_LADDER;
                  ...
                  if ((verysmall(mtmp->data) || throws_rocks(mtmp->data) ||
                    ...
                    if (!onscary(xx, yy, mtmp)) {
                      if ((t->ttyp == TRAPDOOR || t->ttyp == HOLE) &&
                          !is_floater(mtmp->data) && !mtmp->isshk &&
...
#define nomore(x) \
  ...
  for (obj = mtmp->minvent; obj; obj = obj->nobj) {
    ...
    if (obj->otyp == WAN_DIGGING && obj->spe > 0 && !stuck && !t &&
        ...
        !(mtmp->data == &mons[PM_VLAD_THE_IMPALER] && In_V_tower(&player.uz))) {
      m.defensive = obj;
    ...
    if (obj->otyp == SCR_TELEPORTATION && mtmp->mcansee &&
        haseyes(mtmp->data) &&
        (!obj->cursed || (!(mtmp->isshk && inhishop(mtmp)) && !mtmp->isgd &&
                          !mtmp->ispriest))) {
 ...
 */
int use_defensive(Monster *mtmp) {
  int i, fleetim, how = 0;
  Object *otmp = m.defensive;
  ...
  switch (m.has_defense) {
    case MUSE_UNICORN_HORN:
      ...
      if (!mtmp->mcansee) {
        ...
        if (vismon) pline("%s seems steadier now.", Monnam(mtmp));
      } else
    ...
    mon_tele:
      ...
      if ((
			...
			mon_has_amulet(mtmp) ||
#endif
              On_W_tower_level(&player.uz)) &&
          !rn2(3)) {
        if (vismon) pline("%s seems disoriented for a moment.", Monnam(mtmp));
        return 2;
      }
    ...
    case MUSE_SCR_TELEPORTATION: {
      ...
      if (obj_is_cursed || mtmp->mconf) {
        ...
        if (nlev == depth(&player.uz)) {
          if (vismon) pline("%s shudders for a moment.", Monnam(mtmp));
          return 2;
        }
        get_level(&flev, nlev);
    ...
    case MUSE_WAN_DIGGING: {
      ...
      if (IS_FURNITURE(levl[mtmp->mx][mtmp->my].typ) ||
          ...
          (sstairs.sx && sstairs.sx == mtmp->mx && sstairs.sy == mtmp->my)) {
    ...
    case MUSE_WAN_CREATE_MONSTER: {
      ...
      if (mon && canspotmon(mon) && oseen) makeknown(WAN_CREATE_MONSTER);
      return 2;
    }
    case MUSE_SCR_CREATE_MONSTER: {
      coord cc;
      MonsterType *pm = 0, *fish = 0;
      int cnt = 1;
      Monster *mon;
      bool known = FALSE;
    ...
    case MUSE_UPSTAIRS:
      ...
      if (ledger_no(&player.uz) == 1) {
        if (mon_has_special(mtmp)) return 0;
        if (vismon) pline("%s escapes the dungeon!", Monnam(mtmp));
        mongone(mtmp);
        return 2;
      }
      m_flee(mtmp);
...
int rnd_defensive_item(Monster *mtmp) {
  MonsterType *pm = mtmp->data;
...
try_again:
  switch (rn2(8 + (difficulty > 3) + (difficulty > 6) + (difficulty > 8))) {
    case 6:
    case 9:
      if (level.flags.noteleport && ++trycnt < 2) goto try_again;
 ...
 * found.
 */
bool find_offensive(Monster *mtmp) {
  Object *obj;
...
#define nomore(x) \
  ...
  for (obj = mtmp->minvent; obj; obj = obj->nobj) {
    ...
    if (obj->otyp == SCR_EARTH &&
        ...
        dist2(mtmp->mx, mtmp->my, mtmp->mux, mtmp->muy) <= 2 && mtmp->mcansee &&
        haseyes(mtmp->data)
#ifdef REINCARNATION
      ...
      m.has_offense = MUSE_SCR_EARTH;
		...
		if (Object->otyp == SCR_FIRE && resists_fire(mtmp)
...
STATIC_PTR
int mbhitm(Monster *mtmp, Object *otmp) {
  int tmp;
  ...
  switch (otmp->otyp) {
    case WAN_STRIKING:
      ...
      if (mtmp == &youmonst) {
        ...
        if (cansee(mtmp->mx, mtmp->my) && zap_oseen) makeknown(WAN_STRIKING);
      } else {
        miss("wand", mtmp);
        if (cansee(mtmp->mx, mtmp->my) && zap_oseen) makeknown(WAN_STRIKING);
      }
...
STATIC_OVL void mbhit(
  ...
  while (range-- > 0) {
    ...
    if (find_drawbridge(&x, &y)) switch (obj->otyp) {
        case WAN_STRIKING:
    ...
    if (!ZAP_POS(typ) || (IS_DOOR(typ) && (levl[bhitpos.x][bhitpos.y].doormask &
                                           (D_LOCKED | D_CLOSED)))) {
      bhitpos.x -= ddx;
      bhitpos.y -= ddy;
      break;
 ...
 */
int use_offensive(Monster *mtmp) {
  int i;
  Object *otmp = m.offensive;
  bool oseen;

  /* offensive potions are not drunk, they're thrown */
  if (otmp->oclass != POTION_CLASS && (i = precheck(mtmp, otmp)) != 0) return i;
    ...
    case MUSE_SCR_EARTH: {
      ...
      for (x = mmx - 1; x <= mmx + 1; x++) {
        for (y = mmy - 1; y <= mmy + 1; y++) {
          ...
          if (isok(x, y) && !closed_door(x, y) && !IS_ROCK(levl[x][y].typ) &&
              !IS_AIR(levl[x][y].typ) &&
              (((x == mmx) && (y == mmy)) ? !otmp->blessed : !otmp->cursed) &&
            ...
            if (mtmp2 && !amorphous(mtmp2->data) &&
                !passes_walls(mtmp2->data) && !noncorporeal(mtmp2->data) &&
                !unsolid(mtmp2->data)) {
              Object *helmet = which_armor(mtmp2, W_ARMH);
              int mdmg;

              if (cansee(mtmp2->mx, mtmp2->my)) {
                pline("%s is hit by %s!", Monnam(mtmp2), doname(otmp2));
              ...
              if (helmet) {
                if (is_metallic(helmet)) {
                  ...
                  if (canspotmon(mtmp2))
                    pline("%s's %s does not protect %s.", Monnam(mtmp2),
                          xname(helmet), mhim(mtmp2));
                }
              }
              mtmp2->mhp -= mdmg;
              if (mtmp2->mhp <= 0) {
...
int rnd_offensive_item(Monster *mtmp) {
  MonsterType *pm = mtmp->data;
  int difficulty = monstr[(monsndx(pm))];
...
bool find_misc(Monster *mtmp) {
  ...
  m.misc = nullptr;
  m.has_misc = 0;
  if (is_animal(mdat) || mindless(mdat)) return 0;
  if (player.uswallow && stuck) return FALSE;
   ...
   * you can see them...
   */
  if (dist2(x, y, mtmp->mux, mtmp->muy) > 36) return FALSE;

  if (!stuck && !immobile && !mtmp->cham && monstr[monsndx(mdat)] < 6) {
    bool ignore_boulders =
...
static MonsterType *muse_newcham_mon(Monster *mon) {
  Object *m_armr;
...
int use_misc(Monster *mtmp) {
  ...
  switch (m.has_misc) {
    case MUSE_POT_GAIN_LEVEL:
      ...
      m_useup(mtmp, otmp);
      if (!grow_up(mtmp, (Monster *)0)) return 1;
    ...
    case MUSE_BULLWHIP:
      ...
      if (uwep && !rn2(5)) {
        ...
        if (vismon)
          pline("%s flicks a bullwhip towards your %s!", Monnam(mtmp), hand);
        ...
        switch (where_to) {
          case 1: /* onto floor beneath mon */
            pline("%s yanks %s from your %s!", Monnam(mtmp), the_weapon, hand);
    ...
    case 0:
      return 0; /* i.e. an exploded wand */
    default:
      impossible("%s wanted to perform action %d?", Monnam(mtmp), m.has_misc);
      break;
  }
  return 0;
}

STATIC_OVL void you_aggravate(Monster *mtmp) {
...
int rnd_misc_item(Monster *mtmp) {
  MonsterType *pm = mtmp->data;
  int difficulty = monstr[(monsndx(pm))];
...
bool searches_for_item(Monster *mon, Object *obj) {
  int typ = obj->otyp;
  ...
  if (typ == WAN_MAKE_INVISIBLE || typ == POT_INVISIBILITY)
    return (bool)(!mon->minvis && !mon->invis_blkd &&
  ...
  switch (obj->oclass) {
    case WAND_CLASS:
      ...
      if (typ == WAN_DIGGING) return (bool)(!is_floater(mon->data));
      if (typ == WAN_POLYMORPH) return (bool)(monstr[monsndx(mon->data)] < 6);
    ...
    case POTION_CLASS:
      if (typ == POT_HEALING || typ == POT_EXTRA_HEALING ||
        ...
        return TRUE;
      if (typ == POT_BLINDNESS && !attacktype(mon->data, AT_GAZE)) return TRUE;
      break;
    case SCROLL_CLASS:
      ...
      break;
    case AMULET_CLASS:
      if (typ == AMULET_OF_LIFE_SAVING) return (bool)(!nonliving(mon->data));
      if (typ == AMULET_OF_REFLECTION) return TRUE;
      break;
    case TOOL_CLASS:
      if (typ == PICK_AXE) return (bool)needspick(mon->data);
      if (typ == UNICORN_HORN)
        return (bool)(!obj->cursed && !is_unicorn(mon->data));
      if (typ == FROST_HORN || typ == FIRE_HORN) return (obj->spe > 0);
      break;
    case FOOD_CLASS:
      if (typ == CORPSE)
        return (
            bool)(((mon->misc_worn_check & W_ARMG) &&
                   touch_petrifies(&mons[obj->corpsenm])) ||
                  (!resists_ston(mon) && (obj->corpsenm == PM_LIZARD ||
                                          (acidic(&mons[obj->corpsenm]) &&
                                           obj->corpsenm != PM_GREEN_SLIME))));
      if (typ == EGG) return (bool)(touch_petrifies(&mons[obj->corpsenm]));
      break;
    default:
      break;
  }
...
bool mon_reflects(Monster *mon, const char *str) {
  Object *orefl = which_armor(mon, W_ARMS);
  ...
  if (orefl && orefl->otyp == SHIELD_OF_REFLECTION) {
    ...
    if (str) pline(str, s_suffix(mon_nam(mon)), "weapon");
    return TRUE;
    ...
    if (str) pline(str, s_suffix(mon_nam(mon)), "armor");
    return TRUE;
  } else if (mon->data == &mons[PM_SILVER_DRAGON] ||
             mon->data == &mons[PM_CHROMATIC_DRAGON]) {
    /* Silver dragons only reflect when mature; babies do not */
    if (str) pline(str, s_suffix(mon_nam(mon)), "scales");
    return TRUE;
  }
  return FALSE;
}
...
bool ureflects(const char *fmt, const char *str) {
  ...
  if (EReflecting & W_ARMS) {
    ...
    /* Due to wielded artifact weapon */
    if (fmt && str) pline(fmt, str, "weapon");
    return TRUE;
    ...
    if (fmt && str) pline(fmt, str, "armor");
    return TRUE;
  } else if (youmonst.data == &mons[PM_SILVER_DRAGON]) {
    if (fmt && str) pline(fmt, str, "scales");
    return TRUE;
  }
  return FALSE;
}
...
bool munstone(Monster *mon, bool by_you) {
  Object *obj;
...
STATIC_OVL void mon_consume_unstone(Monster *mon, Object *obj, bool by_you,
                                    bool stoning) {
  ...
  if (canseemon(mon)) {
    ...
    obj->quan = 1L;
    pline("%s %ss %s.", Monnam(mon), (obj->otyp == POT_ACID) ? "quaff" : "eat",
  ...
  if (obj->otyp == CORPSE && obj->corpsenm == PM_LIZARD && mon->mconf) {
    mon->mconf = 0;
    if (canseemon(mon)) pline("%s seems steadier now.", Monnam(mon));
  }
