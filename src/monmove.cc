...
STATIC_OVL void watch_on_duty(Monster *mtmp) {
  ...
  if (mtmp->mpeaceful &&
      in_town(player.ux + player.dx, player.uy + player.dy) && mtmp->mcansee &&
...
STATIC_OVL void distfleeck(Monster *mtmp, int *inrange, int *nearby,
                           int *scared) {
...
int dochug(Monster *mtmp) {
  ...
  if (!mtmp->mcanmove || (mtmp->mstrategy & STRAT_WAITMASK)) {
    ...
    if (mtmp->mcanmove && (mtmp->mstrategy & STRAT_CLOSE) && !mtmp->msleeping &&
  ...
  if (mdat->msound == MS_SHRIEK && !um_dist(mtmp->mx, mtmp->my, 1))
    m_respond(mtmp);
  if (mdat == &mons[PM_MEDUSA] && couldsee(mtmp->mx, mtmp->my)) m_respond(mtmp);
  ...
  if (find_defensive(mtmp)) {
    if (use_defensive(mtmp) != 0) return 1;
  ...
  if (mdat == &mons[PM_WATCHMAN] || mdat == &mons[PM_WATCH_CAPTAIN])
    ...
    if (canseemon(mtmp)) pline("%s concentrates.", Monnam(mtmp));
    ...
    if (mtmp->mpeaceful && (!Conflict || resist(mtmp, RING_CLASS, 0, 0)))
      ...
      if (m_sen || (Blind_telepat && rn2(2)) || !rn2(10)) {
        int dmg;
        pline(
            "It locks on to your %s!",
            m_sen ? "telepathy" : Blind_telepat ? "latent telepathy" : "mind");
        dmg = rnd(15);
    ...
    for (m2 = fmon; m2; m2 = nmon) {
      ...
      if ((telepathic(m2->data) && (rn2(2) || m2->mblinded)) || !rn2(10)) {
        if (cansee(m2->mx, m2->my)) pline("It locks on to %s.", mon_nam(m2));
        m2->mhp -= rnd(15);
        if (m2->mhp <= 0)
...
/*	Now the actual movement phase	*/
  ...
  if (!nearby || mtmp->mflee || scared || mtmp->mconf || mtmp->mstun ||
      (mtmp->minvis && !rn2(3)) || (mdat->mlet == S_LEPRECHAUN &&
                                    !player.ugold && (mtmp->mgold || rn2(2))) ||
#else
  ...
  if (!nearby || mtmp->mflee || scared || mtmp->mconf || mtmp->mstun ||
    ...
    if (dist2(mtmp->mx, mtmp->my, player.ux, player.uy) <= 49 &&
        !mtmp->mspec_used) {
      struct Attack *a;
    ...
    switch (tmp) {
      ...
      case 3: /* absolutely no movement */
        ...
        if (mtmp->isgd && (mtmp->mhp < 1 || (mtmp->mx == 0 && mtmp->my == 0)))
  ...
  if (!mtmp->msleeping && mtmp->mcanmove && nearby) quest_talk(mtmp);
...
static const char practical[] = {WEAPON_CLASS, ARMOR_CLASS, GEM_CLASS,
                                 FOOD_CLASS, 0};
static const char magical[] = {AMULET_CLASS, POTION_CLASS, SCROLL_CLASS,
 ...
 */
  ...
  if (is_covetous(ptr)) {
    xchar tx = STRAT_GOALX(mtmp->mstrategy), ty = STRAT_GOALY(mtmp->mstrategy);
  ...
  if (ptr == &mons[PM_MAIL_DAEMON]) {
    if (flags.soundok && canseemon(mtmp)) verbalize("I'm late!");
    mongone(mtmp);
    return (2);
  }
#endif
...
#ifdef GOLDOBJ
    Object *lepgold, *ygold;
#endif
    bool should_see =
        (couldsee(omx, omy) && (levl[gx][gy].lit || !levl[omx][omy].lit) &&
         (dist2(omx, omy, gx, gy) <= 36));

    if (!mtmp->mcansee || (should_see && Invis && !perceives(ptr) && rn2(11)) ||
        ...
        if (xx >= lmx && xx <= oomx && yy >= lmy && yy <= oomy) {
          ...
          if ((mtoo = m_at(xx, yy)) != 0 &&
              (mtoo->msleeping || mtoo->mundetected ||
               (mtoo->mappearance && !mtoo->iswiz) || !mtoo->data->mmove))
            continue;

          if (((likegold && otmp->oclass == COIN_CLASS) ||
               (likeobjs && index(practical, otmp->oclass) &&
                (otmp->otyp != CORPSE ||
                 (ptr->mlet == S_NYMPH && !is_rider(&mons[otmp->corpsenm])))) ||
  ...
  if (mmoved) {
    ...
    if (((IS_ROCK(levl[nix][niy].typ) && may_dig(nix, niy)) ||
      ...
      if (closed_door(nix, niy)) {
        if (!(mw_tmp = mtmp->weapon()) || !is_pick(mw_tmp) || !is_axe(mw_tmp))
    ...
    if (!m_in_out_region(mtmp, nix, niy)) return 3;
  ...
  if (mmoved == 1 || mmoved == 3) {
        ...
        if (here->doormask & (D_LOCKED | D_CLOSED) && amorphous(ptr)) {
          if (flags.verbose && canseemon(mtmp))
            pline("%s %s under the door.", Monnam(mtmp),
                  (ptr == &mons[PM_FOG_CLOUD] || ptr == &mons[PM_YELLOW_LIGHT])
                      ? "flows"
                      : "oozes");
    ...
    if (OBJ_AT(mtmp->mx, mtmp->my) && mtmp->mcanmove) {
      ...
      if (setlikes) {
        ...
        likemagic = (likes_magic(ptr) && pctload < 85);
        likerock =
            (throws_rocks(ptr) && pctload < 50 && !In_sokoban(&player.uz));
        conceals = hides_under(ptr);
      }
    ...
    if (hides_under(ptr) || ptr->mlet == S_EEL) {
      ...
      if (mtmp->mundetected || (mtmp->mcanmove && !mtmp->msleeping && rn2(5)))
        mtmp->mundetected =
            (ptr->mlet != S_EEL)
...
void set_apparxy(Monster *mtmp) {
  ...
  if (!gotu) {
    ...
    } while (!isok(mx, my) || (disp != 2 && mx == mtmp->mx && my == mtmp->my) ||
...
#endif
        typ != SACK && typ != BAG_OF_HOLDING && typ != BAG_OF_TRICKS &&
        !Is_candle(obj) && typ != OILSKIN_SACK && typ != LEASH &&
        typ != STETHOSCOPE && typ != BLINDFOLD && typ != TOWEL &&
        typ != TIN_WHISTLE && typ != MAGIC_WHISTLE && typ != MAGIC_MARKER &&
        typ != TIN_OPENER && typ != SKELETON_KEY && typ != LOCK_PICK)
      return FALSE;
