...
bool goodpos(int x, int y, Monster *mtmp, unsigned gpflags) {
   ...
   * oh well.
   */
  if (mtmp != &youmonst && x == player.ux && y == player.uy
#ifdef STEED
      && (!player.usteed || mtmp != player.usteed)
#endif
      )
    return FALSE;
  ...
  if (closed_door(x, y) && (!mdat || !amorphous(mdat))) return FALSE;
...
bool enexto_core(coord *cc, xchar xx, xchar yy, MonsterType *mdat,
                 unsigned entflags) {
#define MAX_GOOD 15
...
STATIC_OVL bool teleok(int x, int y, bool trapok) {
  ...
  if (!tele_jump_ok(player.ux, player.uy, x, y)) return FALSE;
...
void teleds(int nux, int nuy, bool allow_drag) {
  ...
  if (ball_active) {
    if (!carried(uball) && distmin(nux, nuy, uball->ox, uball->oy) <= 2)
      ...
      if (!allow_drag || distmin(player.ux, player.uy, nux, nuy) > 1) {
  ...
  if (hides_under(youmonst.data))
    player.uundetected = OBJ_AT(nux, nuy);
  else if (youmonst.data->mlet == S_EEL)
    player.uundetected = is_pool(nux, nuy);
  else {
    player.uundetected = 0;
    /* mimics stop being unnoticed */
    if (youmonst.data->mlet == S_MIMIC) youmonst.m_ap_type = M_AP_NOTHING;
  }
  ...
  /* must set player.ux, player.uy after drag_ball(), which may need to know
     the old position if allow_drag is true... */
  player.ux = nux;
  player.uy = nuy;
  fill_pit(player.ux0, player.uy0);
  if (ball_active) {
    if (!ball_still_in_range && !allow_drag) placebc();
  }
  ...
  /* Move your steed, too */
  if (player.usteed) {
    player.usteed->mx = nux;
    player.usteed->my = nuy;
  }
#endif
  /*
...
void tele() {
  ...
  if ((player.uhave.amulet || On_W_tower_level(&player.uz)) && !rn2(3)) {
  ...
  if ((Teleport_control && !Stunned)
    ...
    if (unconscious()) {
...
#ifdef STEED
      char buf[BUFSZ];
      if (player.usteed) sprintf(buf, " and %s", mon_nam(player.usteed));
#endif
      ...
      cc.x = player.ux;
      cc.y = player.uy;
      if (getpos(&cc, TRUE, "the desired position") < 0) return; /* abort */
...
int dotele() {
  ...
  if (trap && (!trap->tseen || trap->ttyp != TELEP_TRAP)) trap = 0;
  ...
  if (trap) {
    ...
    if (trap)
      You("%s onto the teleportation trap.", locomotion(youmonst.data, "jump"));
  }
  if (!trap) {
    bool castit = FALSE;
    int sp_no = 0, energy = 0;

    if (!Teleportation || (player.ulevel < (Role_if(PM_WIZARD) ? 8 : 12) &&
    ...
    if (player.uhunger <= 100 || ACURR(A_STR) < 6) {
#ifdef WIZARD
      if (!wizard) {
#endif
...
void level_tele() {
  ...
  if ((player.uhave.amulet || In_endgame(&player.uz) || In_sokoban(&player.uz))
#ifdef WIZARD
      && !wizard
#endif
      ) {
  ...
  if ((Teleport_control && !Stunned)
    ...
    do {
      ...
      if (wizard && !strcmp(buf, "?")) {
        ...
        if ((newlev = (int)print_dungeon(TRUE, &destlev, &destdnum))) {
          ...
          if (In_endgame(&newlevel) && !In_endgame(&player.uz)) {
            sprintf(buf, "Destination is earth level");
            if (!player.uhave.amulet) {
              Object *obj;
              obj = MakeSpecificObject(AMULET_OF_YENDOR, TRUE, FALSE);
              if (obj) {
                obj = addinv(obj);
    ...
    if (newlev == 0) {
      if (trycnt >= 10) goto random_levtport;
     ...
     * we let negative values requests fall into the "heaven" loop.
     */
    if (In_quest(&player.uz) && newlev > 0)
      newlev = newlev + dungeons[player.uz.dnum].depth_start - 1;
  } else { /* involuntary level tele */
  random_levtport:
    newlev = random_teleport_level();
    if (newlev == depth(&player.uz)) {
      You(shudder_for_moment);
      return;
    }
  }
...
#ifdef WIZARD
  if (In_endgame(&player.uz)) { /* must already be wizard */
    int llimit = dunlevs_in_dungeon(&player.uz);
  ...
  if (newlev < 0 && !force_dest) {
    if (*player.ushops0) {
      ...
      u_left_shop(player.ushops0, TRUE);
      /* you're now effectively out of the shop */
      *player.ushops0 = *player.ushops = '\0';
      in_mklev = FALSE;
    }
    if (newlev <= -10) {
  ...
  if (killer) { /* the chosen destination was not survivable */
    ...
    lsav = player.uz;   /* save current level, see below */
    player.uz.dnum = 0; /* main dungeon */
    player.uz.dlevel = (newlev <= -10) ? -10 : 0; /* heaven or surface */
    done(DIED);
    ...
    player.uz = lsav; /* restore player.uz so escape code works */
  }
  ...
  } else if (player.uz.dnum == medusa_level.dnum &&
             newlev >= dungeons[player.uz.dnum].depth_start +
                           dunlevs_in_dungeon(&player.uz)) {
#ifdef WIZARD
 ...
 */
    ...
    if (!wizard)
#endif
      if (Inhell && !player.uevent.invoked &&
          newlev >= (dungeons[player.uz.dnum].depth_start +
                     dunlevs_in_dungeon(&player.uz) - 1)) {
        newlev = dungeons[player.uz.dnum].depth_start +
                 dunlevs_in_dungeon(&player.uz) - 2;
        pline("Sorry...");
      }
    /* no teleporting out of quest dungeon */
    if (In_quest(&player.uz) && newlev < depth(&qstart_level))
 ...
 */
  ...
  if (player.utotype && !flags.mon_moving) deferred_goto();
}
...
void domagicportal(Trap *ttmp) {
  ...
  if (!on_level(&player.uz, &player.uz0)) return;
   ...
   */
  if (In_endgame(&player.uz) && !player.uhave.amulet) {
...
void tele_trap(Trap *trap) {
  if (In_endgame(&player.uz) || Antimagic) {
...
void level_tele_trap(Trap *trap) {
  ...
  if (Antimagic || In_endgame(&player.uz)) {
    You_feel("a wrenching sensation.");
    return;
  }
  if (!Blind)
...
STATIC_OVL bool rloc_pos_ok(int x, int y, Monster *mtmp) {
  ...
  if (!xx) {
    ...
    if (dndest.nlx && On_W_tower_level(&player.uz))
      return ((yy & 2) != 0) ^ /* inside xor not within */
             !within_bounded_area(x, y, dndest.nlx, dndest.nly, dndest.nhx,
                                  dndest.nhy);
    if (updest.lx && (yy & 1) != 0) /* moving up */
      return (within_bounded_area(x, y, updest.lx, updest.ly, updest.hx,
                                  updest.hy) &&
              (!updest.nlx ||
               !within_bounded_area(x, y, updest.nlx, updest.nly, updest.nhx,
                                    updest.nhy)));
...
bool rloc(Monster *mtmp, bool suppress_impossible) {
  ...
  if (mtmp->iswiz && mtmp->mx) { /* Wizard, not just arriving */
    if (!In_W_tower(player.ux, player.uy, &player.uz))
    ...
    if (goodpos(x, y, mtmp, 0)) goto found_xy;
  }
  ...
  do {
    x = rn1(COLNO - 3, 2);
    y = rn2(ROWNO);
    if ((trycount < 500) ? rloc_pos_ok(x, y, mtmp) : goodpos(x, y, mtmp, 0))
      goto found_xy;
  } while (++trycount < 1000);
  ...
  if (!suppress_impossible) impossible("rloc(): couldn't relocate monster");
  return FALSE;
  ...
  if (teleport_pet(mtmp, force_it)) {
    ...
    if ((tt == HOLE || tt == TRAPDOOR)) {
      ...
      if (nlev == depth(&player.uz)) {
        if (in_sight) pline("%s shudders for a moment.", Monnam(mtmp));
        return 0;
      }
...
void rloco(Object *obj) {
    ...
    if (!--try_limit) break;
           ...
           (restricted_fall &&
            (!within_bounded_area(tx, ty, dndest.lx, dndest.ly, dndest.hx,
                                  dndest.hy) ||
             (dndest.nlx && within_bounded_area(tx, ty, dndest.nlx, dndest.nly,
  ...
  if (flooreffects(obj, tx, ty, "fall")) {
    ...
    if (costly_spot(otx, oty) &&
      ...
      if (costly_spot(player.ux, player.uy) &&
          index(player.urooms, *in_rooms(otx, oty, 0)))
...
int random_teleport_level() {
  int nlev, max_depth, min_depth, cur_depth = (int)depth(&player.uz);

  if (!rn2(5) || Is_knox(&player.uz)) return cur_depth;
   ...
   */
  min_depth = In_quest(&player.uz) ? dungeons[player.uz.dnum].depth_start : 1;
  max_depth = dunlevs_in_dungeon(&player.uz) +
              (dungeons[player.uz.dnum].depth_start - 1);
  /* can't reach the Sanctum if the invocation hasn't been performed */
  if (Inhell && !player.uevent.invoked) max_depth -= 1;
  ...
  if (nlev > max_depth) {
    ...
    if (Is_botlevel(&player.uz)) nlev -= rnd(3);
  }
  if (nlev < min_depth) {
    nlev = min_depth;
    if (nlev == cur_depth) {
...
bool u_teleport_mon(Monster *mtmp, bool give_feedback) {
  ...
  if (mtmp->ispriest && *in_rooms(mtmp->mx, mtmp->my, TEMPLE)) {
    if (give_feedback) pline("%s resists your magic!", Monnam(mtmp));
    return FALSE;
  } else if (level.flags.noteleport && player.uswallow &&
             mtmp == player.ustuck) {
    if (give_feedback) You("are no longer inside %s!", mon_nam(mtmp));
    unstuck(mtmp);
    (void)rloc(mtmp, FALSE);
  } else if (is_rider(mtmp->data) && rn2(13) &&
             enexto(&cc, player.ux, player.uy, mtmp->data))
