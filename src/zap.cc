...
STATIC_DCL void costly_cancel(Object *);
...
int bhitm(Monster *mtmp, Object *otmp) {
  ...
  Object *obj;
  bool disguised_mimic =
      (mtmp->data->mlet == S_MIMIC && mtmp->m_ap_type != M_AP_NOTHING);

  if (player.uswallow && mtmp == player.ustuck) reveal_invis = FALSE;
  ...
  switch (otyp) {
    ...
    case SPE_FORCE_BOLT:
      ...
      if (resists_magm(mtmp)) { /* match effect on player */
        ...
        dmg = d(2, 12);
        if (dbldam) dmg *= 2;
        if (otyp == SPE_FORCE_BOLT) dmg += spell_damage_bonus();
    ...
    case SPE_TURN_UNDEAD:
      ...
      if (is_undead(mtmp->data)) {
        ...
        dmg = rnd(8);
        if (dbldam) dmg *= 2;
        if (otyp == SPE_TURN_UNDEAD) dmg += spell_damage_bonus();
    ...
    case POT_POLYMORPH:
      if (resists_magm(mtmp)) {
        ...
        if (mtmp->cham == CHAM_ORDINARY && !rn2(25)) {
          ...
          for (obj = mtmp->minvent; obj; obj = obj->nobj) bypass_obj(obj);
          /* flags.bypasses = TRUE; ## for make_corpse() */
          /* no corpse after system shock */
          xkilled(mtmp, 3);
        } else if (newcham(mtmp, (MonsterType *)0, (otyp != POT_POLYMORPH),
                           FALSE)) {
          if (!Hallucination && canspotmon(mtmp)) makeknown(otyp);
        }
      }
      break;
    ...
    case SPE_STONE_TO_FLESH:
      if (monsndx(mtmp->data) == PM_STONE_GOLEM) {
        ...
        if (newcham(mtmp, &mons[PM_FLESH_GOLEM], FALSE, FALSE)) {
          if (canseemon(mtmp)) pline("%s turns to flesh!", name);
        } else {
          if (canseemon(mtmp))
    ...
    case SPE_DRAIN_LIFE:
      dmg = rnd(8);
      if (dbldam) dmg *= 2;
      if (otyp == SPE_DRAIN_LIFE) dmg += spell_damage_bonus();
      if (resists_drli(mtmp))
        ...
        if (mtmp->mhp <= 0 || mtmp->mhpmax <= 0 || mtmp->m_lev < 1)
          ...
          mtmp->m_lev--;
          if (canseemon(mtmp)) pline("%s suddenly seems weaker!", Monnam(mtmp));
        }
      }
      break;
    default:
...
Monster *montraits(Object *obj, coord *cc) {
  ...
  if (mtmp2) {
    ...
    if (mtmp2->mhpmax <= 0 && !is_rider(mtmp2->data)) return (Monster *)0;
    ...
    if (!mtmp2->m_id) mtmp2->m_id = mtmp->m_id;
    mtmp2->mx = mtmp->mx;
    mtmp2->my = mtmp->my;
    mtmp2->mux = mtmp->mux;
    mtmp2->muy = mtmp->muy;
...
Monster *revive(Object *obj) {
  ...
  if (obj->otyp == CORPSE) {
    ...
    if (obj->where == OBJ_CONTAINED) {
      ...
      if (!get_obj_location(obj, &x, &y, 0)) return (Monster *)0;
    }
    if (in_container) {
    ...
    if (MON_AT(x, y)) {
      ...
      if (enexto(&new_xy, x, y, &mons[montype])) x = new_xy.x, y = new_xy.y;
    }
    ...
    if (cant_create(&montype, TRUE)) {
      ...
      if (obj->oxlth && (obj->oattached == OATTACHED_MONST)) {
        ...
        if (mtmp && mtmp->mtame && !mtmp->isminion) wary_dog(mtmp, TRUE);
      } else
      ...
      if (mtmp) {
        if (obj->oxlth && (obj->oattached == OATTACHED_M_ID)) {
          unsigned m_id;
          Monster *ghost;
          (void)memcpy((genericptr_t) & m_id, (genericptr_t)obj->oextra,
                       sizeof(m_id));
          ...
          if (ghost && ghost->data == &mons[PM_GHOST]) {
            ...
            x2 = ghost->mx;
            y2 = ghost->my;
            if (ghost->mtame) savetame = ghost->mtame;
            if (canseemon(ghost))
        ...
        if (obj->onamelth) mtmp = christen_monst(mtmp, ONAME(obj));
    ...
    if (mtmp) {
      if (obj->oeaten) mtmp->mhp = eaten_stat(mtmp->mhp, obj);
      ...
      switch (obj->where) {
        ...
        case OBJ_FLOOR:
          ...
          if (obj->quan > 1L) obj = SplitObject(obj, 1L);
          delobj(obj);
          newsym(x, y);
          break;
        case OBJ_MINVENT:
...
STATIC_OVL void revive_egg(Object *obj) {
  /*
...
int unturn_dead(Monster *mon) {
  ...
  while ((otmp = otmp2) != 0) {
    ...
    if ((mtmp2 = revive(otmp)) != 0) {
      ...
      if (youseeit) {
        if (!once++)
          strcpy(owner, (mon == &youmonst) ? "Your" : s_suffix(Monnam(mon)));
...
void cancel_item(Object *obj) {
  ...
  switch (obj->otyp) {
    ...
    case RIN_INCREASE_ACCURACY:
      if ((obj->owornmask & W_RING) && u_ring) player.uhitinc -= obj->spe;
      break;
    case RIN_INCREASE_DAMAGE:
      if ((obj->owornmask & W_RING) && u_ring) player.udaminc -= obj->spe;
      break;
...
bool drain_item(Object *obj) {
  ...
  if (!obj || (!objects[obj->otyp].oc_charged && obj->oclass != WEAPON_CLASS &&
      ...
      obj->spe <= 0)
    return (FALSE);
  if (obj_resists(obj, 10, 90)) return (FALSE);
  ...
  switch (obj->otyp) {
    ...
    case RIN_ADORNMENT:
      ...
      break;
    case RIN_INCREASE_ACCURACY:
      if ((obj->owornmask & W_RING) && u_ring) player.uhitinc--;
      break;
    case RIN_INCREASE_DAMAGE:
      if ((obj->owornmask & W_RING) && u_ring) player.udaminc--;
      break;
    case HELM_OF_BRILLIANCE:
      if ((obj->owornmask & W_ARMH) && (obj == uarmh)) {
        ABON(A_INT)--;
 ...
 */
STATIC_OVL void polyuse(Object *objhdr, int mat, int minwt) {
  Object *otmp, *otmp2;
...
STATIC_OVL void create_polymon(Object *obj, int okind) {
  ...
  if (!(mvitals[pm_index].mvflags & G_GENOD)) mdat = &mons[pm_index];
 ...
 */
Object *poly_obj(Object *obj, int id) {
  Object *otmp;
  xchar ox, oy;
  ...
  if (id == STRANGE_OBJECT) { /* preserve symbol */
    ...
    do {
      if (otmp) delobj(otmp);
      otmp = MakeRandomObject(obj->oclass, FALSE);
    } while (--try_limit > 0 &&
...
#undef USES_CORPSENM
  ...
  if (is_damageable(otmp)) otmp->oerodeproof = obj->oerodeproof;

  /* Keep chest/box traps and poisoned ammo if we may */
  if (obj->otrapped && Is_box(otmp)) otmp->otrapped = TRUE;

  if (obj->opoisoned && is_poisonable(otmp)) otmp->opoisoned = TRUE;
  ...
  if (obj_location == OBJ_INVENT) {
    if (id == STRANGE_OBJECT)
      ...
      if (otmp->owornmask & LEFT_RING) uleft = otmp;
      if (otmp->owornmask & RIGHT_RING) uright = otmp;
      if (otmp->owornmask & W_WEP) uwep = otmp;
      if (otmp->owornmask & W_SWAPWEP) uswapwep = otmp;
      if (otmp->owornmask & W_QUIVER) uquiver = otmp;
      goto no_unwear;
    }
  }
...
no_unwear:
  ...
  if ((!carried(otmp) || obj->unpaid) &&
      ...
      costly_spot(ox, oy)) {
    Monster *shkp = shop_keeper(*in_rooms(ox, oy, SHOPBASE));
    ...
    if ((!obj->no_charge ||
      ...
      if (shkp->mpeaceful) {
        if (*player.ushops &&
            *in_rooms(player.ux, player.uy, 0) ==
 ...
 */
int bhito(Object *obj, Object *otmp) {
...
#ifdef DEBUG
      pline("%s for a moment.", Tobjnam(Object, "pulsate"));
#endif
      obj->bypass = 0;
    }
  }
  ...
  if (obj == uball) {
    ...
    switch (otmp->otyp) {
      ...
      case SPE_POLYMORPH:
        ...
        if (obj_shudders(obj)) {
          if (cansee(obj->ox, obj->oy)) makeknown(otmp->otyp);
          do_osshock(obj);
          break;
        }
...
#ifdef INVISIBLE_OBJECTS
        Object->oinvis = TRUE;
        newsym(Object->ox, Object->oy); /* make object disappear */
#endif
        break;
      case WAN_UNDEAD_TURNING:
      case SPE_TURN_UNDEAD:
      ...
      case SPE_WIZARD_LOCK:
        if (Is_box(obj))
          ...
          res = 0;
        if (res /* && otmp->oclass == WAND_CLASS */) makeknown(otmp->otyp);
        break;
      ...
      case SPE_STONE_TO_FLESH:
        ...
        switch (objects[obj->otyp].oc_class) {
          ...
          case TOOL_CLASS: /* figurine */
          {
            ...
            if (mon) {
              delobj(obj);
              if (cansee(mon->mx, mon->my)) pline_The("figurine animates!");
              break;
            }
            goto makecorpse;
          }
...
int bhitpile(Object *obj, int (*fhito)(Object *, Object *), int tx, int ty) {
  int hitanything = 0;
  ...
  if (poly_zapped >= 0) create_polymon(level.objects[tx][ty], poly_zapped);
 ...
 */
int zappable(Object *wand) {
  if (wand->spe < 0 || (wand->spe == 0 && rn2(121))) return 0;
...
void zapnodir(Object *obj) {
  ...
  switch (obj->otyp) {
    ...
    case WAN_CREATE_MONSTER:
      known = create_critters(rn2(23) ? 1 : rn1(7, 2), (MonsterType *)0);
      break;
    case WAN_WISHING:
      known = TRUE;
      if (Luck + rn2(5) < 0) {
...
int dozap() {
  ...
  if (!zappable(obj))
    ...
    if (!Blind) pline("%s glows and fades.", The(xname(obj)));
...
int zapyourself(Object *obj, bool ordinary) {
  int damage = 0;
  char buf[BUFSZ];
  ...
  switch (obj->otyp) {
    ...
    case SPE_WIZARD_LOCK:
      break;
    case WAN_PROBING:
      for (obj = invent; obj; obj = obj->nobj) obj->dknown = 1;
    ...
    case SPE_STONE_TO_FLESH: {
      ...
      if (player.umonnum == PM_STONE_GOLEM) (void)polymon(PM_FLESH_GOLEM);
 ...
 */
STATIC_OVL bool zap_steed(Object *obj) {
  int steedhit = FALSE;
 ...
 */
bool cancel_monst(Monster *mdef, Object *obj, bool youattack,
                  bool allow_cancel_kill, bool self_cancel) {
  bool youdefend = (mdef == &youmonst);
  static const char writing_vanishes[] = "Some writing vanishes from %s head!";
  ...
  if (youdefend) {
    ...
    if (is_were(mdef->data) && mdef->data->mlet != S_HUMAN) were_change(mdef);

    if (mdef->data == &mons[PM_CLAY_GOLEM]) {
      if (canseemon(mdef)) pline(writing_vanishes, s_suffix(mon_nam(mdef)));
...
STATIC_OVL bool zap_updown(Object *obj) {
  ...
  switch (obj->otyp) {
    ...
    case SPE_WIZARD_LOCK:
      ...
      if ((levl[x][y].typ == DRAWBRIDGE_DOWN)
        ...
        losehp(rnd((uarmh && is_metallic(uarmh)) ? 2 : 6), "falling rock",
               KILLED_BY_AN);
        if ((otmp = MakeSpecificObjectAt(ROCK, x, y, FALSE, FALSE)) != 0) {
    ...
    case SPE_STONE_TO_FLESH:
      if (Is_airlevel(&player.uz) || Is_waterlevel(&player.uz) || Underwater ||
          (Is_qstart(&player.uz) && player.dz < 0)) {
        pline(nothing_happens);
...
void weffects(Object *obj) {
  ...
  exercise(A_WIS, TRUE);
#ifdef STEED
  if (player.usteed && (objects[otyp].oc_dir != NODIR) && !player.dx &&
      !player.dy && (player.dz > 0) && zap_steed(obj)) {
    disclose = TRUE;
  } else
#endif
    ...
    if (player.uswallow) {
      ...
      (void)bhit(player.dx, player.dy, rn1(8, 6), ZAPPED_WAND, bhitm, bhito,
                 obj, NULL);
    }
    /* give a clue if obj_zapped */
    if (obj_zapped) You_feel("shuddering vibrations.");
    ...
    if (otyp == WAN_DIGGING || otyp == SPE_DIG)
      ...
      buzz(otyp - SPE_MAGIC_MISSILE + 10, player.ulevel / 2 + 1, player.ux,
           player.uy, player.dx, player.dy);
...
STATIC_OVL int spell_hit_bonus(int skill) {
  ...
  if (dex < 4)
    ...
    hit_bon +=
        dex - 14; /* Even increment for dextrous heroes (see weapon.c abon) */
...
Monster *bhit(
    ...
    int (*fhitm)(Monster *, Object *), int (*fhito)(Object *, Object *),
  ...
  while (range-- > 0) {
    ...
    if (weapon == ZAPPED_WAND && find_drawbridge(&x, &y)) switch (obj->otyp) {
        case WAN_OPENING:
        case SPE_KNOCK:
        ...
        case WAN_STRIKING:
        case SPE_FORCE_BOLT:
          if (typ != DRAWBRIDGE_UP) destroy_drawbridge(x, y);
          makeknown(obj->otyp);
          break;
      }
    ...
    if (fhito) {
      if (bhitpile(obj, fhito, bhitpos.x, bhitpos.y)) range--;
    } else {
      if (weapon == KICKED_WEAPON &&
    ...
    if (weapon != ZAPPED_WAND && weapon != INVIS_BEAM) {
      ...
      if ((weapon == KICKED_WEAPON) &&
          (is_pool(bhitpos.x, bhitpos.y) || is_lava(bhitpos.x, bhitpos.y)))
        break;
#ifdef SINKS
  ...
  for (ct = 0; ct < 10; ct++) {
    ...
    if (bhitpos.x == player.ux && bhitpos.y == player.uy) { /* ct == 9 */
      if (Fumbling || rn2(20) >= ACURR(A_DEX)) {
        /* we hit ourselves */
        (void)thitu(10, rnd(10), nullptr, "boomerang");
        break;
      } else { /* we catch it */
        tmp_at(DISP_END, 0);
    ...
    case ZT_DEATH:                            /* death/disintegration */
      if (abs(type) != ZT_BREATH(ZT_DEATH)) { /* death */
        if (mon->data == &mons[PM_DEATH]) {
          ...
          if (mon->mhpmax >= MAGIC_COOKIE) mon->mhpmax = MAGIC_COOKIE - 1;
          mon->mhp = mon->mhpmax;
          tmp = 0;
          break;
        }
        ...
        if (resists_disint(mon)) {
          ...
          *ootmp = which_armor(mon, W_ARM);
          if ((otmp2 = which_armor(mon, W_ARMC)) != 0) m_useup(mon, otmp2);
        } else {
          ...
          if ((otmp2 = which_armor(mon, W_ARMC)) != 0) m_useup(mon, otmp2);
#ifdef TOURIST
          if ((otmp2 = which_armor(mon, W_ARMU)) != 0) m_useup(mon, otmp2);
#endif
        }
...
STATIC_OVL void zhitu(int type, int nd, const char *fltxt, xchar sx, xchar sy) {
  ...
  if (Half_spell_damage && dam && type < 0 &&
      (type > -20 || type < -29)) /* !Breath */
    dam = (dam + 1) / 2;
 ...
 */
int burn_floor_paper(int x, int y, bool give_feedback, bool u_caused) {
  Object *obj, *obj2;
...
void buzz(int type, int nd, xchar sx, xchar sy, int dx, int dy) {
  ...
  if (player.uswallow) {
    ...
    if (!player.ustuck)
      ...
      pline("%s rips into %s%s", The(fltxt), mon_nam(player.ustuck),
            exclam(tmp));
    /* Using disintegration from the inside only makes a hole... */
    if (tmp == MAGIC_COOKIE) player.ustuck->mhp = 0;
    if (player.ustuck->mhp < 1) killed(player.ustuck);
    return;
  }
  ...
  while (range-- > 0) {
    ...
    if (isok(sx, sy) && (lev = &levl[sx][sy])->typ) {
      ...
      if (cansee(sx, sy)) {
        ...
        if (ZAP_POS(lev->typ) || cansee(lsx, lsy)) tmp_at(sx, sy);
      ...
      if (zap_hit(find_mac(mon), spell_type)) {
        if (mon_reflects(mon, (char *)0)) {
          ...
          if (is_rider(mon->data) && abs(type) == ZT_BREATH(ZT_DEATH)) {
            if (canseemon(mon)) {
              ...
              pline("%s body reintegrates before your %s!",
                    s_suffix(Monnam(mon)), (eyecount(youmonst.data) == 1)
                                               ? body_part(EYE)
          ...
          if (tmp == MAGIC_COOKIE) { /* disintegration */
            Object *otmp2, *m_amulet = mlifesaver(mon);
...
#define oresist_disintegration(obj)                                        \
  (objects[obj->otyp].oc_oprop == DISINT_RES || obj_resists(obj, 5, 50) || \
   is_quest_artifact(obj) || obj == m_amulet)
            ...
            if (!otmp) {
              ...
              if (canseemon(mon))
                pline("%s %s is disintegrated!", s_suffix(Monnam(mon)),
  ...
  tmp_at(DISP_END, 0);
  if (type == ZT_SPELL(ZT_FIRE)) explode(sx, sy, type, d(12, 6), 0, EXPL_FIERY);
  if (shopdamage)
    pay_for_damage(abstype == ZT_FIRE ? "burn away" : abstype == ZT_COLD
                                                          ? "shatter"
                                                          : abstype == ZT_DEATH
                                                                ? "disintegrate"
                                                                : "destroy",
...
int zap_over_floor(xchar x, xchar y, int type, bool *shopdamage) {
  ...
  if (abstype == ZT_FIRE) {
    ...
    if (is_ice(x, y)) {
      ...
      if (cansee(x, y)) pline("Steam billows from the fountain.");
      rangemod -= 1;
      dryup(x, y, type > 0);
    }
  } else if (abstype == ZT_COLD && (is_pool(x, y) || is_lava(x, y))) {
    bool lava = is_lava(x, y);
    bool moat = (!lava && (lev->typ != POOL) && (lev->typ != WATER) &&
                 !Is_medusa_level(&player.uz) && !Is_waterlevel(&player.uz));
    ...
    if (lev->typ == WATER) {
      ...
      if (lev->typ == DRAWBRIDGE_UP) {
        ...
        if (!lava) lev->icedpool = (lev->typ == POOL ? ICED_POOL : ICED_MOAT);
  ...
  if (closed_door(x, y)) {
    ...
    switch (abstype) {
      ...
      case ZT_DEATH:
        ...
        if (abs(type) != ZT_BREATH(ZT_DEATH)) goto def_case;
        new_doormask = D_NODOOR;
      ...
      def_case:
        if (cansee(x, y)) {
          pline_The("door absorbs %s %s!", (type < 0) ? "the" : "your",
                    abs(type) < ZT_SPELL(0) ? "bolt" : abs(type) < ZT_BREATH(0)
                                                           ? "spell"
                                                           : "blast");
        } else
          You_feel("vibrations.");
  ...
  if ((mon = m_at(x, y)) != 0) {
    ...
    if (type >= 0) {
      setmangry(mon);
      if (mon->ispriest && *in_rooms(mon->mx, mon->my, TEMPLE)) ghod_hitsu(mon);
      if (mon->isshk && !*player.ushops) hot_pursuit(mon);
    }
  }
  return rangemod;
}
...
void fracture_rock(Object *obj) {
  ...
  if (obj->where == OBJ_FLOOR) {
    ...
    if (!does_block(obj->ox, obj->oy, &levl[obj->ox][obj->oy]))
      unblock_point(obj->ox, obj->oy);
    if (cansee(obj->ox, obj->oy)) newsym(obj->ox, obj->oy);
  }
}
  ...
  for (obj = invent; obj; obj = obj2) {
    ...
    switch (dmgtyp) {
      ...
      case AD_FIRE:
        ...
        if (obj->otyp == SCR_FIRE || obj->otyp == SPE_FIREBALL) skip++;
        if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
          skip++;
          if (!Blind)
            pline("%s glows a strange %s, but remains intact.", The(xname(obj)),
                  hcolor("dark red"));
        }
        quan = obj->quan;
        switch (osym) {
...
#if 0
			    if (Object == current_wand) { skip++; break; }
#endif
            dindx = 5;
            dmg = rnd(10);
            break;
    ...
    if (!skip) {
      ...
      pline("%s %s %s!", mult, xname(obj), (cnt > 1L)
...
int resist(Monster *mtmp, char oclass, int damage, int tell) {
  ...
  if (dlev > 50)
    ...
    dlev = is_mplayer(mtmp->data) ? player.ulevel : 1;
        ...
        otmp, player.uswallow
                  ...
                  : (Is_airlevel(&player.uz) || Is_waterlevel(&player.uz) ||
                     ...
                     levl[player.ux][player.uy].typ >= ICE)
                        ...
                        : "Oops!  %s to the floor!",
        The(aobjnam(otmp, Is_airlevel(&player.uz) || player.uinwater ? "slip"
                                                                     : "drop")),
        (const char *)0);
