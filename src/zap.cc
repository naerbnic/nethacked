    ...
    case POT_POLYMORPH:
      if (resists_magm(mtmp)) {
        ...
        if (mtmp->cham == CHAM_ORDINARY && !rn2(25)) {
          ...
          xkilled(mtmp, 3);
        } else if (newcham(mtmp, nullptr, (otyp != POT_POLYMORPH),
                           FALSE)) {
...
void probe_monster(Monster *mtmp) {
  ...
  if (mtmp->minvent) {
    ...
    (void)display_minventory(mtmp, MINV_ALL, nullptr);
  } else {
...
#undef USES_CORPSENM
  ...
  if (!IsFlammable(otmp) && !is_rustprone(otmp))
    otmp->oeroded = 0;
  if (!is_corrodeable(otmp) && !IsRottable(otmp))
    otmp->oeroded2 = 0;
        ...
        switch (objects[obj->otyp].oc_class) {
          case ROCK_CLASS: /* boulders and statues */
            if (obj->otyp == BOULDER) {
              ...
              if (!animate_statue(obj, oox, ooy, ANIMATE_SPELL, nullptr)) {
                Object *item;
              makecorpse:
...
void zapnodir(Object *obj) {
  ...
  switch (obj->otyp) {
    ...
    case WAN_CREATE_MONSTER:
      known = create_critters(rn2(23) ? 1 : rn1(7, 2), nullptr);
      break;
    case WAN_WISHING:
      known = TRUE;
      if (Luck + rn2(5) < 0) {
...
int dozap() {
  ...
  if (!zappable(obj))
    ...
    return (1);
  } else if (!(objects[obj->otyp].oc_dir == NODIR) && !getdir(nullptr)) {
    if (!Blind)
  ...
  *ootmp = nullptr;
  switch (abstype) {
    ...
    case ZT_ACID:
      ...
      tmp = d(nd, 6);
      if (!rn2(6))
        erode_obj(mon->weapon(), TRUE, TRUE);
      if (!rn2(6))
...
STATIC_OVL void zhitu(int type, int nd, const char *fltxt, xchar sx, xchar sy) {
  ...
  switch (abs(type) % 10) {
    ...
    case ZT_ACID:
      ...
      if (!rn2(player.twoweap ? 3 : 6))
        erode_obj(uwep, TRUE, TRUE);
      if (player.twoweap && !rn2(3))
...
#define oresist_disintegration(obj)                                        \
  (objects[obj->otyp].oc_oprop == DISINT_RES || obj_resists(obj, 5, 50) || \
   is_quest_artifact(obj) || obj == m_amulet)
      ...
      nomul(0, 0);
#ifdef STEED
      if (player.usteed && !rn2(3) && !mon_reflects(player.usteed, nullptr)) {
        mon = player.usteed;
        goto buzzmonst;
      } else
#endif
...
retry:
  ...
  if (!otmp) {
    ...
    otmp = readobjnam(nullptr, nullptr, TRUE);
    if (!otmp)
