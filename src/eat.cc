...
STATIC_DCL Object *touchfood(Object *);
STATIC_DCL void do_reset_eat();
STATIC_DCL void done_eating(bool);
STATIC_DCL void cprefx(int);
STATIC_DCL int intrinsic_possible(int, MonsterType *);
STATIC_DCL void givit(int, MonsterType *);
STATIC_DCL void cpostfx(int);
STATIC_DCL void start_tin(Object *);
STATIC_DCL int eatcorpse(Object *);
STATIC_DCL void start_eating(Object *);
STATIC_DCL void fprefx(Object *);
STATIC_DCL void accessory_has_effect(Object *);
STATIC_DCL void fpostfx(Object *);
STATIC_DCL int bite();
STATIC_DCL int edibility_prompts(Object *);
STATIC_DCL int rottenfood(Object *);
STATIC_DCL void eatspecial();
STATIC_DCL void eataccessory(Object *);
STATIC_DCL const char *foodword(Object *);
 ...
 */
bool is_edible(Object *obj) {
  /* protect invocation tools but not Rider corpses (handled elsewhere)*/
  /* if (obj->oclass != FOOD_CLASS && obj_resists(obj, 0, 0)) */
  if (objects[obj->otyp].oc_unique) return FALSE;
  ...
  if (metallivorous(youmonst.data) && is_metallic(obj) &&
    ...
    return TRUE;
  if (player.umonnum == PM_GELATINOUS_CUBE && is_organic(obj) &&
...
static struct {
  Object *piece; /* the thing being eaten, or last thing that
...
STATIC_OVL const char *food_xname(Object *food, bool the_pfx) {
  const char *result;
...
STATIC_OVL void choke(Object *food) {
  /* only happens if you were satiated */
  if (player.uhs != SATIATED) {
    if (!food || food->otyp != AMULET_OF_STRANGULATION) return;
  } else if (Role_if(PM_KNIGHT) && player.ualign.type == A_LAWFUL) {
    ...
    if (food) {
      ...
      if (food->oclass == COIN_CLASS) {
        ...
        if (food->otyp == CORPSE && (mons[food->corpsenm].geno & G_UNIQ)) {
          if (!type_is_pname(&mons[food->corpsenm])) killer = the(killer);
...
STATIC_OVL void recalc_wt() {
  Object *piece = victual.piece;

#ifdef DEBUG
  debugpline("Old weight = %d", piece->owt);
  debugpline("Used time = %d, Req'd time = %d", victual.usedtime,
             victual.reqtime);
#endif
 ...
 * in do_reset_eat().
 */
void food_disappears(Object *obj) {
...
void food_substitution(Object *old_obj, Object *new_obj) {
...
int eatfood() {
  if (!victual.piece || (!carried(victual.piece) &&
                         !obj_here(victual.piece, player.ux, player.uy))) {
    /* maybe it was stolen? */
    do_reset_eat();
    return (0);
  }
...
STATIC_OVL bool maybe_cannibal(int pm, bool allowmsg) {
  if (!CANNIBAL_ALLOWED() && your_race(&mons[pm])) {
    if (allowmsg) {
      if (Upolyd) You("have a bad feeling deep inside.");
...
STATIC_OVL void cprefx(int pm) {
  ...
  if (touch_petrifies(&mons[pm]) || pm == PM_MEDUSA) {
    if (!Stone_resistance &&
      ...
      if (victual.piece) victual.eating = FALSE;
      return; /* lifesaved */
    }
  }
  ...
  switch (pm) {
    ...
    case PM_FAMINE: {
      ...
      if (revive_corpse(victual.piece)) victual.piece = nullptr;
      return;
    }
    case PM_GREEN_SLIME:
    ...
    /* Fall through */
    default:
      if (acidic(&mons[pm]) && Stoned) fix_petrification();
      break;
  }
}
...
STATIC_OVL int intrinsic_possible(int type, MonsterType *ptr) {
  switch (type) {
    case FIRE_RES:
#ifdef DEBUG
 ...
 */
STATIC_OVL void givit(int type, MonsterType *ptr) {
  int chance;
  ...
  if (ptr->mlevel <= rn2(chance)) return; /* failed die roll */
...
STATIC_OVL void cpostfx(int pm) {
  ...
  switch (pm) {
    case PM_NEWT:
      ...
      if (rn2(3) || 3 * player.uen <= 2 * player.uenmax) {
        int old_uen = player.uen;
        player.uen += rnd(3);
        if (player.uen > player.uenmax) {
          if (!rn2(3)) player.uenmax++;
          player.uen = player.uenmax;
        }
        if (old_uen != player.uen) {
    ...
    case PM_HUMAN_WERERAT:
      ...
      player.ulycn = PM_WERERAT;
      break;
    case PM_HUMAN_WEREJACKAL:
      catch_lycanthropy = TRUE;
      player.ulycn = PM_WEREJACKAL;
      break;
...
STATIC_PTR
void costly_tin(const char *verb) {
...
int opentin() {
  ...
  if (!carried(tin.tin) && !obj_here(tin.tin, player.ux, player.uy))
  ...
  if (tin.usedtime < tin.reqtime) return (1); /* still busy */
  ...
  if (tin.tin->spe != 1) {
    ...
    if (!vegan(&mons[tin.tin->corpsenm])) player.uconduct.unvegan++;
    if (!vegetarian(&mons[tin.tin->corpsenm])) violated_vegetarian();
...
STATIC_OVL void start_tin(Object *otmp) {
  int tmp;
  ...
  no_opener:
    ...
    if (Glib) {
      pline_The("tin slips from your %s.", makeplural(body_part(FINGER)));
      if (otmp->quan > 1L) {
        otmp = SplitObject(otmp, 1L);
      }
      if (carried(otmp))
...
STATIC_OVL int rottenfood(Object *obj) {
  ...
  if (!rn2(4)) {
    ...
    else if (Levitation || Is_airlevel(&player.uz) || Is_waterlevel(&player.uz))
      what = "you lose control of", where = "yourself";
    else
      what = "you slap against the",
      where =
#ifdef STEED
          (player.usteed) ? "saddle" :
#endif
...
STATIC_OVL int eatcorpse(Object *otmp) {
  ...
  /* KMH, conduct */
  if (!vegan(&mons[mnum])) player.uconduct.unvegan++;
  ...
  if (mnum != PM_ACID_BLOB && !stoneable && rotted > 5L) {
    ...
    if (Sick_resistance) {
      ...
      if (Sick && (sick_time > Sick)) sick_time = (Sick > 1L) ? Sick - 1L : 1L;
      if (!uniq)
...
STATIC_OVL void start_eating(Object *otmp) {
#ifdef DEBUG
 ...
 */
STATIC_OVL void fprefx(Object *otmp) {
  switch (otmp->otyp) {
    case FOOD_RATION:
      if (player.uhunger <= 200)
        pline(Hallucination ? "Oh wow, like, superior, man!"
                            : "That food really hit the spot!");
      else if (player.uhunger <= 700)
      ...
      pline("This %s is %s", singular(otmp, xname),
            otmp->cursed ? (Hallucination ? "grody!" : "terrible!")
                         : (otmp->otyp == CRAM_RATION ||
                            otmp->otyp == K_RATION || otmp->otyp == C_RATION)
                               ? "bland."
  ...
  if (!rn2(otmp->oclass == RING_CLASS ? 3 : 5)) {
    switch (otmp->otyp) {
      default:
        ...
        if (!(player.uprops[objects[typ].oc_oprop].intrinsic & FROMOUTSIDE))
          accessory_has_effect(otmp);

        player.uprops[objects[typ].oc_oprop].intrinsic |= FROMOUTSIDE;
        ...
        switch (typ) {
          ...
          case RIN_LEVITATION:
            ...
            player.uprops[LEVITATION].intrinsic = oldprop;
            if (!Levitation) {
              float_up();
      ...
      case RIN_ADORNMENT:
        accessory_has_effect(otmp);
        if (adjattrib(A_CHA, otmp->spe, -1)) makeknown(typ);
        break;
      case RIN_GAIN_STRENGTH:
        accessory_has_effect(otmp);
        if (adjattrib(A_STR, otmp->spe, -1)) makeknown(typ);
        break;
      case RIN_GAIN_CONSTITUTION:
        accessory_has_effect(otmp);
        if (adjattrib(A_CON, otmp->spe, -1)) makeknown(typ);
        break;
      case RIN_INCREASE_ACCURACY:
        accessory_has_effect(otmp);
        player.uhitinc += otmp->spe;
        break;
      case RIN_INCREASE_DAMAGE:
        accessory_has_effect(otmp);
        player.udaminc += otmp->spe;
        break;
      case RIN_PROTECTION:
        accessory_has_effect(otmp);
        HProtection |= FROMOUTSIDE;
        player.ublessed += otmp->spe;
        flags.botl = 1;
        break;
      case RIN_FREE_ACTION:
        /* Give sleep resistance instead */
        if (!(HSleep_resistance & FROMOUTSIDE)) accessory_has_effect(otmp);
        if (!Sleep_resistance) You_feel("wide awake.");
        HSleep_resistance |= FROMOUTSIDE;
        break;
      ...
      case AMULET_OF_RESTFUL_SLEEP: /* another bad idea! */
        if (!(HSleeping & FROMOUTSIDE)) accessory_has_effect(otmp);
...
#else
    if (otmp->where == OBJ_FREE) DeallocateObject(otmp);
#endif
    else
...
STATIC_OVL const char *foodword(Object *otmp) {
...
STATIC_OVL void fpostfx(Object *otmp) {
  switch (otmp->otyp) {
    case SPRIG_OF_WOLFSBANE:
      if (player.ulycn >= LOW_PM || is_were(youmonst.data)) you_unwere(TRUE);
      break;
    case CARROT:
    ...
    case LUMP_OF_ROYAL_JELLY:
      ...
      gainstr(otmp, 1);
      if (Upolyd) {
        player.mh += otmp->cursed ? -rnd(20) : rnd(20);
        if (player.mh > player.mhmax) {
          if (!rn2(17)) player.mhmax++;
          player.mh = player.mhmax;
        } else if (player.mh <= 0) {
          rehumanize();
        }
      } else {
        player.uhp += otmp->cursed ? -rnd(20) : rnd(20);
        if (player.uhp > player.uhpmax) {
          if (!rn2(17)) player.uhpmax++;
          player.uhp = player.uhpmax;
    ...
    case EUCALYPTUS_LEAF:
      if (Sick && !otmp->cursed) make_sick(0L, (char *)0, TRUE, SICK_ALL);
      if (Vomiting && !otmp->cursed) make_vomiting(0L, TRUE);
      break;
  }
  return;
}
 ...
 */
STATIC_OVL int edibility_prompts(Object *otmp) {
  ...
  bool cadaver = (otmp->otyp == CORPSE), stoneorslime = FALSE;
  ...
  if (stoneorslime) {
    sprintf(buf, "%s like %s could be something very dangerous! %s", foodsmell,
  ...
  if (cadaver && !vegetarian(&mons[mnum]) && !player.uconduct.unvegetarian &&
      Role_if(PM_MONK)) {
  ...
  if (Upolyd && player.umonnum == PM_RUST_MONSTER && is_metallic(otmp) &&
      otmp->oerodeproof) {
  ...
  if (!player.uconduct.unvegan &&
    ...
    sprintf(buf, "%s foul and unfamiliar to you. %s", foodsmell, eat_it_anyway);
    if (yn_function(buf, ynchars, 'n') == 'n')
      return 1;
    else
      return 2;
...
int doeat() /* generic "eat" command funtion (see cmd.c) */
{
  ...
  if (is_metallic(otmp) && player.umonnum == PM_RUST_MONSTER &&
      otmp->oerodeproof) {
    otmp->rknown = TRUE;
    if (otmp->quan > 1L) {
      if (!carried(otmp))
  ...
  if (otmp->oclass != FOOD_CLASS) {
    ...
    victual.usedtime = 0;
    victual.canchoke = (player.uhs == SATIATED);
    ...
    if (material == LEATHER || material == BONE || material == DRAGON_HIDE) {
      player.uconduct.unvegan++;
      violated_vegetarian();
    } else if (material == WAX)
      player.uconduct.unvegan++;
    player.uconduct.food++;

    if (otmp->cursed) (void)rottenfood(otmp);
    ...
    if (otmp->oclass == WEAPON_CLASS && otmp->opoisoned) {
      ...
      pline("This %s is delicious!", otmp->oclass == COIN_CLASS
                                         ? foodword(otmp)
                                         : singular(otmp, xname));
  ...
  if (otmp == victual.piece) {
     ...
     */
    if (player.uhs != SATIATED) victual.canchoke = FALSE;
...
#endif
  victual.reqtime =
      (basenutrit == 0 ? 0 : rounddiv(victual.reqtime * (long)otmp->oeaten,
                                      basenutrit));
#ifdef DEBUG
 ...
 */
STATIC_OVL int bite() {
  if (victual.canchoke && player.uhunger >= 2000) {
    choke(victual.piece);
    return 1;
  }
  if (victual.doreset) {
...
{
  if (player.uinvulnerable) return; /* you don't feel hungrier */

  if ((!player.usleep || !rn2(10)) /* slow metabolic rate while asleep */
      && (carnivorous(youmonst.data) || herbivorous(youmonst.data)) &&
      !Slow_digestion)
    player.uhunger--; /* ordinary food consumption */
  ...
  if (moves % 2) { /* odd turns */
    ...
    if (HRegeneration || ((ERegeneration & (~W_ART)) &&
      ...
      player.uhunger--;
    if (near_capacity() > SLT_ENCUMBER) player.uhunger--;
  } else { /* even turns */
    if (Hunger) player.uhunger--;
    /* Conflict uses up food too */
    if (HConflict || (EConflict & (~W_ARTI))) player.uhunger--;
    ...
    switch ((int)(moves % 20)) { /* note: use even cases only */
      ...
      case 12:
        ...
        break;
      case 16:
        if (player.uhave.amulet) player.uhunger--;
        break;
      default:
        break;
    }
...
void sync_hunger() {
	...
	if(is_fainted()) {
		...
		flags.soundok = 0;
		nomul(-10+(player.uhunger/10), "fainted from lack of food");
...
void newuhs(bool incr) {
  ...
  int h = player.uhunger;

  newhs = (h > 1000) ? SATIATED : (h > 150)
                                      ? NOT_HUNGRY
                                      : (h > 50) ? HUNGRY : (h > 0) ? WEAK
                                                                    : FAINTING;
  ...
  if (newhs == FAINTING) {
    ...
    if (player.uhs <= WEAK || rn2(20 - player.uhunger / 10) >= 19) {
    ...
    } else if (player.uhunger < -(int)(200 + 20 * ACURR(A_CON))) {
      player.uhs = STARVED;
      flags.botl = 1;
      bot();
  ...
  if (newhs != player.uhs) {
    if (newhs >= WEAK && player.uhs < WEAK)
      losestr(1); /* this may kill you -- see below */
    else if (newhs < WEAK && player.uhs >= WEAK)
      losestr(-1);
    switch (newhs) {
      case HUNGRY:
        if (Hallucination) {
          ...
          You((!incr)
                  ...
                  : (player.uhunger < 145) ? "feel hungry."
      ...
      case WEAK:
        if (Hallucination)
          ...
          pline("%s needs food, badly!",
                (Role_if(PM_WIZARD) || Role_if(PM_VALKYRIE)) ? urole.name.m
                                                             : "Elf");
        else
          You((!incr) ? "feel weak now." : (player.uhunger < 45)
                                               ? "feel weak."
    ...
    flags.botl = 1;
    bot();
    if ((Upolyd ? player.mh : player.uhp) < 1) {
...
Object *floorfood(const char *verb, int corpsecheck) {
  Object *otmp;
  char qbuf[QBUFSZ];
  char c;
  ...
  if (!can_reach_floor() ||
#ifdef STEED
      (feeding && player.usteed) || /* can't eat off floor while riding */
#endif
  ...
  if (feeding && metallivorous(youmonst.data)) {
    ...
    if (ttmp && ttmp->tseen && ttmp->ttyp == BEAR_TRAP) {
      ...
      sprintf(qbuf, "There is a bear trap here (%s); eat it?",
              (player.utrap && player.utraptype == TT_BEARTRAP) ? "holding you"
                                                                : "armed");
    ...
    if (youmonst.data != &mons[PM_RUST_MONSTER] &&
        (gold = g_at(player.ux, player.uy)) != 0) {
      if (gold->quan == 1L)
        sprintf(qbuf, "There is 1 gold piece here; eat it?");
      else
        sprintf(qbuf, "There are %ld gold pieces here; eat them?", gold->quan);
      if ((c = yn_function(qbuf, ynqchars, 'n')) == 'y') {
        return gold;
      } else if (c == 'q') {
        return nullptr;
...
skipfloor:
   ...
   */
  otmp =
      getobj(feeding ? (const char *)allobj : (const char *)comestibles, verb);
  if (corpsecheck && otmp)
...
int eaten_stat(int base, Object *obj) {
...
void consume_oeaten(Object *obj, int amt) {
  /*
