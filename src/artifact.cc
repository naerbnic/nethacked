...
STATIC_DCL bool Mb_hit(Monster *magr, Monster *mdef, Object *, int *, int, bool,
                       char *);
...
STATIC_DCL bool attacks(int, Object *);
...
int touch_artifact(Object *obj, Monster *mon) {
  ...
  if (yours) {
    badclass = self_willed && ((oart->role != NON_PM && !Role_if(oart->role)) ||
    ...
    badclass =
        self_willed && oart->role != NON_PM && oart != &artilist[ART_EXCALIBUR];
...
STATIC_OVL int spec_applies(const Artifact *weap, Monster *mtmp) {
  ...
  if (weap->spfx & SPFX_DMONS) {
    ...
    Object *defending_weapon = (yours ? uwep : mtmp->weapon());
...
STATIC_OVL bool Mb_hit(Monster *magr, Monster *mdef, Object *mb, int *dmgptr,
                       int dieroll, bool vis, char *hittee) {
  MonsterType *old_uasmon;
  const char *verb;
  bool youattack = (magr == &youmonst), youdefend = (mdef == &youmonst),
  ...
  *dmgptr += rnd(4); /* (2..3)d4 */
  ...
  if (youattack || youdefend || vis) {
    result = TRUE;
    pline_The("magic-absorbing blade %s %s!", vtense((const char *)0, verb),
              hittee);
 ...
 */
bool artifact_hit(Monster *magr, Monster *mdef, Object *otmp, int *dmgptr,
                  int dieroll) {
  ...
  if (attacks(AD_MAGM, otmp)) {
    if (realizes_damage)
      pline_The("imaginary widget hits%s %s%c",
                !spec_dbon_applies ? "" : "!  A hail of magic missiles strikes",
                hittee, !spec_dbon_applies ? '.' : '!');
    return realizes_damage;
  }
    ...
    if (otmp->oartifact == ART_TSURUGI_OF_MURAMASA && dieroll == 1) {
      ...
      if (!youdefend) {
        ...
        if (noncorporeal(mdef->data) || amorphous(mdef->data)) {
          pline("%s slices through %s %s.", wepdesc, s_suffix(mon_nam(mdef)),
                mbodypart(mdef, NECK));
          return TRUE;
        }
        *dmgptr = 2 * mdef->mhp + FATAL_DAMAGE_MODIFIER;
    ...
    if (!youdefend) {
      if (vis) {
        if (otmp->oartifact == ART_STORMBRINGER)
          pline_The("%s blade draws the life from %s!", hcolor(NH_BLACK),
                    mon_nam(mdef));
        else
          pline("%s draws the life from %s!", The(distant_name(otmp, xname)),
                mon_nam(mdef));
      }
      if (mdef->m_lev == 0) {
        *dmgptr = 2 * mdef->mhp + FATAL_DAMAGE_MODIFIER;
    ...
    switch (oart->inv_prop) {
      ...
      case ENERGY_BOOST: {
        ...
        if (epboost > 120)
          ...
          epboost = player.uenmax - player.uen;
        if (epboost) {
      ...
      case CHARGE_OBJ: {
        Object *otmp = getobj(recharge_type, "charge");
        bool b_effect;
      ...
      case CREATE_PORTAL: {
        ...
        if (player.uhave.amulet || In_endgame(&player.uz) ||
            In_endgame(&newlev) || newlev.dnum == player.uz.dnum) {
...
bool artifact_light(Object *obj) {
