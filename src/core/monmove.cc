...
not_special:
  if (player.uswallow && !mtmp->mflee && player.ustuck != mtmp)
    return (1);
  omx = mtmp->mx;
  omy = mtmp->my;
  gx = mtmp->mux;
...
#ifdef GOLDOBJ
    if (typ == COIN_CLASS && Object->quan > 100L)
      return FALSE;
#endif
