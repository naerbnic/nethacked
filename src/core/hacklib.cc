...
NetHack, except that rounddiv may call panic().
      ...
      return type     routine name    argument type(s)
        ...
        bool		fuzzymatch	(const char *,const char *,const
char *,bool)
        void		setrandom	(void)
        int		getyear		(void)
        char *		yymmdd		(time_t)
...
pmatch_top:
  ...
  if (!p)                       /* end of pattern */
    ...
    return ((bool)((!*patrn || pmatch(patrn, strng - 1))
                       ? TRUE
...
bool fuzzymatch(const char *s1, const char *s2, const char *ignore_chars,
                bool caseblind) {
  char c1, c2;
...
int getyear() { return (1900 + getlt()->tm_year); }
