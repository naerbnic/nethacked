...
void explode(int x, int y, int type, int dam, char olet, int expltype) {
        ...
        if (mtmp->mhp < 1)
          ...
          switch (adtyp) {
            ...
            case AD_COLD:
              ...
              break;
            case AD_DISN:
              explmask[i][j] |= (olet == WAND_CLASS) ? (nonliving(mtmp->data) ||
                                                        is_demon(mtmp->data))
                                                     : resists_disint(mtmp);
              break;
            case AD_ELEC:
        ...
        for (i = 0; i < 3; i++)
          for (j = 0; j < 3; j++) {
            if (explmask[i][j] == 1)
               ...
               */
              show_glyph(i + x - 1, j + y - 1, cmap_to_glyph(shield_static[k]));
          }
        curs_on_u(); /* will flush screen and output */
        delay_output();
      }
  ...
  if (dam)
    for (i = 0; i < 3; i++)
      for (j = 0; j < 3; j++) {
        ...
        if (type >= 0)
          (void)zap_over_floor((xchar)(i + x - 1), (xchar)(j + y - 1), type,
                               &shopdamage);
        ...
        if (player.uswallow && mtmp == player.ustuck) {
          if (is_animal(player.ustuck->data))
            ...
            pline("%s gets slightly %s!", Monnam(player.ustuck),
                  (adtyp == AD_FIRE)
                      ? "toasted"
                      : (adtyp == AD_COLD)
                            ? "chilly"
    ...
    if (player.uhp <= 0 || (Upolyd && player.mh <= 0)) {
      if (Upolyd) {
        ...
        if (olet == MON_EXPLODE) {
          ...
          sprintf(killer_buf, "caught %sself in %s own %s", uhim(), uhis(),
                  str);
  ...
  if (shopdamage) {
    pay_for_damage(adtyp == AD_FIRE ? "burn away" : adtyp == AD_COLD
                                                        ? "shatter"
                                                        : adtyp == AD_DISN
                                                              ? "disintegrate"
                                                              : "destroy",
...
long scatter(int sx, int sy, int blastforce, unsigned int scflags,
             Object *obj) {
  Object *otmp;
  int tmp;
  int farthest = 0;
  ...
  while ((otmp = individual_object ? obj : level.objects[sx][sy]) != 0) {
    ...
    if (!used_up) {
      stmp = (struct scatter_chain *)alloc(sizeof(struct scatter_chain));
    ...
    for (stmp = schain; stmp; stmp = stmp->next) {
      if ((stmp->range-- > 0) && (!stmp->stopped)) {
        ...
        if (!isok(bhitpos.x, bhitpos.y)) {
          ...
          bhitpos.y -= stmp->dy;
          stmp->stopped = TRUE;
        } else if (!ZAP_POS(typ) || closed_door(bhitpos.x, bhitpos.y)) {
          bhitpos.x -= stmp->dx;
          bhitpos.y -= stmp->dy;
          stmp->stopped = TRUE;
