...
void container_impact_dmg(Object *obj) {
  ...
  for (otmp = obj->cobj; otmp; otmp = otmp2) {
    ...
    if (result) {
      ...
      if (costly)
        loss += stolen_value(otmp, x, y, (bool)shkp->mpeaceful, TRUE);
      if (otmp->quan > 1L)
        useup(otmp);
      else {
...
STATIC_OVL int kick_object(xchar x, xchar y) {
  ...
  if (kickobj->quan > 1L && !isgold)
    kickobj = SplitObject(kickobj, 1L);
...
void obj_delivery() {
  ...
  for (otmp = migrating_objs; otmp; otmp = otmp2) {
    otmp2 = otmp->nobj;
    if (otmp->ox != player.uz.dnum || otmp->oy != player.uz.dlevel)
      continue;
