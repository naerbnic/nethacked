...
#define IS_TREE(typ) ((typ) == TREE || (level.flags.arboreal && (typ) == STONE))
 ...
 */
#define MON_AT(x, y) \
  (level.monsters[x][y] != nullptr && !(level.monsters[x][y])->mburied)
#define MON_BURIED_AT(x, y) \
  (level.monsters[x][y] != nullptr && (level.monsters[x][y])->mburied)
#ifndef STEED
#define place_monster(m, x, y) \
  ((m)->mx = (x), (m)->my = (y), level.monsters[(m)->mx][(m)->my] = (m))
#endif
