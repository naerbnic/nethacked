...
void outrumor(int truth, int mechanism) {
  static const char fortune_msg[] = "This cookie has a scrap of paper inside.";
  const char *line;
  char buf[BUFSZ];
  bool reading = (mechanism == BY_COOKIE || mechanism == BY_PAPER);
...
void outoracle(bool special, bool delphi) {
  ...
  if (oracles) {
    ...
    if (delphi)
      putstr(tmpwin, 0,
             special ? "The Oracle scornfully takes all your money and says:"
