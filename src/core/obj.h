...
#define greatest_erosion(otmp) \
  (int)((otmp)->oeroded > (otmp)->oeroded2 ? (otmp)->oeroded : (otmp)->oeroded2)
#define MAX_ERODE 3
...
#define is_blade(otmp)                                                         \
  (otmp->oclass == WEAPON_CLASS && objects[otmp->otyp].oc_skill >= P_DAGGER && \
...
#define is_boots(otmp) \
  (otmp->oclass == ARMOR_CLASS && objects[otmp->otyp].oc_armcat == ARM_BOOTS)
#define is_gloves(otmp) \
  (otmp->oclass == ARMOR_CLASS && objects[otmp->otyp].oc_armcat == ARM_GLOVES)
#define is_cloak(otmp) \
  (otmp->oclass == ARMOR_CLASS && objects[otmp->otyp].oc_armcat == ARM_CLOAK)
#define is_shirt(otmp) \
  (otmp->oclass == ARMOR_CLASS && objects[otmp->otyp].oc_armcat == ARM_SHIRT)
#define is_suit(otmp) \
  (otmp->oclass == ARMOR_CLASS && objects[otmp->otyp].oc_armcat == ARM_SUIT)
#define is_elven_armor(otmp)                                                   \
  ((otmp)->otyp == ELVEN_LEATHER_HELM || (otmp)->otyp == ELVEN_MITHRIL_COAT || \
   (otmp)->otyp == ELVEN_CLOAK || (otmp)->otyp == ELVEN_SHIELD ||              \
...
#define is_dwarvish_armor(otmp)                                               \
  ((otmp)->otyp == DWARVISH_IRON_HELM ||                                      \
   (otmp)->otyp == DWARVISH_MITHRIL_COAT || (otmp)->otyp == DWARVISH_CLOAK || \
...
#define is_dwarvish_obj(otmp)                                   \
  (is_dwarvish_armor(otmp) || (otmp)->otyp == DWARVISH_SPEAR || \
   (otmp)->otyp == DWARVISH_SHORT_SWORD || (otmp)->otyp == DWARVISH_MATTOCK)
