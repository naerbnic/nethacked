...
bool next_to_u() {
  ...
  if (player.usteed && mon_has_amulet(player.usteed))
    return FALSE;
#endif
  return (TRUE);
}
    ...
    if (dist2(player.ux, player.uy, mtmp->mx, mtmp->my) >
      ...
      if (!um_dist(mtmp->mx, mtmp->my, 3)) {
        ...
        if (um_dist(mtmp->mx, mtmp->my, 5) || (mtmp->mhp -= rnd(2)) <= 0) {
          ...
          if (mtmp->mhp > 0)
            player.uconduct.killer = save_pacifism;
        } else {
...
STATIC_OVL int use_mirror(Object *obj) {
  ...
  if (mtmp->msleeping) {
    if (vis)
      pline("%s is too tired to look at your mirror.", Monnam(mtmp));
...
#define attr_trouble(Y) trouble_list[trouble_count++] = attr2trbl(Y)
  ...
  if (Sick)
    prop_trouble(SICK);
  if (Blinded > (long)player.ucreamed)
