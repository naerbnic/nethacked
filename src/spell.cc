...
STATIC_DCL int throwspell();
STATIC_DCL void cast_protection();
      ...
      if (uarmg) {
        if (uarmg->oerodeproof || !is_corrodeable(uarmg)) {
          ...
          if (uarmg->greased) {
            ...
            Your("gloves corrode%s!", uarmg->oeroded2 + 1 == MAX_ERODE
                                          ? " completely"
                                          : uarmg->oeroded2 ? " further" : "");
            uarmg->oeroded2++;
          }
        } else
          Your("gloves %s completely corroded.", Blind ? "feel" : "look");
        break;
      }
...
STATIC_OVL void deadbook(Object *book2) {
  ...
  if (invocation_pos(player.ux, player.uy) &&
    ...
    Object *otmp;
    bool arti1_primed = FALSE, arti2_primed = FALSE, arti_cursed = FALSE;
  ...
  raise_dead:
    ...
    if (!rn2(3) && ((mtmp = makemon(&mons[PM_MASTER_LICH], player.ux, player.uy,
                                    NO_MINVENT)) != 0 ||
                    (mtmp = makemon(&mons[PM_NALFESHNEE], player.ux, player.uy,
                                    NO_MINVENT)) != 0)) {
      mtmp->mpeaceful = 0;
      set_malign(mtmp);
    }
...
int spell_skilltype(int booktype) { return (objects[booktype].oc_skill); }
      ...
      if (role_skill >= P_SKILLED) {
        if (throwspell()) {
          ...
          while (n--) {
            ...
            player.dx = cc.x + rnd(3) - 2;
            player.dy = cc.y + rnd(3) - 2;
...
STATIC_OVL bool dospellmenu(const char *prompt, int splaction, int *spell_no) {
  ...
  for (i = 0; i < MAXSPELL && spellid(i) != NO_SPELL; i++) {
    sprintf(buf, iflags.menu_tab_sep ? "%s\t%-d%s\t%s\t%-d%%"
                                     : "%-20s  %2d%s   %-12s %3d%%",
            spellname(i), spellev(i), spellknow(i) ? " " : "*",
            spelltypemnemonic(spell_skilltype(spellid(i))),
            100 - percent_success(i));

    any.a_int = i + 1; /* must be non-zero */
    add_menu(tmpwin, NO_GLYPH, &any, spellet(i), 0, ATR_NONE, buf,
...
void dump_spells() {
  ...
  dump("  ", buf);
  for (i = 0; i < MAXSPELL && spellid(i) != NO_SPELL; i++) {
    sprintf(buf, "%c - %-20s  %2d%s   %-12s %3d%%", spellet(i), spellname(i),
            spellev(i), spellknow(i) ? " " : "*",
            spelltypemnemonic(spell_skilltype(spellid(i))),
            100 - percent_success(i));
    dump("  ", buf);
  }
...
STATIC_OVL int percent_success(int spell) {
  ...
  difficulty =
      (spellev(spell) - 1) * 4 - ((skill * 6) + (player.ulevel / 3) + 1);
