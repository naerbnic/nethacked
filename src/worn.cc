...
void setworn(Object *obj, long mask) {
...
void mon_set_minvis(Monster *mon) {
  mon->perminvis = 1;
  if (!mon->invis_blkd) {
    mon->minvis = 1;
...
void mon_adjust_speed(Monster *mon, int adjust, Object *obj) {
  Object *otmp;
...
void update_mon_intrinsics(Monster *mon, Object *obj, bool on, bool silently) {
  int unseen;
  uchar mask;
  Object *otmp;
 ...
 */
void m_dowear(Monster *mon, bool creation) {
#define RACE_EXCEPTION TRUE
  ...
  if (mindless(mon->data) && (!creation || (mon->data->mlet != S_MUMMY &&
...
STATIC_OVL void m_dowear_type(Monster *mon, long flag, bool creation,
  ...
  if (!creation) {
    if (canseemon(mon)) {
      ...
      if (old)
        ...
        buf[0] = '\0';
      pline("%s%s puts on %s.", Monnam(mon), buf, distant_name(best, doname));
    } /* can see it */
...
Object *which_armor(Monster *mon, long flag) {
  Object *obj;
...
void bypass_obj(Object *obj) {
  obj->bypass = 1;
  flags.bypasses = TRUE;
}

void mon_break_armor(Monster *mon, bool polyspot) {
  Object *otmp;
  ...
  if (breakarm(mdat)) {
    ...
    if ((otmp = which_armor(mon, W_ARMU)) != 0) {
      if (vis) {
        if (sliparm(mon->data))
          pline("%s seeps right through %s shirt!", Monnam(mon), ppronoun);
        else
  ...
  noride:
    ...
    if (touch_petrifies(player.usteed->data) && !Stone_resistance && rnl(3)) {
      ...
      sprintf(buf, "falling off %s", an(player.usteed->data->mname));
      instapetrify(buf);
    }
    dismount_steed(DISMOUNT_FELL);
  }
 ...
 */
int racial_exception(Monster *mon, Object *obj) {
  const MonsterType *ptr = raceptr(mon);
