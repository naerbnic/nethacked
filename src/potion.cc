...
void make_sick(long xtime, const char *cause, bool talk, int type) {
  ...
  if (xtime > 0L) {
    ...
    if (!old) {
      ...
      /* already sick */
      if (talk) You_feel("%s worse.", xtime <= Sick / 2L ? "much" : "even");
    }
    ...
    /* was sick, now not */
    player.usick_type &= ~type;
    if (player.usick_type) { /* only partly cured */
  ...
  if (Sick) {
    ...
    if (cause) {
      (void)strncpy(player.usick_cause, cause, sizeof(player.usick_cause));
      player.usick_cause[sizeof(player.usick_cause) - 1] = 0;
    } else
...
bool make_hallucinated(long xtime, bool talk, long mask) {
  ...
  if (mask) {
    ...
    if (!EHalluc_resistance && (!!HHallucination != !!xtime)) changed = TRUE;
...
STATIC_OVL void ghost_from_bottle() {
  Monster *mtmp = makemon(&mons[PM_GHOST], player.ux, player.uy, NO_MM_FLAGS);
...
int dodrink() {
  ...
  if (IS_FOUNTAIN(levl[player.ux][player.uy].typ) && !Levitation) {
...
#ifdef SINKS
  /* Or a kitchen sink? */
  if (IS_SINK(levl[player.ux][player.uy].typ)) {
  ...
  switch (otmp->otyp) {
    ...
    case SPE_RESTORE_ABILITY:
      ...
      if (otmp->cursed) {
        ...
        for (ii = 0; ii < A_MAX; ii++) {
          lim = AMAX(i);
          if (i == A_STR && player.uhs >= 3) --lim; /* WEAK */
          if (ABASE(i) < lim) {
            ABASE(i) = lim;
            flags.botl = 1;
    ...
    case POT_HALLUCINATION:
      ...
      (void)make_hallucinated(
          itimeout_incr(HHallucination, rn1(200, 600 - 300 * GetBUCSign(otmp))),
          TRUE, 0L);
      break;
    case POT_WATER:
      if (!otmp->blessed && !otmp->cursed) {
        pline("This tastes like water.");
        player.uhunger += rnd(10);
        newuhs(FALSE);
        break;
      }
      unkn++;
        ...
        if (otmp->blessed) {
          ...
          if (player.ulycn >= LOW_PM) {
            Your("affinity to %s disappears!",
                 makeplural(mons[player.ulycn].mname));
            if (youmonst.data == &mons[player.ulycn]) you_unwere(FALSE);
          ...
          healup(d(2, 6), 0, 0, 0);
          if (player.ulycn >= LOW_PM && !Upolyd) you_were();
          exercise(A_CON, TRUE);
        }
      } else {
        if (otmp->blessed) {
          ...
          exercise(A_WIS, TRUE);
          exercise(A_CON, TRUE);
          if (player.ulycn >= LOW_PM) you_unwere(TRUE); /* "Purified" */
    ...
    case POT_FRUIT_JUICE: {
      ...
      newsym(player.ux, player.uy); /* see yourself! */
    ...
    case SPE_DETECT_MONSTERS:
      ...
      if (monster_detect(otmp, 0)) return (1); /* nothing detected */
      exercise(A_WIS, TRUE);
      break;
    case POT_OBJECT_DETECTION:
    case SPE_DETECT_TREASURE:
      if (object_detect(otmp, 0)) return (1); /* nothing detected */
      exercise(A_WIS, TRUE);
      break;
    case POT_SICKNESS:
      ...
      if (otmp->blessed) {
        ...
        if (Role_if(PM_HEALER))
          ...
          if (!Poison_resistance) {
            if (otmp->fromsink)
              losehp(rnd(10) + 5 * !!(otmp->cursed), "contaminated tap water",
                     KILLED_BY);
            else
              losehp(rnd(10) + 5 * !!(otmp->cursed), "contaminated potion",
                     KILLED_BY_AN);
          }
          exercise(A_CON, FALSE);
        }
    ...
    case POT_CONFUSION:
      if (!Confusion)
        ...
        nothing++;
      make_confused(
          itimeout_incr(HConfusion, rn1(7, 16 - 8 * GetBUCSign(otmp))), FALSE);
      break;
    case POT_GAIN_ABILITY:
      if (otmp->cursed) {
        ...
        for (ii = A_MAX; ii > 0; ii--) {
          ...
          if (adjattrib(i, 1, itmp) && !otmp->blessed) break;
        }
      }
      break;
    case POT_SPEED:
...
#ifdef STEED
          && !player.usteed /* heal_legs() would heal steeds legs */
#endif
          ) {
        heal_legs();
        unkn++;
    ...
    case POT_BLINDNESS:
      if (Blind) nothing++;
      make_blinded(
          itimeout_incr(Blinded, rn1(200, 250 - 125 * GetBUCSign(otmp))),
          (bool)!Blind);
      break;
    case POT_GAIN_LEVEL:
      if (otmp->cursed) {
        ...
        if ((ledger_no(&player.uz) == 1 && player.uhave.amulet) ||
            Can_rise_up(player.ux, player.uy, &player.uz)) {
          const char *riseup = "rise up, through the %s!";
          if (ledger_no(&player.uz) == 1) {
            You(riseup, ceiling(player.ux, player.uy));
            goto_level(&earth_level, FALSE, FALSE, FALSE);
          } else {
            int newlev = depth(&player.uz) - 1;
            d_level newlevel;
      ...
      if (otmp->blessed)
         ...
         */
        player.uexp = rndexp(TRUE);
      break;
    case POT_HEALING:
      You_feel("better.");
      healup(d(6 + 2 * GetBUCSign(otmp), 4), !otmp->cursed ? 1 : 0,
    ...
    case POT_GAIN_ENERGY: /* M. Stephenson */
    {
      ...
      player.uenmax += (otmp->cursed) ? -num : num;
      player.uen += (otmp->cursed) ? -num : num;
    ...
    case POT_ACID:
      if (Acid_resistance) /* Not necessarily a creature who _likes_ acid */
        pline("This tastes %s.", Hallucination ? "tangy" : "sour");
      else {
        pline("This burns%s!", otmp->blessed
                                   ? " a little"
                                   : otmp->cursed ? " a lot" : " like acid");
        losehp(d(otmp->cursed ? 2 : 1, otmp->blessed ? 4 : 8), "potion of acid",
               KILLED_BY_AN);
        exercise(A_CON, FALSE);
      }
...
void healup(int nhp, int nxtra, bool curesick, bool cureblind) {
  if (nhp) {
    if (Upolyd) {
      player.mh += nhp;
      if (player.mh > player.mhmax) player.mh = (player.mhmax += nxtra);
    } else {
      player.uhp += nhp;
      if (player.uhp > player.uhpmax) player.uhp = (player.uhpmax += nxtra);
    }
  }
...
const char *bottlename() { return bottlenames[rn2(SIZE(bottlenames))]; }
...
void potionhit(Monster *mon, Object *obj, bool your_fault) {
  ...
  if (isyou) {
    switch (obj->otyp) {
      case POT_OIL:
        if (obj->lamplit) splatter_burning_oil(player.ux, player.uy);
        break;
      case POT_POLYMORPH:
        You_feel("a little %s.", Hallucination ? "normal" : "strange");
      ...
      case POT_ACID:
        if (!Acid_resistance) {
          ...
          losehp(d(obj->cursed ? 2 : 1, obj->blessed ? 4 : 8), "potion of acid",
                 KILLED_BY_AN);
        }
        break;
    }
    ...
    switch (obj->otyp) {
      ...
      case POT_SICKNESS:
        ...
        if (dmgtype(mon->data, AD_DISE) ||
            ...
            resists_poison(mon)) {
          if (canseemon(mon)) pline("%s looks unharmed.", Monnam(mon));
          break;
        }
      do_illness:
      ...
      case POT_BLINDNESS:
        ...
        break;
      case POT_WATER:
        if (is_undead(mon->data) || is_demon(mon->data) || is_were(mon->data)) {
          if (obj->blessed) {
        ...
        break;
      case POT_OIL:
        if (obj->lamplit) splatter_burning_oil(mon->mx, mon->my);
        break;
      case POT_ACID:
  ...
  if ((distance == 0 || ((distance < 3) && rn2(5))) &&
    ...
    docall(obj);
  if (*player.ushops && obj->unpaid) {
    Monster *shkp = shop_keeper(*in_rooms(player.ux, player.uy, SHOPBASE));
...
void potionbreathe(Object *obj) {
  ...
  switch (obj->otyp) {
    ...
    case POT_GAIN_ABILITY:
      ...
      break;
    case POT_FULL_HEALING:
      if (Upolyd && player.mh < player.mhmax) player.mh++, flags.botl = 1;
      if (player.uhp < player.uhpmax) player.uhp++, flags.botl = 1;
    /*FALL THROUGH*/
    case POT_EXTRA_HEALING:
      if (Upolyd && player.mh < player.mhmax) player.mh++, flags.botl = 1;
      if (player.uhp < player.uhpmax) player.uhp++, flags.botl = 1;
    ...
    case POT_INVISIBILITY:
      if (!Blind && !Invis) {
        kn++;
        pline("For an instant you %s!", See_invisible
    ...
    case POT_SPEED:
      ...
      break;
    case POT_BLINDNESS:
      if (!Blind && !player.usleep) {
        kn++;
        pline("It suddenly gets dark.");
      }
      make_blinded(itimeout_incr(Blinded, rnd(5)), FALSE);
      if (!Blind && !player.usleep) Your(vision_clears);
      break;
    case POT_WATER:
      if (player.umonnum == PM_GREMLIN) {
        ...
        if (obj->blessed && youmonst.data == &mons[player.ulycn])
          you_unwere(FALSE);
  ...
  if (obj->dknown) {
    if (kn)
      makeknown(obj->otyp);
    else if (!objects[obj->otyp].oc_name_known && !objects[obj->otyp].oc_uname)
      docall(obj);
  }
}
...
STATIC_OVL short mixtype(Object *o1, Object *o2) {
  ...
  switch (o1->otyp) {
    ...
    case AMETHYST: /* "a-methyst" == "not intoxicated" */
      if (o2->otyp == POT_BOOZE) return POT_FRUIT_JUICE;
      break;
    case POT_GAIN_LEVEL:
    case POT_GAIN_ENERGY:
      switch (o2->otyp) {
    ...
    case SCROLL_CLASS:
      if (obj->otyp != SCR_BLANK_PAPER
        ...
        if (!Blind) {
          ...
          pline_The("scroll%s %s.", oq1 ? "" : "s", otense(obj, "fade"));
        }
        if (obj->unpaid && costly_spot(player.ux, player.uy)) {
    ...
    case SPBOOK_CLASS:
      if (obj->otyp != SPE_BLANK_PAPER) {
        if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
          ...
          if (!Blind) {
            bool oq1 = obj->quan == 1L;
            pline_The("spellbook%s %s.", oq1 ? "" : "s", otense(obj, "fade"));
          }
          if (obj->unpaid && costly_spot(player.ux, player.uy)) {
            You("erase it, you pay for it.");
            CreateBillDummyObject(obj);
...
int dodip() {
  ...
  allowall[1] = '\0';
  if (!(obj = getobj(allowall, "dip"))) return (0);

  here = levl[player.ux][player.uy].typ;
  ...
  if (IS_FOUNTAIN(here)) {
    ...
    tmp = waterbody_name(player.ux, player.uy);
#ifdef PARANOID
    ...
    if (yn(qbuf) == 'y') {
      if (Levitation) {
        floating_above(tmp);
#ifdef STEED
      } else if (player.usteed && !is_swimmer(player.usteed->data) &&
  ...
  if (potion->otyp == POT_WATER) {
    ...
    if (potion->blessed) {
      if (obj->cursed) {
        ...
        if (useeit) {
          ...
          pline("%s %s with a%s %s aura.", Your_buf,
                aobjnam(obj, "softly glow"), index(vowels, *tmp) ? "n" : "",
                tmp);
        }
        Bless(obj);
        obj->bknown = 1;
      ...
      if (obj->blessed) {
        ...
        if (useeit) {
          tmp = hcolor(NH_BLACK);
          pline("%s %s with a%s %s aura.", Your_buf, aobjnam(obj, "glow"),
  ...
  if (potion->otyp == POT_INVISIBILITY && !Object->oinvis) {
    ...
    if (!Blind) {
      if (!See_invisible)
        pline("Where did %s go?", the(xname(Object)));
      else
        You("notice a little haziness around %s.", the(xname(Object)));
    }
    goto poof;
    ...
    if (!Blind) {
      if (!See_invisible)
        pline("So that's where %s went!", the(xname(Object)));
      else
        pline_The("haziness around %s disappears.", the(xname(Object)));
    }
    goto poof;
  }
#endif
...
more_dips:
  ...
  if ((obj->otyp == OIL_LAMP || obj->otyp == MAGIC_LAMP) &&
    ...
    if (obj->lamplit || potion->lamplit) {
      useup(potion);
      explode(player.ux, player.uy, 11, d(6, 6), 0, EXPL_FIERY);
      exercise(A_WIS, FALSE);
      return 1;
    }
  ...
  if ((obj->otyp == UNICORN_HORN || obj->otyp == AMETHYST) &&
    ...
    /* with multiple merged potions, split off one and
       just clear it */
    if (potion->quan > 1L) {
      singlepotion = SplitObject(potion, 1L);
    } else
      singlepotion = potion;
    ...
    singlepotion = hold_another_object(singlepotion, "You juggle and drop %s!",
...
void djinni_from_bottle(Object *obj) {
  ...
  if (!(mtmp = makemon(&mons[PM_DJINNI], player.ux, player.uy, NO_MM_FLAGS))) {
...
Monster *split_mon(
  ...
  if (mon == &youmonst) {
    mtmp2 = cloneu();
    if (mtmp2) {
      mtmp2->mhpmax = player.mhmax / 2;
      player.mhmax -= mtmp2->mhpmax;
      flags.botl = 1;
    ...
    if (mtmp2) {
      ...
      if (canspotmon(mon)) pline("%s multiplies%s!", Monnam(mon), reason);
    }
  }
  return mtmp2;
}
