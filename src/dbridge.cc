 ...
 */
bool is_db_wall(int x, int y) { return ((bool)(levl[x][y].typ == DBWALL)); }
...
struct entity {
  Monster *emon;      /* youmonst for the player */
  MonsterType *edata; /* must be non-zero for record to be valid */
  int ex, ey;
};
...
STATIC_OVL
struct entity *e_at(int x, int y) {
  int entitycnt;
...
STATIC_OVL void m_to_e(Monster *mtmp, int x, int y, struct entity *etmp) {
  etmp->emon = mtmp;
  if (mtmp) {
    etmp->ex = x;
    etmp->ey = y;
...
/* #define e_strg(etmp, func) (is_u(etmp)? nullptr : func(etmp->emon)) */
...
STATIC_OVL const char *E_phrase(struct entity *etmp, const char *verb) {
  ...
  if (is_u(etmp))
    ...
    strcat(wholebuf, vtense(nullptr, verb));
  return (wholebuf);
}
...
STATIC_OVL bool e_survives_at(struct entity *etmp, int x, int y) {
  ...
  if (is_db_wall(x, y))
    return ((bool)(is_u(etmp) ? Passes_walls : passes_walls(etmp->edata)));
  return (TRUE);
}
...
#define mk_message(dest) ((dest & 1) ? "" : nullptr)
#define mk_corpse(dest) ((dest & 2) ? AD_DGST : AD_PHYS)
    ...
    for (entitycnt = 0; entitycnt < ENTITIES; entitycnt++) {
      if (etmp != &(occupants[entitycnt]) &&
        ...
        occupants[entitycnt].edata = nullptr;
    }
#undef mk_message
#undef mk_corpse
  }
 ...
 */
  ...
  if (is_db_wall(etmp->ex, etmp->ey)) {
    ...
    e_died(
        etmp, e_inview ? 3 : 2, /* CRUSHING is arbitrary */
        (is_pool(etmp->ex, etmp->ey))
            ? DROWNING
            : (is_lava(etmp->ex, etmp->ey)) ? BURNING : CRUSHING); /*no corpse*/
    return;
  }
}
...
void close_drawbridge(int x, int y) {
  ...
  (void)revive_nasty(x, y, nullptr);
  (void)revive_nasty(x2, y2, nullptr);
  delallobj(x, y);
  delallobj(x2, y2);
  if ((t = t_at(x, y)) != 0)
    deltrap(t);
...
void destroy_drawbridge(int x, int y) {
  ...
  if ((lev1->drawbridgemask & DB_UNDER) == DB_MOAT ||
    ...
    if (lev1->typ == DRAWBRIDGE_UP) {
      ...
      if (cansee(x, y))
        pline_The("drawbridge collapses into the %s!", lava ? "lava" : "moat");
      else if (flags.soundok)
        You_hear("a loud *SPLASH*!");
    }
