  ...
  while (mtmp) {
    if (!mtmp->dead()) {
      ...
      if (distm < distance) {
        ...
        if (distm < distance / 3 && !resist(mtmp, TOOL_CLASS, 0, NOTELL))
    ...
    if (!mtmp->dead() && mtmp->data->mlet == S_SNAKE && mtmp->mcanmove &&
      ...
      if (canseemon(mtmp)) {
        if (!could_see_mon)
          ...
          pline("%s freezes, then sways with the music%s.", Monnam(mtmp),
...
void awaken_soldiers() {
  ...
  while (mtmp) {
    if (!mtmp->dead() && is_mercenary(mtmp->data) &&
          ...
          do_pit:
            ...
            if (mtmp) {
              ...
              if (Levitation || Flying || is_clinger(youmonst.data)) {
                ...
                losehp(rnd(6), "fell into a chasm", NO_KILLER_PREFIX);
    ...
    if (Is_stronghold(&player.uz)) {
      ...
      if (!strcmp(buf, tune)) {
        ...
        if (ok) { /* There is a drawbridge near */
          ...
          if (tumblers)
            if (gears)
              You_hear("%d tumbler%s click and %d gear%s turn.", tumblers,
                       plur(tumblers), gears, plur(gears));
            else
              You_hear("%d tumbler%s click.", tumblers, plur(tumblers));
          else if (gears) {
