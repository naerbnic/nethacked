...
STATIC_VAR const short skill_names_indices[P_NUM_SKILLS] = {
    0,                    DAGGER,           KNIFE,
    AXE,                  PICK_AXE,         SHORT_SWORD,
    BROADSWORD,           LONG_SWORD,       TWO_HANDED_SWORD,
    SCIMITAR,             PN_SABER,         CLUB,
    MACE,                 MORNING_STAR,     FLAIL,
    PN_HAMMER,            QUARTERSTAFF,     PN_POLEARMS,
    SPEAR,                JAVELIN,          TRIDENT,
    LANCE,                BOW,              SLING,
    CROSSBOW,             DART,             SHURIKEN,
    ...
    PN_MATTER_SPELL,      PN_BARE_HANDED,   PN_TWO_WEAPONS,
#ifdef STEED
    PN_RIDING
#endif
};
...
STATIC_VAR const char *const odd_skill_names[] = {
    ...
    "two weapon combat",  "riding",          "polearms",
    "saber",              "hammer",          "whip",
...
STATIC_VAR const char *const barehands_or_martial[] = {"bare handed combat",
                                                       "martial arts"};
...
int hitval(Object *otmp, Monster *mon) {
  ...
  if (Is_weapon && otmp->blessed && (is_demon(ptr) || is_undead(ptr))) tmp += 2;
...
int dmgval(Object *otmp, Monster *mon) {
  ...
  if (bigmonst(ptr)) {
    if (objects[otyp].oc_wldam) tmp = rnd(objects[otyp].oc_wldam);
    switch (otyp) {
      case IRON_CHAIN:
      case CROSSBOW_BOLT:
      case MORNING_STAR:
    ...
    if (objects[otyp].oc_wsdam) tmp = rnd(objects[otyp].oc_wsdam);
    switch (otyp) {
      case IRON_CHAIN:
      case CROSSBOW_BOLT:
      case MACE:
  ...
  if (objects[otyp].oc_material <= LEATHER && thick_skinned(ptr))
    ...
    tmp = 0;
  if (ptr == &mons[PM_SHADE] && objects[otyp].oc_material != SILVER) tmp = 0;
  ...
  if (Is_weapon || otmp->oclass == GEM_CLASS || otmp->oclass == BALL_CLASS ||
    ...
    if (otmp->blessed && (is_undead(ptr) || is_demon(ptr))) bonus += rnd(4);
...
#define Oselect(x) \
  if ((otmp = oselect(mtmp, x)) != 0) return (otmp);
...
static const int rwep[] = {
    DWARVISH_SPEAR,            SILVER_SPEAR,  ELVEN_SPEAR,  SPEAR,
    ORCISH_SPEAR,              JAVELIN,       SHURIKEN,     YA,
    SILVER_ARROW,              ELVEN_ARROW,   ARROW,        ORCISH_ARROW,
    CROSSBOW_BOLT,             SILVER_DAGGER, ELVEN_DAGGER, DAGGER,
    ORCISH_DAGGER,             KNIFE,         FLINT,        ROCK,
...
static const int pwep[] = {
    HALBERD,  BARDICHE, SPETUM, BILL_GUISARME, VOULGE,
    RANSEUR,  GUISARME, GLAIVE, LUCERN_HAMMER, BEC_DE_CORBIN,
    FAUCHARD, PARTISAN, LANCE};
...
static const short hwep[] = {
    ...
    TSURUGI,              RUNESWORD,      DWARVISH_MATTOCK,   TWO_HANDED_SWORD,
    BATTLE_AXE,           KATANA,         UNICORN_HORN,       CRYSKNIFE,
    TRIDENT,              LONG_SWORD,     ELVEN_BROADSWORD,   BROADSWORD,
    SCIMITAR,             SILVER_SABER,   MORNING_STAR,       ELVEN_SHORT_SWORD,
    DWARVISH_SHORT_SWORD, SHORT_SWORD,    ORCISH_SHORT_SWORD, MACE,
    AXE,                  DWARVISH_SPEAR, SILVER_SPEAR,       ELVEN_SPEAR,
    SPEAR,                ORCISH_SPEAR,   FLAIL,              BULLWHIP,
    QUARTERSTAFF,         JAVELIN,        AKLYS,              CLUB,
    PICK_AXE,
#ifdef KOPS
    RUBBER_HOSE,
#endif /* KOPS */
    WAR_HAMMER,           SILVER_DAGGER,  ELVEN_DAGGER,       DAGGER,
    ORCISH_DAGGER,        ATHAME,         SCALPEL,            KNIFE,
    WORM_TOOTH};
...
Object *select_hwep(Monster *mtmp) {
  ...
  for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
    if (otmp->oclass == WEAPON_CLASS && otmp->oartifact &&
        ...
        ((strong && !wearing_shield) || !objects[otmp->otyp].oc_bimanual))
      return otmp;
  }
  ...
  for (i = 0; i < SIZE(hwep); i++) {
    if (hwep[i] == CORPSE && !(mtmp->misc_worn_check & W_ARMG)) continue;
...
int mon_wield_item(Monster *mon) {
  ...
  switch (mon->weapon_check) {
    ...
    case NEED_AXE:
      ...
      if (!obj || which_armor(mon, W_ARMS)) obj = m_carrying(mon, AXE);
      break;
    ...
    if (mw_tmp && mw_tmp->cursed && mw_tmp->otyp != CORPSE) {
      if (canseemon(mon)) {
        ...
        if (obj->otyp == PICK_AXE) {
          ...
          pline("%s %s %s!", s_suffix(Monnam(mon)), xname(mw_tmp), welded_buf);
        }
        mw_tmp->bknown = 1;
      }
      mon->weapon_check = NO_WEAPON_WANTED;
    ...
    if (canseemon(mon)) {
      ...
      if (obj->cursed && obj->otyp != CORPSE) {
        pline("%s %s to %s %s!", Tobjnam(obj, "weld"),
              is_plural(obj) ? "themselves" : "itself", s_suffix(mon_nam(mon)),
              mbodypart(mon, HAND));
        obj->bknown = 1;
      }
    }
...
void dump_weapon_skill() { enhance_skill(TRUE); }
...
#endif
{
  ...
  if (!want_dump)
#endif
    if (wizard && yn("Advance skills without practice?") == 'y') speedy = TRUE;
#endif
  ...
  do {
      ...
      for (i = skill_ranges[pass].first; i <= skill_ranges[pass].last; i++) {
        ...
        if (want_dump) {
          if (P_SKILL(i) > P_UNSKILLED) {
            sprintf(buf2, "%-*s [%s]", longest, P_NAME(i),
          ...
          if (wizard) {
            if (!iflags.menu_tab_sep)
              sprintf(buf, " %s%-*s %-12s %5d(%4d)", prefix, longest, P_NAME(i),
                      sklnambuf, P_ADVANCE(i),
                      practice_needed_to_advance(P_SKILL(i)));
            else
              sprintf(buf, " %s%s\t%s\t%5d(%4d)", prefix, P_NAME(i), sklnambuf,
                      P_ADVANCE(i), practice_needed_to_advance(P_SKILL(i)));
          } else
#endif
          {
            if (!iflags.menu_tab_sep)
              sprintf(buf, " %s %-*s [%s]", prefix, longest, P_NAME(i),
                      sklnambuf);
            else
    ...
    strcpy(buf,
           (to_advance > 0) ? "Pick a skill to advance:" : "Current skills:");
#ifdef WIZARD
    if (wizard && !speedy)
      sprintf(eos(buf), "  (%d slot%s available)", player.weapon_slots,
              plur(player.weapon_slots));
#endif
#ifdef DUMP_LOG
    if (want_dump) {
      dump("", "");
...
void add_weapon_skill(int n) {
  ...
  if (before < after) give_may_advance_msg(P_NONE);
}
...
void lose_weapon_skill(int n) {
  ...
  while (--n >= 0) {
    ...
    if (player.weapon_slots) {
      ...
      if (P_SKILL(skill) <= P_UNSKILLED) panic("lose_weapon_skill (%d)", skill);
...
#ifdef STEED
  if (urole.petnum == PM_PONY) P_SKILL(P_RIDING) = P_BASIC;
#endif
