...
STATIC_PTR int ckunpaid(Object *);
STATIC_PTR int ckvalidcat(Object *);
#ifdef DUMP_LOG
...
Object *merge_choice(Object *objlist, Object *obj) {
  Monster *shkp;
  int save_nocharge;
...
int merged(Object **potmp, Object **pobj) {
  Object *otmp = *potmp, *obj = *pobj;
...
/* temporary special case for gold objects!!!! */
    ...
    if (obj->owornmask && carried(otmp)) {
      ...
      setnotworn(obj);
		...
		else if (Object->owornmask && mcarried(otmp)) {
		    if (Object == MON_WEP(otmp->ocarry)) {
...
void addinv_core1(Object *obj) {
  if (obj->oclass == COIN_CLASS) {
    ...
    if (is_quest_artifact(obj)) {
      if (player.uhave.questart) impossible("already have quest artifact?");
...
*/
Object *addinv(Object *obj) {
  Object *otmp, *prev;

  if (obj->where != OBJ_FREE) panic("addinv: obj not free");
 ...
 */
void carry_obj_effects(Object *obj) {
 ...
 */
Object *hold_another_object(Object *obj, const char *drop_fmt,
...
void useupall(Object *obj) {
  setnotworn(obj);
  freeinv(obj);
  obfree(obj, nullptr); /* deletes contents also */
}

void useup(Object *obj) {
...
void consume_obj_charge(Object *obj, bool maybe_unpaid) {
...
void freeinv_core(Object *obj) {
  if (obj->oclass == COIN_CLASS) {
    ...
    if (is_quest_artifact(obj)) {
      if (!player.uhave.questart) impossible("don't have quest artifact?");
...
bool have_lizard() {
  ...
  for (otmp = invent; otmp; otmp = otmp->nobj)
    if (otmp->otyp == CORPSE && otmp->corpsenm == PM_LIZARD) return (TRUE);
  return (FALSE);
}

Object *o_on(unsigned int id, Object *objchn) {
  Object *temp;

  while (objchn) {
    if (objchn->o_id == id) return (objchn);
    if (Has_contents(objchn) && (temp = o_on(id, objchn->cobj))) return temp;
    objchn = objchn->nobj;
  }
  return ((Object *)0);
}
...
/*
 * getobj returns:
 *	Object *xxx:	object to do something with.
 *	nullptr	error return: no object.
...
#endif
 */
Object *getobj(const char *let, const char *word) {
  Object *otmp;
  char ilet;
...
#ifndef GOLDOBJ
  if (*let == COIN_CLASS)
    let++, usegold = TRUE, allowgold = (player.ugold ? TRUE : FALSE);
#else
...
#ifndef GOLDOBJ
      if (!(allowcnt == 2 && cnt < player.ugold)) cnt = player.ugold;
...
void silly_thing(const char *word, Object *otmp) {
  ...
  if (s1) {
    ...
    if (otyp == LENSES || is_gloves(otmp) || is_boots(otmp)) what = "those";
...
STATIC_PTR int ckunpaid(Object *otmp) { return ((int)(otmp->unpaid)); }
...
static const char removeables[] = {ARMOR_CLASS,  WEAPON_CLASS, RING_CLASS,
                                   AMULET_CLASS, TOOL_CLASS,   0};
...
int ggetobj(char const *word, int (*fn)(Object *), int mx,
  ...
  iletct = collect_obj_classes(ilets, invent, FALSE,
#ifndef GOLDOBJ
                               (allowgold != 0),
#endif
                               filter, &itemcount);
  ...
  ilets[iletct++] = 'i';
  if (!combo) ilets[iletct++] = 'm'; /* allow menu presentation on request */
  ilets[iletct] = '\0';

  for (;;) {
    sprintf(qbuf, "What kinds of thing do you want to %s? [%s]", word, ilets);
    getlin(qbuf, buf);
  ...
  if (m_seen)
     ...
     */
    if (combo && allflag && resultflags) *resultflags |= ALL_FINISHED;
    return cnt;
  }
}
 ...
 */
int askchain(Object **objchn,
             char const *olets, /* olets is an Obj Class char array */
             int allflag, int (*fn)(Object *), int (*ckfn)(Object *), int mx,
             char const *word) {
  Object *otmp, *otmp2, *otmpo;
  char sym, ilet;
  int cnt = 0, dud = 0, tmp;
  ...
  nodot = (!strcmp(word, "nodot") || !strcmp(word, "drop") || ident || takeoff);
  ininv = (*objchn == invent);
...
nextclass:
  ilet = 'a' - 1;
  if (*objchn && (*objchn)->oclass == COIN_CLASS) ilet--; /* extra iteration */
  ...
  if (olets && *olets && *++olets) goto nextclass;
  if (!takeoff && (dud || cnt))
    pline("That was all.");
  else if (!dud && !cnt)
...
void fully_identify_obj(Object *otmp) {
  makeknown(otmp->otyp);
...
#ifndef GOLDOBJ
  if (obj->oclass == COIN_CLASS) return GOLD_SYM;
#endif
  if (!flags.invlet_constant) {
    obj->invlet = NOINVSYM;
    reassign();
 ...
 */
void prinv(const char *prefix, Object *obj, long quan) {
  if (!prefix) prefix = "";
...
char *xprname(Object *obj, const char *txt, char let, bool dot, long cost,
  ...
  if (cost != 0 || let == '*') {
    ...
    sprintf(li, "%c - %-45s %6ld %s", (dot && use_invlet ? obj->invlet : let),
 ...
 * containers.
 */
STATIC_OVL Object *find_unpaid(Object *list, Object **last_found) {
  Object *obj;
  ...
  while (list) {
    if (list->unpaid) {
      if (*last_found) {
        ...
        if (list == *last_found) *last_found = (Object *)0;
      } else
        return (*last_found = list);
    }
    if (Has_contents(list)) {
      if ((obj = find_unpaid(list->cobj, last_found)) != 0) return obj;
    }
    list = list->nobj;
  }
  return (Object *)0;
 ...
 */
#ifdef DUMP_LOG
static char display_pickinv(char const *lets, bool want_reply, long *out_cnt,
...
#endif
{
  ...
  if (want_disp) {
    ...
    if (flags.perm_invent) {
      ...
      if (win == WIN_ERR) win = local_win = create_nhwindow(NHW_MENU);
    } else
      win = WIN_INVEN;
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!flags.sortpack || otmp->oclass == *invlet) {
      if (flags.sortpack && !classcount) {
        any.a_void = 0; /* zero */
#ifdef DUMP_LOG
        if (want_dump) dump("  ", let_to_name(*invlet, FALSE));
        if (want_disp)
#endif
      ...
      if (want_disp)
#endif
        add_menu(win, obj_to_glyph(otmp), &any, ilet, 0, ATR_NONE, doname(otmp),
                 MENU_UNSELECTED);
    }
  }
#else /* SORTLOOT */
 ...
 * contained objects.
 */
int count_unpaid(Object *list) {
  int count = 0;

  while (list) {
    if (list->unpaid) count++;
    if (Has_contents(list)) count += count_unpaid(list->cobj);
    list = list->nobj;
  }
  return count;
}
 ...
 */
int count_buc(Object *list, int type) {
  int count = 0;
  ...
  while (list) {
    ...
    switch (type) {
      case BUC_BLESSED:
        ...
        break;
      case BUC_CURSED:
        if (list->oclass != COIN_CLASS && list->bknown && list->cursed) count++;
        break;
      case BUC_UNCURSED:
        if (list->oclass != COIN_CLASS && list->bknown && !list->blessed &&
            !list->cursed)
        ...
        break;
      case BUC_UNKNOWN:
        if (list->oclass != COIN_CLASS && !list->bknown) count++;
        break;
      default:
...
STATIC_OVL void dounpaid() {
  ...
  if (count == 1) {
    ...
    pline("%s", xprname(otmp, distant_name(otmp, doname),
                        marker ? otmp->invlet : CONTAINED_SYM, TRUE,
                        unpaid_cost(otmp), 0L));
    return;
  }
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj) {
      ...
      if (otmp->unpaid) {
        if (!flags.sortpack || otmp->oclass == *invlet) {
          ...
          otmp->unpaid = 0;
          putstr(win, 0, xprname(otmp, distant_name(otmp, doname), ilet, TRUE,
                                 cost, 0L));
  ...
  if (count > num_so_far) {
    ...
    if (flags.sortpack) putstr(win, 0, let_to_name(CONTAINED_SYM, TRUE));
    /*
    ...
    for (otmp = invent; otmp; otmp = otmp->nobj) {
      if (Has_contents(otmp)) {
        ...
        while (find_unpaid(otmp->cobj, &marker)) {
          ...
          putstr(win, 0, xprname(marker, distant_name(marker, doname),
...
STATIC_OVL bool this_type_only(Object *obj) {
...
int dotypeinv() {
  ...
  if (flags.menu_style != MENU_TRADITIONAL) {
    if (flags.menu_style == MENU_FULL || flags.menu_style == MENU_PARTIAL) {
      traditional = FALSE;
      i = UNPAID_TYPES;
...
int look_here(int obj_cnt, bool picked_some) {
  ...
  if (player.uswallow && player.ustuck) {
    ...
    You("%s to %s what is lying in %s.", Blind ? "try" : "look around", verb,
        &fbuf[12]);
    otmp = mtmp->minvent;
    if (otmp) {
  ...
  if (dfeature) sprintf(fbuf, "There is %s here.", an(dfeature));

  if (!otmp || is_lava(player.ux, player.uy) ||
  ...
  if (skip_objects) {
    if (dfeature) pline(fbuf);
    read_engr_at(player.ux, player.uy); /* Eric Backus */
    There("are %s%s objects here.", (obj_cnt <= 10) ? "several" : "many",
          picked_some ? " more" : "");
...
int dolook() { return look_here(0, FALSE); }

bool will_feel_cockatrice(Object *otmp, bool force_touch) {
...
void feel_cockatrice(Object *otmp, bool force_touch) {
  char kbuf[BUFSZ];
...
void stackobj(Object *obj) {
  ...
  for (otmp = level.objects[obj->ox][obj->oy]; otmp; otmp = otmp->nexthere)
    if (otmp != obj && merged(&obj, &otmp)) break;
  return;
}

/* returns TRUE if obj  & otmp can be merged */
STATIC_OVL bool mergable(Object *otmp, Object *obj) {
  ...
  if (Is_candle(obj) && obj->age / 25 != otmp->age / 25) return (FALSE);
...
STATIC_OVL bool tool_in_use(Object *obj) {
...
int doprtool() {
  ...
  for (otmp = invent; otmp; otmp = otmp->nobj)
    if (tool_in_use(otmp)) lets[ct++] = obj_to_let(otmp);
  lets[ct] = '\0';
  if (!ct)
...
int doprinuse() {
  ...
  for (otmp = invent; otmp; otmp = otmp->nobj)
    if (is_worn(otmp) || tool_in_use(otmp)) lets[ct++] = obj_to_let(otmp);
  lets[ct] = '\0';
  if (!ct)
 ...
 */
STATIC_VAR const char *names[] = {
    0,         "Illegal objects", "Weapons",          "Armor",
    "Rings",   "Amulets",         "Tools",            "Comestibles",
    "Potions", "Scrolls",         "Spellbooks",       "Wands",
    "Coins",   "Gems",            "Boulders/Statues", "Iron balls",
    "Chains",  "Venoms"};

static const char oth_symbols[] = {CONTAINED_SYM, '\0'};
...
STATIC_OVL void invdisp_nothing(const char *hdr, const char *txt) {
  ...
  end_menu(win, (char *)0);
  if (select_menu(win, PICK_NONE, &selected) > 0) free((genericptr_t)selected);
  destroy_nhwindow(win);
  return;
}

/* query_objlist callback: return things that could possibly be worn/wielded */
STATIC_OVL bool worn_wield_only(Object *obj) {
 ...
 */
Object *display_minventory(Monster *mon, int dflags, char *title) {
  Object *ret;
#ifndef GOLDOBJ
  Object m_gold;
#endif
...
#ifndef GOLDOBJ
  int do_all = (dflags & MINV_ALL) != 0, do_gold = (do_all && mon->mgold);
#else
  ...
  if (do_all ? (mon->minvent != 0)
    ...
    if (do_gold) {
       ...
       */
      m_gold = zeroobj;
      m_gold.otyp = GOLD_PIECE;
      m_gold.oclass = COIN_CLASS;
      m_gold.quan = mon->mgold;
      m_gold.dknown = 1;
    ...
    n = query_objlist(title ? title : tmp, mon->minvent, INVORDER_SORT,
                      &selected, (dflags & MINV_NOLET) ? PICK_NONE : PICK_ONE,
 ...
 */
Object *display_cinventory(Object *obj) {
  Object *ret;
  char tmp[QBUFSZ];
  int n;
  menu_item *selected = 0;
...
STATIC_OVL bool only_here(Object *obj) {
