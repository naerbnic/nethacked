...
static bool ReadColorMap(FILE *fd, int number);
...
bool fopen_gif_file(const char *, const char *);
bool read_gif_tile(pixel (*)[]);
int fclose_gif_file();
...
static void read_header(FILE *fd) {
  ...
  for (;;) {
    ...
    if (c == '!') { /* Extension */
      if (!ReadOK(fd, &c, 1)) {
        Fprintf(stderr, "EOF / read error on extension function code\n");
        exit(EXIT_FAILURE);
      }
...
static int LWZReadByte(FILE *fd, int flag, int input_code_size) {
  ...
  while ((code = GetCode(fd, code_size, FALSE)) >= 0) {
    if (code == clear_code) {
      ...
      if (count != 0)
        Fprintf(stderr, "missing EOD in data stream (common occurrence)\n");
      return -2;
    }
...
static void ReadInterleavedImage(FILE *fd, int len, int height) {
  ...
  while ((v = LWZReadByte(fd, FALSE, (int)input_code_size)) >= 0) {
    PPM_ASSIGN(image[ypos][xpos], ColorMap[CM_RED][v], ColorMap[CM_GREEN][v],
               ColorMap[CM_BLUE][v]);
...
bool read_gif_tile(pixel (*pixels)[TILE_X]) {
  ...
  if (GifScreen.Interlace) {
    for (j = 0; j < TILE_Y; j++) {
      for (i = 0; i < TILE_X; i++) {
        pixels[j][i] =
            image[curr_tiles_down * TILE_Y + j][curr_tiles_across * TILE_X + i];
      }
    }
  } else {
    for (j = 0; j < TILE_Y; j++) {
