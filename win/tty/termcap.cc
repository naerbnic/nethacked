...
int xputc(int c) { return putchar(c); }
...
void tty_delay_output() {
  ...
  if (flags.null)
    ...
    int cmlen = strlen(tgoto(nh_CM, ttyDisplay->curx, ttyDisplay->cury));
...
void term_start_raw_bold() { xputs(nh_HI); }

void term_end_raw_bold() { xputs(nh_HE); }

#ifdef TEXTCOLOR

void term_end_color() { xputs(nh_HE); }
