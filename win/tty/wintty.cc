...
STATIC_DCL void invert_all_on_page(winid, tty_menu_item *, tty_menu_item *,
                                   char);
...
static const char default_menu_cmds[] = {
    ...
    MENU_UNSELECT_PAGE, MENU_INVERT_PAGE,  0 /* null terminator */
};
    ...
    if (pick4u == 'y' || flags.initrace == ROLE_RANDOM || flags.randomall) {
      ...
      if (n > 1) {
        ...
        for (i = 0; races[i].noun; i++)
          if (ok_race(flags.initrole, i, flags.initgend, flags.initalign)) {
            any.a_int = i + 1; /* must be non-zero */
            add_menu(win, NO_GLYPH, &any, races[i].noun[0], 0, ATR_NONE,
    ...
    if (pick4u == 'y' || flags.initgend == ROLE_RANDOM || flags.randomall) {
      ...
      if (n > 1) {
        ...
        for (i = 0; i < ROLE_GENDERS; i++)
          if (ok_gend(flags.initrole, flags.initrace, i, flags.initalign)) {
            any.a_int = i + 1;
            add_menu(win, NO_GLYPH, &any, genders[i].adj[0], 0, ATR_NONE,
                     genders[i].adj, MENU_UNSELECTED);
          }
        any.a_int = pick_gend(flags.initrole, flags.initrace, flags.initalign,
                              PICK_RANDOM) +
                    1;
        if (any.a_int == 0) /* must be non-zero */
    ...
    if (pick4u == 'y' || flags.initalign == ROLE_RANDOM || flags.randomall) {
      ...
      if (n > 1) {
        ...
        for (i = 0; i < ROLE_ALIGNS; i++)
          if (ok_align(flags.initrole, flags.initrace, flags.initgend, i)) {
            any.a_int = i + 1;
            add_menu(win, NO_GLYPH, &any, aligns[i].adj[0], 0, ATR_NONE,
                     aligns[i].adj, MENU_UNSELECTED);
          }
        any.a_int = pick_align(flags.initrole, flags.initrace, flags.initgend,
                               PICK_RANDOM) +
                    1;
        if (any.a_int == 0) /* must be non-zero */
...
winid tty_create_nhwindow(int type) {
  ...
  if (maxwin == MAXWIN) return WIN_ERR;
  ...
  if (newwin->maxrow) {
    newwin->data = (char **)alloc(sizeof(char *) * (unsigned)newwin->maxrow);
    newwin->datlen = (short *)alloc(sizeof(short) * (unsigned)newwin->maxrow);
    if (newwin->maxcol) {
    ...
    if (newwin->type == NHW_MESSAGE) newwin->maxrow = 0;
  } else {
    newwin->data = (char **)0;
...
STATIC_OVL void erase_menu_or_text(winid window, struct WinDesc *cw,
                                   bool clear) {
  if (cw->offx == 0)
    if (cw->offy) {
      tty_curs(window, 1, 0);
  ...
  switch (cw->type) {
    case NHW_MESSAGE:
      if (ttyDisplay->toplin) {
        home();
        cl_end();
        if (cw->cury) docorner(1, cw->cury + 1);
        ttyDisplay->toplin = 0;
      }
      break;
    case NHW_STATUS:
...
STATIC_OVL void set_all_on_page(winid window, tty_menu_item *page_start,
 ...
 * zero).
 */
STATIC_OVL void invert_all(winid window, tty_menu_item *page_start,
                           tty_menu_item *page_end, char acc) {
  tty_menu_item *curr;
  bool on_curr_page;
...
STATIC_OVL void process_menu_window(winid window, struct WinDesc *cw) {
  ...
  char *cp, *rp, resp[QBUFSZ], gacc[QBUFSZ], *msave, *morestr;
    ...
    if (n > 0) /* at least one group accelerator found */
      for (rp = gacc, curr = cw->mlist; curr; curr = curr->next)
        if (curr->gselector && !index(gacc, curr->gselector) &&
            (cw->how == PICK_ANY || gcnt[GSELIDX(curr->gselector)] == 1)) {
          *rp++ = curr->gselector;
    ...
    if (!page_start) {
      ...
      if (cw->npages > 0) {
        ...
        for (page_lines = 0, curr = page_start; curr != page_end;
          ...
          bool menucolr = FALSE;
#endif
          if (curr->selector) *rp++ = curr->selector;
 ...
 */
          ...
          for (n = 0, cp = curr->str;
               *cp && (int)++ttyDisplay->curx < (int)ttyDisplay->cols;
               cp++, n++)
            if (n == 2 && curr->identifier.a_void != 0 && curr->selected) {
              if (curr->count == -1L)
      ...
      *rp = 0;
      ...
      if (cw->npages > 1)
        sprintf(cw->morestr, "(%d of %d)", curr_page + 1, (int)cw->npages);
      else if (msave)
    ...
    switch (morc) {
      ...
      case MENU_SELECT_PAGE:
        if (cw->how == PICK_ANY) set_all_on_page(window, page_start, page_end);
        break;
      ...
      case MENU_INVERT_ALL:
        if (cw->how == PICK_ANY) invert_all(window, page_start, page_end, 0);
        break;
      default:
...
STATIC_OVL void process_text_window(winid window, struct WinDesc *cw) {
  ...
  for (n = 0, i = 0; i < cw->maxrow; i++) {
    ...
    if (cw->data[i]) {
      ...
      term_start_attr(attr);
      for (cp = &cw->data[i][1];
           *cp && (int)++ttyDisplay->curx < (int)ttyDisplay->cols; cp++)
        (void)putchar(*cp);
      term_end_attr(attr);
    }
  }
  if (i == cw->maxrow) {
    ...
    cl_end();
    dmore(cw, quitchars);
    if (morc == '\033') cw->flags |= WIN_CANCELLED;
  }
}
  ...
  switch (cw->type) {
    case NHW_MESSAGE:
      if (ttyDisplay->toplin == 1) {
        ...
        ttyDisplay->toplin = 0;
      cw->curx = cw->cury = 0;
      if (!cw->active) iflags.window_inited = TRUE;
      break;
    case NHW_MAP:
      end_glyphout();
      if (blocking) {
    ...
    case NHW_MENU:
      ...
      cw->offx =
          (uchar)(int)max((int)10, (int)(ttyDisplay->cols - cw->maxcol - 1));
      if (cw->type == NHW_MENU) cw->offy = 0;
      if (ttyDisplay->toplin == 1) tty_display_nhwindow(WIN_MESSAGE, TRUE);
...
void tty_dismiss_nhwindow(winid window) {
  ...
  switch (cw->type) {
    case NHW_MESSAGE:
      if (ttyDisplay->toplin) tty_display_nhwindow(WIN_MESSAGE, TRUE);
    /*FALLTHRU*/
    case NHW_STATUS:
    case NHW_BASE:
    case NHW_MAP:
...
void tty_destroy_nhwindow(winid window) {
  ...
  if (cw->type == NHW_MESSAGE) iflags.window_inited = 0;
  if (cw->type == NHW_MAP) clear_screen();
...
void tty_putstr(winid window, int attr, const char *str) {
  ...
  if (str == (const char *)0 ||
    ...
    return;
  if (cw->type != NHW_MESSAGE) str = compress_str(str);
  ...
  switch (cw->type) {
    ...
    case NHW_STATUS:
      ...
      for (i = cw->curx + 1, n0 = cw->cols; i < n0; i++, nb++) {
        ...
        if (*ob != *nb) tty_putsym(WIN_STATUS, i, cw->cury, *nb);
        if (*ob) ob++;
      }
    ...
    case NHW_TEXT:
      ...
      if (cw->cury >= cw->rows) {
        ...
        for (i = 0; i < cw->maxrow; i++) tmp[i] = cw->data[i];
        if (cw->data) free((genericptr_t)cw->data);
        cw->data = tmp;

        for (i = cw->maxrow; i < cw->rows; i++) cw->data[i] = 0;
      }
      if (cw->data[cw->cury]) free((genericptr_t)cw->data[cw->cury]);
      n0 = strlen(str) + 1;
...
#else /* DEF_PAGER */
  {
    ...
    if (!f) {
      ...
      while (dlb_fgets(buf, BUFSZ, f)) {
        ...
        if (wins[datawin]->flags & WIN_CANCELLED) break;
      }
 ...
 * later.
 */
void tty_add_menu(winid window, int glyph, const anything *identifier, char ch,
...
void tty_end_menu(winid window, const char *prompt) {
  ...
  if (cw->plist_size < cw->npages + 1 /*need 1 slot beyond last*/) {
    ...
    cw->plist =
        (tty_menu_item **)alloc(cw->plist_size * sizeof(tty_menu_item *));
  }
...
void tty_update_inventory() { return; }

void tty_mark_synch() { (void)fflush(stdout); }
...
void tty_cliparound(int x, int y) {
  ...
  if (clipx != oldx || clipy != oldy) {
    if (on_level(&player.uz0, &player.uz) && !restoring) (void)doredraw();
  }
}
#endif /* CLIPPING */
...
void tty_print_glyph(winid window, xchar x, xchar y, int glyph) {
...
#ifdef CLIPPING
  if (clipping) {
    if (x <= clipx || y < clipy || x >= clipxmax || y >= clipymax) return;
  }
#endif
  ...
  if (color != ttyDisplay->color) {
    if (ttyDisplay->color != NO_COLOR) term_end_color();
...
int tty_nhgetch() {
  ...
  if (ttyDisplay && ttyDisplay->toplin == 1) ttyDisplay->toplin = 2;
  return i;
}
 ...
 */
/*ARGSUSED*/
int tty_nh_poskey(int *x, int *y, int *mod) { return tty_nhgetch(); }

void win_tty_init() { return; }
