 ...
 public:
  ...
  class iterator {
   public:
    ...
    bool operator!=(iterator other) { return i_ != other.i_; }
  ...
  T range_start() const { return start_; }

  T range_end() const { return end_; }

  iterator begin() const { return iterator(start_); }

  iterator end() const { return iterator(end_); }

  bool contains(T val) const { return start_ <= val && val < end_; }
...
class ArrayRange {
 public:
  ...
  class iterator {
   public:
    iterator(ArrayType &array, typename Range<RangeType>::iterator iter)
   ...
   public:
    ...
    decltype(array_[*iter_]) & operator*() { return array_[*iter_]; }
  };

  iterator begin() { return iterator(array_, range_.begin()); }

  iterator end() { return iterator(array_, range_.end()); }
...
class ArrayRange<BaseType *, RangeType> {
 public:
  ...
  class iterator {
   public:
    iterator(BaseType *array, typename Range<RangeType>::iterator iter)
        : array_(array), iter_(iter) {}
   ...
   public:
    // We're forced to go in order to get the typing to work right, so...
    BaseType &operator*() { return array_[*iter_]; }
  };
